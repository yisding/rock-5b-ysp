...
- `` `2026-07-16-rockchip-bsp-driver-quality.md` `` — Rockchip's BSP is feature-strong and functionally effective on fixed hardware, but its private MPP/RGA stacks remain below mature mainline drivers in hostile-input validation, resource lifetimes, framework integration, review traceability, and branch maintenance; upstream-derived areas such as DRM are materially healthier, so the judgment is scoped rather than BSP-wide — CODE-INSPECTED / SOURCE-INSPECTED / MEASURED / INFERRED for comparative ratings.
