...
- `` `2026-07-04-rga3-im2d-error-irq.md` `` — RGA3 core0 `INTR[0x2]` on direct upstream librga virtual-buffer samples was root-caused to a forward-port DMA/IOMMU contract gap: missing BSP large-segment setup (`13afe70c8271`), RGA IOVAs being allocated too close to 4 GiB and wrapping in 32-bit registers (`6b9dba7abcd0`), and missing fail-closed validation for non-single-segment or 32-bit-wrapping DMA mappings; runtime validation pending after rebuild — MEASURED / ROOT-CAUSED / FIX IN TREE.
