...
- `` `2026-07-13-rock5b-u-boot-fit-dtb-race.md` `` — controlled delay proves that the FIT packer can embed a 0-byte `u-boot.dtb` because its original rule waited for `dts/dt.dtb` rather than the copied file; making `u-boot.dtb` a prerequisite prevents the induced race — MEASURED / SOURCE-INSPECTED.
