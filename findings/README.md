...
- `` `2026-07-11-rock5b-u-boot-four-way-comparison.md` `` — Armbian 26.2.1 vendor and 26.5.1 current are almost the same Radxa 2017.09 fork/config, but the failing 26.2 and 26.5 vendor ITBs contain a zero-byte required U-Boot control DTB while 26.5 current contains 12,752 bytes; a missing Makefile dependency makes this a parallel-build race, Radxa tip still has it plus a real SPL-relocation change, and upstream replaces the vendor stack with Bootstd/EFI/Linux DT/binman — SOURCE-INSPECTED / CONFIG-INSPECTED / CODE-INSPECTED / INFERRED for causal attribution pending the 26.5-current hardware test.
