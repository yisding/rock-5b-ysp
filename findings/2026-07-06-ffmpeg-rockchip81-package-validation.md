# ffmpeg-rockchip-81 package validation failures and MPP root cause

> Scope: local package build from `/home/yi/Code/ffmpeg/ffmpeg-rockchip-81`
> into `packaging/ffmpeg-rockchip81/`, plus on-board smoke validation on ROCK 5B.
> Source tree: `ffmpeg-rockchip-81` branch `refactor/section-c` at
> `75638e7f0b17`.
...
- After the MPP root-cause pass below, the same FFmpeg binary also decoded
  the H.264 RKMPP test stream successfully when run against a matching
  from-source MPP runtime.

Failures and blockers are below.

## Failure 1: sandbox `/dev` is not usable for codec/RGA validation

Inside the default command sandbox, `/dev` was a small tmpfs with `nodev`; it did
not expose `/dev/mpp_service`, `/dev/rga`, `/dev/dma_heap/*`, or `/dev/dri`.
...
kernel or FFmpeg result. Hardware validation must run outside the sandbox, on the
board, or through a runner with the real device namespace.

## Failure 2: RKMPP decode fails in the installed MPP runtime
...
- the newly built packaged binary under `packaging/ffmpeg-rockchip81/build/`,
- the existing `/home/yi/Code/ffmpeg/ffmpeg-rockchip-81/ffmpeg` binary,
- the installed system `ffmpeg 7:8.1.2-1+rk1`.

Root-cause follow-up:

- The installed runtime selected by normal `pkg-config rockchip_mpp` is
...
- Symbol/string inspection of the installed library found H.264 HAL symbols
  ...
  `mpp_parser_api_register_mpp_h264d_wrapper`,
  `mpp_singleton_add_mpp_h264d`).
- The current local `~/Code/rockchip-userspace/mpp-rockchip` source is tag
  `1.0.12` at `1375813c`; its pkg-config template advertises
  `Version: 1.3.10`.
- A clean `/tmp` build of that exact source produced
  `librockchip_mpp.so.1 -> librockchip_mpp.so.0` with the missing parser
  registration symbols and `mpp_sgln_base_add`.
- Re-running the same packaged FFmpeg binary with
  `LD_LIBRARY_PATH=/tmp/mpp-rockchip-1.0.12-build2/mpp` decoded the H.264
  RKMPP stream to `null` successfully, with MPP reporting:
...
Conclusion: this failure is caused by the installed `/usr` MPP userspace
library being incomplete or mismatched for decoder parser registration. It is
not caused by the FFmpeg forward port and was not caused by the kernel in this
test.
...
decode smoke passes; the remaining validation item is to package or install the
matching MPP runtime and rerun the full decode/RGA transcode suite.
...
uploaded through the RKMPP hwcontext.

The corrected software-source smoke passed:
...
The local conformance MPP demo binaries failed before they could be used as an
independent decoder/encoder comparison:

```text
/home/yi/Code/rockchip-conformance/out/mpp/bin/mpi_dec_test:
  undefined symbol: mpp_sgln_base_add

/home/yi/Code/rockchip-conformance/out/mpp/bin/mpi_enc_test:
  undefined symbol: mpp_sgln_base_add
```

Root-cause follow-up:

- The demo binaries have `NEEDED librockchip_mpp.so.1` but no `RPATH` or
  `RUNPATH`, so the dynamic loader selected the installed `/usr` library.
- The matching local library exists at
  `/home/yi/Code/rockchip-conformance/out/mpp/lib/librockchip_mpp.so.1`.
  It exports `mpp_sgln_base_add` and the decoder parser registration symbols.
- Running the demos with
  `LD_LIBRARY_PATH=/home/yi/Code/rockchip-conformance/out/mpp/lib` removes the
  undefined-symbol failure and reaches normal argument/runtime checks.
- That conformance output library reports MPP commit `c2c1ee5`, so it is not
  the exact current `1.0.12` source build, but it is already newer/different
  than the installed `/usr` library ABI.

Conclusion: those binaries were built against a newer/different MPP ABI than
the installed `/usr/lib/aarch64-linux-gnu/librockchip_mpp.so.1`.

Impact: use a matching `LD_LIBRARY_PATH` for those demos, rebuild them against
the installed MPP, or avoid using them as evidence for the packaged FFmpeg
until the runtime mismatch is fixed.
...
The builder now normalizes package-root permissions with:

```bash
chmod -R u=rwX,go=rX "$PKGROOT"
```

The final archive contents show normal `0755` directories and executable modes.
