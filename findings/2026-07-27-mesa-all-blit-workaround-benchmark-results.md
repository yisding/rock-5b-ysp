...
> Source: YSP `e2ad2cf` (`blit_workaround_bench.c` and `run_blit_workaround_bench.py`); Mesa `0c1cf4a71b4`
> Date: 2026-07-27
...
The run produced 6,512 timing samples with zero disjoint flags. Correctness
separated the A/B at both known-problem geometries: every off process failed
...
The performance half is not decision-grade. The documented 500 MHz lock could
not be applied because this session had no noninteractive root path: `sudo`
required a password. The devfreq range therefore remained 300 MHz to 1 GHz,
and the runner's before/after current-frequency samples included 300, 400, 500,
800, and 1,000 MHz across the matrix. CPU 6 also remained under the `ondemand`
governor. The boot was the KASAN kernel
`6.18.40-video-port-kasan-rockchip-rk3588`.

Independently of clock control, the primary batched GPU query did not scale
with operation count. At `256x256`, the GPU median remained about 2.6
microseconds for points from `N=1` through `N=256`; at `12288x1`, it remained
about 1.2 microseconds at both `N=256` and `N=1024`. Per-process fitted GPU
slopes were consequently near zero, sometimes negative, and frequently had
poor `R²`. The resulting paired `on - off` slope deltas must not be quoted as
the workaround cost. The query apparently does not own the intended deferred
tile work; that attribution remains to be proved with a trace or corrected
timing boundary.
...
- **Exercise:** each matrix row used
  ...
  --cpu 6 -- ... --samples 11 --warmups 2 --ring 4 --schedule both`, with the
  row's size and count list.
- **Pass/fail signal:** all six runner invocations exited 0; the runner required
  the patched driver's `PAN_BLIT_DEPTH_BIAS=<mode>` acknowledgement. Off-mode
  exit 2 was accepted only for the two expected baseline correctness failures.
- **Artifacts:** raw logs, before/after metadata, and `SHA256SUMS` are in the
  machine-local, untracked bundle
  `/home/yi/Code/mesa-blit-workaround-bench-20260727` (544 KiB).
...
4. rerun on a non-KASAN kernel with GPU devfreq fixed at 500 MHz and a
...
other Mali generations; or fixed-clock performance. The test instrumentation
forces all V9-V10 internal fullscreen blits in on mode, so it validates the
proposed broad policy's measured functional subset, not every path reached by
that policy.
