...
adds opt-in BGRA, XRGB, and RGB565 fd-backed legacy display-rotation artifacts
alongside the existing RKNN/RKNPU preprocessing and GStreamer-shaped legacy
conversion artifacts. A 2026-07-05 re-check also found
...
A late 2026-07-06 symbol-level pass found one more useful class of public
sources, but again no new ioctl family. BELABOX's `gstreamer-rockchip` fork
uses RGA as an MPP/GStreamer conversion helper: it prefers dma-buf fds, falls
back to mapped virtual input, writes to dma-buf output, and drives legacy
`c_RkRgaBlit()` for format conversion and 0/90/180/270-degree rotation across
NV12/NV21/YUYV/RGB565/RGB888/RGBA/BGRA/XRGB/BGRx-style formats. BrightSign's
NPU gaze extension uses `wrapbuffer_virtualaddr()`, `imresize()`, and
`imcvtcolor()` for RTSP/USB camera preprocessing into RKNN input. `drmclone`
uses IM2D fd-or-virtual imports, handle wrapping, copy/resize/rotate, and
RGB565/BGRA/XRGB/NV12/NV21 DRM-format mapping. `YSCV`'s Rust wrapper
dynamically loads `librga.so` and exposes the same legacy blit surface over
fd-or-virtual RGB/NV12/NV21/I420 surfaces. These hits reinforce the existing
fd import, virtual import, RGB-family rotation, and NV12-family conversion
priorities; they do not justify physical-address import, Android allocator/HWC
compatibility, old DRM-RGA ioctls, or RGA2-Pro FBC source modes.
...
| GStreamer MPP forks | [`BELABOX/gstreamer-rockchip` `gstmpp.c`](https://github.com/BELABOX/gstreamer-rockchip/blob/6453290d26121659111ce3d444ec33624de7506a/gst/rockchipmpp/gstmpp.c) | MPP/GStreamer format mapping, dma-buf preferred input, virtual fallback input, dma-buf output, legacy `c_RkRgaBlit()` convert/rotate, env-gated disable path |
...
| OpenCV / camera capture | [`opencv-mobile` RKAIQ V4L2 capture](https://github.com/nihui/opencv-mobile/blob/3151145cbfe44b1802004d4fe532cf307594b477/highgui/src/capture_v4l2_rk_aiq.cpp), [`rockchip-hdmi-capture-rtsp` RGA converter](https://github.com/pablocpas/rockchip-hdmi-capture-rtsp/blob/2e10fb8cdde9224c901ce4aed304c436d01835a6/src/rga_converter.cpp), [`brightsign-npu-gaze-extension` `inference.cpp`](https://github.com/brightsign/brightsign-npu-gaze-extension/blob/889ea9554034ae766ea7f665ff8bd20cf022460c/src/inference.cpp) | V4L2/MPP dma-buf fd import, dma-heap or encoder fd destination import, handle wrapping, virtual camera-frame wrapping, `imresize()`/`imcvtcolor()`/`imcvtcolor_t()` for camera/streaming color conversion |
...
| Language bindings | [`varphone/rkrga`](https://github.com/varphone/rkrga), [`YSCV` `rga.rs`](https://github.com/enthropy7/YSCV/blob/f0344820ac03b49aa9cc4662ba110ec10e056c85/crates/yscv-video/src/rga.rs), [`vicharak-in/zig-rga`](https://github.com/vicharak-in/zig-rga/blob/35616b3d33216d4b0289a72b3de53796cf9f7bfa/src/im2d_single.zig) | Rust/Zig exposure of the same C `librga` / legacy blit/fill/IM2D ABI, often through dynamic loading; not a distinct kernel feature demand |
| Runtime wrappers / G2D shims | [`w-0x1f/linux-media` C# wrapper](https://github.com/w-0x1f/linux-media/blob/439ac160af49987a928368ff48e8ca77ea8ae994/linux-media-rockchip-rga/RGA.cs), [`posix-bsp-perf` RGA G2D shim](https://github.com/zczjx/posix-bsp-perf/blob/e17acf4670c3cd6f722d083b2695b2e3b37cdd45/bsp/bsp_g2d/impl/rk_rga/rkrga.cpp), [`drmclone` `rga_helper.cpp`](https://github.com/wxd9199/drmclone/blob/72f39d3668c29f8933a149de840d2d63387e6207/src/rga_helper.cpp) | Thin wrapper over legacy blit/fill/flush plus fd/virtual IM2D wrapping, resize/color-convert/rectangle/rotate, DRM-format-to-RGA-format mapping, and thread-local RGA3 scheduler-core selection |
