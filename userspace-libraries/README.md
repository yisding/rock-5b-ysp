...
| Owns | The package-level entry point here, the deep explanation in [`../docs/02`](../docs/02-how-the-userspace-libs-work.md), ABI detail in [`../docs/03`](../docs/03-dev-uapis.md), and build staging through [`../ffmpeg/README.md`](../ffmpeg/README.md). |
...
|----------|---------------|
| What does libmpp hide from an app? | [`../docs/02-how-the-userspace-libs-work.md`](../docs/02-how-the-userspace-libs-work.md) Part A |
| What does librga hide from an app? | [`../docs/02-how-the-userspace-libs-work.md`](../docs/02-how-the-userspace-libs-work.md) Part B |
| What ioctls cross into the kernel? | [`../docs/03-dev-uapis.md`](../docs/03-dev-uapis.md) |
| Why does GRD use upstream FFmpeg instead of `ffmpeg-rockchip`? | [`../ffmpeg/IMPLEMENTATION-COMPARISON.md`](../ffmpeg/IMPLEMENTATION-COMPARISON.md) |
| Which ABI facts were learned during the rewrite track? | [`../docs/13-rewrite-drivers.md`](../docs/13-rewrite-drivers.md) |

## Common traps

| Trap | Explanation |
|------|-------------|
| Device access is three nodes, not one | Non-root MPP encode needs `/dev/mpp_service` and `/dev/dma_heap/*`; RGA also needs `/dev/rga`. Install [`../scripts/99-rockchip-codec.rules`](../scripts/99-rockchip-codec.rules) or the [`../packaging/codec-udev/`](../packaging/codec-udev/README.md) deb. |
| `librga` has source even when the official drop ships a prebuilt `.so` | The buildable source lineage is documented in [`../docs/10-gotchas.md`](../docs/10-gotchas.md) and the staging recipe in [`../ffmpeg/README.md`](../ffmpeg/README.md). |
| `h264_rkmpp` does not always mean the same implementation | `ffmpeg-rockchip` and upstream FFmpeg 8.1.2 both expose rkmpp names, but the control surface differs. See [`../ffmpeg/IMPLEMENTATION-COMPARISON.md`](../ffmpeg/IMPLEMENTATION-COMPARISON.md). |
