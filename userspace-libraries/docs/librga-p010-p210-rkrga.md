...
The short version: P010/P210 through the legacy `c_RkRgaBlit()` path is only safe
when librga copies the 10-bit layout flags into the kernel request. The older
source tree did not. Jellyfin's packaged Rockchip path relies on a patched librga
branch that does copy those fields.
...
## nyanmisaka's librga patch

The `nyanmisaka/rk-mirrors` `jellyfin-rga` branch adds exactly the legacy-path
copy we were missing:
...
We replayed that three-file delta onto `../librga-src`:

| File | Result |
|------|--------|
| `core/NormalRga.cpp` | Added FBCE fixup, full-CSC fixup, and 10-bit layout field copies. |
...
| Option | Risk profile |
|--------|--------------|
| Build and stage the patched `../librga-src` | Best match for Jellyfin-style legacy RKRGA P010 use. |
...
| Keep compact NV15/NV20 handling | Lower risk because compact is the default kernel interpretation, though RGA2/RGA3 restrictions still apply. |
