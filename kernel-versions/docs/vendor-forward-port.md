...
> per-change table), see [vendor delta](../../kernel-drivers/docs/vendor-delta.md).
...
re-syncing against a newer BSP or kernel, see [resyncing guide](../../kernel-drivers/docs/resyncing.md).
...
- **`rockchip_iommu.h` is no longer a compat shim.** The forward-port now keeps
  ...
  VSI provider described in [RK3588 AV1 decode, IOMMU, and userspace paths](../../kernel-drivers/av1/docs/av1-rk3588.md).
...
2. **`iovad` moved to offset 0** (`mpp_iommu.h:20-29`). The cookie dropped its
   ...
   iovad) != 0)` (`mpp_iommu.c:719`). See [resyncing guide](../../kernel-drivers/docs/resyncing.md) for why this
...
[device-tree glossary](../../kernel-drivers/docs/device-tree.md) for the authoritative expansion) that
