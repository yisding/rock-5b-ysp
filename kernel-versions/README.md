...
| Evidence boundary | This directory owns stable comparisons, not moving install recommendations. [`../status.md`](../status.md) owns the forward-port, rewrite, and maximum-mainline verdicts; [`../kernel-drivers/docs/forward-port-status.md`](../kernel-drivers/docs/forward-port-status.md) owns accumulated forward-port evidence. A build is not proof of a board boot. |
...
| [`docs/hdmi21-frl-forward-port-plan.md`](docs/hdmi21-frl-forward-port-plan.md) | Two-stage plan to backport the Collabora HDMI 2.0 scrambling and HDMI 2.1 FRL stacks (as reconciled in maxline) onto the ysp 6.18 kernel, lifting the current 340 MHz/4K@30 ceiling. Design only; nothing started. |
