...
The in-depth companion to [kernel driver guide](../../kernel-drivers/docs/how-the-drivers-work.md). That one
...
> cited sources: [source-tree pins](../../docs/source-trees.md). To actually **build and
...
**In plain terms.** The kernel drivers (how-the-drivers-work.md) are a low-level "front door":
...
flowchart TB
  ...
  subgraph K["kernel drivers — how-the-drivers-work.md"]
...
is [`video-libraries/ffmpeg/docs/implementation-comparison.md`](../../video-libraries/ffmpeg/docs/implementation-comparison.md).
...
| driver client | `osal/driver/mpp_service.c` | issues `INIT_CLIENT_TYPE`, `SET_REG_WRITE/READ`, `POLL_HW_FINISH` (how-the-drivers-work.md §3) |
...
sequenceDiagram
  ...
  Note over K: kernel patches IOVAs, programs VDPU381,<br/>hardware decodes, IRQ (how-the-drivers-work.md §3,§6,§9)
...
runs it → `encode_get_packet` (bitstream out). The **key link to how-the-drivers-work.md:** the
...
start the task). [uAPI guide](../../kernel-drivers/docs/dev-uapis.md) documents that wire format.

## A5. Buffers — where the dma-bufs come from

**In plain terms.** The big frame buffers that get shared zero-copy (how-the-drivers-work.md §5)
...
- Buffers live in **`MppBufferGroup`s** (pools), so the decoder recycles a fixed
  ...
  **imported and refcounted** on that session's used-list ([kernel driver guide §5](../../kernel-drivers/docs/how-the-drivers-work.md)),
...
[`mpp-library-architecture.md`](../mpp/docs/mpp-library-architecture.md) source map just names
...
sequenceDiagram
  ...
  Note over K: scheduler picks RGA3 core0/1 or RGA2,<br/>IOMMU-maps buffers, runs the op (how-the-drivers-work.md §4,§6)
...
zero-copy, same buffer as how-the-drivers-work.md §5:
...
[`kernel-drivers/tests/transcode-test.sh`](../../kernel-drivers/tests/transcode-test.sh).
...
5. The **kernel** (how-the-drivers-work.md) patches IOVAs, picks a core (CCU/DCHS or RGA
...
> [`video-libraries/ffmpeg/README.md`](../../video-libraries/ffmpeg/README.md); all tree pins:
