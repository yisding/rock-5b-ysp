...
| Patch | Target file | Bundles (full hunk→finding map: [docs/11 §Per-patch map](../../docs/11-bsp-audit.md#per-patch-hunk--finding-map); verdicts: [VERIFICATION.md](VERIFICATION.md)) |
|-------|-------------|------------------------------------------------------------------|
...
git apply --reject "$YSP"/patches/cleanup-draft/mpp_iommu.patch
#   …or, to keep the commit message:
git am "$YSP"/patches/cleanup-draft/mpp_iommu.patch
