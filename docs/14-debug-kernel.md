...
- **Reproducing [`docs/11`](11-bsp-audit.md) findings**: several HIGH findings
...
philosophy as [`docs/08`](08-armbian-packaging.md).
...
| Memory sanitizers | `KASAN` (`GENERIC`, `INLINE`, `VMALLOC`), `PAGE_OWNER`, `PAGE_POISONING`, `DEBUG_PAGEALLOC`, `PAGE_TABLE_CHECK`, `DMA_API_DEBUG(_SG)`, `DEBUG_SG`, `DEBUG_LIST`, `DEBUG_PLIST`, `DEBUG_NOTIFIERS` | UAF/OOB (the docs/11 HIGH class), DMA mapping misuse (dma-buf import paths, docs/01 §6), corrupted lists |
...
> deadlock trap in [`docs/10`](10-gotchas.md) is about `rmdir`ing a live
...
[`docs/10`](10-gotchas.md) § Runtime ("KASAN/vermagic kernel-variant
...
**non-debug combined kernel** ([`docs/04`](04-status.md)). The config's
