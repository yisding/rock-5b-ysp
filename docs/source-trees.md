...
| 8 | Clean-room rewrite drivers | [rewrite-driver track](../kernel-drivers/docs/rewrite-drivers.md) | `yisding/linux-rock5b` branch `rk3588-rewrite-6.18` @ `a45474faa3f3` + branch `rk3588-rewrite-mainline` @ `14dc104e4b33`, see §8 |
...
- branch `rk3588-rewrite-6.18`, commit `a45474faa3f3` ("media: rockchip:
  cover rga config acquire ownership"), matching the clean dev worktree
  `/home/yi/Code/linux-6.18-rkvenc`.
- branch `rk3588-rewrite-mainline`, commit `14dc104e4b33` (same subject),
...
acquire-fd ownership/no-release-fence-export coverage, legacy async blit and
modern request-submit acquire/release-fence coverage, mixed-task RGA3-to-RGA2 core
