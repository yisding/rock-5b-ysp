...
| 8 | Clean-room rewrite drivers | [rewrite-driver track](../kernel-drivers/docs/rewrite-drivers.md) | maintained tips `rk3588-rewrite-6.18@37ae7459656b` on `v6.18.42` and `rk3588-rewrite-mainline@02bf372dac70` on `v7.2-rc6`, at tracked-source parity as of 2026-08-05; current normal-profile build and 305-signal audit pass; predecessor 6.18 `19634f4eebba` has exact 92+152 KUnit, MPP, and partial RGA hardware evidence, while the fixed tips remain runtime-unverified; package composites `rk3588-rewrite-armbian-6.18.38@8daf5e9513b8` and `rk3588-rewrite-armbian-7.2-rc3@24f7424fb958` are historical; see §8 |
...
| libmpp VP9 presentation-event repair | `github.com/yisding/mpp`, branch `ysp/main` | `a8b19653af1a0b23754afafd7de72919fa8d0c0c` (2026-08-05); distinct slot-queue occurrences plus owned show-existing frame snapshots. Hardware-validated and public; installed-package integration remains | [`VP9 show-existing repair`](../findings/2026-08-04-libmpp-vp9-show-existing-reference-slot-leak.md#2026-08-05-repair-and-validation) |
| libmpp (PPA packaging tree) | `github.com/yisding/mpp`, branch `ysp/main` | `a8b19653` (2026-08-05), eleven commits past `1.0.12@1375813c`; exported as `1.5.0+git20260805.a8b19653+ds-0ubuntu1~rk1` with unused Windows binaries removed. Source publication `18657949` is accepted/Pending; arm64 build `33468629` has a successful log and is in `Uploading build`; installed package remains the predecessor | [`packaging/ppa/README.md`](../packaging/ppa/README.md), [`VP9 show-existing repair`](../findings/2026-08-04-libmpp-vp9-show-existing-reference-slot-leak.md#2026-08-05-repair-and-validation) |
...
| ffmpeg-rockchip-81 canonical tip | `github.com/yisding/ffmpeg-rockchip-81`, branch `main` | `8b57e531d1fc` (`n8.2-dev-2444-g8b57e531d1`), 73 patch commits over current FFmpeg `master@ceabc9b306f5`; carries the reworked nyanmisaka stack, all unique refactor/Jellyfin correctness commits, and the encoder static-format/concurrency fix | [`video-libraries/ffmpeg/docs/rebase-notes.md`](../video-libraries/ffmpeg/docs/rebase-notes.md) §7, [`video-libraries/ffmpeg/README.md`](../video-libraries/ffmpeg/README.md) |
...
| ffmpeg-rockchip-81 normal-PPA encoder line | `github.com/yisding/ffmpeg-rockchip-81`, branch `fix/rkmpp-output-timeout` | `c9428bedaa45448d79c629f6be83a41257ac6167`, based on the FFmpeg 8.0 Rockchip line; retains transient synchronous RKMPP backpressure and the unused-following-reference fix, then fixes asynchronous input ownership across reset/close. Exported locally as `7:8.0.3+rockchip+git20260730.c9428bedaa-0ubuntu1~rk1`; source validation plus 10/10 close and 10/10 flush/reuse hardware gates pass. Predecessor `33a651a55b` remains the exact Published and installed runtime package exercised in the isolated complete HEVC sweep and normal matrix | [`packaging/ppa/README.md`](../packaging/ppa/README.md), [lifetime fix](../findings/2026-07-30-ffmpeg-rkmpp-async-frame-lifetime-fix.md), [`findings/2026-07-19-grd-rkmpp-encoder-wedge-mpp-input-backpressure.md`](../findings/2026-07-19-grd-rkmpp-encoder-wedge-mpp-input-backpressure.md), [NUT fix](../findings/2026-07-29-hevc-nut-radl-and-unused-rps-reference-fixes.md) |
...
| rockchip-vaapi maintained fork | `github.com/yisding/rockchip-vaapi`, branch `main` | `184d7d4` (2026-08-04), pushed to `fork/main`; exported as signed source `1.0.11+ysp12-0ubuntu1~rk1`. Carries the interlaced-decode fix (MPP's internal deinterlacer disabled), the 8192x8192 picture-size limits, and roadmap Phase 6 for `VAEntrypointVideoProc`; upstream remains `woodyst/rockchip-vaapi@e8c64dd` | [`video-libraries/vaapi/README.md`](../video-libraries/vaapi/README.md), [`roadmap qualification`](../findings/2026-07-29-rockchip-vaapi-roadmap-phase2-phase4-closure.md), [`Firefox RDD policy`](../findings/2026-07-26-firefox-rdd-rockchip-vaapi-policy.md) |
...
- branch `rk3588-rewrite-6.18`, commit `37ae7459656b` ("media: rockchip:
  ...
  `rk3588-rewrite-mainline@02bf372dac70` carries the same
  ...
  case already covered by stronger compile-time assertions, reducing the live
  gate to 84 MPP plus 148 RGA cases. Parent `0a2d7b9414f58` makes both
  lifecycle suites explicitly opt-in instead of following `KUNIT_ALL_TESTS`.
  Earlier `9af4a8816f25` initializes the reset/import
  fixture's DCHS spinlock after booted `P91d6-Cad24` disproved the preceding
  final-fixture claim. Parent `f6ebe28a3f66` initializes the sibling abort
  fixture's DCHS spinlock and makes the observational MPP debug-state snapshot
  return `-EBUSY` instead of waiting indefinitely on a contended service mutex.
  Earlier `6b55e022ce49` initializes the session mutex in
  ...
  KUnit runs after every initcall, so the driver now
  ...
  Earlier `835b19f81d2b` moves the large MPP batch fixture to heap storage;
...
- branch `rk3588-rewrite-mainline`, commit `02bf372dac70` ("media: rockchip:
  rga-rewrite: fix librga DMA mapping"). It carries the same maintained rewrite
  ...
  debug-state fix `394d80552960`, mutex/nested-allocation fix `9aa6ef7e97b2`,
  and lifecycle fix `fac707773158`; earlier
  ...
  the earlier replay tip is preserved as
...
clean-archive `normal` profile passed at 6.18 `37ae7459656b` and mainline
`02bf372dac70`, building Rockchip and VSI IOMMU support, both KUnit-enabled
...
case-builder and comparator validation, including 143 GStreamer case builders.
That same run passed a syzlang ABI-marker check over 26 Rockchip markers; that
check and the description it reads have since moved to the private
`rock-5b-security` repository, so the runner here no longer performs it. The
...
`../rock-5b/build/rockchip-conformance` (`/home/yi/Code/rock-5b/build/rockchip-conformance` on the dev box),
...
The 2026-07-30 codec audit and mainline correction work in
...
used one shared Linux object store and separate clean worktrees:

| Ref | Pin | Role |
|-----|-----|------|
| `origin/master` | `3708dd9488440e35a165aee2bb2a1a7b1d0d5777` | Torvalds mainline on 2026-07-30 |
| `mainline-rkvdec-hantro-fixes-ready` | `c28b6586f74f7fb37c071174b66a445cf4ce0884` | seven prepared mainline-only RKVDEC/Hantro corrections |
...
The clean maxline audit checkout is
...
The prepared mainline correction checkout is
`/home/yi/Code/rock-5b/kernel/linux-mainline-codec-fixes`. Its source contents
are reconstructed from the recorded base and the repository's patch archive:

```bash
git worktree add ../linux-mainline-codec-fixes \
  3708dd9488440e35a165aee2bb2a1a7b1d0d5777
git -C ../linux-mainline-codec-fixes am \
