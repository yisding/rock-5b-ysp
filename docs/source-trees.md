...
| 8 | Clean-room rewrite drivers | [rewrite-driver track](../kernel-drivers/docs/rewrite-drivers.md) | local branch `rk3588-rewrite-6.18` @ `6aefb879f6e7` + branch `rk3588-rewrite-mainline` @ `ff0631beca29`, see §8 |
...
- branch `rk3588-rewrite-6.18`, commit `6aefb879f6e7` ("media: rockchip:
  cover legacy rga no-op ioctls"), committed in the dev worktree
  `/home/yi/Code/linux-6.18-rkvenc`.
- branch `rk3588-rewrite-mainline`, commit `ff0631beca29` ("media: rockchip:
  cover legacy rga no-op ioctls"),
...
dispatch KUnit coverage for current librga's post-blit compatibility path, and the
...
(`../linux-6.18-rkvenc@6aefb879f6e7` and `../linux@ff0631beca29`) passed that
