...
| 8 | Clean-room rewrite drivers | [rewrite-driver track](../kernel-drivers/docs/rewrite-drivers.md) | local branch `rk3588-rewrite-6.18` @ `02e6026c3a00` + branch `rk3588-rewrite-mainline` @ `60983387706b`, including display-tail BGRA partial-alpha-blend RGA3 KUnit coverage after the collector-level dormant MPP batch-server rejection and Route B RGA3 userptr IOMMU mapping/debugfs attribution slices; see §8 |
...
- branch `rk3588-rewrite-6.18`, commit `02e6026c3a00` ("media: rockchip:
  rga-rewrite: cover display alpha blend"), committed in the dev worktree
...
- branch `rk3588-rewrite-mainline`, commit `60983387706b` ("media: rockchip:
  rga-rewrite: cover display alpha blend"),
...
boundary, plus RGA3 display-tail BGRA partial alpha-blend command-emission
coverage for the opt-in public UI/display smoke shape. The 6.18 branch also has the forward-port recovery cleanup that moves
...
(`../kernel/linux-6.18-rkvenc@02e6026c3a00` and `../kernel/linux@60983387706b`) passed that
archive build gate warning-free on 2026-07-06 after the RGA3 display-tail
partial-alpha-blend coverage was added; the same pins also passed
`REWRITE_BUILD_PROFILES="memory race" kernel-drivers/tests/rewrite-build-gate.sh all`
warning-free for KASAN/fault-injection and KCSAN/lockdep compile coverage.
`VALIDATE_ONLY=1
