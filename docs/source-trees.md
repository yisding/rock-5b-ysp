...
| 8 | Clean-room rewrite drivers | [rewrite-driver track](../kernel-drivers/docs/rewrite-drivers.md) | `yisding/linux-rock5b` branch `rk3588-rewrite-6.18` @ `0d9d87ad57fa` + branch `rk3588-rewrite-mainline` @ `f05d39966e5d`, see §8 |
...
- branch `rk3588-rewrite-6.18`, commit `0d9d87ad57fa` ("media: rockchip:
  clarify rga legacy direct imports"), matching the clean dev worktree
  `/home/yi/Code/linux-6.18-rkvenc`.
- branch `rk3588-rewrite-mainline`, commit `f05d39966e5d` (same subject),
...
profiles, plus the RKVDEC2 CCU-mode update that keeps HARD opt-in while the
