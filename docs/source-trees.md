...
| 8 | Clean-room rewrite drivers | [rewrite-driver track](../kernel-drivers/docs/rewrite-drivers.md) | `yisding/linux-rock5b` branch `rk3588-rewrite-6.18` @ `20b5e5f09f5a` + branch `rk3588-rewrite-mainline` @ `fdfb6dfa8e6b`, see §8 |
...
- branch `rk3588-rewrite-6.18`, commit `20b5e5f09f5a` ("media: rockchip:
  cover rga3 border side copies"), matching the clean dev worktree
  `/home/yi/Code/linux-6.18-rkvenc`.
- branch `rk3588-rewrite-mainline`, commit `fdfb6dfa8e6b` (same subject),
