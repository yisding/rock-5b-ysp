...
| 8 | Clean-room rewrite drivers | [rewrite-driver track](../kernel-drivers/docs/rewrite-drivers.md) | `yisding/linux-rock5b` branch `rk3588-rewrite-6.18` @ `fe69cd59f9d5` + branch `rk3588-rewrite-mainline` @ `1c302a05c7ff`, see §8 |
...
- branch `rk3588-rewrite-6.18`, commit `fe69cd59f9d5` ("media: rockchip:
  test rewrite timing counter routing"), matching the clean dev worktree
  `/home/yi/Code/linux-6.18-rkvenc`.
- branch `rk3588-rewrite-mainline`, commit `1c302a05c7ff` (same subject),
...
plus KUnit coverage for per-core timing-counter routing.
