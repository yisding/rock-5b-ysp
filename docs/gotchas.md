...
| Userspace libraries | Rockchip `*-dma32` dma-heaps are about low DMA-addressable memory for 32-bit-limited hardware, **not** 32-bit ARM applications. Missing DMA32 heap names on the 6.18 forward-port are a BSP ABI/sample-compatibility gap, not a known correctness requirement for maintained MPP/RGA paths | [`vendor-libraries/docs/how-the-userspace-libs-work.md` § A5.1](../vendor-libraries/docs/how-the-userspace-libs-work.md), [`findings/2026-07-04-rga3-im2d-error-irq.md`](../findings/2026-07-04-rga3-im2d-error-irq.md) |
| RGA | `RGA3_core0 INTR[0x2]` is the **RGA MMU interrupt**, not the DMA32 heap problem. The copy/resize samples use malloc-backed virtual imports, so the live hypothesis is RGA3 + mainline Rockchip IOMMU glue; decide it by fault-IOVA logs and BSP parity for the exact same sample binaries | [`findings/2026-07-04-rga3-im2d-error-irq.md`](../findings/2026-07-04-rga3-im2d-error-irq.md), [`kernel-drivers/docs/forward-port-status.md`](../kernel-drivers/docs/forward-port-status.md) |
...
`kernel-drivers/scripts/install-combined-kernel.sh` so the installer matches the new deb.
