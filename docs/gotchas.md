...
| RGA | `RGA3_core0 INTR[0x2]` is the **RGA MMU interrupt**, not the DMA32 heap problem. The direct librga virtual-buffer samples exposed a forward Rockchip IOMMU gap: losing BSP's `dma_set_max_seg_size(..., DMA_BIT_MASK(32))` let `dma_map_sg()` return fragmented IOVA ranges even though RGA programs only the first segment as a single span | [`findings/2026-07-04-rga3-im2d-error-irq.md`](../findings/2026-07-04-rga3-im2d-error-irq.md), [`kernel-drivers/docs/forward-port-status.md`](../kernel-drivers/docs/forward-port-status.md) |
