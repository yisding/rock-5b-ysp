...
| C02 | Boot firmware and root-media selection | `TRACKED` | [`../boot-firmware/`](../boot-firmware/README.md) owns the stable U-Boot model/comparison; [`status.md` track 12](../status.md#dashboard) owns dated SPI, raw-SD, SD-root, and NVMe-root evidence. | Run the next gate in track 12 with UART output, exact image/build identity, component inventory, and a recoverable media backup. |
| C03 | Kernel delivery, boot, validation, and rollback | `TRACKED` | Status tracks 1, 3, 4, 9, and 13 distinguish the combined, DKMS, rewrite, PPA, and maximum-mainline paths; [`../install.md`](../install.md) owns the operator flow. The documented SD rescue + `kernel-revert.sh` commands are operator-validated for the forward-port path. | Complete each delivery path's own install/boot/functional gate; do not transfer the forward-port SD-recovery result to DKMS, rewrite, or maximum-mainline packages without testing them. |
...
| C12 | Audio devices, playback, and capture | `NARROW` | [`GRD audio redirection`](../apps/gnome-remote-desktop/docs/audio-redirection.md) records ES8316 ALSA enumeration, a live FluidSynth/PulseAudio playback stream, successful RDP PCM negotiation, and the split-stack failure that left GRD's native PipeWire graph without a sink. The PipeWire migration is live-validated as of 2026-07-21: installed `exp9` audibly renders PCM RDP output to a macOS client ([`status.md` W10](../status.md#watch-w10)). Compressed-audio interoperability and repeated focus/resume runs remain untested. | Install the complete PipeWire audio stack, then test RDP output and input plus each physically used playback/capture path for channel mapping, mute/volume, reconnect, and reboot persistence. |
...
| C16 | RK3588 NPU / RKNN + RKLLM runtimes | `UNASSESSED` | The [`RKNPU/RKNN guide`](../kernel-drivers/rknpu/docs/how-rknpu-works.md) owns a pinned source-level account of conversion, proprietary userspace, memory, submission, and kernel execution; the [`RKLLM survey`](../kernel-drivers/rknpu/docs/rkllm-large-language-models.md) documents the LLM sibling stack from public docs. Neither a CNN nor an LLM runtime job has been run on this distro. | Pin the RKNN or RKLLM runtime/model, record the render node or `/dev/rknpu` and driver identity (`≥ 0.9.6`), run a known-output sample (or a small `.rkllm` generation) on the selected core(s), and capture faults/tokens-per-second. |
...
| C19 | Package upgrade, coexistence, and recovery | `TRACKED` | Status tracks 9 and 10, [`../packaging/`](../packaging/README.md), and [`../install.md`](../install.md) own delivery and rollback constraints. The documented SD rescue + `kernel-revert.sh` flow has been used successfully by the operator. | Test clean migration and stale-package cleanup on the board; keep the known-good SD rescue media available for kernel changes. |
...
| C21 | Video post-processing and deinterlacing | `TRACKED` | The [`IEP2 project`](../kernel-drivers/iep2/README.md) owns the RK3588 IEP2 path. KASAN/lockdep package `Pcf86-Cc271` bound client 28 and passed focused TFF/BFF, decoder-vproc, negative, encoding-race, close/completion, and soak gates; the Published production `6.18.42` kernel also runs standalone IEP2. Field-parity testing found a separate libmpp I1O1 BFF bootstrap defect. | Runtime-verify the libmpp BFF fix, broaden mode/geometry/output-quality coverage, and retain the untriggered software-timeout path as an explicit fault-injection gap. |
