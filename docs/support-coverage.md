...
| C16 | RK3588 NPU / RKNN runtime | `UNASSESSED` | The [`RKNPU/RKNN guide`](../kernel-drivers/rknpu/docs/how-rknpu-works.md) owns a pinned source-level account of conversion, proprietary userspace, memory, submission, and kernel execution, but the repo has no runtime job on this distro. | Pin the RKNN runtime/model, record the render node or `/dev/rknpu` and driver identity, run a known-output sample on each selected core, and capture faults/performance. |
