...
| C21 | Video post-processing and deinterlacing | `UNASSESSED` | The [`IEP2 project`](../kernel-drivers/iep2/README.md) establishes that RK3588 has IEP2 rather than VDPP, records the BSP/libmpp path, and measures that the current forward-port kernel omits client 28. No deinterlaced output has been produced. | Forward-port and bind IEP2, exercise a known interlaced source through both a focused IEP2 test and libmpp decoder vproc, inspect field order/output, and capture faults plus reboot/soak behavior. |
