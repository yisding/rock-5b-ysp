...
| C03 | Kernel delivery, boot, validation, and rollback | `TRACKED` | Status tracks 1, 3, 4, 9, and 13 distinguish the combined, DKMS, rewrite, PPA, and maximum-mainline paths; [`../install.md`](../install.md) owns the validated operator flow. | Complete the board install/reboot/revert gate for each delivery path before describing it as usable. |
...
| C21 | Video post-processing and deinterlacing | `UNASSESSED` | The [`IEP2 project`](../kernel-drivers/iep2/README.md) establishes that RK3588 has IEP2 rather than VDPP and records the BSP/libmpp path. Its 6.18 source port and safety fixes build as KASAN package `Pcf86-Cc271`, but the installed kernel still omits client 28 and no deinterlaced output has been produced. | Boot `Pcf86-Cc271`, bind client 28, exercise a known interlaced source through both the focused test and libmpp decoder vproc, inspect field order/output, and capture recovery plus reboot/soak behavior. |
