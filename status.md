...
| 4 | Clean-room rewrite drivers | 🚧 Kernel #8 at `gf37186832202` booted and exposed residual decoder and RGA3 failures. The operator explicitly authorized source-only Phase 2/3 work without waiting for boot qualification. Maintained tips 6.18 `1784358650e3c41abbd5154cbd1737da07d968de` and mainline `e71d368cc48374638eafbfdf1e9c6864cabbd118` retain the Phase 2 owners plus Phase 3A–H activation checkpoints. Phase 3H makes the active-slot claim token own the exact containing-job reference and carries typed core or hard-CCU group/core recovery proof through recovered error-IRQ, timeout, fault, abort/close, remove/shutdown, and dependent terminal paths. Exact restore refusal transfers that reference and generation into a service quarantine tombstone, retains resources and the dispatch lease, closes selected-core/owned-CCU admission, blocks remove teardown, and lets shutdown retain ownership until reboot. Clean IRQ/`CCU_DONE` and pre-doorbell `START_FAILURE` keep the Phase 3G legacy release boundary; there is still no `RECLAIMABLE`, general clean retirement, or final outcome arbitration. The tips pass strict checkpatch, byte identity, the exact 103 MPP + 152 RGA manifest, the 2,068-signal production ownership inventory, unchanged 306-signal KUnit-debt audit, all eight warning-fatal clean-archive profiles, and the repository-wide handoff gate. This is source/compile evidence only. The predecessor exact 6.18 package P692f with source stamp `(gab69ece99864)` remains uninstalled and unbooted; no package or boot contains Phase 2 or Phase 3. Runtime KUnit, decoder/reset-contention/recovery, solo RGA3 vpp, overlay-chain, AV1/VSI, performance, fuzz, and soak qualification remain open. The separate RGA2 plain-system-heap 1 MiB SWIOTLB staging design remains deliberately unimplemented while active ownership changes. | 2026-08-09 | [Phase 3H recovered-terminal quarantine](./findings/2026-08-09-rewrite-phase-3h-mpp-recovered-terminal-quarantine.md), [Phase 3G retry retirement proof](./findings/2026-08-09-rewrite-phase-3g-mpp-retry-retirement-proof.md), [ownership refactor plan](./kernel-drivers/docs/rewrite-ownership-refactor-plan.md), [RGA2 staging design](./findings/2026-08-08-rewrite-rga2-dmabuf-staging-design.md), [rga3 vpp corruption](./findings/2026-08-07-rga3-cross-process-vpp-corruption-lead.md), [overlay-chain gaps](./findings/2026-08-07-rewrite-rga-blend-chain-swiotlb-and-rga3-iommu-fault.md) |
...
| 4 | Clean-room rewrite drivers | Build/package/install/boot the exact Phase 3H tip, require all 255 KUnit cases and a fatal-free lockdep interval, and force the restore-refusal quarantine plus same-session H.26x, dual-core reset contention/recovery, solo RGA3 vpp, and overlay-chain cases. Before outcome arbitration, extend typed closure to clean IRQ/`CCU_DONE` and pre-doorbell `START_FAILURE`, then define `RECLAIMABLE` only after every async/resource owner drains. | [Phase 3H boundary](./findings/2026-08-09-rewrite-phase-3h-mpp-recovered-terminal-quarantine.md#boundary-and-next-gate), [ownership refactor plan](./kernel-drivers/docs/rewrite-ownership-refactor-plan.md), [RGA3 fixed-tip gate](./findings/2026-08-08-rga3-cross-process-contention-harness-plan.md#verification-gate), [RGA2 staging design](./findings/2026-08-08-rewrite-rga2-dmabuf-staging-design.md#verification-gate), [overlay-chain boundary](./findings/2026-08-07-rewrite-rga-blend-chain-swiotlb-and-rga3-iommu-fault.md#boundary), [rewrite validation plan](./kernel-drivers/docs/rewrite-validation-plan.md) |
