...
| 5 | ffmpeg tree | ⚠️ `ffmpeg-rockchip-81` builds and packages. The AV1 MP4/MKV fix (`be367abfe6`) and decoder `pix_fmts` cleanup (`8356739686`) are committed and pushed directly on `main`; `refactor/section-c` merged the new main and remains PR #1. The decoder, `ffmpeg`, and focused FATE test pass. Packaging now exports `main@be367abfe6` with no quilt series; the signed source was accepted as pending publication `18615674`. RK3588 MP4/MKV hardware re-validation remains pending. | 2026-07-11 | [Kodi/ffmpeg finding](./findings/2026-07-11-kodi-ffmpeg-rockchip-hwaccel.md) |
...
| 9 | Launchpad PPA | ⚠️ PPA work is in progress, but the full stack is not installable yet: MPP and librga source + arm64 binaries are public; Rockchip-81 FFmpeg `~rk5` built successfully, and its replacement from `main@be367abfe6` was accepted as pending source publication `18615674` with no build record yet. Co-installable `ffmpeg-rockchip` build `33387375` succeeded. Kernel `~rk2` retries remain pending/building, while GRD and the optional GDM greeter ACL remain held. Kernel board install/revert validation is pending. | 2026-07-11 | [`packaging/ppa/`](packaging/ppa/README.md) |
...
| Launchpad PPA publication state | Source acceptance, build state, and binary publication can change after upload without local file changes | 2026-07-11 | Launchpad API check 21:10 PDT: `main@be367abfe6` FFmpeg version `7:8.1.2+rockchip81+git20260711.be367abfe6-0ubuntu1~rk1` is pending source publication [`18615674`](https://launchpad.net/~yi-ding/+archive/ubuntu/ubuntu-rock-5b/+sourcepub/18615674), with no build record yet. Previous `~rk5` is Published and its arm64 build succeeded. MPP `1375813c` + librga arm64 binaries are public. |
...
| `ffmpeg-rockchip-81` tip | Main carries correctness fixes; refactor work remains isolated in PR #1. | 2026-07-11 | `main` tip `be367abfe6`; `refactor/section-c` tip `844d95e047`, with updated main merged and [PR #1](https://github.com/yisding/ffmpeg-rockchip-81/pull/1) clean. PPA source exports the exact main tip. |
