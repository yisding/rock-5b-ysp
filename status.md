...
| 4 | Clean-room rewrite drivers | 🚧 Kernel #8 at `gf37186832202` booted and exposed residual decoder and RGA3 failures. The provisional Phase 1 boundary is source-complete, and the operator explicitly authorized source-only Phase 2 work without waiting for boot qualification. Maintained tips 6.18 `53a7fa1acbc00` and mainline `ba8e11de18a8e` construct stable MPP reset-domain identity and membership, route complete single-target power-deassert and recovery pulses through domain state and a nonzero epoch, and retain the old hard-CCU group pulse for the cluster follow-up. They pass strict checkpatch, byte identity, exact 96 MPP + 152 RGA manifests, the 564-signal production ownership inventory, the unchanged 306-signal KUnit-debt audit, and warning-fatal clean-archive `normal`, `test-disabled`, KASAN/fault-injection `memory`, and KCSAN/lockdep `race` builds of both IOMMU providers, rewrite objects, and Rock 5B DTB. This is compile/source evidence only. The predecessor exact 6.18 package P692f with source stamp `(gab69ece99864)` remains uninstalled and unbooted; no package or boot contains Phase 2. Runtime KUnit, decoder/reset-contention/recovery, solo RGA3 vpp, overlay-chain, AV1/VSI, performance, fuzz, and soak qualification remain open. The separate RGA2 plain-system-heap 1 MiB SWIOTLB staging design remains deliberately unimplemented while active ownership changes. | 2026-08-08 | [Phase 2 finding](./findings/2026-08-08-rewrite-phase-2-reset-domain-construction.md), [Phase 1 build finding](./findings/2026-08-08-rewrite-phase-1-focused-build-gate.md), [ownership refactor plan](./kernel-drivers/docs/rewrite-ownership-refactor-plan.md), [RGA2 staging design](./findings/2026-08-08-rewrite-rga2-dmabuf-staging-design.md), [rga3 vpp corruption](./findings/2026-08-07-rga3-cross-process-vpp-corruption-lead.md), [overlay-chain gaps](./findings/2026-08-07-rewrite-rga-blend-chain-swiotlb-and-rga3-iommu-fault.md) |
...
| 4 | Clean-room rewrite drivers | Continue Phase 2 with read-only `rk_mpp_cluster` topology construction, then migrate the hard-CCU participant pulse as one non-interleavable domain transaction before moving group power or admission. Keep the source-only boundary explicit: separately build/package/install/boot the exact tip, require all 248 KUnit cases and a fatal-free lockdep interval, and replay same-session H.26x, dual-core reset contention/recovery, solo RGA3 vpp, and overlay-chain cases. Terminal arbitration still waits for a retained generation-tagged activation/task execution, not a stateless job wrapper. | [Phase 2 gate](./findings/2026-08-08-rewrite-phase-2-reset-domain-construction.md#verification-gate), [ownership refactor plan](./kernel-drivers/docs/rewrite-ownership-refactor-plan.md), [RGA3 fixed-tip gate](./findings/2026-08-08-rga3-cross-process-contention-harness-plan.md#verification-gate), [RGA2 staging design](./findings/2026-08-08-rewrite-rga2-dmabuf-staging-design.md#verification-gate), [overlay-chain boundary](./findings/2026-08-07-rewrite-rga-blend-chain-swiotlb-and-rga3-iommu-fault.md#boundary), [rewrite validation plan](./kernel-drivers/docs/rewrite-validation-plan.md) |
