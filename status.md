...
| 4 | Clean-room rewrite drivers | 🚧 The isolated 6.18 AV1 line is `rk3588-rewrite-av1-6.18@e58c57e50d0a0`, based on the complete shared rewrite through `rk3588-rewrite-6.18@c5faabf9d00b0`; the shared mainline pin remains `rk3588-rewrite-mainline@7481ab327d7e`. Three independent adversarial reviews found and fixed five gaps: VSI refresh after masked faults, fault-vs-completion ownership, AFBC level-IRQ quiescence, post-offset DMA provenance, and full AFBC output capacity. Raw AFBC MMIO is now kernel-owned, all 103 real region-based translation indices are exercised, and discriminating regressions cover the races and boundaries. The AV1 branch now has **237 cases** (90 MPP + 147 RGA). At the committed tip, focused GCC/Clang builds and clean-archive `normal`, KASAN/fault-injection `memory`, and KCSAN/lockdep `race` profiles pass; strict patch check reports 0 errors/warnings/checks. **It has not been packaged, installed, booted, or run on the ROCK 5B**, so there is no booted 237-case report, direct AV1 decode result, AFBC result, injected two-fault recovery evidence, or hardware parity claim. | 2026-07-26 | [adversarial validation](./findings/2026-07-26-av1-rewrite-adversarial-validation.md), [AV1 rewrite assessment](./kernel-drivers/av1/docs/av1-rewrite-assessment.md), [conformance-gap audit](./kernel-drivers/docs/rewrite-conformance-gap-audit.md) |
...
| 9 | Launchpad PPA | ⚠️ The normal PPA now has a client-side-complete source upload for the 6.18.40 forward-port kernel `6.18.40+rk3588av1fwport20260725-0ubuntu1~rk1`; Launchpad publication and arm64 build are pending confirmation. GRD 50.2 source `18632058` is accepted/Pending and arm64 build `33422570` is running. The rest of the normal system stack, both dedicated FFmpeg comparisons, both rewrite-kernel replacements, and experimental GRD `~exp3` have Published sources and successful arm64 builds. Optional GDM upload and board migration/kernel/GRD runtime gates remain open. | 2026-07-25 | [`packaging/ppa/`](packaging/ppa/README.md) |
...
|---|-------|------------|-------------|
| 1 | Kernel forward-port | `0074` and `0075` hardware gates are closed on the booted 2026-07-25 KASAN build: RGA 10-bit stride/UV-offset, fresh-librga P010/NV15, forced `split_arg=4` H.264/H.265 slice overflow, KASAN MPP, ioctl fuzz, and root gates are green. The production source package `6.18.40+rk3588av1fwport20260725-0ubuntu1~rk1` is built, signed, source-verified, and `dput`-uploaded client-side. Remaining: confirm Launchpad source publication and arm64 build, install it from the PPA, validate conformance/rollback, and keep broad GStreamer/librga userspace/environment failures outside the kernel verdict. | [6.18.40 KASAN validation](./findings/2026-07-25-forward-port-6-18-40-kasan-full-validation.md), [UV-offset finding](./findings/2026-07-24-rga-10bit-uv-plane-offset-still-pixel-scaled.md), [slice-FIFO finding](./findings/2026-07-20-rkvenc2-slice-fifo-terminal-drop.md) |
| 2 | BSP-audit fixes | Correctness + destructive `0058`-`0068` gates are green on booted `Pabd5-C4ad2`; `0069`/`0070` (mm_session UAF) verified on `Pc1f8-C9fc5`. The previously-skipped **root-only gates were run on `Pc1f8` 2026-07-23**: encoder ✓, transcode ✓, rga-mmu-debug ✓ (the `0070` mm_session gate — no D-state wedge), vp9-show-existing ✓ (board survived; the crash's proven fix is `0057`, clientless-RELEASE_FD reproducer returns `-EINVAL`), and iommu-machinery-fuzz surfaced a **new** scattered-userptr zero-output bug now fixed as `0071` (reject). **`0071` is now runtime-verified** on a booted KASAN `#8` build (`av1-fwport@4401383a6d9b5`, tail `0001`–`0071`, built 2026-07-23 07:39): the `rga-iommu-fuzz` scatter probe shows the driver rejecting a non-16-aligned IOMMU base (`-EINVAL`) instead of silent zero output, and the fuzz oracle was corrected to expect that reject ([validation run](./findings/2026-07-23-forward-port-current-tip-full-validation-run.md)). The root-only gates were re-run green on this `0071` tip (`20260723-141619`): encoder/transcode/rga-mmu/iommu-fuzz PASS, `mpp-debug-capture` expected SKIP, and VP9 `show_existing_frame` survived clean (30×4 loops, `flagged_kernel_lines=0`). The production (non-KASAN) Launchpad build of this tail (`…20260723~rk1`) was installed from the PPA, booted, and **passed the full conformance set + root gates on 2026-07-24** incl. a clean VP9 `show_existing_frame` survival and perf numbers ([production run](./findings/2026-07-24-production-ppa-kernel-full-conformance-run.md)). Remaining: validate rollback. | [Port record](./findings/2026-07-22-bsp-high-current-tip-port.md), [list_add finding](./findings/2026-07-22-mpp-process-request-list-add-double-add-warn.md), [runtime gate inventory](./kernel-drivers/patches/cleanup-draft/verification.md#runtime-gate-result-record-here-when-run) |
...
| 4 | Clean-room rewrite drivers | Package the adversarially reviewed AV1 tip `e58c57e50d0a0`, boot KASAN and KCSAN variants, persist 237 green KUnit results, then require bit-exact AV1 raster decode, 8/10-bit AFBC, VSI fault/timeout recovery, paired clean-dmesg/counter/artifact evidence, AVS2, and H.264/H.265 low-delay slice polling. Carry the AV1 commits to mainline only after the 6.18 hardware baseline is understood. | [adversarial AV1 validation](./findings/2026-07-26-av1-rewrite-adversarial-validation.md), [remaining rewrite hardware gates](./kernel-drivers/docs/rewrite-conformance-gap-audit.md#remaining-gaps-and-hardware-gates) |
...
| 9 | Launchpad PPA | Confirm the 6.18.40 forward-port source is accepted/Published and the arm64 build succeeds, then install, boot, and revert the co-installable kernel on the ROCK 5B. | [Kernel package checklist](./packaging/ppa/kernel-forward-port/README.md#remaining-checklist) |
