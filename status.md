...
| 7 | GNOME Remote Desktop backend | 🚧 Installed `50.2+rkmpp+git20260721.13.cf60b4d` reaches GDM handover and AVC420 negotiation, then its first RKMPP H.264 smoke frame kills the encoder worker with a kernel oops before encoder hardware start — 3/3 production boots, so there is **no login video on the production kernel**. Confirmed a kernel-config defect, not a GRD one: `CONFIG_DMABUF_DEBUG` scrambles the scatterlist the system heap then syncs. No GRD defect is implicated. Two 2026-07-28 logins on a kernel without the option were clean end to end, with `Created h264_rkmpp encode session … (encoder dimensions 2064x1296)` for both greeter and user session. Sustained focus/resume and audio gates unblock once the config fix is boot-verified on production. | 2026-07-28 | [root cause + fix](./findings/2026-07-28-dmabuf-debug-mangle-sg-table-is-the-sg-writer.md), [third repro](./findings/2026-07-27-grd-sg-oops-third-repro-ioctl-window-measured.md), [oops trace](./findings/2026-07-27-grd-rkmpp-system-heap-sg-corruption-oops.md), [testing](./apps/gnome-remote-desktop/docs/testing.md) |
...
| 7 | GNOME Remote Desktop backend | Blocked on track 1, but now only on a rebuild: re-run the focus/resume and audio gates on a production kernel built without `CONFIG_DMABUF_DEBUG`. They can meanwhile be exercised on any kernel that lacks the option — the KASAN image passes this path 9/9 — provided results are recorded as that kernel's evidence rather than production's. | [focus/resume gate](./apps/gnome-remote-desktop/docs/testing.md#10-exp6exp7-macos-focusresume-gate), [root cause + fix](./findings/2026-07-28-dmabuf-debug-mangle-sg-table-is-the-sg-writer.md#verification-gate) |
