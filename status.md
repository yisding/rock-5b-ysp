...
| 9 | Launchpad PPA | ⚠️ PPA work is in progress, but the full stack is not installable yet: MPP and librga source + arm64 binaries are public; baseline FFmpeg build `33381225` succeeded; Rockchip-81 FFmpeg `~rk2` source `18614542` published but arm64 build `33387355` failed on unsupported `--disable-omx`; `~rk3` source `18614555` is pending with arm64 build `33387380` currently building; co-installable `ffmpeg-rockchip` source `18614552` is pending and arm64 build `33387375` succeeded. Forward-port kernel source `18614540` is published but arm64 build `33387353` failed on missing `mkimage`; alpha rewrite kernel sources `18614549` and `18614550` are pending with arm64 builds `33387366` and `33387367` currently building. GRD and the optional GDM greeter ACL remain held. Kernel board install/revert validation is pending. | 2026-07-10 | [`packaging/ppa/`](packaging/ppa/README.md) |
...
| Launchpad PPA publication state | Source acceptance, build state, and binary publication can change after upload without local file changes | 2026-07-10 | Launchpad API check at 23:30 PDT: Rockchip-81 FFmpeg `~rk2` source `18614542` is `Published` but arm64 build `33387355` failed at configure on unsupported `--disable-omx`; replacement `~rk3` source `18614555` is `Pending` with arm64 build `33387380` `Currently building` on `bos03-arm64-014`; co-installable `ffmpeg-rockchip` source `18614552` is `Pending` with arm64 build `33387375` `Successfully built`. Forward-port kernel source `18614540` is `Published` but arm64 build `33387353` `Failed to build` on missing `mkimage`; alpha kernel sources `18614549` and `18614550` are `Pending` with arm64 builds `33387366` and `33387367` `Currently building`. MPP and librga source + arm64 binaries were already public. GRD and GDM ACL are not uploaded. Re-check before telling users to install from the PPA. |
