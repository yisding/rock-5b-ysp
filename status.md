...
| 4 | Clean-room rewrite drivers | Replace the job-owned hard-CCU coordinator/powered-core flags and array with one cluster power lease while preserving the exact acquire/transfer/release order; do not fold descriptor admission or the reset/IOMMU recovery result into that mechanical migration. Keep the source-only boundary explicit: separately build/package/install/boot the exact tip, require all 251 KUnit cases and a fatal-free lockdep interval, and replay same-session H.26x, dual-core reset contention/recovery, solo RGA3 vpp, and overlay-chain cases. Terminal arbitration still waits for a retained generation-tagged activation/task execution, not a stateless job wrapper. | [group-reset gate](./findings/2026-08-08-rewrite-phase-2-hard-ccu-reset-ownership.md#verification-gate), [ownership refactor plan](./kernel-drivers/docs/rewrite-ownership-refactor-plan.md), [RGA3 fixed-tip gate](./findings/2026-08-08-rga3-cross-process-contention-harness-plan.md#verification-gate), [RGA2 staging design](./findings/2026-08-08-rewrite-rga2-dmabuf-staging-design.md#verification-gate), [overlay-chain boundary](./findings/2026-08-07-rewrite-rga-blend-chain-swiotlb-and-rga3-iommu-fault.md#boundary), [rewrite validation plan](./kernel-drivers/docs/rewrite-validation-plan.md) |
