...
| 9 | Launchpad PPA | ⚠️ The latest forward-port kernel source, `6.18.41+rk3588av1fwport20260802-0ubuntu1~rk1`, is Published as source `18654047`; arm64 build `33461848` is currently building. The preceding `20260801` kernel source `18652965` is Published and arm64 build `33460058` succeeded. The normal stack and comparison/rewrite archives otherwise retain their package-specific recorded states; exact kernel install, board validation, and rollback remain open. | 2026-08-02 | [`packaging/ppa/`](packaging/ppa/README.md) |
...
|---|-------|------------|-------------|
| 1 | Kernel forward-port | Confirm arm64 build `33461848` succeeds and publishes the `…20260802~rk1` binaries; install that exact package, verify boot identity, then run MPP/FFmpeg, librga/RGA, GStreamer, ABI, RDP encode, fatal-journal, and rollback gates. | [kernel package checklist](./packaging/ppa/kernel-forward-port/README.md#remaining-checklist) |
...
| 9 | Launchpad PPA | Confirm arm64 build `33461848` succeeds and publishes binaries, then install, boot, validate, and revert the co-installable kernel on the ROCK 5B. | [kernel package checklist](./packaging/ppa/kernel-forward-port/README.md#remaining-checklist) |
...
| W05 | [Launchpad PPA publication](#watch-w05) | 2026-08-02 | Forward-port kernel source `18654047` is Published and arm64 build `33461848` is currently building. Its `20260801` predecessor is Published as source `18652965` with successful arm64 build `33460058`. Other packages retain their package-specific recorded states. |
...
| W16 | [Forward-port kernel-fix tail](#watch-w16) | 2026-08-02 | The exported tail is contiguous `0001`–`0087` at public GitHub tip `5b87d46eefdcb`. Signed `6.18.41+rk3588av1fwport20260802-0ubuntu1~rk1` passed patch-only/source-provenance checks; source `18654047` is Published, build `33461848` is currently building, and all seven-tail runtime gates remain pending. The preceding `0001`–`0080` source is Published and built successfully. |
...
| W18 | [rockchip-vaapi fork state](#watch-w18) | 2026-08-02 | Fork tip is `main@aee5926`; `df14bb6` supersedes the Firefox Main10 diagnosis — the invalid `GR16` chroma fourcc was ours, both chroma-retry patches are deleted, and `ysp7` imports zero-copy in both Firefox processes. Upstream remains `e8c64dd` and unrevived. Installed driver is `1.0.11+ysp5`; signed final source `1.0.11+ysp6-0ubuntu1~rk1` and its exact binaries pass source/binary, Lintian-error, and isolated lifecycle gates but are not uploaded or installed. Exact Published MPP/FFmpeg binaries pass the isolated complete sweep, normal/ASan matrices, and a 7,200-second/216,005-frame 4K decode soak with no RSS or fd growth, but still need host installation. Firefox packaging/runtime, physical HDR/mpv, Chromium GL, clean-image install, and P010 encode remain open. |
...
- **State 2026-08-02:** The maintained/public branch and exported series are
  ...
  PDT. Launchpad source `18654047` is Published and arm64 build `33461848` is
  currently building. All booted hardware gates for `0081`–`0087` remain
