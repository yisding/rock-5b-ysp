...
| 9 | Launchpad PPA | ⚠️ The normal PPA provides the complete nine-source system stack. Forward-port Linux **6.18.51** / exact `0001`–`0097` is Published as source publication `18730790`, its arm64 build `33593313` succeeded, and all three binaries are Published, making it the archive's install candidate. It has not been installed or booted. Its 6.18.44 predecessor built successfully and is now Superseded. The last Published, installed, and booted kernel remains `20260808` / 6.18.43 / `0096`, whose conformance stops at one RGA2 USERPTR librga failure. Two experimental kernels were also uploaded on 2026-09-13: the `7.3-rc2` mainline rewrite alpha to `rock5b-kernel72rc2-rewrite` and the `6.18.51` rewrite alpha to `rock5b-kernel618-rewrite`; both are source-only with no compile or boot evidence. Maximum-mainline `public` and `wip` reached a PPA for the first time, accepted as `Pending` source publications in `ubuntu-rock-5b-experimental`. The exact Published FFmpeg package is installed; VA-API still needs exact PPA-binary replay. [W05](#watch-w05) owns live publication identities. | 2026-09-13 | [update finding](./findings/2026-09-13-kernel-lines-updated-to-latest-upstreams.md), [PPA kernel record](./packaging/ppa/kernel-forward-port/README.md), [0097 finding](./findings/2026-08-11-forward-port-rga2-contiguous-userptr-rejection.md) |
...
| 13 | Maximum-mainline kernel | Run the `wip` full-tree compile, then confirm the Launchpad builds of both uploaded profiles, install `public` with serial recovery, and prove boot, storage, network, display, suspend, and rollback before `wip`. | [recovery-first test order](./kernel-versions/maxline/README.md#install-and-test-order) |
...
| W05 | [Launchpad PPA publication](#watch-w05) | 2026-09-13 | Forward-port 6.18.51 Published and built, all three binaries Published. Four experimental sources accepted. The installed/booted artifact is still `20260808`. |
...
- **State 2026-09-13:** Five kernel source uploads, each verified against the
  ...
  - `rock5b-kernel618-rewrite`: `6.18.51+rk3588rewritealpha20260913-0ubuntu1`
    is accepted and `Pending`.
  - `ubuntu-rock-5b-experimental`: both maxline profiles,
    `7.3.0~rc2+git20260913+rk3588maxline{public,wip}-0ubuntu1`, are accepted
    and `Pending` — the first maxline packages ever to reach an archive.
  No arm64 build result exists for any of these four experimental sources.
  A successful `dput` proves client-side transfer only; the
