...
| 4 | Clean-room rewrite drivers | 🚧 The conformance freeze occurred in the pre-workload debug-state snapshot after an earlier KUnit/ABI fault abandoned an MPP service mutex; the official MPP cases had not started. Current byte-identical tips 6.18 `f6ebe28a3f66` and mainline `394d80552960` repair the final observed fixture lock, make only the observational state reader fail fast, and add an automatic fatal-interval/live-lockdep gate before ABI/media work. Current 6.18 KASAN/lockdep package `P91d6-Cad24` is built and payload-verified but not installed or booted. | 2026-07-27 | [freeze attribution, repair, and package](./findings/2026-07-27-rewrite-mpp-preflight-freeze.md) |
...
| 4 | Clean-room rewrite drivers | Boot `P91d6-Cad24`; require exact 233-case KTAP, the new clean complete-interval/live-lockdep artifact, a clean aged kmemleak scan, restored runtimes and every expected core before isolated ABI replay and full conformance. | [compound next gate](./findings/2026-07-27-rewrite-mpp-preflight-freeze.md#current-kasan-package) |
