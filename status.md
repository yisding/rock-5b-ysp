...
| 5 | ffmpeg tree | 🚧 The intermittent H.26x full-suite corruption is root-caused to the rewrite driver's same-session dual-core rkvdec dispatch (MPP `a8b19653` and the FFmpeg lifetime fix are exonerated by a 9/20-vs-9/20 differential with a 0/20 `-fast_parse 0` control); a three-commit driver fix through `f37186832202` is committed and compile-verified, with the rebuilt kernel's boot + 40/40 repro + full-suite replay gate still pending. Its published/install state belongs to [W05](#watch-w05); installed-package GRD fallback/recreation and AV1 container validation remain open. | 2026-08-07 | [dispatch-race finding](./findings/2026-08-07-rewrite-mpp-same-session-dual-core-dispatch-race.md), [intermittent decode finding](./findings/2026-08-06-rewrite-kasan-media-suite-userspace-fixes-and-intermittent-h264.md), [lifetime fix and gate](./findings/2026-07-30-ffmpeg-rkmpp-async-frame-lifetime-fix.md), [FFmpeg project](./video-libraries/ffmpeg/README.md) |
