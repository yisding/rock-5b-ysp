...
| 9 | Launchpad PPA | ✅ **2026-08-05:** the last API recheck found all nine normal-PPA sources Published. The forward-port kernel milestone is closed: `6.18.42+rk3588av1fwport20260804-0ubuntu1~rk1` (`0092`) source/build and all three arm64 packages are live, and the exact image, DTB, and headers are installed and previously booted through broad production validation. MPP `1.5.0+git20260805.a8b19653+ds-0ubuntu1~rk1` is live, installed, and runtime-validated through its retained VP9 oracle, 120-process stress, four-codec differential, and 12/12 official suite. VA-API ysp13 source/build and driver/config binaries are also live; its same-version local build is validated and installed, while exact PPA-binary replay remains track 14's gate. After that recheck, the signed FFmpeg `c9428bedaa` source upload completed client-side; its Launchpad acceptance/build/publication state was deliberately not queried. The documented SD rescue + `kernel-revert.sh` path is operator-validated. The clean-install transaction remains a separate migration test for machines carrying incompatible stacks. | 2026-08-05 | [`packaging/ppa/`](packaging/ppa/README.md), [MPP package closure](./findings/2026-08-04-libmpp-vp9-show-existing-reference-slot-leak.md#installed-ppa-package-closure), [kernel production validation](./findings/2026-08-04-forward-port-6-18-42-0092-production-validation.md), [recovery finding](./findings/2026-08-04-forward-port-sd-rescue-rollback-used.md) |
...
| 9 | Launchpad PPA | Confirm Launchpad accepted, built, and published the uploaded FFmpeg `c9428bedaa` source; then install it and replay its GRD fallback/recreation gate. Refresh and exercise the clean-install transaction separately only when a machine actually migrates from an incompatible stack. | [PPA package matrix and migration path](./packaging/ppa/README.md#current-state), [FFmpeg integration gate](./findings/2026-07-30-ffmpeg-rkmpp-async-frame-lifetime-fix.md#verification-gate) |
