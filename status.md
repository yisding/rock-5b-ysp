...
| 5 | ffmpeg tree | ⚠️ `ffmpeg-rockchip-81` forward-port at `75638e7f0b17` builds and packages. MPP runtime resolved: the board's stock MPP (`750e76e`) fails parser registration, but the PPA MPP (`1375813c` = tag 1.0.12) is installed and `h264_rkmpp`/`hevc_rkmpp` decode is verified. Three fork-packaging bugs fixed (rkmpp `pix_fmts`/FATE, `RELEASE_NOTES`, `ffmpeg-doc` arch) and a full arch+indep build validated locally. Open: `av1_rkmpp` drops the mp4/mkv sequence header (works from IVF). | 2026-07-11 | [Kodi/ffmpeg finding](./findings/2026-07-11-kodi-ffmpeg-rockchip-hwaccel.md) |
...
| 9 | Launchpad PPA | ⚠️ PPA work is in progress, but the full stack is not installable yet: MPP and librga source + arm64 binaries are public; baseline FFmpeg build `33381225` succeeded; Rockchip-81 FFmpeg is now at `~rk5` after fixing three fork-packaging bugs (`~rk3` failed FATE, `~rk4` failed on `RELEASE_NOTES` then `ffmpeg-doc` arch) — `~rk5` **built successfully** on Launchpad (binaries publishing), validated locally as a full arch+indep build emitting every deb; co-installable `ffmpeg-rockchip` source `18614552` is pending and arm64 build `33387375` succeeded. Kernel `~rk2` retries add `u-boot-tools`: forward-port source `18614559` build `33387391`, alpha 6.18 source `18614560` build `33387392`, and alpha 7.2-rc2 source `18614561` build `33387393` are all pending/currently building. GRD and the optional GDM greeter ACL remain held. Kernel board install/revert validation is pending. | 2026-07-11 | [`packaging/ppa/`](packaging/ppa/README.md) |
...
| Launchpad PPA publication state | Source acceptance, build state, and binary publication can change after upload without local file changes | 2026-07-11 | Launchpad API check ~09:20 PDT: Rockchip-81 FFmpeg `~rk3` (FATE) and `~rk4` (build `33387871`, `ffmpeg-doc` arch) both `Failed to build`; `~rk5` source `Published` and arm64 build **`Successfully built`** (binaries publishing). MPP `1375813c` + librga arm64 binaries are public and installed on the builder. Co-installable `ffmpeg-rockchip` `18614552` build `33387375` `Successfully built`. GRD and GDM ACL not uploaded. Confirm the `~rk5` `libavcodec63`/`libav*` binaries have published in the arm64 index before telling users to install the fork ffmpeg from the PPA. |
...
| `ffmpeg-rockchip-81` tip | It has moved mid-documentation several times; package validation used the local branch `refactor/section-c`, not the older status-row tip. | 2026-07-11 | `75638e7f0b17`; PPA `ffmpeg` now carries the fork quilt patch `0001-rkmppdec-do-not-advertise-decoder-pix_fmts.patch` plus the `RELEASE_NOTES`/`ffmpeg-doc` packaging fixes. Failure ledgers: [`findings/2026-07-11-kodi-ffmpeg-rockchip-hwaccel.md`](findings/2026-07-11-kodi-ffmpeg-rockchip-hwaccel.md), [`findings/2026-07-06-ffmpeg-rockchip81-package-validation.md`](findings/2026-07-06-ffmpeg-rockchip81-package-validation.md) |
| `av1_rkmpp` container extradata | Kodi AV1 playback (mp4/mkv) will fail until the fork feeds the container sequence header to MPP | 2026-07-11 | `av1_rkmpp` decodes IVF (in-band) but fails mp4/mkv with `No sequence header available`; h264/hevc unaffected. Fork bug; fix = an av1 bsf / send `av1C` OBUs. [finding §3](findings/2026-07-11-kodi-ffmpeg-rockchip-hwaccel.md) |
