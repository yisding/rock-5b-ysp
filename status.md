...
| 4 | Clean-room rewrite drivers | 🚧 Active bring-up: current heads have clean normal build coverage and broad device-free validation exists, but there is still no booted hardware-validation record. | 2026-07-06 | [rewrite-driver track](./kernel-drivers/docs/rewrite-drivers.md) §6 |
...
| 9 | Launchpad PPA | ⚠️ Userspace PPA is in progress, but not installable yet: `mpp`, `librga`, `ffmpeg`, and GRD source packaging are imported; public APT currently has MPP/librga source only, empty binary indexes, no public FFmpeg/GRD source, FFmpeg baseline `Pending` in Launchpad API, and Rockchip-81 FFmpeg/GRD held. | 2026-07-06 | [`packaging/ppa/`](packaging/ppa/README.md) |
...
presentation scoreboard plus the watchlist of facts that can change silently.
...
| Launchpad PPA publication state | Source acceptance, build state, and binary publication can change after upload without local file changes | 2026-07-06 | Public APT/API check at `2026-07-06T16:43:41-07:00`: public source index has MPP and librga `-0ubuntu2~rk1`; librga `-0ubuntu3~rk1` is still `Pending` in Launchpad API; arm64/amd64 binary indexes are empty; FFmpeg is absent from the public APT index while the upstream FFmpeg baseline source is `Pending` in Launchpad API; Rockchip-81 FFmpeg held, GRD not uploaded. Re-check before telling users to install from the PPA. |
...
| Repository-wide license decision | A public conference/release handoff needs a clear license; until then the repo is presentable as an integration record, not a redistributable release | 2026-07-06 | No repository-wide license granted; current boundary recorded in [`LICENSE.md`](LICENSE.md). |
| **Dev-box-only artifacts** (single point of failure) | Loss = unrecoverable: GRD async-PBO/MemFd prototype worktrees and the headless-harness driver script. The `mpp`/`librga`/`ffmpeg`/GRD PPA packaging and the rewrite-driver branches are no longer in this bucket. | 2026-07-06 | Remaining capture actions live in [`apps/gnome-remote-desktop/docs/baseline.md`](./apps/gnome-remote-desktop/docs/baseline.md) §7 and [`apps/gnome-remote-desktop/docs/profiling.md`](./apps/gnome-remote-desktop/docs/profiling.md) §4; PPA-only gaps are tracked separately in [`packaging/README.md`](packaging/README.md#remaining-ppa-import-gaps). |
