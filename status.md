...
|---|-------|--------------|----------|--------|
| 1 | Kernel forward-port | ⚠️ The series is unchanged at `0001`–`0097` (tip `e7ff978398825`) and now rides Linux **6.18.51**. All 97 patches applied to 6.18.51 with no reject, and both top-ranked resync hazards are clear at that base: `struct iommu_dma_cookie` is the unchanged 6.18.44 layout with `iovad` first, and `IOMMU_COOKIE_NONE` still exists. The exact 6.18.51 source is signed, uploaded, and Published as source publication `18730790`; its arm64 build is in progress and no binary, install, or boot exists for it. The last artifact that is actually installed and booted remains 6.18.43 / `0001`–`0096`, which passes identity, ABI, all 12 MPP cases, and 30/31 required librga cases and still fails gray256 on the high contiguous USERPTR admission gap that source `0097` fixes. The 6.18.44 / `0097` predecessor did build successfully on Launchpad but was never installed. `0095`–`0097` runtime, exact-tail sanitizer, hostile, root-counter, and display gates all remain open. | 2026-09-13 | [update finding](./findings/2026-09-13-kernel-lines-updated-to-latest-upstreams.md), [0097 finding](./findings/2026-08-11-forward-port-rga2-contiguous-userptr-rejection.md), [0095–0096 audit finding](./findings/2026-08-08-forward-port-rga-mpp-ownership-audit-fixes.md), [production validation](./findings/2026-08-04-forward-port-6-18-42-0092-production-validation.md), [series status](./kernel-drivers/docs/forward-port-status.md) |
...
| W05 | [Launchpad PPA publication](#watch-w05) | 2026-09-13 | Forward-port 6.18.51 Published, arm64 build in progress. Three more kernel lines uploaded. The installed/booted artifact is still `20260808`. |
...
- **State 2026-09-13:** Five kernel source uploads, each verified against the
  ...
  - Normal PPA: forward-port `6.18.51+rk3588av1fwport20260913-0ubuntu1~rk1` is
    **Published** as source publication
    ...
    `Currently building`. No binary, install, or boot exists for it yet.
...
- **State 2026-09-13:** The board is unchanged — `6.18.43-ysp-rockchip64` /
  ...
  KASAN/KCSAN/lockdep/DMABUF_DEBUG off, and there are no `*-rewrite` paths.
  Two identities that were open on 2026-08-11 are now settled: the 6.18.44 /
  `0097` source published and built successfully, and it was never installed.
  So `0097` still has **no** runtime evidence of any kind.
