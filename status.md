...
| 9 | Launchpad PPA | ⚠️ PPA work is in progress, but the full stack is not installable yet: MPP and librga source + arm64 binaries are public; fixed FFmpeg baseline `7:8.1.2-1+rk2` is accepted as Pending source publication `18610234` with arm64 build `33381225` queued; Rockchip-81 FFmpeg and GRD remain held. The optional GDM greeter ACL native source wrapper is now in-repo but not uploaded. Forward-port kernel source package `linux-rockchip64-ysp` exports locally, but binary-build and board install/revert validation are pending. | 2026-07-09 | [`packaging/ppa/`](packaging/ppa/README.md) |
...
| Launchpad PPA publication state | Source acceptance, build state, and binary publication can change after upload without local file changes | 2026-07-08 | Public APT/API check at `2026-07-08T22:50:31-07:00`: MPP `-0ubuntu2~rk1` and librga `-0ubuntu3~rk1` source + arm64 binaries are public; fixed baseline FFmpeg `7:8.1.2-1+rk2` is Pending as source publication `18610234`, arm64 build `33381225` is `Needs building`; old baseline build `33366878` failed on missing `frei0r-plugins`; Rockchip-81 FFmpeg and GRD are not uploaded. Re-check before telling users to install from the PPA. |
