...
| 4 | Clean-room rewrite drivers | 🚧 Booted KASAN/lockdep package `P91d6-Cad24` completed its then-current exact 85/85 MPP plus 148/148 RGA KTAP, but MPP case 83 disabled lockdep, so the compound gate remains red. Current byte-identical tips 6.18 `51ea9d1ca537` and mainline `03da898b03f1f` isolate the 84+148 fixtures behind local services, remove KUnit production unbind/reprobe, make reviewed resource paths assertion-safe, and add deterministic completion/race synchronization. Qualification now requires the exact ordered manifest and source/config/package identity; the expanded audit reports 325 known signals with zero cross-tree drift. These tips remain unbooted. | 2026-07-28 | [KUnit fixture and evidence contract](./kernel-drivers/docs/rewrite-kunit.md), [rationalization plan](./kernel-drivers/docs/rewrite-kunit-rationalization-plan.md) |
...
| 4 | Clean-room rewrite drivers | Build and boot a source-bound successor package from `51ea9d1ca537`; require the exact ordered 84+148 manifest, a clean outer-KTAP interval with live lockdep, matching source/config/package identity, a clean aged kmemleak scan, every runtime/core, ABI replay, and full conformance. | [qualification contract](./kernel-drivers/docs/rewrite-kunit.md#capture-a-reproducible-result) |
