...
|---|-------|------------|-------------|
| 1 | Kernel forward-port | Rebuild the production source package with `CONFIG_DMABUF_DEBUG` now disabled in the tracked config, install it, and take one RDP login at `2064x1296` on a fresh boot. Production oopses 3/3 boots and 7× within a single boot, so one clean login — with `Created h264_rkmpp encode session` in the GRD journal proving the encoder path actually ran — closes this. Keep a kernel with the option still enabled installed and bootable until it passes, so the comparison can be re-run. No instrument is needed: the guard, the hardware watchpoint, and the `linux-rockchip64-ysp-sgguard` upload are all superseded. | [root cause + fix](./findings/2026-07-28-dmabuf-debug-mangle-sg-table-is-the-sg-writer.md#verification-gate), [kernel package checklist](./packaging/ppa/kernel-forward-port/README.md#remaining-checklist) |
