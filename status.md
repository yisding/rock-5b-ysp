...
| 4 | Clean-room rewrite drivers | 🚧 Booted KASAN/lockdep package `P91d6-Cad24` completed its then-current exact 85/85 MPP plus 148/148 RGA KTAP, but MPP case 83 disabled lockdep, so the compound gate remains red. Current byte-identical tips 6.18 `cd71f985a784c` and mainline `7dcb4c3b5a981` land the [2026-07-29 review-round-2 fixes](./findings/2026-07-29-rewrite-driver-review-round-2.md): legacy librga 90/270-degree rotates now validate (previously all failed `-EINVAL`), the hard-CCU chain has one writer, abort kicks the scheduler, RGA fences get per-fence contexts plus module pinning, shared-IRQ MMIO is power-gated, and the session-abort KUnit double free is repaired; the audit baseline is 327 signals with zero cross-tree drift. These tips remain unbooted. **2026-07-29 late: the silent conformance wedge is root-caused** — dual-core `mpi_dec_mt_h264` stalled the interconnect through the rewrite's split soft-CCU arm/start window (a sibling completion could re-gate an armed-but-unstarted core; single-core decode was immune, and no rewrite mpp suite had ever actually completed). Fix `7d92b06a4c5ba` (mainline) = `eb78ceed2fd67` (6.18) restores the BSP arm→`CORE_STA`+`START` order under `ccu->run_lock`, splits the coordinator lockdep classes (lockdep had been disabling itself at the first submit), and makes the poll-wait predicates lockless (`__might_sleep` splat); adversarially reviewed, compile-verified, boot + dual-core re-run pending. `build-kernel.sh` now stamps ` g<sha12>` into the build timestamp (`uname -v`; Armbian packaging rejects release-string divergence) and the KUnit identity gate parses it from there; a rewrite-debug build from `eb78ceed2fd67` is in flight. | 2026-07-29 | [soft-CCU wedge root cause](./findings/2026-07-29-rewrite-soft-ccu-dual-core-wedge.md), [KUnit fixture and evidence contract](./kernel-drivers/docs/rewrite-kunit.md), [rationalization plan](./kernel-drivers/docs/rewrite-kunit-rationalization-plan.md), [review round 2](./findings/2026-07-29-rewrite-driver-review-round-2.md) |
