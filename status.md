...
|---|-------|--------------|----------|--------|
| 1 | Kernel forward-port | ⚠️ Published/installed 6.18.43 now boots its matching YSP DTB and passes identity, ABI, and all 12 MPP cases, but full conformance stops at three RGA2-only large-USERPTR librga cases. Source patches `0093`–`0094` separately repair driver-owned USERPTR segmentation and exporter-owned high DMA-BUF staging; both are compile-verified and unbooted. The older `0092` functional/recovery and encode-soak verdict remains valid for its exact 6.18.42 artifact; exact-tail sanitizer, hostile, root-counter, and display gates remain open. | 2026-08-08 | [USERPTR finding](./findings/2026-08-08-forward-port-rga2-userptr-swiotlb-segments.md), [DMA-BUF staging finding](./findings/2026-08-08-forward-port-rga2-dmabuf-staging.md), [production validation](./findings/2026-08-04-forward-port-6-18-42-0092-production-validation.md), [series status](./kernel-drivers/docs/forward-port-status.md) |
...
| 4 | Clean-room rewrite drivers | 🚧 Kernel #8 at `gf37186832202` booted and exposed residual decoder and RGA3 failures. The pre-refactor 6.18 `c20fc8c1cbf76` and mainline `09e39082007dd` tips add per-session RKVDEC dispatch serialization and an RGA command-publication barrier and passed warning-fatal clean-archive `normal`/`test-disabled` builds plus exact 94/152 manifests. Maintained source tips 6.18 `fd068ad6aae65` and mainline `a0dd1f6d68bd6` now also funnel MPP reset, both active slots, RKVDEC dispatch and coordinator/core-chain power leases, backend hardware publication/start ownership, session-locked terminal outcome publication, and RGA execution-map retirement. Run-lock assertions cover active publication, start, IOMMU refresh, and RGA backend-start funnels; RGA completion/destruction warn on inherited mappings, and MPP destruction warns on an inherited live CCU lease. Both exact tips pass warning-fatal clean-archive `normal` and `test-disabled` builds of the IOMMU providers, rewrite objects, and Rock 5B DTB. Mirrored sources are byte-identical, the complete Phase 1 deltas pass strict checkpatch, and the expanded 484-signal ownership plus 308-signal KUnit source audits pass. The inventory now also freezes MPP power transitions, every PM/clock backend call, `power_count` writes, and both watchdog-arm entry sets. Independent review found that a stateless terminal wrapper would change existing races or misassociate evidence across reused job generations, so the provisional Phase 1 source boundary stops here; real arbitration and snapshot closure wait for a retained activation/task-execution owner. Exact 6.18 package `6.18.43-S7b92-D6d03-P910c-Cad24-H1c44-HK01ba-Vc222-B3ab8-R448a` is built and inspected with source stamp `gfd068ad6aae6`; its 35 rewrite warnings are confined to KUnit fixture frames under KASAN, with no production rewrite warning. It remains uninstalled and unbooted; reboot, KUnit-runtime, and hardware qualification are deferred and must pass before Phase 2. The barrier remains only a candidate for the silent RGA3 corruption and fault until runtime replay. The separate RGA2 plain-system-heap 1 MiB SWIOTLB staging design is recorded but deliberately unimplemented during the ownership refactor; AV1/VSI, recovery, performance, fuzz, and soak remain open. | 2026-08-08 | [build finding](./findings/2026-08-08-rewrite-phase-1-focused-build-gate.md), [ownership refactor plan](./kernel-drivers/docs/rewrite-ownership-refactor-plan.md), [RGA2 staging design](./findings/2026-08-08-rewrite-rga2-dmabuf-staging-design.md), [rga3 vpp corruption](./findings/2026-08-07-rga3-cross-process-vpp-corruption-lead.md), [overlay-chain gaps](./findings/2026-08-07-rewrite-rga-blend-chain-swiotlb-and-rga3-iommu-fault.md), [dispatch-race finding](./findings/2026-08-07-rewrite-mpp-same-session-dual-core-dispatch-race.md) |
...
| 9 | Launchpad PPA | ⚠️ The normal PPA provides the complete nine-source system stack, and its forward-port 6.18.43 image/DTB/headers are Published, installed, and booted. Kernel qualification is partial because that exact `0092` package stops at the RGA2 librga failure; source patches `0093`–`0094` are not packaged or published. The FFmpeg successor is Published but uninstalled, and VA-API still needs exact PPA-binary replay. [W05](#watch-w05) owns live publication identities. | 2026-08-08 | [PPA kernel record](./packaging/ppa/kernel-forward-port/README.md), [USERPTR finding](./findings/2026-08-08-forward-port-rga2-userptr-swiotlb-segments.md), [DMA-BUF staging finding](./findings/2026-08-08-forward-port-rga2-dmabuf-staging.md) |
...
|---|-------|------------|-------------|
| 1 | Kernel forward-port | Build, install, and boot exact `0094`; pass the three focused RGA2 USERPTR samples, force the high-system-heap DMA-BUF staging/reuse/copy-back path, and then run the complete production conformance matrix. After functional closure, exercise the exact tail under KASAN/lockdep and capture the remaining root/hostile evidence. | [0094 verification gate](./findings/2026-08-08-forward-port-rga2-dmabuf-staging.md#verification-gate), [0093 USERPTR gate](./findings/2026-08-08-forward-port-rga2-userptr-swiotlb-segments.md#verification-gate), [production validation gaps](./findings/2026-08-04-forward-port-6-18-42-0092-production-validation.md#remaining-qualification-gaps) |
...
| 4 | Clean-room rewrite drivers | Qualify the provisional Phase 1 boundary: install and boot the inspected exact-tip 6.18 package with recovery retained, require the exact 246-case KUnit manifest, replay the same-session H.26x loop plus solo RGA3 vpp and overlay-chain cases, and retain the corrected librga DMA-BUF/USERPTR counter contract. Do not begin Phase 2 or introduce terminal arbitration until these gates pass; the later transition owner must be a retained, generation-tagged activation/task execution, not a stateless job wrapper. A persistent RGA3 fault needs command-image and mapping capture; implement the separately recorded RGA2 staging design only after the active ownership refactor settles. | [build finding](./findings/2026-08-08-rewrite-phase-1-focused-build-gate.md), [ownership refactor plan](./kernel-drivers/docs/rewrite-ownership-refactor-plan.md), [RGA3 fixed-tip gate](./findings/2026-08-08-rga3-cross-process-contention-harness-plan.md#verification-gate), [RGA2 staging design](./findings/2026-08-08-rewrite-rga2-dmabuf-staging-design.md#verification-gate), [overlay-chain boundary](./findings/2026-08-07-rewrite-rga-blend-chain-swiotlb-and-rga3-iommu-fault.md#boundary), [rewrite validation plan](./kernel-drivers/docs/rewrite-validation-plan.md) |
...
| 9 | Launchpad PPA | Build and publish a 6.18.43 successor carrying `0093`–`0094`, install it with recovery retained, and close its full conformance plus forced-staging gates; then install exact Published FFmpeg and replay its GRD integration gate. | [0094 verification gate](./findings/2026-08-08-forward-port-rga2-dmabuf-staging.md#verification-gate), [W05 publication record](#watch-w05), [FFmpeg integration gate](./findings/2026-07-30-ffmpeg-rkmpp-async-frame-lifetime-fix.md#verification-gate) |
...
| W16 | [Forward-port kernel-fix tail](#watch-w16) | 2026-08-08 | Installed `6.18.43-ysp-rockchip64` now boots the matching DTB and passes identity/ABI/MPP, then fails three RGA2-only librga cases. Maintained source `65f5b67940a79` / `0001`–`0094` separately repairs USERPTR segmentation and exporter-owned high DMA-BUF staging; it is compile-verified but unpackaged and unbooted. |
...
- **State 2026-08-07:** Official Launchpad API rechecks list nine Published
  source packages in the normal PPA. Forward-port 6.18.43 source publication
...
- **State 2026-08-08:** `6.18.43-ysp-rockchip64` (source
  ...
  `rk3588-video-6.18@65f5b67940a79` / `0001`–`0094` contains the focused,
  compile-verified USERPTR segment-sizing and high DMA-BUF staging fixes but is
  not packaged or booted. See W26,
  ...
  and the [DMA-BUF staging finding](./findings/2026-08-08-forward-port-rga2-dmabuf-staging.md).
