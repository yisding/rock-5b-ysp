...
| 4 | Clean-room rewrite drivers | Audit the remaining Phase 1 terminal-reason and snapshot seam, but land it only with an activation transition owner that preserves generation and fault semantics; do not begin Phase 2 while exact-tip hardware qualification is deferred. After the behavior-preserving Phase 1 checkpoints settle, build, install, and boot the exact resulting 6.18 rewrite-debug tip; require the exact 246-case KUnit manifest, replay the same-session H.26x loop plus solo RGA3 vpp and overlay-chain cases, and retain the corrected librga DMA-BUF/USERPTR counter contract. A persistent RGA3 fault needs command-image and mapping capture; implement the separately recorded RGA2 staging design only after the active ownership refactor settles. | [ownership refactor plan](./kernel-drivers/docs/rewrite-ownership-refactor-plan.md), [RGA3 fixed-tip gate](./findings/2026-08-08-rga3-cross-process-contention-harness-plan.md#verification-gate), [RGA2 staging design](./findings/2026-08-08-rewrite-rga2-dmabuf-staging-design.md#verification-gate), [overlay-chain boundary](./findings/2026-08-07-rewrite-rga-blend-chain-swiotlb-and-rga3-iommu-fault.md#boundary), [rewrite validation plan](./kernel-drivers/docs/rewrite-validation-plan.md) |
