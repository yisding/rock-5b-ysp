...
|---|-------|--------------|----------|--------|
| 1 | Kernel forward-port | ⚠️ **2026-08-04:** maintained source/export advanced to `7d53bc7a3adc` / `0001`–`0092`, closing the RGA job-task UAF plus IOMMU/decoder recovery lifetime gaps; the six affected objects pass arm64 `W=1`, but the tail is unbooted. The Published package remains `7615b69a744af` / `0089` / `6.18.42+rk3588av1fwport20260803-0ubuntu1~rk1`, installed and booted as `6.18.42-ysp-rockchip64`; standalone IEP2 and 17/17 pinned tier-1 VA-API vectors pass. Build/package/boot and targeted safety gates for `0090`–`0092`, then full conformance and soaks, remain. The documented SD-card rescue + `kernel-revert.sh` commands are operator-validated. | 2026-08-04 | [fix finding](./findings/2026-08-04-forward-port-rga-uaf-recovery-safety-fixes.md), [kernel package](./packaging/ppa/kernel-forward-port/README.md), [series status](./kernel-drivers/docs/forward-port-status.md), [recovery finding](./findings/2026-08-04-forward-port-sd-rescue-rollback-used.md) |
...
| 9 | Launchpad PPA | ⚠️ Forward-port kernel `6.18.42+rk3588av1fwport20260803-0ubuntu1~rk1` (`0089`) is Published with all three arm64 binaries, installed, and booted; maintained source is now three unpackageged, unbooted safety patches ahead at `0092`. GRD replacement source `18654077` (binary `247717203`, build `33461880`) is Published/successful and installed. The normal stack and comparison/rewrite archives otherwise retain their package-specific recorded states. Building/publishing the successor kernel, current-kernel full validation, and clean migration remain open; the documented SD rescue + `kernel-revert.sh` path is operator-validated. | 2026-08-04 | [`packaging/ppa/`](packaging/ppa/README.md), [0092 fix finding](./findings/2026-08-04-forward-port-rga-uaf-recovery-safety-fixes.md), [recovery finding](./findings/2026-08-04-forward-port-sd-rescue-rollback-used.md) |
...
|---|-------|------------|-------------|
| 1 | Kernel forward-port | Build/package and boot exact source tip `7d53bc7a3adc` (`0092`) under KASAN/lockdep, then pass RGA cancellation/session-close and decoder recovery/reset-contention gates before the full campaigns. | [0092 verification gate](./findings/2026-08-04-forward-port-rga-uaf-recovery-safety-fixes.md#verification-gate), [kernel package checklist](./packaging/ppa/kernel-forward-port/README.md#remaining-checklist) |
...
| W05 | [Launchpad PPA publication](#watch-w05) | 2026-08-04 | The `6.18.42` / `0089` kernel is Published with all three binaries, installed, and booted; source `0092` is not uploaded. GRD `~rk2` is Published and installed. Successor publication, validation, and clean migration remain. |
...
| W16 | [Forward-port kernel-fix tail](#watch-w16) | 2026-08-04 | Source/export are `0001`–`0092` / `7d53bc7a3adc` and compile clean; the Published/booted package remains `0089` / `7615b69a744af`, so the new safety tail still needs package and runtime proof. |
...
- **Last checked:** 2026-08-04
- **State 2026-08-04:** The Published package remains at `7615b69a744af`,
  `0001`–`0089`, and
  `6.18.42+rk3588av1fwport20260803-0ubuntu1~rk1`. Launchpad reports the source
  and all three arm64 binaries Published. The package is installed and booted
  ...
  remains open. Maintained source/export `0092` / `7d53bc7a3adc` has not been
  uploaded, so Launchpad carries none of its three safety fixes. GRD replacement
...
- **State 2026-08-04 (source safety tail):** Maintained source and the exported
  ...
  the six affected objects pass arm64 `W=1`. They have not been packaged or
  booted. See the
