...
|---|-------|--------------|----------|--------|
| 1 | Kernel forward-port | ⚠️ The installed `6.18.40+rk3588av1fwport20260725` production kernel includes the hardware-verified `0074`/`0075` tail, but on 2026-07-27 it oopsed during GRD's first H.264 smoke frame: the second entry of the 24-entry system-heap output scatterlist resolved outside RAM after initially mapping successfully. That production evidence is **n=1** — one boot, one handover, one oops. The KASAN/`DEBUG_SG` image did **not** reproduce it (9/9 logins plus 1600 direct encoder sessions clean), and is probably unable to: the corrupt `page_link` is a well-formed `struct page` pointer, which KASAN does not report. The writer remains unproven. | 2026-07-27 | [oops trace](./findings/2026-07-27-grd-rkmpp-system-heap-sg-corruption-oops.md), [non-reproduction](./findings/2026-07-27-grd-sg-corruption-kasan-non-reproduction.md), [kernel status](./kernel-drivers/docs/forward-port-status.md) |
...
| 7 | GNOME Remote Desktop backend | ⚠️ Installed `50.2+rkmpp+git20260721.13.cf60b4d` reached GDM handover and AVC420 negotiation on 2026-07-27, but its first RKMPP H.264 smoke frame triggered a kernel system-heap scatterlist-corruption oops before encoder hardware start — once, on the only production boot that took a handover. On the KASAN kernel the same greeter-handover path created the `h264_rkmpp` session 9/9 times with login video, so the failure is not known to be deterministic. Sustained focus/resume and audio gates stay blocked only until the production base rate is measured. | 2026-07-27 | [oops trace](./findings/2026-07-27-grd-rkmpp-system-heap-sg-corruption-oops.md), [non-reproduction](./findings/2026-07-27-grd-sg-corruption-kasan-non-reproduction.md), [testing](./apps/gnome-remote-desktop/docs/testing.md) |
...
| 14 | Desktop-app HW video (browsers) | 🚧 The renovated rockchip-vaapi fork has measured H.264/VP9 and experimental HEVC Main/Main10, VP9 Profile 2, P010, GStreamer readback, and opt-in H.264/HEVC encode coverage; experimental paths remain hidden. Firefox has a pinned RDD policy patch but no completed package or live sandbox gate, and 10-bit encode input remains unsupported. | 2026-07-26 | [`rockchip-vaapi` project](./video-libraries/vaapi/README.md), [Firefox checkpoint](./findings/2026-07-26-firefox-rdd-package-build-checkpoint.md) |
...
|---|-------|------------|-------------|
| 1 | Kernel forward-port | Boot the production `6.18.40-ysp-rockchip64` kernel and measure the base rate: one RDP login, then the direct encoder loop, then repeated logins. n=1 cannot distinguish a deterministic bug that KASAN masks from a rare one. If it reproduces, put the `system-heap-sg-guard` patch on a non-KASAN forward-port build and escalate to a hardware watchpoint. | [production repro plan](./kernel-drivers/docs/grd-sg-corruption-repro-plan.md), [non-reproduction](./findings/2026-07-27-grd-sg-corruption-kasan-non-reproduction.md) |
...
| 7 | GNOME Remote Desktop backend | Measure the production-kernel login base rate (track 1's plan). If logins pass there, resume the sustained focus-away/return video gate and confirm audible PCM with the normal codec offer rather than holding both on a single unreproduced oops. | [production repro plan](./kernel-drivers/docs/grd-sg-corruption-repro-plan.md), [focus/resume gate](./apps/gnome-remote-desktop/docs/testing.md#10-exp6exp7-macos-focusresume-gate) |
