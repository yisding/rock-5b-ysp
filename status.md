...
|---|-------|--------------|----------|--------|
| 1 | Kernel forward-port | ❌ The installed `6.18.40+rk3588av1fwport20260725` production kernel includes the hardware-verified `0074`/`0075` tail but oopses **deterministically** on GRD's first H.264 smoke frame: 3/3 boots, each on the first login — the third with `mpp_dev_debug` tracing live, so tracing does not mask it. The measured ioctl window is a single output-buffer import (`fd => iova`) with the fatal sync as the adjacent kernel line — no kernel-side MPP activity between them. Always entry 1 of a 24-entry system-heap scatterlist at 1 MiB; the KASAN/`DEBUG_SG` image does not reproduce it (9/9 logins plus 1600 direct encoder sessions clean) and probably cannot — the corrupt `page_link` is a well-formed `struct page` pointer, which KASAN does not report. All three corrupt values sit exactly 256 bytes below a 16 KiB boundary, making that fingerprint near-certain. The writer remains unproven. | 2026-07-27 | [ioctl window + third repro](./findings/2026-07-27-grd-sg-oops-third-repro-ioctl-window-measured.md), [reproduction + capture plan](./findings/2026-07-27-grd-sg-corruption-reproduced-and-ioctl-capture-plan.md), [oops trace](./findings/2026-07-27-grd-rkmpp-system-heap-sg-corruption-oops.md), [non-reproduction](./findings/2026-07-27-grd-sg-corruption-kasan-non-reproduction.md) |
