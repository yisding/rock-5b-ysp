...
| Launchpad PPA publication state | Source acceptance, build state, and binary publication can change after upload without local file changes | 2026-07-10 | Launchpad API check at 23:49 PDT: Rockchip-81 FFmpeg `~rk3` source `18614555` is `Pending` with arm64 build `33387380`; co-installable `ffmpeg-rockchip` source `18614552` is `Pending` and arm64 build `33387375` `Successfully built`. Kernel `~rk2` retries that add `u-boot-tools` were accepted as Pending: forward-port `18614559` / build `33387391` on `bos03-arm64-047`, alpha 6.18 `18614560` / build `33387392` on `bos03-arm64-110`, and alpha 7.2-rc2 `18614561` / build `33387393` on `bos03-arm64-074`, all `Currently building`. MPP and librga source + arm64 binaries were already public. GRD and GDM ACL are not uploaded. Re-check before telling users to install from the PPA. |
