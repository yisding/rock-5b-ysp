...
| 4 | Clean-room rewrite drivers | 🚧 **2026-08-04:** maintained tips are 6.18 `33c30ec6989e` and mainline `9e503f6b16df`; tracked rewrite sources, Kconfig, ABI ledgers, and UAPI are byte-identical. The latest commit repairs non-terminal request-configuration cleanup and corrects the KUnit rotation oracles without changing the **92 MPP + 152 RGA** manifest. Both tips pass the warning-fatal clean-archive `normal` build and the 305-signal audit with zero new or absent signals. This is source/build evidence only. Cleanest boot `#29` (`g8042f13c5459`) predates the review tail; installed package `#30` never booted and also predates current. The VPU981 AV1 backend is implemented in source, but no current-tip KUnit, codec/RGA media, AV1/VSI, recovery, performance, fuzz, or soak hardware proof exists. The proposed cluster/activation/task-execution ownership refactor is a target architecture and is not present in either maintained tree. | 2026-08-04 | [current tip/build finding](./findings/2026-08-04-rewrite-kunit-request-rotation-repair.md), [architecture guide](./kernel-drivers/docs/rewrite-driver-architecture/README.md), [ownership refactor plan](./kernel-drivers/docs/rewrite-ownership-refactor-plan.md), [KUnit fixture and evidence contract](./kernel-drivers/docs/rewrite-kunit.md), [AV1 assessment](./kernel-drivers/av1/docs/av1-rewrite-assessment.md) |
