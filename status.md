...
| 4 | Clean-room rewrite drivers | 🚧 Kernel #8 at `gf37186832202` booted and exposed residual decoder and RGA3 failures. The operator explicitly authorized source-only Phase 2/3 work without waiting for boot qualification. Maintained tips 6.18 `3e6d682519a02` and mainline `e72aaf3244fbf` retain the Phase 2 reset/cluster/recovery and IRQ-register owners plus the Phase 3 activation checkpoints. Phase 3F replaces in-place hard-CCU retry reuse with a distinct retained successor: the predecessor becomes `SUPERSEDED/RETRY_REPLACED`, current/dispatch/active publication switches atomically, stale timeout work keeps its containing-job pin and cannot claim the successor, and every retained activation owns one selected-core reference until all-attempt drop. Logical replacement still precedes per-core stop/recovery to preserve Phase 3E behavior, so `SUPERSEDED` is not quiesced, `RETIRED`, or `RECLAIMABLE`. CCU/link/DCHS and power ownership, typed closure/retirement, final outcome arbitration, and cleanup tails remain open. The tips pass strict checkpatch, byte identity, exact 102 MPP + 152 RGA manifests, the 1688-signal production ownership inventory, the unchanged 306-signal KUnit-debt audit, and all eight warning-fatal clean-archive `normal`, `test-disabled`, `memory`, and `race` profiles. This is compile/source evidence only. The predecessor exact 6.18 package P692f with source stamp `(gab69ece99864)` remains uninstalled and unbooted; no package or boot contains Phase 2 or Phase 3. Runtime KUnit, decoder/reset-contention/recovery, solo RGA3 vpp, overlay-chain, AV1/VSI, performance, fuzz, and soak qualification remain open. The separate RGA2 plain-system-heap 1 MiB SWIOTLB staging design remains deliberately unimplemented while active ownership changes. | 2026-08-09 | [Phase 3F fresh retry activations](./findings/2026-08-09-rewrite-phase-3f-mpp-fresh-retry-activation.md), [Phase 3E claim owner](./findings/2026-08-09-rewrite-phase-3e-mpp-claim-owner.md), [Phase 3D activation slots](./findings/2026-08-09-rewrite-phase-3d-mpp-activation-slots.md), [ownership refactor plan](./kernel-drivers/docs/rewrite-ownership-refactor-plan.md), [RGA2 staging design](./findings/2026-08-08-rewrite-rga2-dmabuf-staging-design.md), [rga3 vpp corruption](./findings/2026-08-07-rga3-cross-process-vpp-corruption-lead.md), [overlay-chain gaps](./findings/2026-08-07-rewrite-rga-blend-chain-swiotlb-and-rga3-iommu-fault.md) |
...
| 4 | Clean-room rewrite drivers | Build/package/install/boot the exact Phase 3F tip, require all 254 KUnit cases and a fatal-free lockdep interval, and replay same-session H.26x, dual-core reset contention/recovery, solo RGA3 vpp, and overlay-chain cases. Before terminal-outcome arbitration, retain one typed quiescence/cleanup result for the superseded attempt and require retirement or quarantine before releasing its remaining resources. | [Phase 3F boundary](./findings/2026-08-09-rewrite-phase-3f-mpp-fresh-retry-activation.md#boundary-and-next-gate), [ownership refactor plan](./kernel-drivers/docs/rewrite-ownership-refactor-plan.md), [RGA3 fixed-tip gate](./findings/2026-08-08-rga3-cross-process-contention-harness-plan.md#verification-gate), [RGA2 staging design](./findings/2026-08-08-rewrite-rga2-dmabuf-staging-design.md#verification-gate), [overlay-chain boundary](./findings/2026-08-07-rewrite-rga-blend-chain-swiotlb-and-rga3-iommu-fault.md#boundary), [rewrite validation plan](./kernel-drivers/docs/rewrite-validation-plan.md) |
