...
| 2 | BSP-audit fixes | 🚧 The original 16 HIGH rows collapse to 13 bugs: two were already fixed by later forward-port work, and all 11 remaining bugs are now ported as current-tip patches `0059`-`0069`. The subset is per-commit checkpatch-clean, the native Rockchip driver subtree compiles, and KASAN/lockdep debug package `Pabd5-C4ad2` passes artifact, embedded-config, ramoops-DTB, and exact compiled-source inspection. Install/boot, targeted hostile-ioctl/KASAN, and codec/RGA regression gates remain. The older full 65-patch MEDIUM/LOW/cleanup series still has its documented divergence and patch-0024 compile defect. | 2026-07-22 | [`forward-port series`](./kernel-drivers/patches/forward-port-rk3588-av1/README.md), [`BSP audit`](./kernel-drivers/docs/bsp-audit.md), [`cleanup-split/`](./kernel-drivers/patches/cleanup-split/README.md) |
...
| 2 | BSP-audit fixes | Install and boot debug package `Pabd5-C4ad2`; run the foreign-fd, RCB/request bounds, acquire-fence, missing-plane/partial-handle, KASAN, and full codec/RGA regression gates with a clean journal. | [Current forward-port series](./kernel-drivers/patches/forward-port-rk3588-av1/README.md), [runtime gate inventory](./kernel-drivers/patches/cleanup-draft/verification.md#runtime-gate-result--record-here-when-run) |
