...
|---|-------|------------|-------------|
| 1 | Kernel forward-port | Production PPA kernel is installed and green on the full conformance set (2026-07-24). Remaining: fix the RKVENC2 slice-FIFO overflow, triage the GStreamer NV12_10 legacy-blit RGA IOMMU fault (plugin size computation vs kernel validation), and validate rollback. | [RGA2 DMA-sync gate](./findings/2026-07-20-rga2-unmapped-page-table-dma-sync.md#verification-gate), [slice-FIFO gate](./findings/2026-07-20-rkvenc2-slice-fifo-terminal-drop.md#verification-gate) |
...
| W05 | [Launchpad PPA publication](#watch-w05) | 2026-07-24 | Production forward-port kernel `…20260723` (complete current tip, `0001`-`0071`) **Published with a successful arm64 build** — installed from the PPA, booted on the board 2026-07-24, and passed the full conformance set + root gates ([run](./findings/2026-07-24-production-ppa-kernel-full-conformance-run.md)). Prior: latest normal FFmpeg, GRD `~exp3`, and both rewrite-kernel replacements Published with successful arm64 builds. |
...
| W13 | [librga P010/P210 series](#watch-w13) | 2026-07-21 | On `P63dd-C4ad2` the `0047` stride fix makes P010 luma bit-exact; chroma still lands wrong because `rga_convert_addr()` derives UV offsets at 1 byte/px — fixed by kernel patch `0048`, booted chroma gate pending. |
