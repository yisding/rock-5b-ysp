...
| 9 | Launchpad PPA | ⚠️ PPA work is in progress, but the full stack is not installable yet: MPP and librga source + arm64 binaries are public; baseline FFmpeg build `33381225` succeeded; Rockchip-81 FFmpeg is now at `~rk5` after fixing three fork-packaging bugs (`~rk3` failed FATE, `~rk4` failed on `RELEASE_NOTES` then `ffmpeg-doc` arch) — `~rk5` uploaded and building, validated locally as a full arch+indep build emitting every deb; co-installable `ffmpeg-rockchip` source `18614552` is pending and arm64 build `33387375` succeeded. Kernel `~rk2` retries add `u-boot-tools`: forward-port source `18614559` build `33387391`, alpha 6.18 source `18614560` build `33387392`, and alpha 7.2-rc2 source `18614561` build `33387393` are all pending/currently building. GRD and the optional GDM greeter ACL remain held. Kernel board install/revert validation is pending. | 2026-07-10 | [`packaging/ppa/`](packaging/ppa/README.md) |
...
| Launchpad PPA publication state | Source acceptance, build state, and binary publication can change after upload without local file changes | 2026-07-11 | Launchpad API check ~09:20 PDT: Rockchip-81 FFmpeg `~rk3` (FATE) and `~rk4` (build `33387871`, `ffmpeg-doc` arch) both `Failed to build`; `~rk5` accepted `Pending` and building (fix validated locally as a full arch+indep build). MPP `1375813c` + librga arm64 binaries are public and installed on the builder. Co-installable `ffmpeg-rockchip` `18614552` build `33387375` `Successfully built`. GRD and GDM ACL not uploaded. Re-check `~rk5` went green before telling users to install the fork ffmpeg from the PPA. |
