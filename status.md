...
| 7 | GNOME Remote Desktop backend | ⚠️ The backend sustains 60 fps; patch `0017` fixes uncached imported-buffer readback and published FFmpeg `da5befc806` fixes transient MPP input backpressure. Two macOS focus-return failures are now isolated: `0018@34145d9`'s reconstructed-ACK recovery is live-validated after two stalled ACKs, and GDB proved idle time was falsely charged as a 42.493-second pipeline stall (`0019@3e4480e`). The final 19-patch functional tip and `exp7` arm64 package build. Diagnostic patch `0020` and local `exp8` add raw client audio-format logging; their source/native arm64 builds pass. Combined install/live validation and a fresh format capture remain. | 2026-07-20 | [`audio diagnosis`](./apps/gnome-remote-desktop/docs/audio-redirection.md), [`ACK wedge`](./findings/2026-07-20-grd-rdpgfx-focus-resume-ack-wedge.md), [`false starvation`](./findings/2026-07-20-grd-focus-return-false-pipeline-starvation.md) |
...
|---|-------|------------|-------------|
| 1 | Kernel forward-port | Fix the RGA2 DMA-sync and RKVENC2 overflow paths, rebuild/boot the current tip and verify `0044`/`0045` by ABI replay, install the GStreamer development dependencies, finish the KASAN matrix, then repeat it on the uploaded production image and validate rollback. | [RGA ABI gate](./findings/2026-07-21-rga-forward-port-abi-gaps.md#remaining-gate), [RGA2 DMA-sync gate](./findings/2026-07-20-rga2-unmapped-page-table-dma-sync.md#verification-gate), [slice-FIFO gate](./findings/2026-07-20-rkvenc2-slice-fifo-terminal-drop.md#verification-gate) |
...
| W16 | [Forward-port kernel-fix tail](#watch-w16) | 2026-07-21 | Patches `0042`/`0043` are KASAN-verified and corrected MPP runs pass; RGA ABI fixes `0044`/`0045` compile but need booted replay, while slice-FIFO/DMA hardening, GStreamer, publication, exact-image validation, and rollback remain. |
...
- **State then:** Diagnostic `~exp2` and recovery `~exp3` narrowed the original
  ...
  package build. Diagnostic patch `0020` and local `exp8` log every client
  `AUDIO_FORMAT`; source/native arm64 builds and packaged-string inspection
  pass. Install/live validation, a fresh format capture,
  publication/promotion, and an upstream GNOME review remain.
...
- **State then:** The maintained series now exports all three fixes: `0041`
  ...
  `0045@27452e30a2cfd` now fix the two RGA ABI replay gaps and compile from the
  KASAN configuration; booted replay remains.
