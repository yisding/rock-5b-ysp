...
| 4 | Clean-room rewrite drivers | 🚧 Current tips are `linux-6.18-rkvenc@c5faabf9d00b` (`rk3588-rewrite-6.18`) and `linux@39475996a7a8` (`rk3588-rewrite-mainline`), with byte-identical rewrite sources. **All six `normal`/`memory`/`race` clean-source gates pass warning-free at those tips.** The first complete ROCK 5B KUnit boot, parent build `P3565-Cad24`, ran all 85 MPP + 147 RGA cases but passed only 77 + 126; its KUnit interval contained eight Oopses, nine KASAN reports, five debug-object warnings, two refcount warnings, and three IRQ/preemption imbalance pairs. The causes split between incomplete simulated-hardware/work/refcount fixtures, stale register expectations, and real acquire-fd, deterministic-error-output, 4:2:2 geometry, feature-routing, dither, and rotated-canvas contracts. Those are fixed at the current tips. KASAN/lockdep package `P3b08-Cad24` contains both rewrite drivers and both suites, but its exact 85/85 + 147/147 clean boot rerun is still pending; full codec/RGA hardware conformance remains a later gate. | 2026-07-26 | [KUnit root causes](./findings/2026-07-26-rewrite-kunit-failure-root-causes.md) |
...
| 4 | Clean-room rewrite drivers | Install and boot repaired KASAN/lockdep package `P3b08-Cad24`, require exactly 85/85 MPP plus 147/147 RGA KUnit cases with zero skips, and scan the complete suite interval for Oops/KASAN/refcount/debug-object/IRQ/preemption warnings. If green, persist that report before moving to paired clean-dmesg/counter/artifact evidence including AVS2 and H.264/H.265 low-delay slice polling. | [KUnit root causes and closing gate](./findings/2026-07-26-rewrite-kunit-failure-root-causes.md#boundary), [remaining rewrite hardware gates](./kernel-drivers/docs/rewrite-conformance-gap-audit.md#remaining-gaps-and-hardware-gates) |
