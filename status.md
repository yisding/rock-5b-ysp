...
|---|-------|--------------|----------|--------|
| 1 | Kernel forward-port | ⚠️ Published/installed `0001`–`0096` 6.18.43 boots its matching YSP DTB and passes identity, ABI, all 12 MPP cases, and 30/31 required librga cases. Patch `0093` removes the former oversized-SG failure, but a high physically contiguous USERPTR is rejected before that transient RGA2 remap; source `0097` fixes all three admission/MMU/address decisions and is strict-checkpatch, full-RGA `W=1 WERROR=1`, and signed-source-package verified on Linux 6.18.44. Launchpad accepted it as Pending source publication `18669946`; arm64 build, boot, and runtime remain unverified. The older `0092` functional/recovery and encode-soak verdict remains valid for its exact 6.18.42 artifact; `0095`–`0097` runtime, exact-tail sanitizer, hostile, root-counter, and display gates remain open. | 2026-08-11 | [0097 finding](./findings/2026-08-11-forward-port-rga2-contiguous-userptr-rejection.md), [0095–0096 audit finding](./findings/2026-08-08-forward-port-rga-mpp-ownership-audit-fixes.md), [production validation](./findings/2026-08-04-forward-port-6-18-42-0092-production-validation.md), [series status](./kernel-drivers/docs/forward-port-status.md) |
...
| 4 | Clean-room rewrite drivers | 🚧 Ownership-refactor Phases 4 and 5 plus boundary hardening are source-complete at 6.18 `d9cbcf21cda1c047c053447a48a908dcb6e4c5d6` and mainline `b6335efd8f98bedabf426a80d224f85a266c8ea4`. The follow-up adds fixed-width MPP UAPI decoding, explicit IOMMU build prerequisites, RGA request preflight, service-wide USERPTR pin accounting, allocation-independent fault latching, conservative execution reclamation, and suspend/resume admission and callback drain. The byte-identical source SHA-256 values are MPP `349709b66254d6f3c1aeb7b5023e4fc2836d3a7dbedadfe287d14ac5d5344c7a` and RGA `86100ff095475551ea043f6b89160b621183a649b865c599718d2194937f6619`; strict checkpatch over both deltas reports zero errors, warnings, and checks. Ownership passes at 2,313/tree, fixture debt at 306/tree, and the manifest is 109 MPP + 152 RGA = 261 cases. All eight warning-fatal profiles and the dedicated test-disabled ABI-mutation gate pass on both exact heads. No package or boot contains Phases 2–5 or the boundary follow-up, so exact-tip KUnit, terminal/failpoint/fallback/copyback, byte-exact artifact differential, decoder/reset/recovery, RGA3, overlay, AV1/VSI, sanitizer, performance, fuzz, and soak qualification remain open. The separate RGA2 1 MiB SWIOTLB staging design remains deliberately unimplemented. | 2026-08-11 | [Phase 4/5 completion](./findings/2026-08-11-rewrite-phases-4-5-task-execution-and-sealed-emission.md), [ownership refactor plan](./kernel-drivers/docs/rewrite-ownership-refactor-plan.md), [RGA2 staging design](./findings/2026-08-08-rewrite-rga2-dmabuf-staging-design.md), [RGA3 fixed-tip gate](./findings/2026-08-08-rga3-cross-process-contention-harness-plan.md), [overlay-chain gaps](./findings/2026-08-07-rewrite-rga-blend-chain-swiotlb-and-rga3-iommu-fault.md) |
...
| 9 | Launchpad PPA | ⚠️ The normal PPA provides the complete nine-source system stack. Forward-port `20260811` Linux 6.18.44 / exact `0001`–`0097` source is signed, verified, client-uploaded, and accepted as Pending source publication `18669946`; arm64 build, binary publication, install, and boot are unverified. The preceding `20260808` / `0096` source, arm64 build, and all three binaries are Published, installed, and booted with partial qualification because conformance stops at one RGA2 USERPTR librga failure. The exact Published FFmpeg package is now installed; VA-API still needs exact PPA-binary replay. [W05](#watch-w05) owns live publication identities. | 2026-08-11 | [0097 finding](./findings/2026-08-11-forward-port-rga2-contiguous-userptr-rejection.md), [PPA kernel record](./packaging/ppa/kernel-forward-port/README.md), [0095–0096 audit finding](./findings/2026-08-08-forward-port-rga-mpp-ownership-audit-fixes.md) |
...
| 13 | Maximum-mainline kernel | 🚧 Public and WIP stacks are rebased onto shipped Linux 7.2 (`v7.2@8d3ae59288f1`) with all integration patches re-exported; no new full compile, package, boot, or hardware result exists yet. | 2026-08-23 | [`maxline/`](./kernel-versions/maxline/README.md) |
...
| 9 | Launchpad PPA | Confirm build and publication of exact `0097`, then install and boot it with recovery retained; close its focused USERPTR, full-conformance, alias-staging, ownership, fault-admission, and soft-CCU fallback gates, then replay the installed exact-PPA FFmpeg/GRD integration gate. | [0097 verification gate](./findings/2026-08-11-forward-port-rga2-contiguous-userptr-rejection.md#verification-gate), [0095–0096 verification gate](./findings/2026-08-08-forward-port-rga-mpp-ownership-audit-fixes.md#verification-gate), [W05 publication record](#watch-w05), [FFmpeg integration gate](./findings/2026-07-30-ffmpeg-rkmpp-async-frame-lifetime-fix.md#verification-gate) |
...
| 13 | Maximum-mainline kernel | Rerun the public full native compile gate, then build/inspect packages, install with serial recovery, and prove boot, storage, network, display, suspend, and rollback before `wip`. | [recovery-first test order](./kernel-versions/maxline/README.md#install-and-test-order) |
...
| W05 | [Launchpad PPA publication](#watch-w05) | 2026-08-11 | All nine normal-PPA source names remain Published at the last confirmed identities. Forward-port `20260808` source, successful arm64 build, and image/DTB/headers are Published; exact installed identities match. |
...
| W16 | [Forward-port kernel-fix tail](#watch-w16) | 2026-08-11 | Installed/booted `0001`–`0096` passes identity/ABI/MPP and 30/31 librga cases; gray256 exposes a high contiguous USERPTR admission gap. Source `e7ff978398825` / `0097` fixes it and is compile- plus signed-source-package verified on Linux 6.18.44; source publication `18669946` is Pending, and runtime remains open. |
