...
| 2 | BSP-audit fixes | On the booted `Pabd5-C4ad2` (decode differential and KASAN MPP suite already green), run the foreign-fd, RCB/request bounds, acquire-fence, missing-plane/partial-handle gates plus the librga/ABI/FFmpeg/GStreamer sweep with a clean journal, and record the result in the run dir and verification.md. | [Port record](./findings/2026-07-22-bsp-high-current-tip-port.md), [runtime gate inventory](./kernel-drivers/patches/cleanup-draft/verification.md#runtime-gate-result--record-here-when-run) |
