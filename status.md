...
|---|-------|--------------|----------|--------|
| 1 | Kernel forward-port | ⚠️ The installed `6.18.40+rk3588av1fwport20260725` production kernel includes the hardware-verified `0074`/`0075` tail, but on 2026-07-27 it oopsed during GRD's first H.264 smoke frame. The second entry of the 24-entry system-heap output scatterlist resolved outside RAM after initially mapping successfully. The corrupting kernel writer remains unproven; the existing matching KASAN/`DEBUG_SG` image is the next discriminator. | 2026-07-27 | [oops trace](./findings/2026-07-27-grd-rkmpp-system-heap-sg-corruption-oops.md), [kernel status](./kernel-drivers/docs/forward-port-status.md) |
...
| 7 | GNOME Remote Desktop backend | ⚠️ Installed `50.2+rkmpp+git20260721.13.cf60b4d` reached GDM handover and AVC420 negotiation on 2026-07-27, but its first RKMPP H.264 smoke frame triggered a kernel system-heap scatterlist-corruption oops before encoder hardware start. Login video is blocked pending reproduction on the existing KASAN kernel; sustained focus/resume and audio gates cannot begin yet. | 2026-07-27 | [oops trace](./findings/2026-07-27-grd-rkmpp-system-heap-sg-corruption-oops.md), [testing](./apps/gnome-remote-desktop/docs/testing.md) |
...
|---|-------|------------|-------------|
| 1 | Kernel forward-port | Boot the existing `P47b9-Cc271` KASAN/`DEBUG_SG` image and repeat one `2064x1296` GRD login while preserving the earliest debug report. Identify or narrow the writer that corrupts the system-heap scatterlist before resuming conformance and rollback. | [oops verification gate](./findings/2026-07-27-grd-rkmpp-system-heap-sg-corruption-oops.md#next-verification-gate), [6.18.40 KASAN validation](./findings/2026-07-25-forward-port-6-18-40-kasan-full-validation.md) |
...
| 7 | GNOME Remote Desktop backend | First pass one macOS login on the KASAN kernel without the RKMPP system-heap oops. Only then repeat the sustained focus-away/return video gate and confirm audible PCM with the normal codec offer. | [oops verification gate](./findings/2026-07-27-grd-rkmpp-system-heap-sg-corruption-oops.md#next-verification-gate), [focus/resume gate](./apps/gnome-remote-desktop/docs/testing.md#10-exp6exp7-macos-focusresume-gate) |
