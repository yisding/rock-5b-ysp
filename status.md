...
| 4 | Clean-room rewrite drivers | 🚧 The isolated 6.18 AV1 line is `rk3588-rewrite-av1-6.18@402fc9c0bd785`, based on the complete shared rewrite through `rk3588-rewrite-6.18@c5faabf9d00b0`; the shared mainline pin remains `rk3588-rewrite-mainline@7481ab327d7e`. Three AV1 commits add sparse VCD/cache/AFBC register regions, dynamic metadata for all 103 built-in translations, the single-core RKMPP AV1 backend, Rockchip/VSI fault-provider lifecycle, checked 8/10-bit AFBC, generic auxiliary IRQs, codec-neutral counters, ABI documentation, and four KUnit cases. The AV1 branch now has **236 cases** (89 MPP + 147 RGA). On 2026-07-26 its committed clean archive passed warning-free `normal`, KASAN/fault-injection `memory`, and KCSAN/lockdep `race` profiles, each building both IOMMU providers, both KUnit-enabled rewrite objects, and the Rock 5B DTB; the KUnit-parser and evidence-audit selftests also pass. **It has not been packaged, installed, booted, or run on the ROCK 5B**, so there is no booted 236-case report, direct AV1 decode result, AFBC result, recovery evidence, or hardware parity claim. | 2026-07-26 | [AV1 rewrite assessment](./kernel-drivers/av1/docs/av1-rewrite-assessment.md), [conformance-gap audit](./kernel-drivers/docs/rewrite-conformance-gap-audit.md) |
...
| 4 | Clean-room rewrite drivers | Package the dedicated AV1 tip, boot KASAN and KCSAN variants, persist 236 green KUnit results, then require bit-exact AV1 raster decode, 8/10-bit AFBC, VSI fault/timeout recovery, paired clean-dmesg/counter/artifact evidence, AVS2, and H.264/H.265 low-delay slice polling. Carry the AV1 commits to mainline only after the 6.18 hardware baseline is understood. | [AV1 rewrite assessment](./kernel-drivers/av1/docs/av1-rewrite-assessment.md), [remaining rewrite hardware gates](./kernel-drivers/docs/rewrite-conformance-gap-audit.md#remaining-gaps-and-hardware-gates) |
