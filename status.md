...
|---|-------|--------------|----------|--------|
| 1 | Kernel forward-port | ⚠️ The series is unchanged at `0001`–`0097` (tip `e7ff978398825`) and now rides Linux **6.18.51**. All 97 patches applied to 6.18.51 with no reject, and both top-ranked resync hazards are clear at that base: `struct iommu_dma_cookie` is the unchanged 6.18.44 layout with `iovad` first, and `IOMMU_COOKIE_NONE` still exists. The exact 6.18.51 source is signed, uploaded, and Published as source publication `18730790`; arm64 build `33593313` **succeeded** and all three binaries are Published, so 6.18.51 is now the normal PPA's install candidate. It has not been installed or booted. The last artifact that is actually installed and booted remains 6.18.43 / `0001`–`0096`, which passes identity, ABI, all 12 MPP cases, and 30/31 required librga cases and still fails gray256 on the high contiguous USERPTR admission gap that source `0097` fixes. The 6.18.44 / `0097` predecessor also built successfully but was never installed, so `0097` has still never run on hardware. `0095`–`0097` runtime, exact-tail sanitizer, hostile, root-counter, and display gates all remain open. | 2026-09-13 | [update finding](./findings/2026-09-13-kernel-lines-updated-to-latest-upstreams.md), [0097 finding](./findings/2026-08-11-forward-port-rga2-contiguous-userptr-rejection.md), [0095–0096 audit finding](./findings/2026-08-08-forward-port-rga-mpp-ownership-audit-fixes.md), [production validation](./findings/2026-08-04-forward-port-6-18-42-0092-production-validation.md), [series status](./kernel-drivers/docs/forward-port-status.md) |
...
| 4 | Clean-room rewrite drivers | 🚧 Both maintained tips were rebased to current upstream on 2026-09-13: 6.18 `rk3588-rewrite-6.18@7c38fc4ffaba7` on **`v6.18.51`** (all 445 commits replayed, zero conflicts) and mainline `rk3588-rewrite-mainline@12a6a61b76aca` on **`v7.3-rc2`** (all 370 replayed, one conflict: upstream `414cb6f3ac621` dropped the global `rk_ops`, so the port keeps that removal and retains only the rewrite's `rk_iommu_ops` forward declaration). The `drivers/video/rockchip/` payload is byte-identical across both rebases, so every source claim below still holds. The drivers remain source-complete through ownership-refactor Phases 4 and 5 plus boundary hardening: fixed-width MPP UAPI decoding, explicit IOMMU build prerequisites, RGA request preflight, service-wide USERPTR pin accounting, allocation-independent fault latching, conservative execution reclamation, and suspend/resume admission and callback drain. Ownership passes at 2,313/tree, fixture debt at 306/tree, and the manifest is 109 MPP + 152 RGA = 261 cases. Two new package composites exist and are uploaded — mainline `rk3588-rewrite-armbian-7.3-rc2@23383677d9809` on a freshly generated Armbian `rockchip64-7.3` layer, and 6.18 `rk3588-rewrite-armbian-6.18.51@a498b9bfc93b0` on a regenerated `rockchip64-6.18` layer with the two colliding media patches disabled — and both built successfully on Launchpad's arm64 builders and have Published binaries, but neither has a focused clean-archive build gate, a local compile, or any install/boot/KUnit evidence, so no package or boot contains Phases 2–5 or the boundary follow-up. Exact-tip KUnit, terminal/failpoint/fallback/copyback, byte-exact artifact differential, decoder/reset/recovery, RGA3, overlay, AV1/VSI, sanitizer, performance, fuzz, and soak qualification all remain open. The separate RGA2 1 MiB SWIOTLB staging design remains deliberately unimplemented. | 2026-09-13 | [update finding](./findings/2026-09-13-kernel-lines-updated-to-latest-upstreams.md), [Phase 4/5 completion](./findings/2026-08-11-rewrite-phases-4-5-task-execution-and-sealed-emission.md), [ownership refactor plan](./kernel-drivers/docs/rewrite-ownership-refactor-plan.md), [RGA2 staging design](./findings/2026-08-08-rewrite-rga2-dmabuf-staging-design.md), [RGA3 fixed-tip gate](./findings/2026-08-08-rga3-cross-process-contention-harness-plan.md), [overlay-chain gaps](./findings/2026-08-07-rewrite-rga-blend-chain-swiotlb-and-rga3-iommu-fault.md) |
...
| 9 | Launchpad PPA | ⚠️ The normal PPA provides the complete nine-source system stack. Forward-port Linux **6.18.51** / exact `0001`–`0097` is Published as source publication `18730790`, its arm64 build `33593313` succeeded, and all three binaries are Published, making it the archive's install candidate. It has not been installed or booted. Its 6.18.44 predecessor built successfully and is now Superseded. The last Published, installed, and booted kernel remains `20260808` / 6.18.43 / `0096`, whose conformance stops at one RGA2 USERPTR librga failure. Two experimental rewrite kernels were also uploaded on 2026-09-13 — the `7.3-rc2` mainline rewrite alpha to `rock5b-kernel72rc2-rewrite` and the `6.18.51` rewrite alpha to `rock5b-kernel618-rewrite` — and both are now Published with successful arm64 builds and Published binaries, though neither is installed or booted. Maximum-mainline `public` and `wip` reached a PPA for the first time, in `ubuntu-rock-5b-experimental`, and both also built successfully with binaries Published; no maxline package is installed or booted. The exact Published FFmpeg package is installed; VA-API still needs exact PPA-binary replay. [W05](#watch-w05) owns live publication identities. | 2026-09-13 | [update finding](./findings/2026-09-13-kernel-lines-updated-to-latest-upstreams.md), [PPA kernel record](./packaging/ppa/kernel-forward-port/README.md), [0097 finding](./findings/2026-08-11-forward-port-rga2-contiguous-userptr-rejection.md) |
...
| 13 | Maximum-mainline kernel | 🚧 Both stacks were rebased from shipped Linux 7.2 to **`v7.3-rc2`**. The release absorbed 55 of the 299 public commits, retiring ten complete proposal series, and the public stack was then refreshed to the current revision of five more (DW-DP v11, usbdp v14, rkisp2 v3, Samsung DC-PHY v4, RK3588 crypto v3), ending at 248 commits with the 19-commit WIP tail replayed on top. The `accel/rocket` RK3588 DVFS series is deliberately deferred: it needs the unmerged RK3576 NPU enablement base that maxline does not carry. The `public` profile passes a full native arm64 `Image modules dtbs` build and the pinned configuration survives `olddefconfig` with every listed RK3588 feature; `wip` passes a focused compile over every directory its tail touches, with the FRL objects genuinely built, but still owes a full-tree build. Both profiles now have signed source packages, reached a PPA for the first time (`ubuntu-rock-5b-experimental`), and both built successfully on Launchpad's arm64 builders with Published binaries — `wip`'s Launchpad build is now the first full-tree compile it has ever had, on the production toolchain. No maxline profile has been installed, booted, or hardware-tested. | 2026-09-13 | [update finding](./findings/2026-09-13-kernel-lines-updated-to-latest-upstreams.md), [`maxline/`](./kernel-versions/maxline/README.md) |
...
|---|-------|------------|-------------|
| 1 | Kernel forward-port | Install and boot the exact `0097` package, now the archive's install candidate and never booted in any form. Repeatedly pass gray256 plus the two historically intermittent RGA2 USERPTR samples, then pass the complete production conformance matrix — currently blocked by intermittent RKVENC encode corruption on `6.18.43+…20260808`, whose kernel A/B is unrun — and both successful-bounce and oversized high-DMA-BUF alias/staging gates. Confirm hard-CCU requests select soft mode, then exercise the exact tail under KASAN/lockdep plus RGA/MPP concurrency and provider fault-admission gates. | [0097 verification gate](./findings/2026-08-11-forward-port-rga2-contiguous-userptr-rejection.md#verification-gate), [0095–0096 verification gate](./findings/2026-08-08-forward-port-rga-mpp-ownership-audit-fixes.md#verification-gate), [RKVENC encode-corruption gate](./findings/2026-08-16-forward-port-rkvenc-intermittent-encode-corruption.md#verification-gate), [production validation gaps](./findings/2026-08-04-forward-port-6-18-42-0092-production-validation.md#remaining-qualification-gaps) |
...
| 4 | Clean-room rewrite drivers | Run the focused clean-archive build gate on both new composites — neither has been compiled at all — then build/install/boot them, require all 261 KUnit cases and a fatal-free lockdep interval, then force terminal-reason permutations, task-generation IRQ/timeout/fault delivery, acquisition failpoints, fallback/copyback, retirement/quarantine balance, and byte-exact MPP/RGA command-artifact differentials before the decoder/reset/recovery, RGA3, overlay, AV1/VSI, sanitizer, performance, fuzz, and soak matrix. | [Phase 4/5 runtime gate](./findings/2026-08-11-rewrite-phases-4-5-task-execution-and-sealed-emission.md#boundary-and-next-gate), [ownership refactor plan](./kernel-drivers/docs/rewrite-ownership-refactor-plan.md), [RGA3 fixed-tip gate](./findings/2026-08-08-rga3-cross-process-contention-harness-plan.md#verification-gate), [RGA2 staging design](./findings/2026-08-08-rewrite-rga2-dmabuf-staging-design.md#verification-gate), [overlay-chain boundary](./findings/2026-08-07-rewrite-rga-blend-chain-swiotlb-and-rga3-iommu-fault.md#boundary), [rewrite validation plan](./kernel-drivers/docs/rewrite-validation-plan.md) |
...
| 9 | Launchpad PPA | All five 2026-09-13 kernel sources are now built and binary-Published. Install and boot the forward-port with recovery retained and close its focused USERPTR, full-conformance, alias-staging, ownership, fault-admission, and soft-CCU fallback gates, then replay the installed exact-PPA FFmpeg/GRD integration gate. | [update finding](./findings/2026-09-13-kernel-lines-updated-to-latest-upstreams.md), [0097 verification gate](./findings/2026-08-11-forward-port-rga2-contiguous-userptr-rejection.md#verification-gate), [W05 publication record](#watch-w05), [FFmpeg integration gate](./findings/2026-07-30-ffmpeg-rkmpp-async-frame-lifetime-fix.md#verification-gate) |
...
| 13 | Maximum-mainline kernel | Both Launchpad builds now confirm `wip` compiles full-tree; install `public` with serial recovery and prove boot, storage, network, display, suspend, and rollback before `wip`. | [recovery-first test order](./kernel-versions/maxline/README.md#install-and-test-order) |
...
|----|------------|--------------|---------|
| W01 | [Armbian media-patch drift](#watch-w01) | 2026-09-13 | `media-0001` changed at Armbian `2a53f158`; both patches stay disabled in our lanes, so no package is affected, but the DT anchors are now unaudited. |
...
| W05 | [Launchpad PPA publication](#watch-w05) | 2026-09-13 | All five 2026-09-13 kernel uploads (forward-port, both rewrite alphas, both maxline profiles) built successfully and are binary-Published. The installed/booted artifact is still `20260808`. |
...
| W16 | [Forward-port kernel-fix tail](#watch-w16) | 2026-09-13 | Board unchanged at `0001`–`0096`; gray256 still fails. `0097` re-cut onto Linux 6.18.51 and Published, resync hazards re-checked clear, but `0097` still has no runtime evidence. |
...
| W17 | [Maximum-mainline proposal-set drift](#watch-w17) | 2026-09-13 | Re-audited on `v7.3-rc2`: ten series went upstream, five were refreshed, one deferred. `lore` now blocks automation; use the patchwork API. |
...
- **State 2026-09-13:** `armbian/build` main is `2a53f158`, and **`media-0001`
  changed** for the first time since this row was opened: `390c2e0bb071` →
  `fb1c0562fa57`. `media-0007` is unchanged at `a2a4143ee2f8`. The changed blob
  did **not** affect either package built from this base, because
  `build-kernel.sh` disables both media patches in the forward-port and 6.18
  rewrite lanes, and all 97 forward-port patches still applied to 6.18.51
  without a reject. The DT hunks of patch 02 are therefore still unaudited
  against the new `media-0001`; audit them before any build that leaves the
  Armbian media patches enabled. Checklist:
  [resyncing guide §4](./kernel-drivers/docs/resyncing.md).
- **State 2026-08-04:** Main was `9c26d236e194` with both blobs byte-identical
  to the 2026-07-11 reading (`390c2e0b` / `a2a4143e`).
...
- **Last checked:** 2026-09-13
- **State 2026-09-13:** Five kernel source uploads, each verified against the
  Launchpad API after transfer:
  - Normal PPA: forward-port `6.18.51+rk3588av1fwport20260913-0ubuntu1~rk1` is
    **Published** as source publication
    ...
    `Successfully built` at 14:22 UTC. `linux-image-ysp-rockchip64`,
    `linux-dtb-ysp-rockchip64`, and `linux-headers-ysp-rockchip64` are all
    Published at that exact version, so it is the archive's install candidate.
    Nothing is installed or booted from it.
  - The same query settles the August question left open below: the
    `6.18.44` / `0097` source did publish and its arm64 build
    [`33490065`](https://launchpad.net/~yi-ding/+archive/ubuntu/ubuntu-rock-5b/+build/33490065)
    reports `Successfully built`. It is now `Superseded` and was never
    installed.
  - `rock5b-kernel72rc2-rewrite`: `7.3.0~rc2+rk3588rewritealpha20260913-0ubuntu1`
    is **Published**, arm64 build
    [`33593381`](https://launchpad.net/~yi-ding/+archive/ubuntu/rock5b-kernel72rc2-rewrite/+build/33593381)
    `Successfully built`, and all three binaries are Published.
  - `rock5b-kernel618-rewrite`: `6.18.51+rk3588rewritealpha20260913-0ubuntu1`
    is **Published**, arm64 build
    [`33593384`](https://launchpad.net/~yi-ding/+archive/ubuntu/rock5b-kernel618-rewrite/+build/33593384)
    `Successfully built`, and all three binaries are Published.
  - `ubuntu-rock-5b-experimental`: both maxline profiles,
    `7.3.0~rc2+git20260913+rk3588maxline{public,wip}-0ubuntu1`, are
    **Published** — the first maxline packages ever to reach an archive —
    with arm64 builds
    ...
    (`wip`) both `Successfully built`, and all binaries Published for both.
  All five kernel lines from this pass are now built and binary-published on
  Launchpad; none is installed or booted. A successful `dput` proves
  client-side transfer only; the
  [PPA kernel record](./packaging/ppa/kernel-forward-port/README.md) owns the
  signed artifact hashes.
- **State 2026-08-11:** Exact Linux 6.18.44 / `0001`–`0097` forward-port
  source package `6.18.44+rk3588av1fwport20260811-0ubuntu1~rk1` is signed,
  checksum-, extraction-, source-, config-, and `dput --check-only`-verified.
  Client-side `dput` transferred all five artifacts at 17:30 PDT and wrote an
  upload marker with SHA-256
  `e351ea71a22ad052e891fc3e31de5655953f968fb60e52d2b9331771233d9c46`.
  An exact-version Launchpad API query immediately afterward returned no
  source record. A later recheck found exact source publication
  [`18669946`](https://launchpad.net/~yi-ding/+archive/ubuntu/ubuntu-rock-5b/+sourcepub/18669946)
  in `Pending` state. Archive acceptance is confirmed; arm64 build, binary
  publication, install, boot, and runtime remain unverified. The
  [PPA kernel record](./packaging/ppa/kernel-forward-port/README.md) owns the
  signed artifact hashes. The preceding forward-port source
  `6.18.43+rk3588av1fwport20260808-0ubuntu1~rk1` publication
  ...
  is `Successfully built`; and image, DTB, and headers binary publications
  `247936301`, `247936299`, and `247936300` are all `Published`. The board's
  installed image/DTB/headers identities match this exact version; W16 owns the
  boot and partial-conformance result. The last confirmed Published and
  installed PPA state therefore contains the complete `0001`–`0096`
  source/binary set.
...
- **Why recheck:** The frozen base pair, maintained split series, PPA package,
  ...
  KASAN evidence.
- **Last checked:** 2026-09-13
- **State 2026-09-13:** The board is unchanged — `6.18.43-ysp-rockchip64` /
  `0001`–`0096` is still the installed and booted tail, so every runtime claim
  in the 2026-08-11 state below still stands, including the gray256 failure.
  What moved is source and packaging only: the series is unchanged at
  `0001`–`0097` (tip `e7ff978398825`) and was re-cut onto Linux **6.18.51**.
  All 97 patches applied with no reject, and both top-ranked resync hazards are
  re-checked clear at that base — `struct iommu_dma_cookie` is byte-for-byte
  the 6.18.44 layout with `iovad` still first, and `IOMMU_COOKIE_NONE` still
  exists in `include/linux/iommu.h`. The packaged tree extracts as Linux
  6.18.51, six key driver and IOMMU files byte-match the branch tip, the config
  keeps `CONFIG_ROCKCHIP_MPP_SERVICE=y` and `CONFIG_ROCKCHIP_MULTI_RGA=y` with
  KASAN/KCSAN/lockdep/DMABUF_DEBUG off, and there are no `*-rewrite` paths. Its
  arm64 build succeeded and all three binaries are Published, so `0097` is now
  the archive's install candidate for the first time.
  Two identities that were open on 2026-08-11 are also settled: the 6.18.44 /
  `0097` source published and built successfully, and it was never installed.
  So `0097` still has **no** runtime evidence of any kind, and closing that is
  now purely an install-and-boot step.
- **State 2026-08-11:** `6.18.43-ysp-rockchip64` is booted from its matching
  `/boot/Image` and `/boot/dtb`; installed image, DTB, and headers all report
  `6.18.43+rk3588av1fwport20260808-0ubuntu1~rk1`, the published
  `rk3588-video-6.18@7698e7018e3d5` / `0001`–`0096` tail. Production
  conformance passes system/matrix identity, ABI replay, all 12 MPP cases, and
  30/31 required librga cases. Gray256 alone is rejected before hardware start
  because a high physically contiguous USERPTR is misclassified as a raw
  direct-address buffer; no SWIOTLB failure or fatal kernel signature appears.
  Source `e7ff978398825` / patch `0097` repairs admission, RGA2-MMU selection,
  and address preparation consistently and passes strict checkpatch plus a
  complete production-config RGA `W=1 WERROR=1` build. Exact Linux 6.18.44
  source package `6.18.44+rk3588av1fwport20260811-0ubuntu1~rk1` is signed,
  extraction/source/config verified, and accepted as Pending source publication
  `18669946`, but build, boot, and runtime remain unverified. See the
...
- **Why recheck:** Upstream Linux, public proposal revisions, and integration
  ...
  repository.
- **Last checked:** 2026-09-13
- **State 2026-09-13:** Rebased onto released Linux `v7.3-rc2@df2908090cda3`
  and re-audited against patchwork. The release absorbed 55 of the 299 public
  commits and retired ten complete series to `upstream`: `pci-port-reset`,
  `pci-wakeirq`, `hdptx-clock-fixes`, `csi-dphy-2500`, `vop2-multi-output`,
  `vop2-yuv-background`, `rkvdec-h265-fixes`, `dp-altmode-race`, plus the
  non-i915 part of `force-color-format` and five reconciled `hdmi-yuv`
  commits. Five series were refreshed to newer public revisions — DW-DP v8→v11,
  usbdp v13→v14, rkisp2 v1→v3, Samsung DC-PHY v2→v4, RK3588 crypto v2→v3 — and
  one, `accel/rocket` RK3588 DVFS v1, is deferred because it requires the
  unmerged RK3576 NPU enablement base. Three loose `accel/rocket` fixes and
  three `crypto: rockchip` follow-up fixes are recorded but unintegrated. Note
  that `lore.kernel.org` is now behind an anti-bot challenge; this audit used
  the `patchwork.kernel.org` API instead, and one series copy there was
  incomplete (the rkisp2 v3 `linux-rockchip` record is missing patch 1/5; the
  `linux-media` copy is complete). Public passes a full native arm64 compile;
  WIP is not compiled.
