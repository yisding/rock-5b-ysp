...
|---|-------|--------------|----------|--------|
| 1 | Kernel forward-port | ❌ The installed `6.18.40+rk3588av1fwport20260725` production kernel includes the hardware-verified `0074`/`0075` tail but oopses **deterministically** on GRD's first H.264 smoke frame: 2/2 boots, each on the first login, always entry 1 of a 24-entry system-heap scatterlist at 1 MiB. The KASAN/`DEBUG_SG` image does not reproduce it (9/9 logins plus 1600 direct encoder sessions clean) and probably cannot — the corrupt `page_link` is a well-formed `struct page` pointer, which KASAN does not report. Both corrupt values sit exactly 256 bytes below a 16 KiB boundary, a ~1-in-16384 shared fingerprint. The writer remains unproven. | 2026-07-27 | [reproduction + capture plan](./findings/2026-07-27-grd-sg-corruption-reproduced-and-ioctl-capture-plan.md), [oops trace](./findings/2026-07-27-grd-rkmpp-system-heap-sg-corruption-oops.md), [non-reproduction](./findings/2026-07-27-grd-sg-corruption-kasan-non-reproduction.md) |
...
|---|-------|------------|-------------|
| 1 | Kernel forward-port | Capture the MPP sub-command sequence on production with `mpp_dev_debug=0x18` and one login — no rebuild needed — to replace the inferred ioctl chain with a measured one. Then finish the guarded non-KASAN forward-port build (`rk3588-video-6.18-sgguard`, started and stopped, ccache warm) and escalate to the hardware watchpoint, which must catch a write storing a value 256 bytes below a 16 KiB boundary. | [reproduction + capture plan](./findings/2026-07-27-grd-sg-corruption-reproduced-and-ioctl-capture-plan.md#next-test-pre-registered), [escalation path](./kernel-drivers/docs/grd-sg-corruption-repro-plan.md#if-it-oopses-again-the-escalation-path) |
