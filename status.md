...
| 9 | Launchpad PPA | ⚠️ PPA work is in progress, but the full stack is not installable yet: MPP and librga source + arm64 binaries are public; fixed FFmpeg baseline `7:8.1.2-1+rk2` is accepted as source publication `18610234` with arm64 build `33381225` still `Needs building`; Rockchip-81 FFmpeg `~rk2` source artifacts are regenerated and source-lintian checked but held unsigned/unuploaded until that baseline build produces a useful result. GRD remains held. The optional GDM greeter ACL native source wrapper is now in-repo but not uploaded. Forward-port kernel source package `linux-rockchip64-ysp` and local arm64 binaries build, and normalized module/DTB payloads match Armbian `6.18.38`; board install/revert validation is pending. | 2026-07-09 | [`packaging/ppa/`](packaging/ppa/README.md) |
...
| Launchpad PPA publication state | Source acceptance, build state, and binary publication can change after upload without local file changes | 2026-07-09 | Launchpad API check on build `33381225`: fixed baseline FFmpeg `7:8.1.2-1+rk2` is still `Needs building` with no builder/log. MPP `-0ubuntu2~rk1` and librga `-0ubuntu3~rk1` source + arm64 binaries were already public at the prior check. Rockchip-81 FFmpeg `~rk2` source artifacts are regenerated and source-lintian checked but held unsigned/unuploaded; GRD is not uploaded. Re-check before telling users to install from the PPA. |
