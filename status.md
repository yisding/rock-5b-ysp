...
| 4 | Clean-room rewrite drivers | 🚧 Kernel #8 at `gf37186832202` booted and exposed residual decoder and RGA3 failures. The pre-refactor 6.18 `c20fc8c1cbf76` and mainline `09e39082007dd` tips add per-session RKVDEC dispatch serialization and an RGA command-publication barrier and passed warning-fatal clean-archive `normal`/`test-disabled` builds plus exact 94/152 manifests. Maintained source tips 6.18 `fe9edef36346c` and mainline `9a25b02ba4d68` now also funnel MPP reset, both active slots, RKVDEC dispatch and coordinator/core-chain power leases, backend hardware publication/start ownership, session-locked terminal outcome publication, and RGA execution-map retirement. Run-lock assertions cover active publication, start, IOMMU refresh, and RGA backend-start funnels; RGA completion/destruction warn on inherited mappings, and MPP destruction warns on an inherited live CCU lease. Mirrored sources are byte-identical, strict checkpatch is clean, and the expanded 484-signal ownership plus 308-signal KUnit source audits pass. The inventory now also freezes MPP power transitions, every PM/clock backend call, `power_count` writes, and both watchdog-arm entry sets. Independent review found that a stateless terminal wrapper would change existing races or misassociate evidence across reused job generations, so the provisional Phase 1 source boundary stops here; real arbitration and snapshot closure wait for a retained activation/task-execution owner. The immediately preceding tips have focused object-build evidence, but the current assertion-only tips are deliberately unbuilt; full build, package, reboot, and hardware qualification are deferred and must pass before Phase 2. The barrier remains only a candidate for the silent RGA3 corruption and fault until runtime replay. The separate RGA2 plain-system-heap 1 MiB SWIOTLB staging design is recorded but deliberately unimplemented during the ownership refactor; AV1/VSI, recovery, performance, fuzz, and soak remain open. | 2026-08-08 | [ownership refactor plan](./kernel-drivers/docs/rewrite-ownership-refactor-plan.md), [RGA2 staging design](./findings/2026-08-08-rewrite-rga2-dmabuf-staging-design.md), [rga3 vpp corruption](./findings/2026-08-07-rga3-cross-process-vpp-corruption-lead.md), [overlay-chain gaps](./findings/2026-08-07-rewrite-rga-blend-chain-swiotlb-and-rga3-iommu-fault.md), [dispatch-race finding](./findings/2026-08-07-rewrite-mpp-same-session-dual-core-dispatch-race.md) |
