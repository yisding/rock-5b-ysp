...
|---|-------|--------------|----------|--------|
| 1 | Kernel forward-port | ❌ The installed `6.18.40+rk3588av1fwport20260725` production kernel includes the hardware-verified `0074`/`0075` tail but oopses **deterministically** on GRD's first H.264 smoke frame: 3/3 boots, each on the first login — the third with `mpp_dev_debug` tracing live, so tracing does not mask it. The measured ioctl window is a single output-buffer import (`fd => iova`) with the fatal sync as the adjacent kernel line — no kernel-side MPP activity between them. Always entry 1 of a 24-entry system-heap scatterlist at 1 MiB; the KASAN/`DEBUG_SG` image does not reproduce it (9/9 logins plus 1600 direct encoder sessions clean), but that comparison is **confounded by toolchain** — production is a Launchpad buildd binary (gcc 15.2.0/binutils 2.46), the KASAN image a local build (gcc 13.3.0/binutils 2.42) — so "KASAN masks it" and "the gcc-13 build lacks it" are both live; memory-layout config, vendor driver source, and Armbian derivation were checked and are not confounded. KASAN probably also cannot see this write: the corrupt `page_link` is a well-formed `struct page` pointer. All three corrupt values sit exactly 256 bytes below a 16 KiB boundary, making that fingerprint near-certain. The writer remains unproven, and source inspection has cleared the vendor MPP driver. | 2026-07-27 | [build-provenance confound](./findings/2026-07-27-kasan-vs-production-build-provenance-confound.md), [source sweep](./findings/2026-07-27-grd-sg-writer-source-sweep-vendor-drivers-cleared.md), [ioctl window + third repro](./findings/2026-07-27-grd-sg-oops-third-repro-ioctl-window-measured.md), [oops trace](./findings/2026-07-27-grd-rkmpp-system-heap-sg-corruption-oops.md), [non-reproduction](./findings/2026-07-27-grd-sg-corruption-kasan-non-reproduction.md) |
...
| 4 | Clean-room rewrite drivers | 🚧 Booted KASAN/lockdep package `P91d6-Cad24` completed exact 85/85 MPP plus 148/148 RGA KTAP, but MPP case 83 reached DCHS release through a second zeroed local service without initializing `rkvenc_dchs_lock`; lockdep disabled itself before the RGA suite, so the compound gate remains red. Current byte-identical tips 6.18 `9af4a8816f259` and mainline `fb5040f08d833` initialize that reset/import fixture too. A source audit found no other reachable uninitialized lock in the later MPP or RGA fixtures, but a successor boot must prove it dynamically. | 2026-07-27 | [boot warning, root cause, fix, and audit](./findings/2026-07-27-rewrite-reset-import-fixture-lockdep.md) |
...
| 7 | GNOME Remote Desktop backend | ❌ Installed `50.2+rkmpp+git20260721.13.cf60b4d` reaches GDM handover and AVC420 negotiation, then its first RKMPP H.264 smoke frame kills the encoder worker with a kernel oops before encoder hardware start — 3/3 production boots, so there is **no login video on the production kernel**. The same greeter-handover path is clean 9/9 on the KASAN kernel, which is a kernel difference, not a GRD one: no GRD defect is implicated. Sustained focus/resume and audio gates cannot run on production until the kernel writer is found. | 2026-07-27 | [third repro](./findings/2026-07-27-grd-sg-oops-third-repro-ioctl-window-measured.md), [oops trace](./findings/2026-07-27-grd-rkmpp-system-heap-sg-corruption-oops.md), [testing](./apps/gnome-remote-desktop/docs/testing.md) |
...
| 9 | Launchpad PPA | ⚠️ The normal stack and comparison/rewrite builds are largely Published, while the 6.18.40 forward-port source is only client-side uploaded and GRD 50.2 was still building at last check. Publication, clean board migration, combined kernel/GRD validation, and rollback remain open. | 2026-07-25 | [`packaging/ppa/`](packaging/ppa/README.md) |
...
|---|-------|------------|-------------|
| 1 | Kernel forward-port | Finish the resumed guarded non-KASAN forward-port build (76 patches, `P37e0`), boot it with `system_heap.sg_guard=0` for one login to prove the local build reproduces like the PPA kernel — read that asymmetrically, since a *clean* result is ambiguous three ways (KASAN layout, gcc 13 vs 15 toolchain, Armbian-archive vintage) and would call for a Launchpad-built guarded variant instead — then re-enable the guard to capture the three data the register block cannot give: the **prior** `page_link` (hence the delta), the damaged table's **owning device** and attachment count, and the first drifting checkpoint. Then arm the hardware watchpoint — the caught store must write `pfn_to_page(A − 4)` for a 1 MiB-aligned `A` outside RAM. Source sweep has cleared the vendor MPP driver as the writer, so CPU-path suspicion moves to the concurrently live module drivers. | [source sweep](./findings/2026-07-27-grd-sg-writer-source-sweep-vendor-drivers-cleared.md#verification-gate), [third repro](./findings/2026-07-27-grd-sg-oops-third-repro-ioctl-window-measured.md#verification-gate), [escalation path](./kernel-drivers/docs/grd-sg-corruption-repro-plan.md#if-it-oopses-again-the-escalation-path) |
...
| 4 | Clean-room rewrite drivers | Build and boot a successor package from `9af4a8816f259`; require exact 233-case KTAP, a clean complete-interval/live-lockdep artifact, a clean aged kmemleak scan, restored runtimes and every expected core before isolated ABI replay and full conformance. | [verification gate](./findings/2026-07-27-rewrite-reset-import-fixture-lockdep.md#boundary-and-verification-gate) |
...
| 7 | GNOME Remote Desktop backend | Blocked on track 1: the kernel writer must be found before a production login can create an encoder at all. The focus/resume and audio gates can meanwhile be exercised on the KASAN kernel, which passes this path 9/9, provided results are recorded as KASAN-kernel evidence rather than production. | [focus/resume gate](./apps/gnome-remote-desktop/docs/testing.md#10-exp6exp7-macos-focusresume-gate), [reproduction](./findings/2026-07-27-grd-sg-corruption-reproduced-and-ioctl-capture-plan.md) |
...
| 14 | Desktop-app HW video (browsers) | Finish the Firefox 152.0.6 arm64 packages, install them, and prove live hardware decode with the RDD sandbox still enabled. | [Firefox package checkpoint](./findings/2026-07-26-firefox-rdd-package-build-checkpoint.md), [RDD policy](./findings/2026-07-26-firefox-rdd-rockchip-vaapi-policy.md) |
