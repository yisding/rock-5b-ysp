...
|---|-------|--------------|----------|--------|
| 1 | Kernel forward-port | ⚠️ Published/installed `0001`–`0096` 6.18.43 boots its matching YSP DTB and passes identity, ABI, all 12 MPP cases, and 30/31 required librga cases. Patch `0093` removes the former oversized-SG failure, but a high physically contiguous USERPTR is rejected before that transient RGA2 remap; source `0097` fixes all three admission/MMU/address decisions and is strict-checkpatch plus full-RGA `W=1 WERROR=1` verified, not packaged or booted. The older `0092` functional/recovery and encode-soak verdict remains valid for its exact 6.18.42 artifact; `0095`–`0097` runtime, exact-tail sanitizer, hostile, root-counter, and display gates remain open. | 2026-08-11 | [0097 finding](./findings/2026-08-11-forward-port-rga2-contiguous-userptr-rejection.md), [0095–0096 audit finding](./findings/2026-08-08-forward-port-rga-mpp-ownership-audit-fixes.md), [production validation](./findings/2026-08-04-forward-port-6-18-42-0092-production-validation.md), [series status](./kernel-drivers/docs/forward-port-status.md) |
...
| 9 | Launchpad PPA | ⚠️ The normal PPA provides the complete nine-source system stack. Forward-port `20260808` / `0001`–`0096` source, arm64 build, and all three binaries are Published, installed, and booted; kernel qualification is partial because conformance stops at one RGA2 USERPTR librga failure. Source `0097` repairs that newly isolated path but is not packaged. The exact Published FFmpeg package is now installed; VA-API still needs exact PPA-binary replay. [W05](#watch-w05) owns live publication identities. | 2026-08-11 | [0097 finding](./findings/2026-08-11-forward-port-rga2-contiguous-userptr-rejection.md), [PPA kernel record](./packaging/ppa/kernel-forward-port/README.md), [0095–0096 audit finding](./findings/2026-08-08-forward-port-rga-mpp-ownership-audit-fixes.md) |
...
| W16 | [Forward-port kernel-fix tail](#watch-w16) | 2026-08-11 | Installed/booted `0001`–`0096` passes identity/ABI/MPP and 30/31 librga cases; gray256 exposes a high contiguous USERPTR admission gap. Source `e7ff978398825` / `0097` fixes it and is compile-verified, not packaged or booted. |
...
- **State 2026-08-11:** Forward-port source
  ...
  boot and partial-conformance result. The PPA therefore contains the complete
  `0001`–`0096` source/binary set, while source patch `0097` remains unpublished.
...
- **State 2026-08-11:** `6.18.43-ysp-rockchip64` is booted from its matching
  ...
  complete production-config RGA `W=1 WERROR=1` build, but is not packaged or
  booted. See the
