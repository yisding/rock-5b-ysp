...
|---|-------|------------|-------------|
| 1 | Kernel forward-port | Finish the resumed guarded non-KASAN forward-port build (76 patches, `P37e0`), boot it with `system_heap.sg_guard=0` for one login to prove the local build reproduces like the PPA kernel, re-enable the guard to name the damaged table's owning device and bracket the window, then arm the hardware watchpoint — the caught write must store a value 256 bytes below a 16 KiB boundary (fingerprint now 3/3). | [third repro gate](./findings/2026-07-27-grd-sg-oops-third-repro-ioctl-window-measured.md#verification-gate), [escalation path](./kernel-drivers/docs/grd-sg-corruption-repro-plan.md#if-it-oopses-again-the-escalation-path) |
