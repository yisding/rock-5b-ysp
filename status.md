...
| 5 | ffmpeg tree | Install and boot the rebuilt rewrite-debug kernel carrying `f37186832202`, require the exact 246-case KUnit manifest green, run the 40/40 `LOAD=1` repro plus `-fast_parse 0` control with both rkvdec cores still in use, and replay the full FFmpeg suite; then install exact Published `c9428bedaa` and repeat the GRD fallback/recreation and AV1 container gates. | [dispatch-race verification gate](./findings/2026-08-07-rewrite-mpp-same-session-dual-core-dispatch-race.md#verification-gate), [lifetime integration gate](./findings/2026-07-30-ffmpeg-rkmpp-async-frame-lifetime-fix.md#verification-gate) |
