...
|---|-------|--------------|----------|--------|
| 1 | Kernel forward-port | ⚠️ The Published `…20260723~rk1` production kernel (tail `0001`–`0071`) booted and passed the full conformance and root-gate sets. Later fixes `0074`/`0075` are hardware-verified on a 6.18.40 KASAN build, but their matching production package still needs Launchpad publication, install, conformance, and rollback proof. | 2026-07-25 | [kernel status](./kernel-drivers/docs/forward-port-status.md) |
...
| 7 | GNOME Remote Desktop backend | ⚠️ Public `release/50.2-rkmpp@cf60b4d` keeps the validated 60 fps RKMPP path, cached-readback fix, bounded recovery, reconnect fixes, and ACK-resume recovery while leaving diagnostics archived. Source/native builds and RDP integration pass; PPA install plus sustained focus/resume and audio validation remain. | 2026-07-21 | [release patches](./apps/gnome-remote-desktop/patches/README.md), [testing](./apps/gnome-remote-desktop/docs/testing.md) |
...
|---|-------|------------|-------------|
| 1 | Kernel forward-port | Confirm the 6.18.40 source and arm64 build in Launchpad, install it from the PPA, then pass full conformance and rollback. | [6.18.40 KASAN validation](./findings/2026-07-25-forward-port-6-18-40-kasan-full-validation.md), [package checklist](./packaging/ppa/kernel-forward-port/README.md#remaining-checklist) |
...
| 7 | GNOME Remote Desktop backend | Install the clean 50.2 PPA build, repeat the macOS focus-away/return video gate, and confirm audible PCM with the normal codec offer. | [focus/resume gate](./apps/gnome-remote-desktop/docs/testing.md#10-exp6exp7-macos-focusresume-gate), [audio boundary](./apps/gnome-remote-desktop/docs/audio-redirection.md) |
