...
| 4 | Clean-room rewrite drivers | 🚧 Armbian-based 6.18.38 and v7.2-rc3 composites pass the warning-free focused build gate and local source/full arm64 package builds; the replacement packages are not uploaded, and no rewrite kernel has booted hardware proof. | 2026-07-16 | [rewrite-driver track](./kernel-drivers/docs/rewrite-drivers.md) §6 |
...
| 9 | Launchpad PPA | ⚠️ The recreated main system PPA and all four dedicated FFmpeg/kernel PPAs have Published sources and binaries. Experimental GRD reconnect-v2 source `18620800` was accepted and build `33399816` succeeded in the isolated staging PPA. Optional GDM upload and board migration/kernel/GRD runtime gates remain open. | 2026-07-15 | [`packaging/ppa/`](packaging/ppa/README.md) |
...
| 4 | Clean-room rewrite drivers | Upload/install one of the Armbian-based replacements, boot it on the board, and capture the first hardware conformance log. | [Rewrite acceptance commands](./kernel-drivers/tests/rewrite-conformance.md#rewrite-acceptance-one-command) |
...
- **State then:** At 20:28 PDT, the recreated `ubuntu-rock-5b` main PPA had all seven current
  ...
  Published. Later that evening, `ubuntu-rock-5b-experimental` accepted GRD
  reconnect-v2 source publication
