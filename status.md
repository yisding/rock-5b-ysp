...
| 4 | Clean-room rewrite drivers | 🚧 Current 6.18/mainline source tips incorporate the five applicable Rockchip 5.10 RGA reliability/cache-safety lessons and pass warning-free normal/memory/race clean-source gates plus device-free conformance validation. Existing Armbian package composites predate that commit, and no current rewrite kernel has booted hardware proof. | 2026-07-17 | [rewrite-driver track](./kernel-drivers/docs/rewrite-drivers.md) §6 |
...
| 4 | Clean-room rewrite drivers | Rebuild/package one current July 17 source tip, boot it on the board, run the focused cache-line/userptr-IOMMU gate, then capture the first full hardware conformance log. | [Rewrite acceptance commands](./kernel-drivers/tests/rewrite-conformance.md#rewrite-acceptance-one-command) |
