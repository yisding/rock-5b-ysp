...
| 4 | Clean-room rewrite drivers | 🚧 Booted KASAN/lockdep package `P91d6-Cad24` completed its then-current exact 85/85 MPP plus 148/148 RGA KTAP, but MPP case 83 disabled lockdep, so the compound gate remains red. Current byte-identical tips 6.18 `cd71f985a784c` and mainline `7dcb4c3b5a981` land the [2026-07-29 review-round-2 fixes](./findings/2026-07-29-rewrite-driver-review-round-2.md): legacy librga 90/270-degree rotates now validate (previously all failed `-EINVAL`), the hard-CCU chain has one writer, abort kicks the scheduler, RGA fences get per-fence contexts plus module pinning, shared-IRQ MMIO is power-gated, and the session-abort KUnit double free is repaired; the audit baseline is 327 signals with zero cross-tree drift. These tips remain unbooted. | 2026-07-29 | [KUnit fixture and evidence contract](./kernel-drivers/docs/rewrite-kunit.md), [rationalization plan](./kernel-drivers/docs/rewrite-kunit-rationalization-plan.md), [review round 2](./findings/2026-07-29-rewrite-driver-review-round-2.md) |
| 5 | ffmpeg tree | ⚠️ Public refs for canonical `main`, `ffmpeg-80`, and `ffmpeg-81` remain at source/FATE-validated tips. The normal PPA's bounded-wait 8.0 branch is built and Published but still needs the combined GRD runtime gate; canonical-tip and AV1 MP4/MKV board validation remain open. | 2026-07-19 | [FFmpeg status](./video-libraries/ffmpeg/README.md) |
...
| 7 | GNOME Remote Desktop backend | 🚧 The full-range BT.709 package experiment fixed the muted client-visible colors after a clean reboot on the tested macOS RDP path. The fix is promoted as `release/50.2-rkmpp@24f4392` and exact package `50.2+rkmpp+git20260729.14.24f4392-0ubuntu1~rk1`; source/native arm64 builds and the RDP integration test pass, TPM/EGL skip as expected, and Launchpad source `18647901` is accepted/Pending with arm64 build `33450532` running. The earlier first-frame production oops is independently root-caused to `CONFIG_DMABUF_DEBUG`, not GRD; kernels without it complete greeter and user-session RKMPP encode. Sustained focus/resume and audio remain open. | 2026-07-29 | [color fix](./findings/2026-07-29-grd-fullrange-bt709-fixes-muted-colors.md), [root cause + kernel fix](./findings/2026-07-28-dmabuf-debug-mangle-sg-table-is-the-sg-writer.md), [testing](./apps/gnome-remote-desktop/docs/testing.md) |
...
| 9 | Launchpad PPA | ⚠️ GRD `50.2+rkmpp+git20260729.14.24f4392-0ubuntu1~rk1` is accepted as source `18647901` and arm64 build `33450532` is running. The normal stack and comparison/rewrite builds are otherwise largely Published, while the 6.18.40 forward-port source remains only client-side uploaded. GRD publication/installation, clean board migration, combined kernel/GRD validation, and rollback remain open. | 2026-07-29 | [`packaging/ppa/`](packaging/ppa/README.md) |
...
| 14 | Desktop-app HW video (browsers) | 🚧 **HEVC Main now ships by default** alongside H.264 and VP9 Profile 0, and the whole decode gate set was re-run on the production-shaped stack — `6.18.40-ysp-rockchip64` (`CONFIG_KASAN` and `CONFIG_DMABUF_DEBUG` both unset) with the installed post-fix `mpp 1.5.0+git20260727.d8c6b88a` and `librga 2.2.0`. The "never measured on the shipping combination" caveat therefore no longer applies to the driver. Evidence widened from 8 pinned vectors to all 163 HEVC Main candidates in the FFmpeg FATE suite: 142 byte-exact, zero driver failures. Stock **VLC 3.0.23 and Firefox 153.0 both hardware-decode H.264 and HEVC** in a real GNOME session — VLC's earlier fallback was a missing `vaDeriveImage`, not only the headless session. Local fork commit `491533e` now rejects an unsupported sub-68-pixel Main10/VP9 Profile 2 context before decode and keeps a matching RGA pre-submit guard. The focused 64×240 gate software-decodes all 48 frames after one context rejection, with zero RGA conversions and zero kernel `no core match` messages; 68×240 context creation and the 320×240/416×240 Main10 regressions remain green. **The shipping artifact is still stale**: all of this used the fork build through `LIBVA_DRIVERS_PATH`, while the installed package remains `1.0.11+ysp1` from 2026-07-21. HEVC Main10 and VP9 Profile 2 stay hidden pending throughput/HDR and broader app qualification, Firefox's sandbox row is unproven, and Chromium is blocked on its own GL stack. | 2026-07-29 | [`rockchip-vaapi` project](./video-libraries/vaapi/README.md), [narrow-AFBC refusal](./findings/2026-07-29-rga-no-core-match-narrow-afbc-10bit.md), [shipping-stack gates](./findings/2026-07-28-vaapi-shipping-stack-gates-hevc-main-and-vlc-firefox-decode.md), [MPP TILES fix](./findings/2026-07-27-rockchip-mpp-hevc-tiles-same-id-pps-update.md) |
...
| 4 | Clean-room rewrite drivers | Build and boot a source-bound successor package from `cd71f985a784c`; require the exact ordered 84+148 manifest, a clean outer-KTAP interval with live lockdep, matching source/config/package identity, a clean aged kmemleak scan, every runtime/core, ABI replay, and full conformance — the first boot of the corrected rotate expectations and the double-free repair. | [qualification contract](./kernel-drivers/docs/rewrite-kunit.md#capture-a-reproducible-result) |
...
| 7 | GNOME Remote Desktop backend | Install the published `50.2+rkmpp+git20260729.14.24f4392-0ubuntu1~rk1` PPA build when available, confirm one clean GDM-to-user handover and RKMPP session, then run the focus/resume and audio gates on a production kernel without `CONFIG_DMABUF_DEBUG`. The color fix is runtime-verified on the tested macOS client; other clients and sustained behavior remain outside that result. | [color result](./findings/2026-07-29-grd-fullrange-bt709-fixes-muted-colors.md), [focus/resume gate](./apps/gnome-remote-desktop/docs/testing.md#10-exp6exp7-macos-focusresume-gate), [root cause + fix](./findings/2026-07-28-dmabuf-debug-mangle-sg-table-is-the-sg-writer.md#verification-gate) |
...
| 14 | Desktop-app HW video (browsers) | Package source commit `491533e` under a version newer than `1.0.11+ysp4`, install it, then re-run the decode, narrow-fallback, VLC and Firefox gates against the installed driver with no `LIBVA_DRIVERS_PATH` override. | [Verification gate](./findings/2026-07-28-vaapi-shipping-stack-gates-hevc-main-and-vlc-firefox-decode.md#verification-gate) |
...
| W05 | [Launchpad PPA publication](#watch-w05) | 2026-07-29 | **GRD `…20260729.14.24f4392` accepted as source `18647901`; arm64 build `33450532` running.** The promoted source adds the runtime-verified full-range BT.709 color signaling fix. **Prior:** `librga …20260725.26a50ef` and the 6.18.40 forward-port kernel were client-side uploaded with Launchpad confirmation still pending at their last check. The normal FFmpeg, prior GRD, production forward-port `…20260723`, rewrite-kernel replacements, and experimental GRD `~exp3` are Published with successful arm64 builds. |
...
- **State 2026-07-29:** Launchpad accepted
  ...
  to `bos03-arm64-101`. The source is Pending and the build is running. This
  is the clean 50.2 release branch plus the runtime-verified full-range BT.709
  color-signaling fix.
...
- **State 2026-07-23:** Root-caused, deterministically reproduced, and escalated to a
  ...
  `0058`-`0068`; in the **submit-now/CVE tier**
