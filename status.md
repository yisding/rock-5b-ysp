...
| 4 | Clean-room rewrite drivers | 🚧 KASAN/lockdep package `P3b08-Cad24`, built from `linux-6.18-rkvenc@c5faabf9d00b`, booted as `6.18.40-video-rewrite-kasan-rockchip64` but failed 1/85 MPP and 8/147 RGA KUnit cases. The failed MPP fixture left freed fake hardware on the live global list; ABI replay then oopsed while holding `hw_lock`, and the official suite blocked in its pre-case debugfs snapshot, so no MPP or RGA workload ran. Independently, overlapping core/IOMMU DT resources made both RGA3 cores fail probe with `-EBUSY`, leaving only RGA2. Published 6.18 tip `0cc483d3ee20` combines residual fixture repair `2241255f4cb2` with disjoint `0x200` RGA3 core resources; published mainline tip `edba1c58a726` carries the byte-identical fixture repair on `v7.2-rc5`. Package `Pf1f5-Cad24` contains the 6.18 fixes, all rewrite/KUnit/debug config symbols, and the corrected ROCK 5B DTB. It is package-inspected but not installed or boot-verified. | 2026-07-26 | [Failed repaired boot and DT fix](./findings/2026-07-26-rewrite-kunit-poisons-runtime-and-rga3-probe-fails.md) |
...
| 4 | Clean-room rewrite drivers | Install and boot KASAN/lockdep package `Pf1f5-Cad24`, then require 85/85 MPP plus 147/147 RGA KUnit cases with zero skips and a warning-clean interval before any userspace ioctl. Also require all intended MPP and all three RGA cores to bind. Run ABI replay alone and prove clean dmesg plus readable debugfs state before resuming paired counter/artifact conformance, including AVS2 and H.264/H.265 low-delay slice polling. | [Failure chain and exact gate](./findings/2026-07-26-rewrite-kunit-poisons-runtime-and-rga3-probe-fails.md#verification-gate), [remaining rewrite hardware gates](./kernel-drivers/docs/rewrite-conformance-gap-audit.md#remaining-gaps-and-hardware-gates) |
