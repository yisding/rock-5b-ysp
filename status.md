...
| 4 | Clean-room rewrite drivers | 🚧 Booted KASAN/lockdep package `P91d6-Cad24` completed its then-current exact 85/85 MPP plus 148/148 RGA KTAP, but MPP case 83 disabled lockdep through an uninitialized local DCHS lock, so the compound gate remains red. Current byte-identical tips 6.18 `669697f23d3df` and mainline `a49eb7575f436` retain that fixture repair, make both lifecycle suites opt-in, and remove the compile-time-owned MPP ABI-layout runtime case. The new 84+148 source gate passes KUnit-enabled and test-disabled clean-archive builds on both lines; its deliberate ABI mutation fails compilation, and the checked fixture-debt audit reports 248 known signals with zero new. The rationalized source remains unbooted. | 2026-07-28 | [pre-phase implementation and build evidence](./findings/2026-07-28-rewrite-kunit-pre-phase-applied.md), [prior boot warning and repair](./findings/2026-07-27-rewrite-reset-import-fixture-lockdep.md) |
...
| 4 | Clean-room rewrite drivers | Build and boot a successor package from `669697f23d3df`; require exact 232-case KTAP (84 MPP + 148 RGA), a clean complete-interval/live-lockdep artifact, a clean aged kmemleak scan, restored runtimes and every expected core before isolated ABI replay and full conformance. | [verification gate](./findings/2026-07-28-rewrite-kunit-pre-phase-applied.md#verification-gate) |
