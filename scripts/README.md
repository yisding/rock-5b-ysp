...
| [`check-doc-consistency.py`](check-doc-consistency.py) | Checks substantive drift and completeness only: nearest-README ownership and nested-README navigation, root patch placement, findings-index coverage and order, paired live-watchlist metadata with authority/recheck/freshness fields plus dated retired-ID stubs, optional ledger rows that map to dashboard tracks, contiguous status-table rows, synchronized kernel package helpers, distinct FFmpeg intended-source/published-install pins, GRD source pins, portable operational defaults, and the tracked-shell mode/shebang contract. It deliberately does not police status prose, dates, general ordering, or project-brief fields. |
| [`report-doc-duplication.py`](report-doc-duplication.py) | Produces the informational organization-migration baseline: identical long sentences, highly similar normalized paragraphs, version/SHA literals repeated across files, and current/as-of language outside designated dated-state owners. Findings require human classification and never fail the repository gate. |
