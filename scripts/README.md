...
| Script | Purpose |
|--------|---------|
| [`check-repo.sh`](check-repo.sh) | Runs the common repository handoff gate, in order: Markdown links and anchors, the `tests/` regression suite, the informational documentation-owner report, ShellCheck at warning-or-higher, the documentation consistency check, and staged/unstaged/untracked whitespace. Every stage runs even if an earlier one fails, so one run reports every problem; the exit summary names the failed stages. The documentation checks always cover the whole repository; ShellCheck and the minutes-long rewrite source-audit tests are scoped to what the branch changed, and `--all` (or `CHECK_REPO_ALL=1`) forces the complete matrix. |
...
| [`check-doc-consistency.py`](check-doc-consistency.py) | Checks substantive drift and completeness only: nearest-README ownership and nested-README navigation, root patch placement, live findings-index coverage/order, no promotion tombstones, active finding ownership for every temporary evidence bundle, paired live-watchlist metadata with authority/recheck/freshness fields plus dated retired-ID stubs, optional ledger rows that map to dashboard tracks, contiguous status-table rows, synchronized kernel package helpers, distinct FFmpeg and GRD intended-source/published-install pins, portable operational defaults, and the tracked-shell mode/shebang contract. It deliberately does not police status prose, dates, general ordering, or project-brief fields. |
| [`report-doc-duplication.py`](report-doc-duplication.py) | Produces the informational owner-review report: identical long sentences, highly similar normalized paragraphs, repeated version/SHA literals, current/as-of language outside dated-state owners, finding-evidence bundles without a live owner, dashboard rows with many routes, and missing project-brief concepts. Findings require human classification and never fail the gate; only the exact evidence-owner/tombstone invariant is enforced by `check-doc-consistency.py`. |
| [`repo_files.py`](repo_files.py) | Shared Git-aware maintained file inventories for the Python checks, with a pruned source-archive fallback, local Markdown-target resolution, temporary finding-evidence ownership discovery, and index-mode lookup for tracked files. |
| [`tests/test_repo_checks.py`](tests/test_repo_checks.py) | Standard-library regression tests for the checks above: file inventory, link classification and repo-escaping links, findings-index and watchlist contracts, synchronized package helpers, portable defaults, the shell shebang/mode contract, operational `--help` safety, kernel-log fatal signature scans, and the rewrite KUnit fixture-debt baseline guard. |
| [`update-findings-index.py`](update-findings-index.py) | Rebuilds the compact `findings/README.md` index from each dated finding's filename and H1 title; detailed summaries and trust tags stay in the finding instead of being copied into the index. |
