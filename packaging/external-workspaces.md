...
| `/home/yi/Code/rockchip-vaapi` | Working checkout of the VA-API-over-MPP driver, forked to `yisding/rockchip-vaapi`. `origin` = upstream woodyst `@e8c64dd` (v1.0.11); `fork` = `git@github.com:yisding/rockchip-vaapi.git`, branch `ysp/cleanup` carries the phase-one correctness/packaging work (built `.deb` `rockchip-vaapi_1.0.11+ysp1_arm64.deb`, board-validated). Source of truth is the fork remote, not this repo; verdict, fixes, and validation in [`findings/2026-07-21-rockchip-vaapi-driver-review.md`](../findings/2026-07-21-rockchip-vaapi-driver-review.md). Reconstructible by cloning the fork. |
