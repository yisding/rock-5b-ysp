...
| Conformance/test harness source | [`../kernel-drivers/tests/`](../kernel-drivers/tests/README.md) and `../kernel-drivers/tests/conformance/` | `/home/yi/Code/rock-5b/build/rockchip-conformance` deployed bundle |
...
| `/home/yi/Code/rock-5b/build/kernel/rock5b-kernel-build/armbian-build/userpatches/config-rock5b-debug-kernel.conf.sh` | Imported to `kernel-drivers/scripts/debug-kernel/`; the wrapper deploys it back into `userpatches/` at build time. |
| `/home/yi/Code/rock-5b/build/kernel/rock5b-kernel-build/armbian-build/userpatches/customize-image.sh` | Stock Armbian example/template, not project source. Leave outside. |
| `/home/yi/Code/rock-5b/build/kernel/rock5b-kernel-build/ffmpeg-stack` | Legacy test/runtime staging and media, not canonical source. Current test scripts live in `kernel-drivers/tests/`. |
...
| `/home/yi/Code/rock-5b/gnome/grd/gnome-remote-desktop` | GRD fork checkout consumed by the [`ppa/build-source-packages.sh`](ppa/build-source-packages.sh) defaults. W10 owns the dated public release/recovery branch heads; historical reconnect, recovery, readback, focus, and audio experiment tips remain in [`../docs/source-trees.md` §5](../docs/source-trees.md#5-gnome-remote-desktop-base) and the [`patches/archive/`](../apps/gnome-remote-desktop/patches/archive/README.md). The exact published `~exp3` source is recoverable from Launchpad publication `18626586`. Generated build trees stay outside this repo. |
...
| `/home/yi/Code/rock-5b/build/gnome/grd-pkg` and `/home/yi/Code/rock-5b/gnome/grd/grd-ppa` | Historical local package/source-package exports and generated `.deb`s. Canonical packaging is in `ppa/gnome-remote-desktop/`, `ppa/gdm-hwenc/`, and `gdm-hwenc/`. |
...
| `/home/yi/Code/rock-5b/build/rockchip-conformance` | Deployed test bundle with logs/assets/build output. The tracked skeleton and patches live under `kernel-drivers/tests/conformance/`. |
| `/home/yi/Code/rock-5b/rockchip-vaapi` | Working checkout of the VA-API-over-MPP driver. `origin` is upstream `woodyst/rockchip-vaapi`; `fork` is `git@github.com:yisding/rockchip-vaapi.git`, with the latest recorded public checkpoint at `main@03e6cb6`. Clone that fork to reconstruct the source; retained probe bundles live under `/home/yi/Code/rock-5b/build/vaapi/` and reproducible objects/test residue are discarded. The maintained capability/evidence boundary and next gate live in [`../video-libraries/vaapi/`](../video-libraries/vaapi/README.md), while volatile fork state is tracked by [`status.md` W18](../status.md#watch-w18). |
| `/home/yi/Code/rock-5b/ubuntu-rockchip` and `/home/yi/Code/rock-5b/ubuntu-rockchip-settings` | Reference clones of Joshua Riek's archived ubuntu-rockchip image builder (`@38dfb49`) and settings package, surveyed 2026-07-21. Read-only quarry; durable disposition is in the [VA-API consumer scorecard](../video-libraries/vaapi/docs/validation.md#consumer-and-sandbox-conclusions). Reconstructible by re-cloning GitHub. |
...
- Old Armbian kernel `.deb` sets under
  `/home/yi/Code/rock-5b/build/kernel/rock5b-kernel-build/armbian-build/output/debs/` are
  rebuild output; keep only the versions needed for immediate rollback/testing.
- `/home/yi/Code/rock-5b/build/kernel/rock5b-kernel-build/armbian-build/cache/` is a build
