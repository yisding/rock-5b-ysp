...
| `/home/yi/Code/rockchip-vaapi` | Working checkout of the VA-API-over-MPP driver, forked to `yisding/rockchip-vaapi`. `origin` = upstream woodyst; `fork` = `git@github.com:yisding/rockchip-vaapi.git`, with active development and pushed checkpoints on `main` (latest recorded here: `b579bad`). Phase 0/1 are complete; Phase 2 has 7/8 HEVC Main vectors exact, generated plus pinned Main10/Profile 2 P010 gates exact, static HDR10 metadata green, and exact stock-GStreamer readback. Version `1.0.11+ysp3` builds safe split driver/config `.deb`s. Experimental one-slice H.264 Main/High and HEVC Main encode pass FFmpeg CQP/CBR/VBR, GStreamer `vah264enc`/`vah265enc`, parser, ASan/UBSan, and concurrent shipping-decode gates; HEVC requires the native MPP CTU64 capability contract. Profiles and encode remain honestly hidden where app/conformance boundaries are open. Source of truth is the fork remote, not this repo; see the [`Main10/P010`](../findings/2026-07-26-rockchip-vaapi-main10-afbc-p010-validation.md), [`H.264 encode`](../findings/2026-07-26-rockchip-vaapi-h264-va-encode-validation.md), and [`HEVC encode`](../findings/2026-07-26-rockchip-vaapi-hevc-va-encode-validation.md) findings. Reconstructible by cloning the fork. |
