...
|-----|------|-------------------------------|
| `ppa:yi-ding/ubuntu-rock-5b` | Normal system stack: codec udev access, MPP, librga, FFmpeg 8.0.3 Rockchip, patched GNOME Remote Desktop, co-installable FFmpeg 6.1 tools, and the Linux 6.18 forward-port kernel. | The seven existing source/binary sets are Published. Replacement forward-port kernel source `18624245` is accepted and arm64 build `33407351` is running; the previous kernel binaries remain Published. This is the only normal system-stack install target. |
...
`2026-07-16T16:07:14-07:00` and in
...
| forward-port kernel | `6.18.38+rk3588av1fwport20260716-0ubuntu1~rk1` under [`kernel-forward-port/`](kernel-forward-port/README.md) | Replacement source publication [`18624245`](https://launchpad.net/~yi-ding/+archive/ubuntu/ubuntu-rock-5b/+sourcepub/18624245) is accepted and arm64 build [`33407351`](https://launchpad.net/~yi-ding/+archive/ubuntu/ubuntu-rock-5b/+build/33407351) is running. Previous source `18619788` and all three prior binaries remain Published. | Carries the Rockchip 5.10 RGA reconciliation and RKVENC2 multi-slice error fix. The full local Armbian integration build passed as `Pf618-Cb831`; board install/revert validation is pending. |
...
| Forward-port kernel source version | `KERNEL_PPA_UPSTREAM_VERSION` | `6.18.38+rk3588av1fwport20260716` |
...
The GRD exporter archives the clean public `GRD_COMMIT` before creating the
orig tarball. `GRD_DELTA` is empty by default; when it is explicitly non-empty,
the helper applies it after the archive export. Override `GRD_REPO`,
`GRD_COMMIT`, `GRD_UPSTREAM_VERSION`, and `GRD_DELTA` together when
reconstructing a different or historical GRD source state.
