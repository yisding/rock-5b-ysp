...
kernel. The source package now builds; the binary package build and board
install/revert validation are still pending.
...
- Full local binary build from the generated source package.
- Payload comparison against the Armbian-built local `.deb`s.
...
1. Build the co-installable image, DTB, and headers packages locally from the
   generated `.dsc`.
2. Compare installed payloads against the Armbian-built local debs for kernel
   image, modules, DTBs, maintainer scripts, and bootloader integration.
3. Validate install, reboot, rollback, and `kernel-revert.sh` recovery on the
   board.
4. Sign the source upload and `dput` it to the PPA only after the binary and
   board gates pass.
