...
1. **Dispatch by compatible** (the driver change in `vendor-forward-port.md`): once
...
The convert (in `kernel-drivers/patches/...-02-...dt.patch`, applied in `rk3588-rock-5b.dtsi`):
...
probes. That is exactly why the vanilla/inline form (`vanilla-kernel.md`) must spell
...
(`hit=0 miss=0`). `kernel-drivers/scripts/build-combined-kernel.sh` passes it correctly. Full
