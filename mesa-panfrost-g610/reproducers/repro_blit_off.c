...
// FBO, and reads back a SUBREGION starting at x = X0 (argv[2], default 8000)
// with glReadPixels(GL_RGBA_INTEGER, GL_UNSIGNED_INT), which forces the
// st_ReadPixels staging blit with src_x1 = X0 and dst_x1 = 0. Any spatial
// shift shows up as output[i].r != X0 + i.
// Verified exact (0 mismatches) at X0 = 1, 623, 8000, 16000 on the
// panfrost-transfer-fragcoord-blit branch (git-2f6e8a6afc), 2026-07-01.
//
...
int main(int argc, char **argv) {
   ...
   int X0 = (argc > 2) ? atoi(argv[2]) : 8000;
   int H = 1;
   ...
   p_glTexImage2D(GL_TEXTURE_2D, 0, GL_RG32UI, W, H, 0, GL_RG_INTEGER, GL_UNSIGNED_INT, src);
   check_gl("texImage");
   ...
   p_glReadPixels(X0, 0, W - X0, H, GL_RGBA_INTEGER, GL_UNSIGNED_INT, dst);
   check_gl("readpixels");

   long mismatches = 0;
   int lo = -4, hi = 4; long hist[9] = {0}; long other = 0, zeros = 0;
   int first_mismatch = -1;
   for (int i = 0; i < (W - X0) * H; i++) {
      unsigned r = dst[i*4+0];
      if (r != (unsigned)(X0 + i)) { mismatches++; if (first_mismatch < 0) first_mismatch = i; }
      if (r == 0 && i != 0) zeros++;
      long shift = (long)r - (long)(X0 + i);
      if (shift >= lo && shift <= hi) hist[shift - lo]++; else other++;
   }
   printf("W=%d X0=%d  mismatches=%ld / %d  (%.1f%%)  first_mismatch=%d  zeros(nonhead)=%ld\n",
          W, X0, mismatches, (W-X0)*H, 100.0*mismatches/((W-X0)*H), first_mismatch, zeros);
   printf("shift histogram (sampled_texel - i):\n");
   for (int s = lo; s <= hi; s++) printf("   shift %+d : %ld\n", s, hist[s - lo]);
   printf("   other    : %ld\n", other);
   printf("i : sampled_r : shift : implied_scale(r/i)\n");
   for (int i = 256; i < W - X0; i *= 2) {
      long r = dst[i*4+0];
