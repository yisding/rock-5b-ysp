...
| 2. Boot lifecycle and KUnit | [Post-reboot preflight](#post-reboot-identity-and-ownership-preflight) | The exact expected 103 MPP + 152 RGA cases ran; their complete boot interval is fatal-free, lockdep is still live, and production services/cores were restored after test isolation. | Media correctness, userspace compatibility, or hardware scheduling beyond the paths the fixtures model. |
...
exact boot/package/source identity
  -> correlated 255-case KUnit report + full fatal/lockdep interval
...
For exact Phase 3H heads 6.18
`1784358650e3c41abbd5154cbd1737da07d968de` and mainline
`e71d368cc48374638eafbfdf1e9c6864cabbd118`, all eight warning-fatal
clean-archive profiles above pass: `normal`, `test-disabled`, `memory`, and
`race` on both lines. The instrumented profiles are compile evidence only, not
booted sanitizer or race evidence. The exact boundary is recorded in the
[Phase 3H finding](../../findings/2026-08-09-rewrite-phase-3h-mpp-recovered-terminal-quarantine.md).
...
`*-kunit.tsv` to report exactly 103 MPP plus 152 RGA cases with no fail/skip,
