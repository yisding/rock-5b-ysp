...
| 2. Boot lifecycle and KUnit | [Post-reboot preflight](#post-reboot-identity-and-ownership-preflight) | The exact expected 109 MPP + 152 RGA cases ran; their complete boot interval is fatal-free, lockdep is still live, and production services/cores were restored after test isolation. | Media correctness, userspace compatibility, or hardware scheduling beyond the paths the fixtures model. |
| 3. ABI boundary | [Raw ABI replay](#raw-abi-replay-comparisons) | On forward-port and rewrite profiles, safe query/import/release/parser behavior and explicit negative contracts match the selected profile after normalization. The stateful replay is excluded from the vendor BSP. | Correct registers, pixels, bitstreams, interrupts, or recovery under active work. |
...
- `memory`: enables KASAN, fault-injection options used by the fail-nth
  ioctl unwind tests, and the debug-object lifecycle family mirrored from
  [debug-kernel.md](../docs/debug-kernel.md).
...
| `rewrite-ownership-source-audit.py` | device-free, source-pinned inventory of the production ownership seams targeted by the refactor: reset/cluster/DMA ownership; IRQ/register leases; typed activation and RGA task-execution references and slot/claim/retry/quarantine transfers; MPP builder/seal/const-backend contracts; immutable RGA task-plan publication; activation storage, closure, reason, outcome, resource, reclaim, and dispatch schemas; power/IOMMU transitions; lifecycle/outcome publication; and the corresponding RGA active/IRQ/timeout, execution-map, command, watchdog, sole task-advance, outcome, terminal, and start owners. Hard contracts reject schema/type/enum/priority drift, mutable MPP backend images, raw-task RGA emitters, unauthorized field writers, duplicate terminal tails, non-central `DONE` publication, incomplete resource drain or retry handoff, unsafe reclaim order, unpinned backend dispatch, obsolete helpers, an unpinned source HEAD, and cross-tree drift even if the TSV is regenerated. `rewrite-build-gate.sh` runs it before every profile. The current boundary-hardening heads pass at exactly 2,313 signals/tree with zero drift. |
| `rewrite-writer-inventory.py` | device-free, source-pinned writer inventory for protected state on top of the ownership audit: every production mention of the MPP/RGA active/timeout/IRQ slot references, activation slot/resource terminal-state writes (including RGA `WRITE_ONCE` forms via function scoping), generation-cookie writes including the `_seq` allocator, RKVDEC dispatch-lease writes, reset-control API calls, and `publish_and_start()` call sites. Generation coverage includes the `activation_generation_seq` allocator and AV1 AFBC armed/status cookies; RGA terminal-state coverage is scoped to execution-lifecycle function names; slot-ref categories inventory every production mention (reads included). New signals fail; resolved signals are allowed; an unpinned current HEAD fails like the sibling audit. Doorbell MMIO remains guarded by the ownership audit's `start-doorbell-write` allowlist instead of a duplicate rule here. `--self-test` generates its baseline from a pristine scratch mirror so the head-pin guard cannot mask detection, then requires the injected synthetic dispatch-lease writer to be named in the failing diff — no kernel build. `rewrite-build-gate.sh` audit modes run it against both trees. |
...
`*-kunit.tsv` to report exactly 109 MPP plus 152 RGA cases with no fail/skip,
