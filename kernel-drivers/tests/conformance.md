...
| `rewrite-ownership-source-audit.py` | device-free, source-pinned inventory of the production ownership seams targeted by the refactor: MPP reset, active-slot access/write, dispatch-lease access/write, CCU-power, semantic IOMMU transitions plus raw backend operations, lifecycle/outcome writes and publication entries, terminal entries, IRQ acknowledgements, and start writers; shared IRQ/fault/watchdog snapshots, activation timing, and terminal/admission-state writers; plus RGA active-slot access/write, execution-map and command release, task-advance, command-writer, raw-task-emitter, outcome writers, terminal entries, and start writers. Its checked TSV permits reviewed signal removal but rejects new signals, an unpinned source HEAD, a whole-category parser disappearance, or cross-tree drift. `rewrite-build-gate.sh` runs it before every profile. |
