...
| `rewrite-ownership-source-audit.py` | device-free, source-pinned inventory of the production ownership seams targeted by the refactor: MPP reset, active-slot, dispatch-lease, CCU-power, IOMMU, terminal, and start writers plus RGA active-slot, task-advance, command-writer, raw-task-emitter, and start writers. Its checked TSV permits reviewed signal removal but rejects new signals, an unpinned source HEAD, a whole-category parser disappearance, or cross-tree drift. `rewrite-build-gate.sh` runs it before every profile. |
