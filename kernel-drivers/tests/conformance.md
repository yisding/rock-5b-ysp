...
| 2. Boot lifecycle and KUnit | [Post-reboot preflight](#post-reboot-identity-and-ownership-preflight) | The exact expected 105 MPP + 152 RGA cases ran; their complete boot interval is fatal-free, lockdep is still live, and production services/cores were restored after test isolation. | Media correctness, userspace compatibility, or hardware scheduling beyond the paths the fixtures model. |
...
exact boot/package/source identity
  -> correlated 257-case KUnit report + full fatal/lockdep interval
...
For exact Phase 3I heads 6.18 `395644689db8f` and mainline
`38768c5cff419`, focused KUnit-enabled MPP objects and all eight warning-fatal
clean-archive profiles above (`normal`, `test-disabled`, `memory`, and `race`
on both lines) compile. Every booted sanitizer, race, and runtime result remains
pending. Strict full-series checkpatch is 0/0/0 over 1,076 lines, ownership is
2,180 signals/tree, fixture debt is 306/tree, and the exact source manifest is
104 MPP + 152 RGA = 256.
The exact boundary is recorded in the
[Phase 3I finding](../../findings/2026-08-09-rewrite-phase-3i-mpp-clean-terminal-observations.md).
...
| `rewrite-ownership-source-audit.py` | device-free, source-pinned inventory of the production ownership seams targeted by the refactor: MPP raw reset backends plus reset-domain binding, registry identity/member-head access, member lifecycle, transaction entry, pending-operation access, and state writes; register/IRQ lease helper entries, accesses, and writes; typed `{activation, generation}` active/timeout reference slots, base/external reference schemas and helper routing, and hard-allowlisted access/write; the activation storage, current-pointer, retained-list, allocation/free and retry-closure/token schemas, plus typed-alias-aware generation/deadline/state/reason/closure accesses, helper entries, field-specific writers, exact enums, reasoned claim callers, retry proof ordering, and fixed IOMMU-fault priority; the exact session-scoped activation-dispatch schema plus hard-allowlisted owner access/write; CCU-power fields and transitions, every raw PM/clock backend call, `power_count` writes, semantic IOMMU transitions plus raw backend operations, lifecycle/outcome writes and publication entries, terminal entries, IRQ acknowledgements, watchdog-arm entries, and start writers; shared IRQ/fault/watchdog snapshots, activation timing, and terminal/admission-state writers; plus RGA active-slot access/write, execution-map and command release, watchdog arms, task-advance, command-writer, raw-task-emitter, outcome writers, terminal entries, and start writers. Its field matcher covers nested embedded fields, typed pointer aliases, parenthesized/indexed dereferences, explicit casts, slot pointees, whole-object assignment and memory writers, prefix/postfix updates, ordinary/compound/shift assignments, store/exchange publishers, and the reset pending counter's non-read atomic publishers in either standard argument position; atomic power-count writes are recognized with the target in either standard argument position. Activation/slot/closure writers, claim and retry-proof routing, recovery-result writers, token escape, and dispatch-owner misuse are rejected even if the TSV is regenerated; obsolete detach/dispatch helpers, schema relocation/duplication/type drift, enum or priority drift, and divergent-tree baseline output fail closed. The checked TSV otherwise permits reviewed signal removal but rejects new signals, an unpinned source HEAD, a whole-category parser disappearance, or cross-tree drift. `rewrite-build-gate.sh` runs it before every profile. Phase 3H additionally freezes the claim token's active-slot job-reference ownership, typed recovered-terminal core/group records, exact quarantine generation, distinct diagnostic/core/group status routing, service tombstone list/count, admission closure, retry refusal after proof failure, and remove/shutdown retention. Phase 3I additionally freezes the exact clean-observation enum/status writers and finish routing, advisory bus-idle fields, impossible-finisher quarantine, removed legacy fallback, and AV1 retained-slot path. Phase 3J freezes the base-biased activation lifetime, typed external `{activation, generation}` plus containing-job reference pairs, clone/move/put helpers, active/timeout/claim/retry/quarantine transfers, retry-refusal quarantine, and final-teardown balance; its exact inventory is 2,251 signals/tree with zero drift. |
...
`*-kunit.tsv` to report exactly 105 MPP plus 152 RGA cases with no fail/skip,
...
empty `active_activation` slot. The software owner must be published before the
