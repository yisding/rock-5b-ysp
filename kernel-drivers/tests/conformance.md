...
| 2. Boot lifecycle and KUnit | [Post-reboot preflight](#post-reboot-identity-and-ownership-preflight) | The exact expected 108 MPP + 152 RGA cases ran; their complete boot interval is fatal-free, lockdep is still live, and production services/cores were restored after test isolation. | Media correctness, userspace compatibility, or hardware scheduling beyond the paths the fixtures model. |
...
| `rewrite-ownership-source-audit.py` | device-free, source-pinned inventory of the production ownership seams targeted by the refactor: reset/cluster/DMA ownership; IRQ/register leases; typed activation references and slot/claim/retry/quarantine transfers; activation storage, closure, reason, outcome, resource, reclaim, and dispatch schemas; power/IOMMU transitions; lifecycle/outcome publication; and the corresponding RGA active-slot, execution-map, command, watchdog, task-advance, outcome, terminal, and start writers. Hard contracts reject schema/type/enum/priority drift, unauthorized field writers, duplicate terminal tails, non-central `DONE` publication, incomplete resource drain or retry handoff, unsafe reclaim order, unpinned backend dispatch, obsolete helpers, an unpinned source HEAD, and cross-tree drift even if the TSV is regenerated. `rewrite-build-gate.sh` runs it before every profile. Completed Phase 3 passes at exactly 2,314 signals/tree with zero drift. |
...
`*-kunit.tsv` to report exactly 108 MPP plus 152 RGA cases with no fail/skip,
