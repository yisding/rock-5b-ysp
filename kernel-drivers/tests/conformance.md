...
| 2. Boot lifecycle and KUnit | [Post-reboot preflight](#post-reboot-identity-and-ownership-preflight) | The exact expected 96 MPP + 152 RGA cases ran; their complete boot interval is fatal-free, lockdep is still live, and production services/cores were restored after test isolation. | Media correctness, userspace compatibility, or hardware scheduling beyond the paths the fixtures model. |
...
exact boot/package/source identity
  -> correlated 248-case KUnit report + full fatal/lockdep interval
...
| `rewrite-ownership-source-audit.py` | device-free, source-pinned inventory of the production ownership seams targeted by the refactor: MPP raw reset backends plus reset-domain binding, registry identity/member-head access, member lifecycle, transaction entry, pending-operation access, and state writes; register/IRQ lease helper entries, accesses, and writes; active-slot access/write, dispatch-lease access/write, CCU-power fields and transitions, every raw PM/clock backend call, `power_count` writes, semantic IOMMU transitions plus raw backend operations, lifecycle/outcome writes and publication entries, terminal entries, IRQ acknowledgements, watchdog-arm entries, and start writers; shared IRQ/fault/watchdog snapshots, activation timing, and terminal/admission-state writers; plus RGA active-slot access/write, execution-map and command release, watchdog arms, task-advance, command-writer, raw-task-emitter, outcome writers, terminal entries, and start writers. Its field matcher covers prefix/postfix updates, ordinary/compound/shift assignments, store/exchange publishers, and the reset pending counter's non-read atomic publishers in either standard argument position; atomic power-count writes are recognized with the target in either standard argument position. The checked TSV permits reviewed signal removal but rejects new signals, an unpinned source HEAD, a whole-category parser disappearance, or cross-tree drift. `rewrite-build-gate.sh` runs it before every profile. |
...
`*-kunit.tsv` to report exactly 96 MPP plus 152 RGA cases with no fail/skip,
