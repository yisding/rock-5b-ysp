...
# Shared result and artifact comparator. Thin suite-specific launchers set the
# suite name and the target project's artifact requirement before sourcing it.
set -euo pipefail

TEST_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
REPO_ROOT=$(cd "$TEST_DIR/../.." && pwd)
: "${SUITE_COMPARE_NAME:?set SUITE_COMPARE_NAME before running suite-compare.sh}"
SUITE_COMPARE_LABEL=${SUITE_COMPARE_LABEL:-$SUITE_COMPARE_NAME}
...
find_latest_summary()
{
	...
	summary=$({ find "$CONFORMANCE_ROOT/logs/$profile" \
		-path "*-$SUITE_COMPARE_NAME-suite/summary.tsv" \
		-type f -printf "%T@ %p\n" 2>/dev/null || true; } | sort -nr |
		awk 'NR == 1 { sub(/^[^ ]+ /, ""); print }')
	if [ -z "$summary" ]; then
		printf 'No %s suite summary found for profile %s\n' \
			"$SUITE_COMPARE_LABEL" "$profile" >&2
		return 1
	fi
