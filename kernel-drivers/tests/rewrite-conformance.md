...
maintenance check, and paired-evidence audit selftest.
...
- RGA: `copy`, `resize`, `cvtcolor`, `fill`, `alpha`, `transform`, `async`, and
  ...
  `c_RkRgaBlit` resize
  paths, and records an NV12 raster-to-tile8x8-to-raster round-trip artifact;
...
legacy RGB resize, tile8x8 round-trip, Gaussian matrix, sequential IM2D task-job copy chains,
...
```bash
VALIDATE_ONLY=1 bash rewrite-conformance-run.sh  # device-free runner/syzlang/syzkaller/ioctl-fuzz/librga-smoke/gstreamer-harness/iommu-fuzz/recovery/case/comparator/evidence wiring check
...
crop/CSC/resize fence chain, and after it added a public-API tile8x8
round-trip artifact. Historical reruns include the checks after generated GStreamer H.264
