...
`vp9enc`, `ivfmux`, or `ivfparse`. AV1 remains opt-in for the shared rewrite
profile; the dedicated 6.18 AV1 branch implements the backend. Set
...
| `rewrite-conformance-run.sh` | same device and dependency access as the selected suites; sequences booted KUnit verification, system-info, ABI replay, MPP, librga, GStreamer, FFmpeg, optional `rkmppenc`, optional debugfs counter checks, and optional comparator steps. Rewrite profiles default `RUN_KUNIT_CHECK=1`, persist an exact 89-MPP/147-RGA report, and require readable before/after dmesg for every suite. `VALIDATE_ONLY=1` additionally checks the KUnit parser, shared dmesg gate, all 31 MPP case builders, and the existing build/parser/comparator/audit helpers. With `RUN_COUNTER_CHECKS=1`, it requires expected hardware/fence-path deltas, zero idle gauges, and present safety counters with no positive delta; per-suite prefix variables still support multicore-spread requirements. |
| `rewrite-kunit-log-check.sh` | runtime mode reads `/sys/kernel/debug/kunit/{rk_mpp_rewrite,rockchip-rga-rewrite}/results`, requires exactly 89 + 147 cases with no fail/skip, and optionally writes `KUNIT_REPORT`; `--selftest` is device-free. |
...
`dmesg-scan.tsv`, requires each selected rewrite suite's run-matched `*-kunit.tsv` to report
exactly 89 MPP plus 147 RGA cases with no fail/skip, and requires the named
official-MPP core matrix. `REQUIRE_DMESG_EVIDENCE=0`,
...
AV1 remains opt-in because the dedicated RKMPP rewrite backend has compile
evidence but no board result. The GStreamer plugin advertises `video/x-av1`, so
the suite has
