...
| 2. Boot lifecycle and KUnit | [Post-reboot preflight](#post-reboot-identity-and-ownership-preflight) | The exact expected 90 MPP + 148 RGA cases ran; their complete boot interval is fatal-free, lockdep is still live, and production services/cores were restored after test isolation. | Media correctness, userspace compatibility, or hardware scheduling beyond the paths the fixtures model. |
...
| Test | Needs |
|------|-------|
| `build-mpp-tests.sh` | optional legacy-comparison builder; no device access and writes staged MPP library/tests under `../rock-5b/rockchip-conformance/out/mpp` |
...
| `librga-suite.sh` | device access for `/dev/rga`, `/dev/dma_heap/*`, optional DRM render nodes, readable debugfs/dmesg for full logs, and the installed `librga-dev` package for the in-repo `ysp_librga_smoke` artifact case; root is the simplest mode. `LIBRGA_LIBDIR` selects an explicit staged or legacy runtime. |
...
`*-kunit.tsv` to report exactly 90 MPP plus 148 RGA cases with no fail/skip,
