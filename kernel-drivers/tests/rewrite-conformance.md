...
maintenance check, and paired-evidence audit selftest, including the
...
rotation plus partial-blend smoke if appliance/display usage becomes part of
the target profile.
...
XRGB fd-backed legacy `c_RkRgaBlit()` display-rotation artifacts plus a
...
- RGA: `copy`, `resize`, `cvtcolor`, `fill`, `alpha`, `transform`, `async`, and
  ...
  paths. It can also opt into BGRA/XRGB display-tail rotation artifacts and a
  BGRA partial-rectangle alpha-blend artifact with
