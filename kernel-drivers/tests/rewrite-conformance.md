...
| `rewrite-kunit-log-check.sh` | runtime mode reads `/sys/kernel/debug/kunit/{rk_mpp_rewrite,rockchip-rga-rewrite}/results`, requires exactly 86 + 120 cases with no fail/skip, and optionally writes `KUNIT_REPORT`; `--selftest` is device-free. |
...
exactly 86 MPP plus 120 RGA cases with no fail/skip, and requires the named
