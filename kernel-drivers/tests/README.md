...
| `rewrite-reset-contention.sh` | **sibling-core reset deassert race measurement** (rkvdec soft CCU) | Provokes the overlap between a core's recovery reset pulse and a sibling submit's unconditional `reset_control_deassert`, then reads `mpp:reset_deassert_contended_count`. `EXPECT=contended` passes when the race is observed (reachability evidence, expected before the per-reset-domain lock lands); `EXPECT=clean` passes when it is not (regression gate, only meaningful once the same workload has shown a non-zero delta beforehand). Root-only, since the counter lives in debugfs; exit `77` when not root, when the decoder binary or input is missing, or when the kernel predates the counter. |
...
> | `EXPECT`, `RESET_LOOPS`, `RESET_SURVIVORS` | `rewrite-reset-contention.sh` | `contended` to require the race be observed (pre-fix reachability), `clean` to require it absent (post-fix regression), or unset to report only; plus the kill-cycle count (default `40`) and the number of concurrent surviving decoders (default `2`) |
