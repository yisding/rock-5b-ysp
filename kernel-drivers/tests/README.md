...
| `librga-smoke.sh` | **direct librga/im2d functional test** on current `/dev/rga` owner | Builds and runs a tiny C++ im2d client against staged `librga`: virtual-address imports, sync `imcopy`/`imresize`/`imfill`, forced RGA3 core-mask + priority copy through `improcess`, and an async acquire/release-fence copy chain waited with `imsync`. This exercises the maintained librga import/submit/core/fence paths independently of FFmpeg. Exit `77` means `/dev/rga` is absent. |
