...
> | `RUN_KUNIT_CHECK` | `rewrite-conformance-run.sh` | defaults to `1` for rewrite profiles; requires the ordered 89 MPP + 150 RGA manifest with no fail/skip, a clean outer-KTAP interval, live lockdep, and source/config/package identity, then persists `$LOG_ROOT/$RUN_ID-kunit.tsv` |
