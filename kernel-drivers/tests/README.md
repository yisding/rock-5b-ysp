...
[`conformance.md`](./conformance.md) so this page stays a clean
newcomer on-ramp.
...
| Developer focus | Keep each test's isolation clear: decoder-only software inputs, encoder PSNR/fault checks, and FFmpeg transcode paths with no software fallback. The rewrite build gate and conformance suites live in [`conformance.md`](./conformance.md). |
| Owns | The smoke tests `test-decode.sh`, `decode-differential.sh`, `encode-test-tiny.sh`, `transcode-test.sh`, `iep2-smoke.sh`, `rewrite-smoke.sh`, `mpp-debug-capture.sh`, `abi-probe.sh`/`abi-probe.c`, `abi-replay.sh`, `librga-smoke.sh`/`librga-smoke.cpp`, the single-purpose gate probes `rga-p010-test.cpp`, `rga-nv15-test.cpp`, `rga-core-match-test.cpp`, `legacy-blit-test.c`, `rga-10bit-legacy-stride-test.c`, and `rga-10bit-uv-offset-test.c`, and the targeted RGA MMU diagnostic `rga-mmu-debug.sh`; the root-gate orchestrator `run-root-gates.sh` (runs the root-only forward-port gates — `encode-test-tiny.sh`, `transcode-test.sh`, `rga-mmu-debug.sh`, `iommu-machinery-fuzz.sh`, `mpp-debug-capture.sh`, and the opt-in destructive `mpp-vp9-show-existing-repro.sh` — each with a per-gate journal fatal scan, forcing the hardware `/usr/bin/ffmpeg`); the recovery/RGA/IOMMU stress tools `rewrite-recovery-stress.sh`, `rewrite-slice-stress.sh`, `rewrite-reset-contention.sh` plus its corrupt-stream builder `rewrite-corrupt-stream.py` and the resumable lock-verification gate `rewrite-reset-lock-gate.sh`, `rewrite-fixes-gate.sh`, `iommu-machinery-fuzz.sh`, `rga-iommu-fuzz.cpp`, and [`IOMMU-FUZZING.md`](./IOMMU-FUZZING.md); the KASAN reset-session gate `reset-session-kasan.sh` and shared `sanitizer-scan.sh`; the GRD system-heap scatterlist-corruption reproducer `grd-sg-oops-repro.sh`; the conformance catalog under [`conformance/`](conformance/README.md), `run-conformance.sh`, `suite-common.sh`, `debugfs-counters.sh`, `rewrite-evidence-audit.sh`, `rewrite-kunit-log-check.sh` plus its ordered `rewrite-kunit-manifest.tsv`, the fixture-debt guard `rewrite-kunit-source-audit.py` plus its checked `rewrite-kunit-source-audit-baseline.tsv`, the production ownership inventory `rewrite-ownership-source-audit.py` plus its checked source-pinned baseline, the protected-state writer inventory gate `rewrite-writer-inventory.py` plus its checked `rewrite-writer-inventory-baseline.tsv` (fails any new production mention of active/timeout/IRQ slot refs — reads included, deliberately stricter — plus new terminal-state writers with RGA scoped to execution-lifecycle function names, generation-cookie writers including the `activation_generation_seq` allocator and AV1 AFBC cookies, RKVDEC dispatch-lease writes, reset-control API calls, and bare or prefixed `publish_and_start` call sites; doorbell MMIO stays guarded by the ownership audit's `start-doorbell-write` allowlist rather than duplicated here; `--self-test` builds a pristine scratch mirror baseline and requires the injected synthetic twin to be named in the failing diff), the owning-module Coccinelle rules under `coccinelle/` (`reset-refresh-required.cocci`, `sealed-image-const.cocci`, `publish-start-doorbell.cocci`, `rga-raw-request-emitter.cocci`) plus the graceful `rewrite-coccinelle-gate.sh` (skips with bootstrap note when `spatch` absent, `STRICT=1` for CI), the `rewrite-sparse-gate.sh` sparse/smatch lane (graceful when `sparse`/`smatch` absent), the standalone MPP precedence checker `mpp-activation-precedence-checker.py` (exhaustive ordered-pair/triple verification of the ten-reason policy table, device-free) and the invariant-sweep model `rewrite-invariant-sweep.py` (three-verdict clean/violated/skipped-inconclusive with trylock discipline, device-free self-test), the MPP/librga/GStreamer/FFmpeg/rkmppenc suite runners and thin comparator launchers, their shared `suite-compare.sh`, `debugfs-counter-check.sh`, `rewrite-build-gate.sh`, `suite-common-selftest.sh`, `suite-compare-selftest.sh`, the retro-item harnesses `pm-stress-knobs.sh`, `rewrite-case-pair-matrix.sh`, and `kunit-manifest-check.sh`, and their build helpers `build-mpp-tests.sh`, `build-librga-samples-full.sh`, `build-gstreamer-rockchip.sh`, and `gstreamer-event-harness.c` (all documented in [`conformance.md`](./conformance.md)); input-regeneration recipes; pass criteria; and observed reference results. |
| Depends on | A validated kernel from [`../scripts/`](../scripts/README.md), installed MPP/librga packages, FFmpeg artifacts from [`video-libraries/ffmpeg/README.md`](../../video-libraries/ffmpeg/README.md), and device access from the codec udev rule. |
| Current state | H.264/H.265 decode, encode, and full HW transcode have been validated on the forward-port; VP9 and AV1 decode are hardware-validated bit-exact on the av1-fwport build as of 2026-07-04 (`decode-differential.sh`; AV1 needs that variant's `mpp_av1dec.c` backend). The rewrite conformance machinery now covers ABI replay, MPP/librga/GStreamer/FFmpeg suites, optional `rkmppenc`, artifact comparators, an exact ordered 89-MPP/150-RGA KUnit manifest with source/config/package identity plus a complete fatal-free outer-KTAP interval and live lockdep, before/after per-suite dmesg gates, safety/idle counter checks, official-MPP H.264/H.265/VP9/AVS2 plus slice-polling cases, fuzz-smoke build checks, and the paired evidence audit; the detailed matrix lives in [`conformance.md`](./conformance.md) and the post-reconciliation findings in [`../docs/rewrite-conformance-gap-audit.md`](../docs/rewrite-conformance-gap-audit.md). MPP pre/post capture now reads an explicit endpoint allowlist and fails closed instead of recursively walking generated procfs/debugfs files. Device-free runner/counter/KUnit-parser/dmesg/default RGA userptr-IOMMU validations pass. Booted rewrite hardware evidence remains incomplete, so `rewrite-evidence-audit.sh` is expected to fail until paired forward-port/rewrite logs, artifacts, counters, clean dmesg reports, compound booted-KUnit evidence, and comparator-clean results exist. The MPP rewrite now carries the AV1 backend and its six device-free KUnit cases (89-case live suite); AV1 hardware conformance remains a separate track from the current rewrite hardware gate. |

## Conformance harness quick start

Driver implementation and build instrumentation are independent selectors. The
standard catalog runs the widest reusable ABI/MPP/librga/GStreamer/FFmpeg set on
every target; rewrite KUnit/counters and focused sanitizer or race tests are
...
sudo bash run-conformance.sh --target rewrite --configuration kcsan \
  --include iommu-stress,recovery-stress,reset-contention
```

Use `--list` to see every catalog row, `--only id1,id2` for a focused run,
`--skip id` for an explicit omission, `--compare-to profile` for paired
...
launchers over the maintained `suite-compare.sh` engine.
...
installed YSP MPP package carries that half and is now the conformance default.
The older pinned MPP checkout remains available for an explicit legacy
comparison, so its copy is also carried as a repo-owned bootstrap patch at
...
| `iep2-smoke.sh` | **IEP2 deinterlacing** through MPP client 28 | Device-free mode checks the driver/register/Kconfig/DT integration plus the timeout-drain, raw-address, span, current-task-lock, auxiliary-IOVA, and fault-callback teardown guards; it optionally builds the MPP archive, both IOMMU provider objects, and ROCK 5B DTB. Runtime requires bound `fdbb0000.iep`/`fdbb0800.iommu`, generates deterministic TFF and BFF interlaced input, runs official `iep2_test` in I5O2, requires exact-size nonzero output, and scans new dmesg. Exit `77` means the boot or userspace test does not expose IEP2. |
| [`iep2/`](./iep2/README.md) | **IEP2 ioctl-level gates** below what `iep2_test` can reach | Three programs (`iep2_negative.c`, `iep2_encoding.c`, `iep2_stress.c`) that drive `/dev/mpp_service` as client 28 directly, covering synchronous rejection of malformed requests, per-task packed-versus-offset-alone address encoding, and close-versus-completion plus import churn. `iep2_test` validates its own inputs and caps at 1920x1088, so it cannot exercise the driver's DMA-span bound or any malformed-request path. They do not scan the kernel log themselves; pair each run with a `journalctl -k` diff. |
| `rewrite-smoke.sh` | **current `/dev/mpp_service` + `/dev/rga` owner**: forward-port or rewrite | Runs the ABI probe plus decode, encode, and transcode gates above in one pass, and snapshots rewrite debugfs counters, including aggregate/per-core timing counters, when present. It defaults MPP to the installed `/usr` prefix, accepts an explicit staged install (`lib` + `bin`) or raw build (`mpp` + `test`) through `MPP_BUILD`, uses `$ROCK5B_WORKSPACE/ffmpeg/ffmpeg-rockchip`, and selects the existing generated 320×240 H.264/H.265 decode clips plus 1080p H.264 transcode asset. Exit `77` means the device nodes are absent on this boot, not that the workload failed. |
...
| `build-gstreamer-rockchip.sh` | **GStreamer plugin and event harness build helper** | Stages JeffyCN's GStreamer Rockchip plugins, applies the maintained [`patches/gstreamer-rockchip/`](./patches/gstreamer-rockchip/README.md) userspace fixes without dirtying the pinned source checkout, and builds the `gstreamer-event-harness` helper used for flush, force-key-unit, seek, and EOS-loop cases. Set `GST_ROCKCHIP_PATCH_DIR=` for an intentional unpatched comparison. Set `GST_EVENT_HARNESS_VALIDATE_BUILD=1` for a device-free event-harness compile/link check; it returns `77` when the GStreamer development pkg-config files are absent, and the top-level validate-only runner records that as a visible skip. |
...
| `iommu-machinery-fuzz.sh` | **RGA3 userptr-IOMMU and RK3588 IOMMU stress** | Builds `rga-iommu-fuzz.cpp`, runs scattered-userptr RGA copy/resize/rotate/cvtcolor correctness checks across all 64 cache-line offsets, and protects inactive source/destination bytes with guards. It reuses `decode-differential.sh` for bit-exact H.264/H.265/VP9/AV1 decode and can run RGA scatter plus AV1 decode concurrently while bracketing dmesg and debugfs counters for IOMMU faults, RGA userptr-IOMMU leaks, boundary-shadow copy coverage, active-shadow leaks, and setup failures. Run on booted hardware, ideally the debug kernel in [`IOMMU-FUZZING.md`](./IOMMU-FUZZING.md). Set `IOMMU_FUZZ_VALIDATE_BUILD=1` for the device-free C++ compile check included by `run-conformance.sh --validate`; that mode does not touch devices, debugfs, or target librga shared libraries. |
| `rewrite-recovery-stress.sh` | **reset/recovery stress harness** | Runs kill/close, reset-opener, and opt-in platform unbind/rebind loops around an explicit busy workload, then runs a post-case liveness command, scans new dmesg lines for fatal signatures, and snapshots MPP/RGA debugfs counter deltas. Set `RECOVERY_VALIDATE_ONLY=1` for device-free config validation; that mode is included by `run-conformance.sh --validate` and is not hardware recovery evidence. Runtime exit `77` means both device nodes are absent. |
| `rewrite-fixes-gate.sh` | **single gate for the 2026-07-31 rewrite fix series** | Orchestrates the checks that actually discriminate for that series — `slice` (the slice-output stress), `signals` (job-control stop/cont across an encode), `abort` (a killed decode must not stall the next job on that core, calibrated against a baseline measured under the same survivor load), `contention` (the sibling-reset counter), and the official MPP suite as baseline regression — into one summary and one exit code. Labels each check **PROVEN** or **GUARD**: only `slice` was observed failing on a kernel without the fixes; `signals` and `abort` pass with and without them and so guard against a future gross regression rather than evidencing the fix. Prints the genuinely uncovered items (RGA fence interruptibility, the RGA import ABI checks, the non-blocking slice poll path) on every run, because a green result says nothing about those. `VALIDATE_ONLY=1` is device-free. |
| `rewrite-slice-stress.sh` | **encoder slice-output regression stress** | Loops the h264/h265 slice-split encode under CPU contention, because the slice-poll stream-end race and the stale-interrupt attribution race both pass a single run most of the time (observed roughly 1-in-20 to 1-in-160 for h264, usually first iteration for h265). Fails on either shape: a non-zero exit, or an exit-0 run whose log shows framing desync (`can not find match frm`, `hal_bufs_get_buf invalid input`) — checking the exit code alone misses the second entirely. Also scans new dmesg for fatal signatures and, when run as root, gates on `aborted/failed/timeout/recovery_failure/spurious_irq` counter deltas. Without root the counter gate is reported UNAVAILABLE and the pass is labelled DEGRADED rather than silently narrowed; `SLICE_REQUIRE_COUNTERS=1` turns that into a failure. Exit `77` means the encoder binary, input, or `/dev/mpp_service` is absent. |
| `rewrite-reset-contention.sh` | **sibling-core reset deassert race measurement** (rkvdec soft CCU) | Provokes the overlap between a core's recovery reset pulse and a sibling submit's unconditional `reset_control_deassert`, then reads `mpp:reset_deassert_contended_count`. Drives the reset side from the **decode error path** — `rewrite-corrupt-stream.py` builds a stream whose slice payloads are damaged, so the soft-CCU IRQ thread resets the core once per frame — alongside clean-stream survivors supplying the sibling submits and one killed decoder exercising the session-abort reset. The kill-only provocation it used before was ~1e-3 of the strength needed and produced a meaningless zero (see the [finding](../../findings/2026-07-31-rkvdec-sibling-reset-deassert-race.md)). Reports `reset_count`, `reset_deassert_count`, and the submit rate next to the contention count, and derives the expected hit count per core (`sum over cores of resets x deassert rate x 10us`) — falling back to the submit rate as a labelled *upper bound* on kernels predating the deassert counter, since a job inheriting a coordinator power hold issues no deassert. When no reset happened, or the expectation is under `RESET_MIN_EXPECT`, it exits `78` **INCONCLUSIVE** rather than claiming the race is absent — a zero contention count is evidence only if the run could have produced a non-zero one. `EXPECT=contended` passes when the race is observed (reachability evidence, expected before the per-reset-domain lock lands); `EXPECT=clean` passes when a provocation of adequate power produces none (regression gate, only meaningful once the same workload has shown a non-zero delta beforehand). Root-only, since the counter lives in debugfs; exit `77` when not root, when the decoder binary, input, or `python3` is missing, or when the kernel predates the counter. |
| `rewrite-reset-lock-gate.sh` | **resumable verification gate for the reset-domain lock** (`b37f6e9825b1`) | Runs the sibling-reset-race verification gate in the order that order matters, on a booted kernel. Six phases: `identity` (the boot really carries the lock SHA, read from the `uname -v` build stamp — everything after it is meaningless otherwise), `kunit`, `irq-bits` (samples `INT_STA` off the existing 64-entry debug event ring for `softreset_rdy`/`dec_bus_sta`/`dec_rdy_sta`, so the hardware-self-reset question needs no counter commit), `perturbation` (`rewrite-smoke.sh` plus the timing counters — a leaf lock on the submit path must not move them), `clean` (**the load-bearing result**: `EXPECT=clean` contention must be zero on the same workload that produced 2 hits per 60 s pre-lock), and `wedge` (repeats the two-stream provocation to discriminate the lock's mechanism from the hard-IRQ MMIO candidate). Safe phases run first and the deliberately risky ones last, because that workload wedged the board roughly 1 run in 3 pre-lock. Each phase fsyncs its verdict, so a wedge costs a reboot rather than the run — re-running the same command skips completed phases; `FORCE=1` redoes them, `SKIP_WEDGE=1` stops after `clean`, `WEDGE_RUNS=N` sets the repetition count. Exit code is the `clean` phase's verdict. Root-only, exit `77` otherwise. |
| `rewrite-corrupt-stream.py` | **corrupt-stream builder for the error path** | Damages slice payloads in an Annex-B h265/h264 elementary stream while leaving parameter sets, SEI, the leading slices, and the NAL framing intact, so userspace still configures the decoder and submits jobs but the hardware reports a decode error on nearly every frame. Every byte written is forced non-zero with bit 6 set so no `00 00 01` start code can be synthesised, and the NAL count is re-checked before the output is written. Used by `rewrite-reset-contention.sh` to turn the driver's error-triggered core reset from a rare event into a per-frame one. |
| `reset-session-kasan.sh` | **narrowed reset-session double-free reproduction** (KASAN kernel only) | Runs `abi-replay.sh` (MPP/RGA session churn incl. `MPP_CMD_RESET_SESSION`) then a one-shot recursive `/proc/mpp_service` snapshot, and scans the kernel log emitted during it. Isolates the [reset-session double-free](../../findings/2026-07-18-mpp-reset-session-dma-double-free-kasan.md): a first-pass KASAN hit on the unpatched fwport kernel, quiet with patch `0042`. A non-zero ABI-contract result is not itself a memory finding; the gate is `flagged_kernel_lines=0`. |
...
| `run-conformance.sh --target … --configuration kasan` | **full standard matrix under KASAN** | Runs the same ABI, MPP, librga, GStreamer, and FFmpeg coverage used for production kernels while every suite scans its bounded kernel-log window with the shared KASAN/KCSAN/fatal signature set. The old KASAN-only MPP wrapper duplicated a subset of `mpp-suite.sh`; the configuration axis now gives the full common suite the same safety gate. Add `reset-session-kasan` or `ioctl-fuzz-kasan` by catalog ID for focused forward-port lifetime and allocation-unwind coverage. |
...
| `iep2-smoke.sh` | device access to `/dev/mpp_service` and the MPP dma-buf allocator; root is strongly preferred so the new-kernel dmesg interval can be required with `IEP2_REQUIRE_DMESG=1`. `IEP2_VALIDATE_ONLY=1` is device-free. |
...
| `build-gstreamer-rockchip.sh` | no device access for build; needs installed MPP, librga, and GStreamer development `.pc` files for the full plugin build. `MPP_PREFIX`/`PKG_SHIM` opt into staged dependencies. `GST_EVENT_HARNESS_VALIDATE_BUILD=1` only needs `gstreamer-1.0` and `glib-2.0` development `.pc` files, and skips with `77` if they are absent. |
...
| `iommu-machinery-fuzz.sh` | **root strongly preferred** — `/dev/rga`, `/dev/mpp_service`, installed librga/MPP, full dmesg, and debugfs counters are needed for the RGA userptr-IOMMU fault and leak signal. `LIBRGA_LIBDIR`/`MPP_BUILD` opt into staged artifacts. `IOMMU_FUZZ_VALIDATE_BUILD=1` is device-free and only compiles the RGA scatter fuzzer object. |
...
| `rewrite-fixes-gate.sh` | **root strongly preferred** — the `contention` check is root-only and skips without it, and the `abort` check falls back to timing alone when `timeout_count` is unreadable, which is the weaker signal. `VALIDATE_ONLY=1` is device-free. |
| `rewrite-slice-stress.sh` | **root preferred, not required** — the encode loop and dmesg scan work unprivileged, but the debugfs counter gate does not (debugfs is `0700`), so a non-root pass is reported DEGRADED. Use `sudo` for the full gate, or `SLICE_REQUIRE_COUNTERS=1` to refuse a degraded run. |
| `rewrite-reset-contention.sh` | **root** — the whole point is reading `reset_deassert_contended_count` from debugfs; without it the harness exits `77` rather than reporting a meaningless pass. |
...
[`conformance.md`](./conformance.md) § Suite privileges.)

> **Paths.** Installed MPP/librga are the default runtime. External assets,
> plugins, and logs remain under the grouped workspace, and every component
> path is env-overridable. Naming matches [`video-libraries/ffmpeg/README.md`](../../video-libraries/ffmpeg/README.md):
>
> | Var | Used by | Meaning |
> |-----|---------|---------|
> | `MPP_BUILD` | decode, encode | an MPP build/install tree with `librockchip_mpp` + `mpi_dec_test`/`mpi_enc_test`. Default `/usr`; an explicit staged install (`lib` + `bin`) or raw CMake build (`mpp` + `test`) is auto-detected too. |
> | `IEP2_TEST`, `IEP2_INPUT`, `IEP2_BFF_INPUT`, `IEP2_LOOPS` | IEP2 smoke | optional official `iep2_test` path, external TFF/BFF raw YUV420 fixtures, and repeat count. Without fixtures the harness uses FFmpeg to generate deterministic interlaced inputs. |
> | `MPP_BIN_DIR`, `MPP_LIBDIR` | `mpp-suite.sh` / GStreamer suite | MPP test-binary directory and optional library override. Defaults are `/usr/bin` and the system dynamic-loader path; set the library directory only for a staged or legacy comparison. |
> | `LIBRGA_LIBDIR` | librga/GStreamer suites | optional staged or legacy librga directory. Empty by default, selecting the installed system library. |
...
> | `FFMPEG_REQUIRE_AV1` | `ffmpeg-suite.sh` | promote AV1 RKMPP decode, AV1->RGA->H.264/H.265 transcodes, AV1 PSNR, and AV1 AFBC probes from bring-up diagnostics to required cases. It defaults to `0` because the source-implemented rewrite VPU981/VSI path has no hardware baseline; set it to `1` for full current-tip qualification. |
...
> | `ABI_PROBE_ABI_ONLY` | `abi-probe.sh` | emit compile-time ioctl/structure constants without opening procfs or either device |
...
> | `LIBRGA_FORCE_RGA_USERPTR_IOMMU` | `librga-suite.sh` / `run-conformance.sh` | set the RGA userptr-IOMMU `userptr_iommu/force_remap` compatibility knob (or legacy `route_b/force_remap`) during the librga suite and restore it at exit; forced counter defaults use `*:attempt`/`*:ok` so either debugfs component name is valid. `LIBRGA_FORCE_ROUTE_B` remains a legacy alias |
> | `LIBRGA_ENABLE_VENDOR_HEAP_CASES` | `librga-suite.sh` | opt into 13 official samples that hard-code `/dev/dma_heap/system-uncached` or `system-uncached-dma32`; disabled by default because upstream-style RK3588 kernels do not expose those vendor heap types |
> | `LIBRGA_SUITE_VALIDATE_LOG_PARSER` | `librga-suite.sh` | set to `1` for the device-free selftest that ensures fatal official-sample log messages override any status and accepts status one only with the sample's explicit terminal success message |
> | `LIBRGA_SUITE_VALIDATE_CASES` | `librga-suite.sh` | set to `1` for the device-free default/opt-in vendor-heap case-list selftest; it also keeps `rga_alpha_rgba5551_demo` and `rga_mosaic_demo` excluded because Rockchip librga 1.10.6_[3] rejects them before an ioctl, and excludes the non-conforming `rga_palette_demo` request sequence documented in the [palette finding](../../findings/2026-08-06-librga-palette-demo-is-not-kernel-conformance.md) |
...
> | `RECOVERY_WORKLOAD_CMD` | `rewrite-recovery-stress.sh` | busy workload command run in its own process group while the harness kills, resets, or unbinds around it; default is a narrowed `run-conformance.sh` profile run |
...
> | `GATE_CHECKS`, `GATE_SLICE_ITERS`, `GATE_ABORT_LOOPS`, `GATE_ABORT_SURVIVORS`, `GATE_SIGNAL_LOOPS` | `rewrite-fixes-gate.sh` | which checks to run (default `slice signals abort contention mpp-suite`) and their depth; `GATE_ABORT_SURVIVORS` holds concurrent decoders across the abort loop so the coordinator cannot autosuspend and clear the state under test |
> | `GATE_CONTENTION_EXPECT` | `rewrite-fixes-gate.sh` | passed through as `EXPECT` to the contention check: `contended`, `clean`, or unset to report only |
> | `SLICE_ITERS`, `SLICE_CODECS`, `SLICE_LOAD`, `SLICE_LOAD_JOBS` | `rewrite-slice-stress.sh` | iterations per codec (default `200`), codecs to run (default `h264 h265`), and whether to hold the CPU busy while encoding (default on, `4` spinners) — the race needs contention, so `SLICE_LOAD=0` makes a pass much weaker |
> | `SLICE_REQUIRE_COUNTERS` | `rewrite-slice-stress.sh` | fail instead of degrading when the debugfs counter gate cannot run (i.e. when not root); default `0` |
> | `EXPECT`, `RESET_DURATION_S`, `RESET_ERROR_STREAMS`, `RESET_SURVIVORS`, `RESET_MIN_EXPECT`, `RESET_KILL` | `rewrite-reset-contention.sh` | `contended` to require the race be observed (pre-fix reachability), `clean` to require it absent (post-fix regression), or unset to report only; plus the provocation length in seconds (default `60`), the number of corrupt-stream decoders driving resets (default `2`), the number of clean-stream decoders driving sibling submits (default `2`), the expected-hit count below which a zero result is reported `78` INCONCLUSIVE instead of pass/fail (default `3`), and whether to also run the mid-decode kill loop (default `1`) |
...
> | `--include rkmppenc` | `run-conformance.sh` | add the optional app-level generated Y4M/raw RGA-resize encode probes plus the diagnostic `--avhw` transcode |
...
> | `REQUIRE_DMESG_EVIDENCE`, `REQUIRE_KUNIT_EVIDENCE` | `rewrite-evidence-audit.sh` | require clean per-suite `dmesg-scan.tsv` records on both profiles and an exact ordered 89-MPP/150-RGA booted-KUnit manifest whose run ID matches each rewrite-candidate suite; both default on for the final audit |
...
> | `RUN_COUNTER_CHECKS` | `run-conformance.sh` | override the target's suite debugfs-counter policy. Rewrite targets default it on and require positive hardware/fence traversal, zero idle gauges, and present safety counters with no positive delta; BSP and forward-port targets default it off. |
> | `--skip kunit` | `run-conformance.sh` | explicitly omit the catalog's rewrite-only ordered KUnit manifest gate. Normal rewrite runs select it automatically; BSP and forward-port plans mark it incompatible. |
...
> | `*_REQUIRED_POSITIVE_COUNTER_PREFIXES` | `run-conformance.sh` / `debugfs-counter-check.sh` | optional multicore spread gate using `component:counter_prefix:min_positive`, e.g. `MPP_REQUIRED_POSITIVE_COUNTER_PREFIXES="mpp:started_rkvdec_core:2"` or `LIBRGA_REQUIRED_POSITIVE_COUNTER_PREFIXES="rga:started_rga3_core:2"` |
> | `CONFORMANCE_COUNTER_DEFAULTS` | `run-conformance.sh` | set `0` to disable the automatic rewrite counter requirements for a documented narrow diagnostic run |
> | `--continue` | `run-conformance.sh` | record a failed test and continue with the remaining selected catalog rows; the harness still exits non-zero at the end |
...
sudo bash rewrite-fixes-gate.sh                           # one gate for the 2026-07-31 fix series
VALIDATE_ONLY=1 bash rewrite-fixes-gate.sh                # device-free gate config check
sudo bash rewrite-slice-stress.sh                         # encoder slice regression stress, full counter gate
SLICE_ITERS=400 SLICE_CODECS=h264 bash rewrite-slice-stress.sh  # long single-codec soak (degraded without root)
sudo EXPECT=contended bash rewrite-reset-contention.sh    # prove the sibling-reset race is reachable (pre-fix)
sudo EXPECT=clean bash rewrite-reset-contention.sh        # regression gate once the reset-domain lock lands
RECOVERY_VALIDATE_ONLY=1 bash rewrite-recovery-stress.sh  # device-free recovery harness config check
sudo RECOVERY_WORKLOAD_CMD='bash "$TEST_DIR/run-conformance.sh" --target rewrite --only mpp,librga' RECOVERY_CASES="kill reset" bash rewrite-recovery-stress.sh  # kill/reset recovery stress around a busy profile run
...
bash run-conformance.sh --target rewrite --validate  # device-free catalog/runner/build/parser/comparator/audit checks
LIBRGA_FORCE_RGA_USERPTR_IOMMU=1 bash run-conformance.sh --target rewrite --validate  # also checks forced userptr-IOMMU counter wiring
bash run-conformance.sh --target rewrite --plan  # show the resolved standard set and compatible opt-ins
sudo bash run-conformance.sh --target rewrite  # full standard rewrite run, including KUnit and counter gates
sudo bash run-conformance.sh --target rewrite --include rkmppenc  # add app-level MPP/RGA evidence
sudo LIBRGA_FORCE_RGA_USERPTR_IOMMU=1 bash run-conformance.sh --target rewrite --only librga  # focused userptr-IOMMU attribution gate
...
For a diagnostic smoke pass in one command (MPP/librga default to the installed
packages; media assets and FFmpeg default under the grouped `rock-5b`
workspace):
...
```bash
sudo bash run-conformance.sh --target forward-port
```

Then boot the rewrite kernel and run:

```bash
sudo bash run-conformance.sh --target rewrite --compare-to forward-port
```

Rewrite targets automatically require counter files, positive hardware-start
and busy-time counters for each selected suite, and zero idle gauges. BSP and
forward-port targets leave rewrite-specific counter checks off. Use
`CONFORMANCE_COUNTER_DEFAULTS=0` only for a deliberately narrow diagnostic pass. To
...
[`conformance.md`](./conformance.md) § VP9 decode via the suites.
...
- **decode correctness (2026-07-04, board build `P1c9d` #8, `decode-differential.sh`):**
  ...
  ~551 / 591 / 741 / 629). The board-stock `/usr/lib` MPP present during that
  historical run failed (`parser not registered`), so that run used the
  conformance-local source build. The installed YSP MPP package has since
  replaced that stock library and is the current default.
