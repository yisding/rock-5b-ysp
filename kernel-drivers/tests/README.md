...
| Current state | H.264/H.265 decode, encode, and full HW transcode have been validated on the forward-port; VP9 and AV1 decode are hardware-validated bit-exact on the av1-fwport build as of 2026-07-04 (`decode-differential.sh`). The rewrite conformance machinery covers ABI replay, MPP/librga/GStreamer/FFmpeg suites, optional `rkmppenc`, artifact comparators, booted 236-case KUnit reports (89 MPP + 147 RGA), before/after dmesg fatal-signature gates, safety/idle counter checks, official-MPP H.264/H.265/VP9/AVS2 plus slice-polling cases, fuzz-smoke build checks, syzlang ABI-marker checks, and the paired evidence audit; the detailed matrix lives in [`rewrite-conformance.md`](./rewrite-conformance.md) and the post-reconciliation findings in [`../docs/rewrite-conformance-gap-audit.md`](../docs/rewrite-conformance-gap-audit.md). The dedicated 6.18 AV1 tip is `rk3588-rewrite-av1-6.18@402fc9c0bd785`, based on the complete shared rewrite through `c5faabf9d00b0`; it passed warning-free clean-archive `normal`, `memory`, and `race` profiles on 2026-07-26, including both Rockchip and VSI IOMMU providers, both rewrite objects with KUnit, and the Rock 5B DTB. Device-free runner/counter/KUnit-parser/dmesg/default RGA userptr-IOMMU validations also pass. Published alpha packages predate this tip. Booted rewrite hardware evidence is missing, so `rewrite-evidence-audit.sh` is expected to fail until paired forward-port/rewrite logs, artifacts, counters, clean dmesg reports, a green booted-KUnit report, and comparator-clean results exist. |
...
> | `REQUIRE_DMESG_EVIDENCE`, `REQUIRE_KUNIT_EVIDENCE` | `rewrite-evidence-audit.sh` | require clean per-suite `dmesg-scan.tsv` records on both profiles and an exact 89-MPP/147-RGA booted-KUnit report whose run ID matches each rewrite-candidate suite; both default on for the final audit |
...
> | `RUN_KUNIT_CHECK` | `rewrite-conformance-run.sh` | defaults to `1` for rewrite profiles; requires exactly 89 MPP and 147 RGA booted KUnit cases with no fail/skip and persists `$LOG_ROOT/$RUN_ID-kunit.tsv` |
