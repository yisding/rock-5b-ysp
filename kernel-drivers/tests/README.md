...
| Owns | The smoke tests `test-decode.sh`, `decode-differential.sh`, `encode-test-tiny.sh`, `transcode-test.sh`, `rewrite-smoke.sh`, `mpp-debug-capture.sh`, `abi-probe.sh`/`abi-probe.c`, `abi-replay.sh`, `librga-smoke.sh`/`librga-smoke.cpp`, the single-purpose gate probes `rga-p010-test.cpp`, `rga-nv15-test.cpp`, `rga-core-match-test.cpp`, `legacy-blit-test.c`, and `rga-10bit-legacy-stride-test.c`, and the targeted RGA MMU diagnostic `rga-mmu-debug.sh`; the root-gate orchestrator `run-root-gates.sh` (runs the root-only forward-port gates — `encode-test-tiny.sh`, `transcode-test.sh`, `rga-mmu-debug.sh`, `iommu-machinery-fuzz.sh`, `mpp-debug-capture.sh`, and the opt-in destructive `mpp-vp9-show-existing-repro.sh` — each with a per-gate journal fatal scan, forcing the hardware `/usr/bin/ffmpeg`); the recovery/RGA/IOMMU stress tools `rewrite-recovery-stress.sh`, `iommu-machinery-fuzz.sh`, `rga-iommu-fuzz.cpp`, and [`IOMMU-FUZZING.md`](./IOMMU-FUZZING.md); the RGA session-close force-free reproducer `rga-session-uaf.sh`/`rga-session-uaf.c` (documented in [`rga-session-uaf.md`](./rga-session-uaf.md)); the KASAN debug-kernel reproducers `kasan-narrowed-repro.sh` and `kasan-mpp-suite.sh`; the sourced helpers `suite-common.sh`, `debugfs-counters.sh`, and `kasan-scan.sh`; the conformance runner/wrappers `rewrite-conformance-run.sh`, `rewrite-evidence-audit.sh`, `rewrite-kunit-log-check.sh`, `mpp-suite.sh`, `mpp-suite-compare.sh`, `librga-suite.sh`, `librga-suite-compare.sh`, `gstreamer-suite.sh`, `gstreamer-suite-compare.sh`, `ffmpeg-suite.sh`, `ffmpeg-suite-compare.sh`, `rkmppenc-suite.sh`, `rkmppenc-suite-compare.sh`, `debugfs-counter-check.sh`, `rewrite-build-gate.sh`, `suite-common-selftest.sh`, `suite-compare-selftest.sh`, the syzkaller checks under `syzkaller/`, and their build helpers `build-mpp-tests.sh`, `build-librga-samples-full.sh`, `build-gstreamer-rockchip.sh`, `gstreamer-event-harness.c` (all documented in [`rewrite-conformance.md`](./rewrite-conformance.md)); input-regeneration recipes; pass criteria; and observed reference results. |
...
| Current state | H.264/H.265 decode, encode, and full HW transcode have been validated on the forward-port; VP9 and AV1 decode are hardware-validated bit-exact on the av1-fwport build as of 2026-07-04 (`decode-differential.sh`; AV1 needs that variant's `mpp_av1dec.c` backend). The rewrite conformance machinery now covers ABI replay, MPP/librga/GStreamer/FFmpeg suites, optional `rkmppenc`, artifact comparators, booted 232-case KUnit reports (85 MPP + 147 RGA), before/after dmesg fatal-signature gates, safety/idle counter checks, official-MPP H.264/H.265/VP9/AVS2 plus slice-polling cases, fuzz-smoke build checks, syzlang ABI-marker checks, and the paired evidence audit; the detailed matrix lives in [`rewrite-conformance.md`](./rewrite-conformance.md) and the post-reconciliation findings in [`../docs/rewrite-conformance-gap-audit.md`](../docs/rewrite-conformance-gap-audit.md). The current tips are `1fe46df86f1ca` on 6.18 (`rk3588-rewrite-6.18`) and `ec9a4a06ecf12` on mainline (`rk3588-rewrite-mainline`), rewrite sources byte-identical across branches; they add the 2026-07-23 `harden rewrite driver recovery` commit atop the five forward-port RGA bugfixes and the adapted RK3588 RGA 5.10 reliability/cache-safety lessons. **All six `normal`/`memory`/`race` clean-source provider/rewrite/DTB profiles passed warning-free at these tips on 2026-07-23** (the earlier 2026-07-17 run covered the pre-bugfix parents). Device-free runner/counter/KUnit-parser/dmesg/default RGA userptr-IOMMU validations also pass. Published alpha packages still contain earlier parents. Booted rewrite hardware evidence is missing, so `rewrite-evidence-audit.sh` is expected to fail until paired forward-port/rewrite logs, artifacts, counters, clean dmesg reports, a green booted-KUnit report, and comparator-clean results exist. The MPP rewrite still has no AV1 backend; AV1 remains a separate implementation track rather than part of the current rewrite hardware gate. |
...
> | `REQUIRE_DMESG_EVIDENCE`, `REQUIRE_KUNIT_EVIDENCE` | `rewrite-evidence-audit.sh` | require clean per-suite `dmesg-scan.tsv` records on both profiles and an exact 86-MPP/120-RGA booted-KUnit report whose run ID matches each rewrite-candidate suite; both default on for the final audit |
...
> | `RUN_KUNIT_CHECK` | `rewrite-conformance-run.sh` | defaults to `1` for rewrite profiles; requires exactly 85 MPP and 147 RGA booted KUnit cases with no fail/skip and persists `$LOG_ROOT/$RUN_ID-kunit.tsv` |
