...
> | `REQUIRE_DMESG_EVIDENCE`, `REQUIRE_KUNIT_EVIDENCE` | `rewrite-evidence-audit.sh` | require clean per-suite `dmesg-scan.tsv` records on both profiles and an exact 90-MPP/147-RGA booted-KUnit report whose run ID matches each rewrite-candidate suite; both default on for the final audit |
...
> | `RUN_KUNIT_CHECK` | `rewrite-conformance-run.sh` | defaults to `1` for rewrite profiles; requires exactly 90 MPP and 147 RGA booted KUnit cases with no fail/skip and persists `$LOG_ROOT/$RUN_ID-kunit.tsv` |
