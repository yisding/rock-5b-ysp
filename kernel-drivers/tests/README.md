...
> | `RUN_KUNIT_CHECK` | `rewrite-conformance-run.sh` | defaults to `1` for rewrite profiles; requires the ordered case manifest with no fail/skip (expected counts come from `rewrite-kunit-manifest.tsv`, 92 MPP + 152 RGA at the current tip), a clean outer-KTAP interval, live lockdep, and source/config/package identity, then persists `$LOG_ROOT/$RUN_ID-kunit.tsv` |
...
VALIDATE_ONLY=1 bash rewrite-conformance-run.sh  # device-free runner/ioctl-fuzz/librga-smoke/gstreamer-harness/iommu-fuzz/recovery/case/comparator/abi-replay wiring check
