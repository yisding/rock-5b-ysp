...
| Current state | H.264/H.265 decode, encode, and full HW transcode have been validated on the forward-port; VP9 and AV1 decode are hardware-validated bit-exact on the av1-fwport build as of 2026-07-04 (`decode-differential.sh`; AV1 needs that variant's `mpp_av1dec.c` backend). The rewrite conformance machinery now covers ABI replay, MPP/librga/GStreamer/FFmpeg suites, optional `rkmppenc`, artifact comparators, booted 208-case KUnit reports, before/after dmesg fatal-signature gates, safety/idle counter checks, official-MPP H.264/H.265/VP9/AVS2 plus slice-polling cases, fuzz-smoke build checks, syzlang ABI-marker checks, and the paired evidence audit; the detailed matrix lives in [`rewrite-conformance.md`](./rewrite-conformance.md) and the post-reconciliation findings in [`../docs/rewrite-conformance-gap-audit.md`](../docs/rewrite-conformance-gap-audit.md). The hardened rewrite parents `0d71ded1690c` on 6.18 and `32696e87c9c7` on mainline include the adapted RK3588 RGA 5.10 reliability/cache-safety lessons and passed all `normal`, `memory`, and `race` clean-source provider/rewrite/DTB profiles warning-free on 2026-07-17; the current tips `8469183da227` on 6.18 and `9ff18809b5e0` on mainline add five forward-port RGA bugfixes (compile-verified on both branches on 2026-07-22, clean-source/boot gates pending re-run). Device-free runner/counter/KUnit-parser/dmesg/default RGA userptr-IOMMU validations also passed that day. Published alpha packages still contain earlier parents. Booted rewrite hardware evidence is missing, so `rewrite-evidence-audit.sh` is expected to fail until paired forward-port/rewrite logs, artifacts, counters, clean dmesg reports, a green booted-KUnit report, and comparator-clean results exist. The MPP rewrite still has no AV1 backend; AV1 remains a separate implementation track rather than part of the current rewrite hardware gate. |
...
> | `RUN_KUNIT_CHECK` | `rewrite-conformance-run.sh` | defaults to `1` for rewrite profiles; requires exactly 86 MPP and 122 RGA booted KUnit cases with no fail/skip and persists `$LOG_ROOT/$RUN_ID-kunit.tsv` |
