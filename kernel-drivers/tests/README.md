...
> | `LIBRGA_SMOKE_EXPECT_FBC_TAIL_REJECT` | `librga-smoke.sh` | optional rewrite-only negative check for AFBC32x8/RFBC64x4 destination modes through public `librga` calls (`0` by default so the smoke records, rather than fails, forward-port behavior) |
...
with `LIBRGA_SMOKE_EXPECT_FBC_TAIL_REJECT=1`. With `LIBRGA_SMOKE_10BIT=1`, it also
