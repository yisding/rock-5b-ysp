...
| Patch `0097` | The first exact-`0096` boot removes the former oversized-SG signature but exposes high physically contiguous USERPTR handles being rejected before their transient RGA2 remap. The [0097 finding](../../findings/2026-08-11-forward-port-rga2-contiguous-userptr-rejection.md) owns the measured failure and the source fix, which is strict-checkpatch and complete-RGA `W=1 WERROR=1` verified. Exact Linux 6.18.44 source is signed-source-package verified and accepted as Pending source publication `18669946`; build and runtime replay are pending |
