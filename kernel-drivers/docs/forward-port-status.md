...
| RGA2/RGA3 operations | Direct librga, FFmpeg RKRGA, preprocessing-shaped, 10-bit, async/fence, forced-core, and recovery cases have hardware evidence across the linked campaigns | Unsupported raw physical imports and several legacy/RGA2-Pro modes remain deliberately rejected |
| ABI and consumer conformance | ABI replay, official MPP, FFmpeg, GStreamer, and direct librga suites have dated results; the exact 0092 production campaign is the latest fully integrated record in this document | Two GStreamer required cases remained classified userspace caps/flush failures in that campaign |
