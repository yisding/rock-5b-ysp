...
| RGA2/RGA3 operations | Direct librga, FFmpeg RKRGA, preprocessing-shaped, 10-bit, async/fence, forced-core, and recovery cases have hardware evidence across the linked campaigns | Published/installed/booted 6.18.43/`0096` passes 30/31 required librga cases. `0093` removes the former oversized USERPTR SG failure, but gray256 exposes a high-contiguous-USERPTR admission gap; source `0097` repairs its admission/MMU/address path and is compile- and signed-source-package verified on Linux 6.18.44, with source publication `18669946` Pending and runtime open. `0095` stages every high-address RGA2 DMA-BUF and closes related ownership gaps, whose focused runtime gates remain open. Unsupported raw physical imports and several legacy/RGA2-Pro modes remain deliberately rejected |
...
| Patch `0093` | The first correct-DTB 6.18.43 run isolated merged 2 MiB RGA2 USERPTR entries exceeding SWIOTLB's per-map limit. Exact `0096` is now published and booted: the old SWIOTLB signature is absent and two of the three historical cases pass, while gray256 reaches a separate high-contiguous-USERPTR admission defect fixed by `0097`. See the [USERPTR finding](../../findings/2026-08-08-forward-port-rga2-userptr-swiotlb-segments.md) |
...
| Patch `0095` | The [ownership audit fix](../../findings/2026-08-08-forward-port-rga-mpp-ownership-audit-fixes.md) stages every high-address RGA2 DMA-BUF through one bounded shared job object and closes RGA request, fence, import, PM, and shutdown ownership gaps. Production and async-disabled full-RGA `W=1 WERROR=1` builds plus final reviews pass; exact `0096` is published and booted with partial conformance, while focused alias-staging/ownership replay remains pending |
| Patch `0096` | MPP session/result/DMA state and Rockchip/VSI fault admission are serialized; post-START pending faults are preserved, and hard RKVDEC2 CCU requests fall back to soft mode before worker selection. Full MPP/IOMMU `W=1 WERROR=1` builds and reviews pass; exact `0096` is published and booted and its ordinary 12-case MPP suite passes, while focused ownership/fault/CCU replay remains pending |
| Patch `0097` | The first exact-`0096` boot removes the former oversized-SG signature but exposes high physically contiguous USERPTR handles being rejected before their transient RGA2 remap. The [0097 finding](../../findings/2026-08-11-forward-port-rga2-contiguous-userptr-rejection.md) owns the measured failure and the source fix, which is strict-checkpatch and complete-RGA `W=1 WERROR=1` verified. Its 6.18.44 source package published and built successfully on Launchpad but was never installed, and it was re-cut unchanged onto Linux 6.18.51 and Published again on 2026-09-13. Patch `0097` therefore still has **no** runtime evidence of any kind; see the [update finding](../../findings/2026-09-13-kernel-lines-updated-to-latest-upstreams.md) |
...
| RGA2 high USERPTR and DMA-BUF mapping | Published/installed/booted `0096` removes the old merged-2-MiB SWIOTLB failure and passes 30/31 librga cases, but gray256 is rejected at policy when a high USERPTR appears physically contiguous. `0097` makes that remappable virtual-memory case take the `0093` transient path. `0095` stages every CPU-accessible high-address DMA-BUF for RGA2-only work; its alias/copyback gate and `0097` runtime remain open. Secure/non-vmap, raw-physical, and over-cap inputs still fail closed |
