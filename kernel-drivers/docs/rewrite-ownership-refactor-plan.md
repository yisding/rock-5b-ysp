...
> typed group/core retirement evidence for the exact predecessor. Phase 3H
> extends the ref-owning claim token through recovered terminal paths and
> transfers an exact restore refusal into a service quarantine tombstone at
> `rk3588-rewrite-6.18@1784358650e3c41abbd5154cbd1737da07d968de` and
> `rk3588-rewrite-mainline@e71d368cc48374638eafbfdf1e9c6864cabbd118`. Their tracked
...
> Phase 3E adds provisional `UNINSTALLED` / `SLOTTED` / `CLAIMED` metadata
> and one reasoned claim helper as the only active-slot detach. Exact identity
> and generation checks precede mutation, a pending IOMMU fault keeps its
> existing priority over IRQ, timeout, and CCU-done claims, and reset/stop
...
> Phase 3H makes a nonempty active-claim token own the containing-job reference
> detached from the active slot. Recovered error-IRQ, timeout, fault, abort/close,
> remove/shutdown, reset-error, and hard-CCU dependent paths move `CLAIMED` to
> `RETIRED` only with typed direct-core or group-plus-core proof. If exact
> restore refuses the token, its reference and generation move to a
> service-owned `QUARANTINED` tombstone with distinct diagnostic, core, and
...
> Phase 3J at
> `rk3588-rewrite-6.18@7481df21ca2b1481a3c4b4d222e3ebed28692544`
> and
> `rk3588-rewrite-mainline@4a632e00c4cd729cb7aa473686bed3ccd2bb271c`
> adds a base reference bias to each activation and replaces the external
> job-pointer adapters with typed `{activation, generation}` references.
> Active, claim, timeout, retry, and quarantine owners pair each activation
> reference with its containing-job reference and use explicit clone, move,
> and put semantics. Dispatch/current/list identities remain borrowed and
> backend resources remain job-shaped.
> `RECLAIMABLE`, activation-aware resource drain, and terminal arbitration
> remain later Phase 3 work. Phase 3J ownership passes at 2,251 signals/tree,
> fixture debt passes at 306/tree, and the source manifest is 105 MPP plus 152
> RGA cases. All eight warning-fatal final-head profiles pass across both kernel
> lines, including both IOMMU providers, both rewrite objects, and the Rock 5B
> DTB. The dedicated test-disabled policy/ABI gate passes on both heads: both
> rewrite KUnit options resolve disabled, the same providers/objects/DTB compile
> warning-fatally, and the deliberate MPP ABI mutation fails at compile time.
> The repository handoff gate passes; runtime results remain pending. There is still no general MPP
...
> strict checkpatch is 0/0/0 over 1,976 lines/tree. The final-source 6.18
> KUnit-enabled object is 6,559,584 bytes at SHA-256
...
> All runtime evidence is pending.
>
...
Checkpoint 3E is present at `969b91ce7d4b` / `8f67fb6fc7d9`: one
`rk_mpp_hw_claim_active_locked()` helper is the only production active-slot
detach. It validates exact activation identity and optional generation before
recording a trigger reason and moving `SLOTTED` to `CLAIMED`; a pending IOMMU
fault preserves its existing priority over IRQ, timeout, and CCU-done claims.
Restore moves the same activation back to `SLOTTED` without changing its
reference, generation, or deadline. Retry remains an in-place `SLOTTED`
generation replacement. The claim reason records the current winner; it does
...
Checkpoint 3H is present at `1784358650e3c41abbd5154cbd1737da07d968de` /
`e71d368cc48374638eafbfdf1e9c6864cabbd118`: a nonempty active-claim token now
owns the exact containing-job reference detached from the active slot. Recovered
terminal paths store typed direct-core or hard-CCU group/core proof before
moving `CLAIMED` to `RETIRED` and releasing that reference. Exact restore
refusal instead transfers the reference and token generation to a
service-owned `QUARANTINED` tombstone, preserves distinct diagnostic/core/group
failure evidence, keeps resources and the RKVDEC dispatch lease, closes
selected-core and owned-CCU admission, blocks remove teardown, and survives
shutdown until reboot. Clean IRQ/`CCU_DONE` and pre-doorbell `START_FAILURE`
...
Checkpoint 3J is present at
`7481df21ca2b1481a3c4b4d222e3ebed28692544` /
`4a632e00c4cd729cb7aa473686bed3ccd2bb271c`: each activation has a base
reference bias, and every dereference-capable external owner carries a typed
`{activation, generation}` reference paired with its containing-job reference.
Active, timeout, claim, retry, and quarantine paths use paired get/clone,
move, and put semantics. Pre-publication retry refusal leaves the predecessor
active and puts the unpublished successor pair; successful A→B publication
moves the predecessor pair through the retry token, and later finish refusal
transfers that exact pair to quarantine. Current,
dispatch, and activation-list identities remain borrowed. The base bias is not
