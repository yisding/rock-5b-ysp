...
|-----|---------------------------------------------|------------|
| Compiled or stale KUnit was treated as current green KUnit | The build profiles enabled both suites, but nothing read the booted results; an unrelated older report could also be combined with newer suite logs. | [`rewrite-kunit-log-check.sh`](../tests/rewrite-kunit-log-check.sh) requires exactly 90 MPP and 147 RGA cases on the AV1 branch (the shared non-AV1 line has 85 MPP cases), with no failure or skip, and the profile runner persists a structured report. The evidence audit requires the report whose run ID matches every selected rewrite-candidate suite. |
...
The isolated branch
`rk3588-rewrite-av1-6.18@e58c57e50d0a0` contains every shared 6.18 rewrite
commit through `c5faabf9d00b0`, followed by the AV1 implementation and
adversarial-repair commits. It adds sparse
...
checked 8/10-bit AFBC programming, codec-neutral counters, and five KUnit
cases. The MPP suite is therefore 90 cases and the combined AV1-branch
requirement is 237. The adversarial pass additionally closes VSI refresh,
fault-versus-completion ownership, AFBC level-IRQ teardown, post-offset
provenance, and whole-output AFBC-capacity gaps.
...
selftests also pass with the 90/147 expectation. This closes the source/build
...
1. Boot KASAN and KCSAN AV1 rewrite kernels, persist the 237-case green KUnit
   report (90 MPP + 147 RGA), and run the full paired suite matrix
