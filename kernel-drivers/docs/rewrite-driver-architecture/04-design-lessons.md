...
|----------|------------------------|---------------------|----------------|
| MPP shared decoder hardware | `rk_mpp_hw`, `rk_mpp_job`, a single-target reset-domain authority, and a separate DMA-group object divide CCU admission, group power, group reset, IOMMU refresh, and quarantine | `rk_mpp_cluster` composes the topology; reset domain and DMA group remain distinct authorities behind it | A new terminal path can otherwise repair reset but omit refresh, group-power release, or peer quarantine. |
