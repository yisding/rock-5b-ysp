...
|----------|------------------------|---------------------|----------------|
| MPP shared decoder hardware | `rk_mpp_cluster` now composes a shadow member topology, while `rk_mpp_hw`, `rk_mpp_job`, the single-target reset-domain authority, and separate DMA-group objects still divide CCU admission, group power, group reset, IOMMU refresh, and quarantine | migrate those transitions behind cluster methods while reset domain and DMA group remain distinct authorities | A new terminal path can otherwise repair reset but omit refresh, group-power release, or peer quarantine. |
...
The as-built model now has a shadow `rk_mpp_cluster`, but no
`rk_mpp_activation`, `rk_rga_task_exec`, or `rk_rga_acquire_set`. The cluster
does not yet own any admission or recovery transition. The
