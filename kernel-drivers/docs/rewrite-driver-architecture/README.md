...
| Target ownership | the MPP cluster owns topology and hard-CCU reset-pulse validation, and a refcounted cluster lease owns member-core power holds; MPP activation and RGA task-execution/acquire-set objects, plus cluster admission/coordinator-power/full-recovery authority, remain refactor goals rather than current behavior claims |
