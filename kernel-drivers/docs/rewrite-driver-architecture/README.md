...
| Build evidence | On 2026-07-27 current 6.18 `f6ebe28` passed clean-archive normal, KASAN/fault-injection, and KCSAN/lockdep profiles; strict checkpatch and source identity with mainline `394d805` pass. The user stopped the current mainline matrix. | The current 6.18 branch builds the IOMMU provider, KUnit-enabled rewrite objects, and ROCK 5B DTB across all three profiles. A build or byte identity is not hardware proof. |
...
| Bootable debug image | Current-tip 6.18.40 KASAN/UBSAN/lockdep/kmemleak package `P91d6-Cad24` is payload-verified with both rewrites, both KUnit suites, exact final patch, and intended ROCK 5B DTB | Packaging is proven for current 6.18 source. It has not been installed or booted. |
| Hardware evidence | Lifecycle-fixed `P3138-Cad24` passed 85/85 MPP plus 148/148 RGA, restored both services, and bound RGA2 plus both RGA3 cores, but later fixture warning/kmemleak evidence kept the compound gate red | KUnit is real board evidence rather than unexecuted scaffold, but a warning-clean current-tip boot and media conformance are still required. |
