...
|-----------|--------------------------------|------------------------------------------|
| Primary objective | Preserve Rockchip behavior while moving the BSP stack to Linux 6.18. | Preserve the current ROCK 5B userspace contract while replacing BSP internals with public kernel APIs and explicit ownership. Current tips are 6.18 `395644689db8f` on `v6.18.42` and mainline `38768c5cff419` on `v7.2-rc6`. |
| Code origin | Rockchip 6.1 BSP, ~87% unchanged (~90% Rockchip-authored), followed by the maintained forward-port/hardening series. | Independent implementation; current tips are 6.18 `395644689db8f` on `v6.18.42` and mainline `38768c5cff419` on `v7.2-rc6`, including the AV1 backend, August adversarial repairs, librga DMA/fence/bounce compatibility, selected-core USERPTR mapping under power, per-session RKVDEC dispatch serialization, RGA command-buffer publication, Phase 1 ownership funnels, Phase 2 reset/cluster/recovery and IRQ-register leases, Phase 3A–G activation identity and retry closure, the Phase 3H ref-owning recovered-terminal claim and service quarantine tombstone, Phase 3I exact clean-terminal observations, BSP-shaped MPP device discovery, explicit RGA3 shared-IRQ wiring, exact KUnit manifest, compile-time ABI layout contract, and reconciled ABI ledgers. |
...
| Kernel-version coupling | Production work is pinned to 6.18 and carries `compat/` glue; resync cost is paid when BSP assumptions meet newer kernel APIs. | The tracked rewrite driver/Kconfig/ABI/uAPI files are byte-identical on 6.18 and the current mainline branch; surrounding kernel, DT, and IOMMU integration differs. Focused KUnit-enabled MPP objects and all four warning-fatal clean-archive profiles compile on each Phase 3I line; both still need independent boot proof. |
...
| Validation | On 2026-08-09, Phase 3I pins MPP SHA-256 `95816d9033e76c86638e13b5ed3a0399b4a30e9d26b338579a624548fcecaada`, strict 0/0/0 over 1,076 lines, focused KUnit-enabled MPP compiles on both lines, all eight warning-fatal clean-archive profiles, ownership 2,180/tree, fixture debt 306/tree, and the exact 104/152 source manifest. The repository-wide handoff gate passes; runtime execution remains pending. The predecessor 6.18 Phase 1 tip `ab69ece998642` was packaged as inspected P692f with stamp `(gab69ece99864)` but remains uninstalled and unbooted. No package or boot carries Phase 2 or Phase 3; exact-tip KUnit, corrected USERPTR GStreamer, MPP/FFmpeg, RGA3 vpp/overlay, aged kmemleak, final librga, performance, and AV1 fault/AFBC proof remain open. |
