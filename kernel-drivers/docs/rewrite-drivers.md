...
| Code origin | Rockchip 6.1 BSP, ~87% unchanged (~90% Rockchip-authored), followed by a 75-commit forward-port/hardening series through `12a7da02bea83`. | Independent implementation; the current 6.18 tip is `835b19f81d2b`, including explicit RGA3 shared-IRQ wiring, heap-backed KUnit work and service fixtures, live-service isolation during KUnit, and ABI ledgers reconciled with the stable implementation. |
...
| Validation | On 2026-07-27, both exact final-fixture commits and their KUnit-isolation descendants passed `rewrite-build-gate.sh all` in the clean-archive `normal` profile, warning-free, building the Rockchip IOMMU provider, both KUnit-enabled rewrite objects, and `rockchip/rk3588-rock-5b.dtb`. Fixed 6.18 tip `835b19f81d2b` then built as package-verified KASAN/lockdep `P259b-Cad24`; its packaged config contains both rewrite suites and the intended instrumentation, disables vendor collisions plus optional DWC PCIe PMU, and its DTB preserves ramoops plus disjoint `0x200` RGA3 core windows. The newly found 2,928-byte batch-fixture frame warning is gone; 31 other KASAN-inflated KUnit frame-size diagnostics remain and are not treated as runtime failures. Earlier on 2026-07-24, the `normal`, `memory`, and `race` profiles completed warning-free at the defect-audit tip, while mainline built clean in `normal`. Note that an in-tree `mpp_rewrite.o` build proves nothing unless `ROCKCHIP_MPP_REWRITE_KUNIT_TEST=y`; the clean-archive and package gates explicitly require both suites. Device-free conformance validation passed in baseline, rewrite-counter, and forced-RGA-userptr-IOMMU modes. The package remains unbooted, so warning-clean KASAN/lockdep hardware proof and media conformance are still open. |
