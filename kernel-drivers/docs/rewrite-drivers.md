...
| Board evidence | Published 6.18 kernel has broad conformance, bit-exact, sanitizer, root-gate, and production-performance evidence; the latest `0074`/`0075` tail is also KASAN-verified. | Lifecycle-fixed package `P3138-Cad24` completed all 85 MPP + 148 RGA cases, restored both services, and bound every expected MPP/RGA core, but later fixture warnings and kmemleak evidence kept the compound gate red. Booted `P91d6-Cad24` again completed exact 85+148 KTAP but disabled lockdep in a second DCHS fixture before RGA ran. Current tips repair that reset/import fixture; a successor warning-clean boot remains required. |
...
| Current KUnit repair | **2026-07-27:** lifecycle-fixed `P3138-Cad24` completed the exact 85/85 MPP plus 148/148 RGA plans and restored both services, proving the preceding boot-order repair. It still exposed warning-cleanliness and fixture-ownership defects despite green KTAP. `P91d6-Cad24` then completed the same exact KTAP totals, but case 83 showed that `rk_mpp_reset_session_hw_active_import_kunit` repeated its sibling abort fixture's defect: its zeroed local service reached DCHS release without initializing `rkvenc_dchs_lock`, disabling lockdep before RGA. Current tips initialize both fixtures. A reachability audit found no other uninitialized lock in the later MPP or RGA fixture paths, but the conformance parser must prove that dynamically by rejecting any fatal line or disabled lockdep before ABI/media work. The next boot must repeat all 233 cases with a fatal-signature-free interval, live lockdep, an aged clean kmemleak scan, restored services, and all expected cores before ABI or media qualification. |
| Current source tips | 6.18 `rk3588-rewrite-6.18@9af4a8816f259` and mainline `rk3588-rewrite-mainline@fb5040f08d833` contain byte-identical rewrite driver and ABI files. They initialize the reset/import fixture's DCHS spinlock over sibling abort-fixture/nonblocking-state repairs `f6ebe28a3f66` / `394d80552960`, mutex/nested-allocation repairs `6b55e022ce49` / `9aa6ef7e97b2`, and lifecycle repairs `db8251eec71a` / `fac707773158`. Earlier `835b19f81d2b` / `79a804a26e005` contain the heap-backed 2,928-byte MPP batch fixture and `3b41eca277c7` / `52d4dfa16825` fix the final capped Debug Objects stack fixture. The audited ledgers match the implemented Kconfig exclusions, RGA errno/validation boundary, 10-bit TILE stride, shared-IRQ and RGA3 capability policy, MPP terminal isolation, and VEPU580 slice-flush fixup. |
...
| Validation | On 2026-07-27, current 6.18 `9af4a8816f259` passed clean-archive normal, KASAN/fault-injection memory, and KCSAN/lockdep race profiles; current mainline `fb5040f08d833` passed the clean-archive normal profile. Strict checkpatch and byte identity between those tips also pass. Parent-source 6.18.40 KASAN/UBSAN/lockdep/kmemleak package `P91d6-Cad24` is payload-verified and booted: exact patch 330 was applied, both 85/148-case suites completed, intended instrumentation was enabled, vendor collisions were disabled, and the DTB retained ramoops plus disjoint RGA3 windows. Its 131.8 MiB image exceeded the stock load gap but fit the existing managed 188 MiB U-Boot map with about 56.2 MiB spare. That boot exposed the reset/import fixture warning summarized above, so warning-clean current-tip KASAN/lockdep hardware proof and media conformance remain open. |
