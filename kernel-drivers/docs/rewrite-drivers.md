...
| In-tree testability | No comparable driver KUnit suite; confidence comes primarily from BSP history, repository reproducers, and extensive board runs. | 84 MPP + 148 RGA KUnit cases (232 total), fail-closed validators, debugfs counters, a checked fixture-debt baseline, and clean-source build profiles. The tests have already exposed fixture/lifetime defects, so their presence is useful evidence rather than a correctness guarantee. |
| Board evidence | Published 6.18 kernel has broad conformance, bit-exact, sanitizer, root-gate, and production-performance evidence; the latest `0074`/`0075` tail is also KASAN-verified. | Lifecycle-fixed package `P3138-Cad24` completed all then-current 85 MPP + 148 RGA cases, restored both services, and bound every expected MPP/RGA core, but later fixture warnings and kmemleak evidence kept the compound gate red. Booted `P91d6-Cad24` again completed exact 85+148 KTAP but disabled lockdep in a second DCHS fixture before RGA ran. Current tips retain the repair and rationalize the live gate to 84+148; a successor warning-clean boot remains required. |
...
| Item | State |
|------|--------------------|
| Code | `drivers/video/rockchip/mpp-rewrite/` (`mpp_rewrite.c` 14,161 lines; +`ABI.rst`, `Kconfig`, `Makefile`) + `drivers/video/rockchip/rga-rewrite/` (`rga_rewrite.c` 24,035 lines; +`ABI.rst`, `Kconfig`, `Makefile`). The trees contain 84 MPP and 148 RGA KUnit cases (232 total). |
| Current KUnit repair | **2026-07-28:** lifecycle-fixed `P3138-Cad24` completed the then-current exact 85/85 MPP plus 148/148 RGA plans and restored both services, proving the preceding boot-order repair. It still exposed warning-cleanliness and fixture-ownership defects despite green KTAP. `P91d6-Cad24` then completed the same exact KTAP totals, but case 83 showed that `rk_mpp_reset_session_hw_active_import_kunit` repeated its sibling abort fixture's defect: its zeroed local service reached DCHS release without initializing `rkvenc_dchs_lock`, disabling lockdep before RGA. Current tips initialize both fixtures, make the lifecycle suites opt-in, remove the compile-time-owned ABI-layout runtime case, and mechanically reject new lexical fixture-debt signals. The next boot must run the current 232 cases with a fatal-signature-free interval, live lockdep, an aged clean kmemleak scan, restored services, and all expected cores before ABI or media qualification. |
| Current source tips | 6.18 `rk3588-rewrite-6.18@cd71f985a784c` and mainline `rk3588-rewrite-mainline@7dcb4c3b5a981` contain byte-identical rewrite driver and ABI files: the 2026-07-29 [review-round-2 fixes](../../findings/2026-07-29-rewrite-driver-review-round-2.md). **Behaviour change:** legacy librga pre-swapped 90/270-degree destination windows now validate in canvas orientation on both backends, so GStreamer/FFmpeg portrait rotates that previously failed `-EINVAL` execute; the rotate KUnit cases submit faithful wire geometry and pin canvas-oriented write windows. The commit also removes the hard-CCU per-core link-table relink (coordinator running list is the only chain writer), kicks the scheduler when abort idles a core, derives the service from the bound core in shared paths, gives every RGA release fence its own dma_fence context plus a module reference, gates hard-handler MMIO on a regs-live count, salvages timed-out-but-finished blits, defaults gauss source alpha opaque, sizes legacy FBC/tile direct imports from their rd_mode layout, and repairs the session-abort KUnit double free. Parents `51ea9d1ca537` / `03da898b03f1f` isolate KUnit fixtures behind local services; `669697f23d3df` / `a49eb7575f436` retire the MPP ABI-layout runtime case in favor of the stronger `static_assert()` set; `0a2d7b9414f58` / `aa18488c8642b` contain the opt-in Kconfig change. The audited ledgers match the implemented Kconfig exclusions, RGA errno/validation boundary, 10-bit TILE stride, shared-IRQ and RGA3 capability policy, MPP terminal isolation, and VEPU580 slice-flush fixup. |
...
| Validation | On 2026-07-29, current 6.18 `cd71f985a784c` and mainline `7dcb4c3b5a981` compile their KUnit-enabled rewrite objects warning-free in-tree; the clean-archive normal/test-disabled/memory/race matrix is the current handoff gate. The source audit baseline moved 325 → 327 for the two deliberate signals of the session-abort double-free fix (the production-freed job must not be KUnit-managed); the audit reports zero new and zero absent signals on both trees, and the gate separately requires byte-identical sources, Kconfig, ABI ledgers, and UAPI. Parent-source 6.18.40 package `P91d6-Cad24` remains the latest boot: both then-current 85/148 suites completed, but the reset/import fixture disabled lockdep. The current 84/148 manifest, fatal-free interval, live lockdep, source/config/package identity, kmemleak, runtime, and media proof remain open on a successor boot — which will be the first to run the corrected rotate expectations and the double-free repair. |
