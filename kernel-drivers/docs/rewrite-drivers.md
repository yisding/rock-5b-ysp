...
| Kernel-version coupling | Production work is pinned to 6.18 and carries `compat/` glue; resync cost is paid when BSP assumptions meet newer kernel APIs. | The tracked rewrite driver/Kconfig/ABI/uAPI files are byte-identical on 6.18 and the current mainline branch; surrounding kernel, DT, and IOMMU integration differs. Both fixed tips pass the normal focused build; predecessor 6.18 `19634f4eebba` is booted, while the current tips still need independent boot proof. |
...
| Board evidence | Published 6.18 kernel has broad conformance, bit-exact, sanitizer, root-gate, and production-performance evidence. | KASAN boot `#2` (`g19634f4eebba`, 2026-08-05) completed exact 92/92 MPP plus 152/152 RGA KTAP with a clean outer interval and live lockdep. The official MPP suite passed 12/12. The direct librga smoke runtime-verifies the earlier handle-plane repair, then exposes independent legacy fd-zero and USERPTR SWIOTLB defects fixed only in the current tip. No current-tip RGA media or AV1 hardware result exists. |
...
| Validation | On 2026-08-05, the warning-fatal clean-archive `normal` profile passed for both maintained tips, compiling Rockchip and VSI IOMMU support, both KUnit-enabled rewrite objects, and the ROCK 5B DTB. The source audit reports **306 known signals, zero new, and zero absent** on both trees, and the identity gate passes for the tracked rewrite sources, Kconfig, ABI ledgers, and UAPI. The exact-tip 6.18 KASAN image/DTB/headers/libc-dev packages also build as `Pc86b-Cad24`; the image embeds `gdf22eeef8757`, the packaged config SHA-256 is `18b72117c018604844b7cb74adb3a24ea25e0134cefe6eb1adf82aea8fc8502d`, and KASAN, lockdep, both rewrite drivers, and both KUnit suites are enabled, now with `BOOTPARAM_HUNG_TASK_PANIC` and `BOOTPARAM_SOFTLOCKUP_PANIC` so a boot-time KUnit stall cannot wedge the board silently again. Its predecessor `P27bb-Cad24` (`g37ae7459656b`) **did not boot**: the legacy fd-zero fence sentinel reached three pending-acquire KUnit cases whose fixtures install a live sync-file on descriptor zero, and the un-gated job wedged `rk_rga_release()` before initramfs — see [the wedge finding](../../findings/2026-08-05-rewrite-kunit-fd-zero-boot-wedge.md). Predecessor 6.18 `19634f4eebba` has exact booted 92/152 KUnit, a clean interval with live lockdep, 12/12 official MPP, and partial librga evidence. The test-disabled, memory, and race profiles were not rerun at the fixed tips. Current-tip install/boot, booted KUnit, aged kmemleak, corrected full librga, performance, and AV1 fault/AFBC proof remain open. |
