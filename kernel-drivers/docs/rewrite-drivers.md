...
| Current KUnit repair | **2026-07-26:** the current tips are `rk3588-rewrite-6.18@c5faabf9d00b` and the byte-identical `rk3588-rewrite-mainline@39475996a7a8`. The first full boot of parent `5a55fa4743b2` ran all 232 cases but passed only 77/85 MPP and 126/147 RGA, with eight Oopses, nine KASAN reports, five debug-object warnings, two refcount warnings, and three IRQ/preemption imbalance pairs in the KUnit interval. The [root-cause record](../../findings/2026-07-26-rewrite-kunit-failure-root-causes.md) separates incomplete fixtures and stale register expectations from the six driver-contract fixes. All six clean-source `normal`/`memory`/`race` profiles pass warning-free at the repaired tips, and KASAN/lockdep package `P3b08-Cad24` contains both rewrite drivers and both suites. Its booted 85+147 clean rerun is pending. The rows below preserve the earlier branch/rebase and package history. |
