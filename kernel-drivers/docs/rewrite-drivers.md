...
| Size (observed 2026-07-06) | MPP ~15,822 lines + RGA3 ~19,171 lines (vendor-delta.md method) | MPP rewrite ~9,256 lines + RGA rewrite ~18,321 lines |
...
| Item | State (2026-07-10) |
|------|--------------------|
| Code | `drivers/video/rockchip/mpp-rewrite/` (`mpp_rewrite.c` 9,256 lines; 9,600 total incl. `ABI.rst`, `Kconfig`, `Makefile`) + `drivers/video/rockchip/rga-rewrite/` (`rga_rewrite.c` 18,321 lines; 18,847 total incl. `ABI.rst`, `Kconfig`, `Makefile`) |
| 6.18 state | committed local branch `rk3588-rewrite-6.18` at **`d1d15a3d052a`** ("iommu: rockchip: harden media fault reporting"), committed in dev worktree `/home/yi/Code/kernel/linux-6.18-rkvenc`. It includes the broad RGA/MPP ABI and performance-path work described in §2/§3, the Rock 5B DT self-containment commit for disabled decoder nodes/IOMMUs/SRAM pools, the BSP-derived forward-port recovery cleanup, Rockchip IOMMU `map_pages`/`unmap_pages` count handling for large dma-buf mappings, provider fault-return fallback, bus-error reporting through the fault path, 32-bit DMA segment sizing for media masters, 6.18-local helper-export runtime-PM guards, rewrite MPP/RGA fault-handler registration through the Rockchip provider-local public hook without the teardown-unsafe legacy domain fallback, explicit KUnit coverage for GStreamer decoder-side 8-bit RGBA/BGRA/RGBx/BGRx RGA output conversions, compact NV12_10LE40/NV16_10LE40 decoder-output scaling to 8-bit NV12/NV16, the remaining 180/270-degree GStreamer public rotation values, display-shaped BGRx 90-degree blits, opt-in display-tail BGRA partial alpha-blend, RGB565 180-degree command emission, and XRGB 270-degree RGA2 fallback command emission, invalid public scheduler-core mask rejection, RKNN/RKNPU RGB888/NV12/NV21 preprocessing plus RGBA crop/letterbox command profiles, zero-count RGA import/release buffer-pool behavior, direct physical-address submit rejection after temporary import rollback, VP9 RKVDEC fd-to-IOVA register translation/validation, direct legacy `RGA_BLIT_SYNC` wait/no-fence ioctl behaviour used by the default `c_RkRgaBlit()` path, legacy RGA flush/result no-op ioctl dispatch used by librga's post-blit compatibility path, `SET_ERR_REF_HACK` copy/discard control handling used by current libmpp probing, dormant batch-server wait-array collector rejection with `-EOPNOTSUPP`, RGA2-Pro RFBC64x4/AFBC32x8 source profiles rejected with `-EOPNOTSUPP` instead of carrying an executable FBCIN path, RGA3 userptr-IOMMU fallback mapping with fail-closed dma-buf span validation, and `rk_rga_rewrite/userptr_iommu/{attempt,ok,active,force_remap}` debugfs counters/knob for development-only fallback attribution. |
| Mainline-master state | committed local branch `rk3588-rewrite-mainline` at **`083bdb98e715`** ("iommu: rockchip: harden media fault reporting"), rebased to official kernel.org `v7.2-rc2` in sibling worktree `/home/yi/Code/kernel/linux`; the pre-rebase tip is preserved as `ysp-backup/rk3588-rewrite-mainline-before-7.2-rc2`. It carries the same rewrite drivers and userspace-facing ABI coverage, the mainline DT/wiring work, the Rockchip IOMMU map-count fix, provider fault-return fallback, bus-error reporting through the fault path, 32-bit DMA segment sizing for media masters, the minimal public `include/soc/rockchip/rockchip_iommu.h` provider fault hook used by the rewrites, and the same GStreamer decoder 8-bit RGB, compact 10-bit YUV-output, 180/270-degree rotation, display-shaped BGRx rot90, opt-in display-tail partial alpha-blend, RGB565 rotation and XRGB RGA2-fallback rotation coverage, invalid public scheduler-core mask rejection, RKNN preprocessing plus RGBA crop/letterbox, zero-count RGA import/release buffer-pool behavior, direct physical-address submit rejection after temporary import rollback, VP9 RKVDEC translation, legacy sync-blit ioctl, legacy RGA no-op ioctl, MPP err-ref control, dormant batch-server wait-array collector rejection, RGA2-Pro FBC source rejection coverage, RGA userptr-IOMMU fallback userptr IOMMU mapping, and the same RGA userptr-IOMMU fallback debugfs counters/force knob. |
| Validation | Focused compile gates pass at the committed tips available so far. After the Rockchip IOMMU provider hardening slice, the default `normal` profile in `kernel-drivers/tests/rewrite-build-gate.sh all` built from `git archive` copies and completed warning-free for `../kernel/linux-6.18-rkvenc` at `d1d15a3d052a` and the pre-rebase `../kernel/linux` at `12f712d71144`, producing the KUnit-enabled `drivers/video/rockchip/mpp-rewrite/mpp_rewrite.o` and `drivers/video/rockchip/rga-rewrite/rga_rewrite.o` targets for both kernels. The rebased `../kernel/linux` at `083bdb98e715` has source-package extraction and `olddefconfig` coverage through `packaging/ppa/kernel-rewrite-alpha-7.2-rc2/`, with rewrite MPP/RGA and KUnit config enabled. The same slice also passed focused clean-archive provider object builds of `drivers/iommu/rockchip-iommu.o` for both kernel lines before commit. Earlier on 2026-07-06, the `normal`, `memory`, and `race` profiles completed warning-free for `../kernel/linux-6.18-rkvenc` at `0a35c26a0fd7` and `../kernel/linux` at `938b1d2032c3`; with scratch cleanup in place, `REWRITE_BUILD_PROFILES="normal memory race" kernel-drivers/tests/rewrite-build-gate.sh all` completed all six profiles in one invocation and left no `rkcompat-rewrite-build.*` directories under `/tmp` afterward. The sanitizer profiles raise `FRAME_WARN` for instrumentation-inflated KUnit stack frames, while the default `normal` profile remains the strict warning gate. `VALIDATE_ONLY=1 kernel-drivers/tests/rewrite-conformance-run.sh`, `VALIDATE_ONLY=1 PROFILE=rewrite RUN_COUNTER_CHECKS=1 kernel-drivers/tests/rewrite-conformance-run.sh`, and the same counter-enabled validation with `LIBRGA_FORCE_RGA_USERPTR_IOMMU=1` also passed device-free syzlang ABI-marker, ioctl mutator build, direct `librga` smoke build, RGA IOMMU scatter-fuzzer build, recovery stress harness config validation, MPP/GStreamer case-builder, FFmpeg/rkmppenc case-list, comparator, counter-default, forced RGA userptr-IOMMU fallback counter-default, and synthetic multicore prefix-counter validation on 2026-07-06. This is still code/ABI-ledger progress rather than proof from a booted rewrite kernel. The broader conformance plan remains the staged `../rockchip-conformance` forward-port-vs-rewrite workflow for MPP, librga, JeffyCN GStreamer, ffmpeg-rockchip, and opt-in `rkmppenc`; booted MPP official-test artifact/timing runs, direct virtual-address RGA userptr-IOMMU fallback RGA3 runs with `userptr_iommu/attempt` and `userptr_iommu/ok` evidence, kill/reset/unbind recovery stress logs, GStreamer state/allocator/RGA-conversion/P010-P210/VP9/display/KMS-capture results, expanded FFmpeg decoder/RGA-filter artifact/timing results, and optional `rkmppenc` app-level logs/comparator output are still missing before lower-priority diagnostic BSP profiles. **UNVERIFIED in this repo**: the workload gate and expanded conformance bundle have not yet passed on hardware through the rewrite; no validation record equivalent to status.md exists yet. |
