...
| Size (observed 2026-07-03) | MPP ~15,822 lines + RGA3 ~19,171 lines (vendor-delta.md method) | MPP rewrite ~8,430 lines + RGA rewrite ~15,924 lines |
...
- **Version tuples are capability keys.** `librga` capability-probing expects
  ...
  (`rga_rewrite.c:15144-15158` — `{3, 2, 0x63318}` / `{3, 0, 0x76831}`,
  rendered `"%x.%01x.%05x"` at `:1258`, hence the hex-looking revision). Report
...
- **Legacy `RGA_CACHE_FLUSH`, `RGA_FLUSH`, `RGA_GET_RESULT`, and
  `RGA2_GET_RESULT` are safe as BSP-compatible no-ops** (`:15108-15113`) —
...
- **Buffer import**: `RGA_IOC_IMPORT_BUFFER`/`RELEASE_BUFFER` for dma-buf fds
  *and* user virtual addresses — VA imports pin user pages, build sg_tables via
  `sg_alloc_table_from_pages()` (`:14531`), map with the public DMA API, and
...
- **Acquire-fence ownership**: when a submitted task clears
  ...
  `release_fence_fd` path for jobs deferred behind an acquire fence.
...
- Optional **KUnit coverage** exists on both rewrite drivers: MPP parser helpers
  ...
  RGA3-to-RGA2 core handoff/requeue selection, legacy async blit and modern
  async request-submit acquire/release-fence ioctls,
