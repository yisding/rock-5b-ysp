...
|-----------|--------------------------------|------------------------------------------|
| Primary objective | Preserve Rockchip behavior while moving the BSP stack to Linux 6.18. | Preserve the current ROCK 5B userspace contract while replacing BSP internals with public kernel APIs and explicit ownership. Current tips are 6.18 `1784358650e3c41abbd5154cbd1737da07d968de` on `v6.18.42` and mainline `e71d368cc48374638eafbfdf1e9c6864cabbd118` on `v7.2-rc6`. |
| Code origin | Rockchip 6.1 BSP, ~87% unchanged (~90% Rockchip-authored), followed by the maintained forward-port/hardening series. | Independent implementation; current tips are 6.18 `1784358650e3c41abbd5154cbd1737da07d968de` on `v6.18.42` and mainline `e71d368cc48374638eafbfdf1e9c6864cabbd118` on `v7.2-rc6`, including the AV1 backend, August adversarial repairs, librga DMA/fence/bounce compatibility, selected-core USERPTR mapping under power, per-session RKVDEC dispatch serialization, RGA command-buffer publication, Phase 1 ownership funnels, Phase 2 reset/cluster/recovery and IRQ-register leases, Phase 3A–G activation identity and retry closure, the Phase 3H ref-owning recovered-terminal claim and service quarantine tombstone, BSP-shaped MPP device discovery, explicit RGA3 shared-IRQ wiring, exact KUnit manifest, compile-time ABI layout contract, and reconciled ABI ledgers. |
...
| In-tree testability | No comparable driver KUnit suite; confidence comes primarily from BSP history, repository reproducers, and extensive board runs. | 103 MPP + 152 RGA KUnit cases (255 total), fail-closed validators, debugfs counters, a checked fixture-debt baseline, and clean-source build profiles. The tests have already exposed fixture/lifetime defects, so their presence is useful evidence rather than a correctness guarantee. |
| Board evidence | Published 6.18 kernel has broad conformance, bit-exact, sanitizer, root-gate, and production-performance evidence. | KASAN boot `#2` (`g19634f4eebba`, 2026-08-05) completed exact 92/92 MPP plus 152/152 RGA KTAP with a clean outer interval and live lockdep. Kernel #8 at `gf37186832202` later exposed residual same-session decode corruption, solo RGA3 vpp corruption, and an overlay-chain RGA3 IOMMU fault; those observations motivated the current source repairs. Phase 3H tips `1784358650e3c41abbd5154cbd1737da07d968de` / `e71d368cc48374638eafbfdf1e9c6864cabbd118` have source/focused-object evidence but remain unbooted, and the RGA2 1 MiB SWIOTLB staging limitation remains open. No current-tip RGA media or AV1 hardware result exists. |
...
| Item | State |
|------|--------------------|
| Code | `drivers/video/rockchip/mpp-rewrite/` (`mpp_rewrite.c`; +`ABI.rst`, `Kconfig`, `Makefile`) + `drivers/video/rockchip/rga-rewrite/` (`rga_rewrite.c`; +`ABI.rst`, `Kconfig`, `Makefile`). The trees contain 103 MPP and 152 RGA KUnit cases (255 total). |
| Current KUnit repair | KASAN boot `Pc86b-Cad24` (`gdf22eeef8757`, 2026-08-05) passes the exact **92 MPP + 152 RGA** manifest: 244 results, zero failures/skips, clean outer-KTAP interval, and live lockdep. It also passes 12/12 official MPP and reaches 21/34 required librga cases. Source stamp `gf37186832202` later booted as kernel #8 and exposed the residual decoder and RGA3 failures documented in the live findings. The current 103/152 manifest and successor ownership changes remain unbooted; all 255 cases and the GStreamer/FFmpeg conformance workloads require exact-tip replay together with an aged clean kmemleak scan, restored services, and all expected cores. |
| Current source tips | 6.18 `rk3588-rewrite-6.18@1784358650e3c41abbd5154cbd1737da07d968de` on `v6.18.42` and mainline `rk3588-rewrite-mainline@e71d368cc48374638eafbfdf1e9c6864cabbd118` on `v7.2-rc6` are the maintained tips. Their provisional Phase 1 funnels are followed by stable reset domains and an `rk_mpp_cluster` topology. The hard-CCU participant pulse validates its pinned coordinator/cores through that cluster and one common reset domain, then records the physical assert/delay/deassert sequence as one non-interleavable epoch. A refcounted cluster power lease replaces the job's fixed powered-core array and transfers unchanged along the existing coordinator chain. Cluster methods validate and own the coordinator running list, link relinks, completion/resend snapshots, soft-CCU arm/START, and hard descriptor publication. Single-core, soft-CCU, idle-fault, and hard-CCU group recovery return typed quiesced/reusable results. Hard recovery deduplicates the already reference-pinned participants' DMA groups, refreshes each group exactly once after reset, and forbids resend if topology, reset, or any refresh is not reusable; terminal isolation remains cleanup-only. Every START publishes a register lease tied to the live reset epoch and direct-core activation generation. Hard IRQ records only a live lease, and the thread rejects absent, consumed, reset-stale, or generation-stale records before it can claim the active slot. Phase 3A–G establish retained activation identity, exact slot/dispatch ownership, fresh hard-CCU retry successors, and typed retry-predecessor closure. Phase 3H makes the active claim token own the exact detached job reference and moves recovered terminals to `RETIRED` only on typed direct-core or group/core proof. Exact restore refusal transfers that reference and generation to a service `QUARANTINED` tombstone with distinct diagnostic/core/group evidence, retains resources and dispatch, closes selected-core/owned-CCU admission, blocks remove, and survives shutdown until reboot. Clean IRQ/`CCU_DONE` and pre-doorbell `START_FAILURE` retain the legacy `CLAIMED` release boundary; `RECLAIMABLE`, general clean retirement, CCU/link/DCHS and power migration, and final result arbitration remain open. The tracked MPP/RGA sources, Kconfig, ABI ledgers, and UAPI are byte-identical between the two maintained branches. VSI retains kernel-version-specific IOMMU attach APIs outside that identical set, with the same admission, retained-fault, callback-drain, and domain-retirement contract. |
...
| Validation | On 2026-08-09, both maintained Phase 3H tips pass the source-pinned 2068-signal production inventory, unchanged 306-signal KUnit-debt audit, exact 103/152 manifest, byte identity, strict checkpatch, all eight warning-fatal clean-archive profiles, and the repository-wide handoff gate. The memory/race profiles remain compile-only evidence. The predecessor 6.18 Phase 1 tip `ab69ece998642` was packaged as inspected P692f with stamp `(gab69ece99864)` but remains uninstalled and unbooted. No package or boot carries Phase 2 or Phase 3; exact-tip KUnit, corrected USERPTR GStreamer, MPP/FFmpeg, RGA3 vpp/overlay, aged kmemleak, final librga, performance, and AV1 fault/AFBC proof remain open. |
