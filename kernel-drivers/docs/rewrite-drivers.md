...
|-----------|--------------------------------|------------------------------------------|
| Primary objective | Preserve Rockchip behavior while moving the BSP stack to Linux 6.18. | Preserve the current ROCK 5B userspace contract while replacing BSP internals with public kernel APIs and explicit ownership. Current tips are 6.18 `7548afe6a8b1b` on `v6.18.42` and mainline `af89363ffa5ed` on `v7.2-rc6`. |
| Code origin | Rockchip 6.1 BSP, ~87% unchanged (~90% Rockchip-authored), followed by the maintained forward-port/hardening series. | Independent implementation; current tips are 6.18 `7548afe6a8b1b` on `v6.18.42` and mainline `af89363ffa5ed` on `v7.2-rc6`, including the AV1 backend, August adversarial repairs, librga DMA/fence/bounce compatibility, selected-core USERPTR mapping under power, per-session RKVDEC dispatch serialization, RGA command-buffer publication, Phase 1 ownership funnels, Phase 2 reset/cluster/recovery and IRQ-register leases, the Phase 3A embedded current-attempt generation/deadline record, BSP-shaped MPP device discovery, explicit RGA3 shared-IRQ wiring, exact KUnit manifest, compile-time ABI layout contract, and reconciled ABI ledgers. |
...
| Validation | On 2026-08-09, both maintained Phase 3A tips pass the source-pinned 1220-signal production inventory, unchanged 306-signal KUnit-debt audit, exact 102/152 manifest, byte identity, strict checkpatch, and all eight warning-fatal clean-archive `normal`, `test-disabled`, KASAN/fault-injection `memory`, and KCSAN/lockdep `race` profiles. The predecessor 6.18 Phase 1 tip `ab69ece998642` was packaged as inspected P692f with stamp `(gab69ece99864)` but remains uninstalled and unbooted. No package or boot carries Phase 2 or Phase 3A; exact-tip KUnit, corrected USERPTR GStreamer, MPP/FFmpeg, RGA3 vpp/overlay, aged kmemleak, final librga, performance, and AV1 fault/AFBC proof remain open. |
