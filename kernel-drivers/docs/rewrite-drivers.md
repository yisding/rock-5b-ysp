...
|-----------|--------------------------------|------------------------------------------|
| Primary objective | Preserve Rockchip behavior while moving the BSP stack to Linux 6.18. | Preserve the current ROCK 5B userspace contract while replacing BSP internals with public kernel APIs and explicit ownership. |
| Code origin | Rockchip 6.1 BSP, ~87% unchanged (~90% Rockchip-authored), followed by a 75-commit forward-port/hardening series through `12a7da02bea83`. | Independent implementation; the current 6.18 tip is `51ea9d1ca537`, including explicit RGA3 shared-IRQ wiring, local KUnit service fixtures with no production unbind/reprobe, assertion-safe fence/FD/work/device cleanup, deterministic race synchronization, initialized locked fixtures, opt-in lifecycle suites, a compile-time-owned ABI layout contract, and ABI ledgers reconciled with the stable implementation. |
| Hardware and codec scope | Broad BSP surface, including RK3588 RKVENC2/RKVDEC2, RGA2/RGA3, the separate RKMPP AV1/VPU981 backend, and legacy SoC helper code. | Deliberately RK3588-focused: RKVENC2/RKVDEC2 and RGA2/RGA3. RKMPP AV1, JPEG, and older VDPU/VPU blocks are outside this slice. |
...
| State and lifetime model | Modular BSP subsystem, but heavily global: service/request/memory managers route work into per-core scheduler state. The hardening tail repairs ownership races without replacing that model. | Session-owned IDs/imports, immutable submitted jobs, retained hardware/import references, exact active-slot claims, generation-tagged recovery, and quarantine when reset cannot prove DMA stopped. |
| DMA/IOMMU model | Compatibility shims plus vendor-derived mapping and recovery machinery, progressively hardened with public fault routing and 32-bit aperture checks. | Public dma-buf/DMA/IOMMU APIs only; mappings are tied to the selected DMA device, literal IOVAs require provenance, and scattered RGA userptr uses a driver-owned contiguous IOVA fallback. |
| Kernel-version coupling | Production work is pinned to 6.18 and carries `compat/` glue; resync cost is paid when BSP assumptions meet newer kernel APIs. | The two driver files are byte-identical on 6.18 and `v7.2-rc5`; surrounding DT/IOMMU integration differs. This is the stronger portability result, although each new kernel still needs build and boot proof. |
| Source organization | Many focused C/header files: easier subsystem-local navigation, but behavior crosses global managers, backend ops, and compatibility layers. | One large C file per driver plus an ABI ledger. Ownership flow is more local, but 14k/24k-line translation units and embedded KUnit bodies are themselves a review and merge burden. |
| In-tree testability | No comparable driver KUnit suite; confidence comes primarily from BSP history, repository reproducers, and extensive board runs. | 92 MPP + 152 RGA KUnit cases (244 total), fail-closed validators, debugfs counters, a checked fixture-debt baseline, and clean-source build profiles. The tests have already exposed fixture/lifetime defects, so their presence is useful evidence rather than a correctness guarantee. |
| Board evidence | Published 6.18 kernel has broad conformance, bit-exact, sanitizer, root-gate, and production-performance evidence; the latest `0074`/`0075` tail is also KASAN-verified. | Boot `#29` (`g8042f13c5459`, 2026-08-02 14:46) completed the then-current exact 89/89 MPP plus 150/150 RGA KTAP with zero fatal signatures across the boot, live lockdep, kmemleak scanning, and every expected MPP/RGA core bound — the first clean compound result, and the historical `P3138-Cad24`/`P91d6-Cad24` lockdep and fixture-warning failures are closed. It predates the 2026-08-02 review fixes, whose `#30` package (`g2f05724a2003`, 92+152) is installed but unbooted, so a successor boot remains required. |
...
`/dev/mpp_service` on the `rk3588-rewrite-6.18` / `rk3588-rewrite-mainline`
branches, while the forward port does. AV1 lives on a separate
...
clean `vsi-iommu` provider and has decoded AV1 bit-exact on hardware; those two
rewrite branches still do not bind this block. The experimental spur
`rk3588-rewrite-av1-6.18` does add an AV1 rewrite backend (source-inspected,
no board evidence yet); see the
...
| Item | State |
|------|--------------------|
| Code | `drivers/video/rockchip/mpp-rewrite/` (`mpp_rewrite.c`; +`ABI.rst`, `Kconfig`, `Makefile`) + `drivers/video/rockchip/rga-rewrite/` (`rga_rewrite.c`; +`ABI.rst`, `Kconfig`, `Makefile`). The trees contain 92 MPP and 152 RGA KUnit cases (244 total). |
| Current KUnit repair | **2026-08-03:** the historical `P3138-Cad24`/`P91d6-Cad24` failures are closed. Boot `#28` (`g187b0d647e6c`, 2026-08-01 20:44) ran 90/90 MPP and 150/152 RGA, and both RGA failures were [fixture lag rather than driver defects](../../findings/2026-08-01-rewrite-kunit-boot-failures-and-suite-audit.md); the WARN splat in that log was the stale `map_hw` fixture tripping a can't-happen guard. Boot `#29` (`g8042f13c5459`) then ran exact 89/89 MPP plus 150/150 RGA twice, on 2026-08-02 at 10:16 and 14:46. The 14:46 boot is the reference result: zero fatal signatures across the entire boot, live lockdep, kmemleak scanning, and all three RGA cores plus the MPP service registered. The 10:16 boot was equally green at its KUnit interval but its lockdep validator later exhausted `MAX_LOCKDEP_CHAINS` at 13:30, ~3 h after the tests, so treat long-lived rewrite boots as lockdep-capacity-bound. The 2026-08-02 review fixes add five cases and are unbooted: `#30` must run the current 244 cases with a fatal-signature-free interval, live lockdep, an aged clean kmemleak scan, restored services, and all expected cores before ABI or media qualification. |
| Current source tips | 6.18 `rk3588-rewrite-6.18@501a2b47f3503` and mainline `rk3588-rewrite-mainline@694aac9b7c0ff` carry the 2026-08-02 [adversarial review](rewrite-driver-adversarial-review-2026-08-02.md) repairs: RGA plane-resolver agreement on a lone `v_addr` placeholder, no teardown of a partially created mapping after its power domain suspended, USERPTR CPU/device ownership tracking that prevents a second copyback, no DMA-API sync after the DMA mapping is abandoned, object-based rather than integer-ID terminal cleanup, an explicit `select SYNC_FILE`, callback-drain failure reflected in the removal result, the one-past-end DMA-BUF address narrowed to VEPU580 bitstream-top word 172, RKVDEC RCB patched with driver-owned IOVAs that survive final validation and are sized by the trusted extent, HARD-CCU fault attribution that never reads a powered-off link register, no release of mappings/commands/power until reset proves the engine stopped, `SET_REG_ADDR_OFFSET` honored for explicit-IOVA jobs, RGA3 pattern-blend rotation refused while its canvas semantics are unresolved, no restart after a poll in the same batch consumed a record, and one execution per staged poll request. **2026-08-03:** porting that commit to mainline exposed one more defect and closed it. The harden commit's recovery fallback called `rockchip_iommu_mask_irq()`, which the mainline branch's deliberately minimal provider surface does not export — it declares only `rockchip_iommu_set_fault_handler()` and `rockchip_iommu_sync_fault_handler()`, while 6.18 carries the full BSP set — so `rga_rewrite.o` failed to build on `v7.2-rc5` while `mpp_rewrite.o` compiled. The fallback proved [unreachable by construction](../../findings/2026-08-03-rewrite-rga-unreachable-iommu-irq-mask.md): the setter returns non-zero only as `-ENODEV` when `rk_iommu_from_dev_checked()` is NULL, and the mask helper opens with the same lookup and returns on NULL, so it could never mask an IRQ or protect a stored token. `501a2b47f3503` removes it as a no-op, matching the equivalent MPP teardown, and mainline `694aac9b7c0ff` cherry-picks it. Current tips are 6.18 `501a2b47f3503` and mainline `694aac9b7c0ff`: rewrite directories byte-identical, and both objects build on 6.18 and `v7.2-rc5`. VSI still has kernel-version-specific IOMMU attach APIs but the same admission, retained-fault, callback-drain, and domain-retirement contract. Earlier [review-round-2](../../findings/2026-07-29-rewrite-driver-review-round-2.md) and [AV1/VSI](../../findings/2026-07-30-rewrite-av1-vsi-fault-afbc-lifecycle-races.md) work remains in both tips. |
| 6.18 rebase/IRQ history | The shared-IRQ checkpoint was **`eb64bc7de327`** ("media: rockchip: rga-rewrite: model shared IRQ wiring"), after published DT/fixture repair `0cc483d3ee20`; the current heap-fixture tip is recorded above. The branch remains based on the latest 6.18 forward-port oracle, `rk3588-video-6.18@12a7da02bea83` ("video: rockchip: rkvenc2: reserve a slice fifo slot for the terminal record"). The previous tip is preserved locally as `ysp-backup/rk3588-rewrite-6.18-before-fwport-20260726@40cf22629cf63`. The rebased stack keeps the 13 rewrite commits previously recorded as `c540d63a8a9be..40cf22629cf63`, now `57e4aaac55eab..5a55fa4743b2`, after all current forward-port commits. **Pre-rebase state (2026-07-24):** committed branch `rk3588-rewrite-6.18` at **`40cf22629cf63`** ("media: rockchip: rga-rewrite: carry the 10-bit byte-stride ABI into TILE"), the tip of a **13-commit series** (`c540d63a8a9be..40cf22629cf63` — a 12-commit defect audit plus the 10-bit TILE byte-stride reconciliation ([TILE finding](../../findings/2026-07-24-rga-10bit-tile-byte-stride-and-fbc-exception.md), pairs with librga fork `4c26ddf`), split by defect class, each commit individually compile-verified so the series is bisectable) that fixes 17 confirmed defects across both drivers — three wake-after-unlock use-after-frees, a stale-timeout-generation reset, a route-B SWIOTLB regression from `0d71ded1690c9`, an MPP abort-sweep list overload (UAF + counter skew), a global-lock `copy_to_user()` DoS, compact-10-bit `x_offset` byte conversion on both cores, the RGA2 color-key enable, and the RGA3 scale/window capability gates. **Behaviour change:** the new RGA3 68x2 minimum moves small blits to RGA2, matching the vendor table. See [audit finding](../../findings/2026-07-24-rewrite-driver-multi-agent-defect-audit.md). The series sits atop **`185d4dcec110`** ("media: rockchip: rga-rewrite: honor the legacy byte-stride ABI for 10-bit rasters") in `/home/yi/Code/rock-5b/kernel/linux-6.18-rkvenc`, which in turn is atop `1fe46df86f1ca` ("harden rewrite driver recovery") and parent `8469183da227` ("port forward-port RGA bugfixes"). Over the July 15 MPP/RGA hardening it layers the July 17 RGA low-voltage quirks/config-error IRQ/cache-line shadows/CSC rule, the ported forward-port RGA bugfixes (10-bit plane offsets, max-seg-size, import double-put, acquire-abort race, job_put NULL guard), the 2026-07-23 recovery-hardening churn (~9k insert/~4.9k delete; RGA KUnit 122 → 147, MPP 86 → 85), and the 2026-07-24 **raster 10-bit byte-stride ABI alignment** (layout/validators/RGA3 write-offset path now match the byte-literal register writers; pairs with fwport `0072` and the librga-fork im2d conversion; [stride finding](../../findings/2026-07-24-rga3-legacy-blit-10bit-stride-convention-fault.md)). A KASAN+lockdep debug Armbian kernel of the pre-rebase tip was built as `P4052-C40aa`; later repaired boots and current build evidence are summarized in the KUnit/validation rows above and below. |
| Mainline rebase/IRQ history | The shared-IRQ checkpoint was **`6ac18425f66c`** ("media: rockchip: rga-rewrite: model shared IRQ wiring"), after published `edba1c58a726` on official kernel.org **`v7.2-rc5@f5098b6bae76`**; the current heap-fixture tip is recorded above. Mainline already carried the non-overlapping `0x200` RGA3 core resources. The pre-rebase repaired tip is preserved as `ysp-backup/rk3588-rewrite-mainline-before-7.2-rc5-20260726@5bae68d8381c`; the older pre-rc2 backup is also retained. |
...
| Validation | On 2026-07-30, the warning-fatal clean-archive `normal` profile passes for current 6.18 and mainline tips, compiling both IOMMU providers, both KUnit-enabled rewrite objects, and both ROCK 5B DTBs; the test-disabled/memory/race profiles remain the broader handoff matrix. The source audit reports 326 known signals, zero new, and zero absent on both trees; the gate separately requires byte-identical rewrite sources, Kconfig, ABI ledgers, and UAPI and now explicitly builds `vsi-iommu.o`. Parent-source 6.18.40 package `P91d6-Cad24` remains the latest boot: both then-current 85/148 suites completed, but the reset/import fixture disabled lockdep. The current 90/148 manifest, fatal-free interval, live lockdep, source/config/package identity, kmemleak, runtime, AV1 fault/AFBC counters, and media proof remain open on a successor boot. |
