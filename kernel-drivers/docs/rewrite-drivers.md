...
| Source organization | Many focused C/header files: easier subsystem-local navigation, but behavior crosses global managers, backend ops, and compatibility layers. | One large C file per driver plus an ABI ledger. Ownership flow is more local, but the current 18,475-line MPP and 26,269-line RGA translation units include large embedded KUnit regions and remain a review/merge burden. |
...
| Current KUnit repair | KASAN boot `Pc86b-Cad24` (`gdf22eeef8757`, 2026-08-05) passes the exact **92 MPP + 152 RGA** manifest: 244 results, zero failures/skips, clean outer-KTAP interval, and live lockdep. It also passes 12/12 official MPP and reaches 21/34 required librga cases. Source stamp `gf37186832202` later booted as kernel #8 and exposed the residual decoder and RGA3 failures documented in the live findings. The current 94/152 manifest and both successor source fixes remain unbooted; all 246 cases and the GStreamer/FFmpeg conformance workloads require exact-tip replay together with an aged clean kmemleak scan, restored services, and all expected cores. |
| Current source tips | 6.18 `rk3588-rewrite-6.18@c20fc8c1cbf76` on `v6.18.42` and mainline `rk3588-rewrite-mainline@09e39082007dd` on `v7.2-rc6` are the maintained tips. They add a per-session RKVDEC dispatch token held through hardware retirement (including fail-closed RESET_SESSION abort handling) and an RGA `dma_wmb()` between complete command emission and the MMIO start doorbell, on top of the earlier dispatch-order, hard-CCU IOTLB, bus-idle, IOMMU, KUnit, and librga compatibility repairs. The tracked MPP/RGA sources, Kconfig, ABI ledgers, and UAPI are byte-identical between the two maintained branches. VSI retains kernel-version-specific IOMMU attach APIs outside that identical set, with the same admission, retained-fault, callback-drain, and domain-retirement contract. |
...
| Validation | On 2026-08-08, warning-fatal clean-archive `normal` and `test-disabled` profiles passed at 6.18 `c20fc8c1cbf76` and mainline `09e39082007dd`, compiling Rockchip and VSI IOMMU support, the MPP/RGA rewrite objects, and the ROCK 5B DTB with zero warnings. The source audit reports **308 known signals, zero new, and zero absent** on both trees; the exact 94/152 manifest and cross-tree identity gates pass. Earlier `memory` profiles passed at 6.18 `67f323aebdf39` and mainline `7a6d4cb075a67`. The 6.18.43 package with source stamp `gf37186832202` was built/package-verified as `P3b3c-Cad24` and that source later booted as kernel #8, where runtime testing proved the decoder fix incomplete and exposed the RGA3 failures. No package or boot yet carries the successor session-serialization or command-publication commits. Exact-tip KUnit, corrected USERPTR GStreamer, MPP/FFmpeg, RGA3 vpp/overlay, aged kmemleak, final librga, performance, and AV1 fault/AFBC proof remain open. |
