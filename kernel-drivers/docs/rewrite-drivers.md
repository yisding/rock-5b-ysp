...
| Code origin | Rockchip 6.1 BSP, ~87% unchanged (~90% Rockchip-authored), followed by a 75-commit forward-port/hardening series through `12a7da02bea83`. | Independent implementation; the current 6.18 tip is `6b55e022ce49`, including explicit RGA3 shared-IRQ wiring, heap-backed KUnit work and service fixtures, boot-lifecycle-safe runtime isolation during KUnit, initialized locked fixtures, assertion-safe nested-allocation cleanup, and ABI ledgers reconciled with the stable implementation. |
...
| Board evidence | Published 6.18 kernel has broad conformance, bit-exact, sanitizer, root-gate, and production-performance evidence; the latest `0074`/`0075` tail is also KASAN-verified. | Lifecycle-fixed package `P3138-Cad24` completed all 85 MPP + 148 RGA cases, restored both services, and bound every expected MPP/RGA core. That boot exposed one fixture mutex warning that disabled lockdep and one 2,048-byte fixture allocation to kmemleak; both are fixed at the current tips, and pinned package `Pe8c5-Cad24` is ready for the compound boot and media proof. |
...
| Current KUnit repair | **2026-07-27:** lifecycle-fixed `P3138-Cad24` completed the exact 85/85 MPP plus 148/148 RGA plans and restored both services, proving the preceding boot-order repair. The compound gate still failed: `rk_mpp_set_err_ref_hack_kunit` locked a zeroed fixture mutex and disabled lockdep, while `rk_mpp_rkvdec2_vp9_translate_validate_kunit` lost a nested production binding table that kmemleak attributed as one 2,048-byte slab object. Current tips initialize the mutex and transfer the binding table immediately to assertion-safe KUnit deferred cleanup. The next boot must repeat all 233 cases with a fatal-signature-free interval, live lockdep, an aged clean kmemleak scan, restored services, and all expected cores before ABI or media qualification. |
| Current source tips | 6.18 `rk3588-rewrite-6.18@6b55e022ce49` and mainline `rk3588-rewrite-mainline@9aa6ef7e97b2` contain byte-identical rewrite driver and ABI files. They fix the final uninitialized-mutex and nested-allocation fixture defects over lifecycle repairs `db8251eec71a` / `fac707773158`. Earlier `835b19f81d2b` / `79a804a26e00` contain the heap-backed 2,928-byte MPP batch fixture and `3b41eca277c7` / `52d4dfa16825` fix the final capped Debug Objects stack fixture. The audited ledgers match the implemented Kconfig exclusions, RGA errno/validation boundary, 10-bit TILE stride, shared-IRQ and RGA3 capability policy, MPP terminal isolation, and VEPU580 slice-flush fixup. |
...
| Validation | On 2026-07-27, final tips 6.18 `6b55e022ce49` and mainline `9aa6ef7e97b2` passed clean-archive normal and KASAN/fault-injection memory profiles; 6.18 also passed KCSAN/lockdep race, while the redundant mainline race run was intentionally stopped. Focused KUnit-enabled MPP, strict checkpatch, source identity, and the KUnit fixture allocation/session audit passed. The 6.18 repair built against exact Linux 6.18.40 as payload-verified KASAN/UBSAN/lockdep/kmemleak package `Pe8c5-Cad24`; its `System.map` contains the new KUnit cleanup wrapper, repaired cases, and device-initcall registrations, its config enables both 85/148-case suites plus the intended instrumentation without vendor collisions or DWC PCIe PMU, and its DTB preserves ramoops plus disjoint RGA3 windows. Note that an in-tree `mpp_rewrite.o` build proves nothing unless `ROCKCHIP_MPP_REWRITE_KUNIT_TEST=y`; the clean-archive and package gates explicitly require both suites. The repaired package remains unbooted, so warning-clean KASAN/lockdep hardware proof and media conformance are still open. |
