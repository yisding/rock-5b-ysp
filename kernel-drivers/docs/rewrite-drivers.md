...
| Code origin | Rockchip 6.1 BSP, ~87% unchanged (~90% Rockchip-authored), followed by a 75-commit forward-port/hardening series through `12a7da02bea83`. | Independent implementation; the current 6.18 tip is `dbc36621b301`, including explicit RGA3 shared-IRQ wiring, heap-backed KUnit work fixtures, live-service isolation during KUnit, and ABI ledgers reconciled with the stable implementation. |
...
`12a7da02bea83` and rewrite tip `dbc36621b301` from
...
| Current KUnit repair | **2026-07-27:** build `#6` passes the complete 85/85 MPP plus 148/148 RGA KTAP plans and binds RGA2 plus both RGA3 cores, but exposes two Debug Objects reports in `rk_rga_timeout_target_replacement_kunit`. That fixture's stack `rk_rga_hw` embeds an ordinary delayed work, contributing work and timer objects. The previous five-report cap hid this pair, correcting the total to eight affected objects across four fixtures. The identical final fix moves the owner to KUnit-managed heap storage at 6.18 `3b41eca277c7` and mainline `52d4dfa16825`. The next boot must pass all 233 cases with no debug-object report; disabling the optional DWC PCIe PMU for that experiment must also leave lockdep enabled before isolated ABI replay. The rows below preserve the earlier branch/rebase and package history. |
| Current source tips | Published 6.18 `rk3588-rewrite-6.18@dbc36621b301` and mainline `rk3588-rewrite-mainline@948db1b44c63` contain byte-identical rewrite driver and ABI files. Their parents `3b41eca277c7` / `52d4dfa16825` fix the final capped stack fixture; the tips additionally isolate KUnit from live service state. The audited ledgers match the implemented Kconfig exclusions, RGA errno/validation boundary, 10-bit TILE stride, shared-IRQ and RGA3 capability policy, MPP terminal isolation, and VEPU580 slice-flush fixup. Both exact tips pass the warning-free clean-archive normal-profile provider/rewrite/DTB build with KUnit enabled. |
...
| Validation | On 2026-07-27, both exact final-fixture commits and their current KUnit-isolation descendants passed `rewrite-build-gate.sh all` in the clean-archive `normal` profile, warning-free, building the Rockchip IOMMU provider, both KUnit-enabled rewrite objects, and `rockchip/rk3588-rock-5b.dtb`. The focused KUnit-enabled 6.18 RGA object, checkpatch, `git diff --check`, and byte-identity comparison also passed. Earlier on 2026-07-24, the `normal`, `memory`, and `race` profiles completed warning-free at the defect-audit tip, while mainline built clean in `normal`; that historical run used a `git ls-files` copy because its fixes were not committed yet. Note that an in-tree `mpp_rewrite.o` build proves nothing unless `ROCKCHIP_MPP_REWRITE_KUNIT_TEST=y`; the clean-archive gate explicitly requires both suites. Device-free conformance validation passed in baseline, rewrite-counter, and forced-RGA-userptr-IOMMU modes. The current tips remain unbooted, so warning-clean KASAN/lockdep hardware proof and media conformance are still open. |
