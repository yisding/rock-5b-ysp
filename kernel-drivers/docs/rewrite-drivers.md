...
> generated elementary-stream decode/transcode, caps-renegotiation, and
> state-loop pipelines so the
...
> reset on caps changes, and decode->encode transcode before
...
| Size (observed 2026-07-03) | MPP ~15,822 lines + RGA3 ~19,171 lines (vendor-delta.md method) | MPP rewrite ~8,754 lines + RGA rewrite ~17,126 lines |
...
- **Userspace-visible priorities after the GStreamer legacy-convert KUnit slice** are
  ...
  handoff, decoder-side RGA rotate/format-convert, in-pipeline H.264/H.265
  encoder caps-renegotiation through two differently sized raw NV12 segments,
  ...
  media-backed GStreamer expansion should add display, seek/EOS loops, AFBC
  diagnostics, and multi-stream decode/encode.  Source review of the plugin shows the
  ...
  color-convert, and 0/90/180/270-degree rotation.  The latest kernel slice
  adds named and matrix RGA KUnit coverage for RGB-family/NV16/NV61-to-NV12
  encoder preprocessing, NV12/NV21/NV16/NV61 plus compact 10-bit decoder output
  to RGB-family formats, and planar I420/YV12-style RGA2 fallback; the direct
  `librga` smoke mirrors the highest-value JeffyCN legacy-convert shapes through
  public `c_RkRgaBlit()`.  The missing evidence is still the
...
- Optional **KUnit coverage** exists on both rewrite drivers: MPP parser helpers
  ...
  profile and format-matrix helpers, and ffmpeg-facing profile helpers via
