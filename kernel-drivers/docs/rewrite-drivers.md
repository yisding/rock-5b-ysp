...
| Size (observed 2026-07-03) | MPP ~15,822 lines + RGA3 ~19,171 lines (vendor-delta.md method) | MPP rewrite ~8,907 lines + RGA rewrite ~17,197 lines |
...
- **Buffer import**: `RGA_IOC_IMPORT_BUFFER`/`RELEASE_BUFFER` for dma-buf fds
  *and* user virtual addresses — VA imports pin user pages, build sg_tables via
  `sg_alloc_table_from_pages()` (`:15050`), map with the public DMA API, and
...
- **Acquire-fence ownership**: when a submitted task clears
  ...
  KUnit now covers both the current `librga` legacy `RGA_BLIT_ASYNC`
  copy-out path (`rga_req.out_fence_fd`) and the modern request-submit
  `release_fence_fd` path for jobs deferred behind an acquire fence, plus
...
- **Userspace-visible priorities after the GStreamer legacy-convert KUnit slice** are
  ...
  linear DMABuf and AFBC decode output, but the remaining media-backed
  GStreamer evidence is still a booted display-plane run plus forward-port vs
  rewrite timing data.  Source review of the plugin
  shows the
  ...
  RGB-family formats, planar I420/YV12-style RGA2 fallback, and the current
...
- Optional **KUnit coverage** exists on both rewrite drivers: MPP parser helpers
  plus hard-CCU/link-table, DCHS, IOMMU-fault, `POLL_HW_IRQ`,
  `SET_SESSION_FD`, and public reset/file-close cleanup helpers via
  ...
  legacy async blit and modern async request-submit
  ...
  and ffmpeg-facing profile helpers via `ROCKCHIP_RGA_REWRITE_KUNIT_TEST`.
...
|------|--------------------|
| Code | `drivers/video/rockchip/mpp-rewrite/` (`mpp_rewrite.c` 8,907 lines; 9,242 total incl. `ABI.rst`, `Kconfig`, `Makefile`) + `drivers/video/rockchip/rga-rewrite/` (`rga_rewrite.c` 17,197 lines; 17,717 total incl. `ABI.rst`, `Kconfig`, `Makefile`) |
| 6.18 state | committed local branch `rk3588-rewrite-6.18` at **`710642eb58e3`** ("media: rockchip: harden rk3588 forward-port recovery"), committed in dev worktree `/home/yi/Code/linux-6.18-rkvenc`. Since `611f5fe047fd`, the committed work added a large RGA feature-coverage push: userptr import coverage, acquire-fence ownership, per-task core masks, request lifetime fixes, RGA2/RGA3 mixed-task scheduling, RGA priority, RGA2 YUV fill/full-CSC/gray/Y400/RFBC/mosaic/ROP/gauss/quantize/alpha-bitmap/OSD/palette, RGA2 packed-YUV422/420 fill, Y4/Y8 compact/full-CSC dither-output coverage, and BPP palette source coverage, RGA3 resize/translate/rotate/flip/padding/color-key/tile8x8/AFBC/RFBC-facing/10-bit/P210/alpha-overlay/overlay-preprocess profiles, AFBC-to-AFBC ffmpeg filter-copy coverage, `immakeBorder()` reflect/wrap top/bottom and left/right side-edge coverage, RGA2 OSD external-flag/default-background/channel-invert/invert-calibration/external-color register coverage, RGA2 `IM_PRE_INTR` line-interrupt programming/line-only IRQ handling, plus MPP `POLL_HW_IRQ`, DCHS-id, hard-CCU resend/recovery, IOMMU-refresh, session-batch-split, `RELEASE_FD`, nonblocking poll coverage, public `RESET_SESSION`/file-close cleanup including hardware-active reset/import cleanup, CCU coordinator dependent queued/active abort coverage, and DT `rockchip,normal-rates` application through the public clock framework, plus RGA request-config staging and reconfiguration resource/acquire-fence/gauss replacement coverage, request-config ioctl staging/acquire-fd ownership/no-release-fence-export coverage, configured request cancel/file-close cleanup, async close cleanup for jobs pending on acquire fences and jobs already queued on hardware, last-hardware pending-acquire cleanup, RGA3 pattern-channel rotate rejection, the RK3588 `im2d_slt` RGB/RGBA-to-YUV three-channel alpha-blend profile, modern async request-submit acquire/release-fence coverage, legacy `RGA_BLIT_ASYNC` acquire/release-fence coverage for current `librga` single-task async calls, mixed-task RGA3-to-RGA2 core handoff/requeue coverage, 6.18 host `libbpf` const-correctness needed by clean BTF-enabled builds on current GCC, MPP KUnit allocation portability, aggregate/per-core MPP/RGA debugfs hardware timing counters, KUnit coverage for per-core timing-counter routing, warning-free RGA scheduler KUnit stack usage, RGA2 packed-YUV420 fill emission for all four packed orderings, JeffyCN GStreamer legacy RGA conversion profile and format-matrix coverage, RGA ABI notes for legacy direct-buffer temporary imports from dma-buf fds or user virtual addresses, RGA direct-buffer fd/userptr classifier coverage, RKVDEC2 DT `rockchip,ccu-mode` parsing with BSP-style SOFT default/direct-core setup and HARD link tables kept opt-in, Rockchip IOMMU `map_pages`/`unmap_pages` count handling for large dma-buf mappings, and the BSP-derived forward-port recovery cleanup: public Rockchip IOMMU hooks, real fault masking/pagefault-done/reset callbacks, 32-bit `MPP_IOC_CFG_V1` compat parsing, reset-path IOMMU-refresh error propagation, and minimal vendor DT bindings. The previous 6.18-only DT commit makes the Rock 5B DT self-contained by adding disabled RK3588 `vdec0`/`vdec1` V4L2-shaped decoder nodes, IOMMUs, and SRAM pools to the base DTS so the board include can retype them to RKMPP without external Armbian media labels. |
| Mainline-master state | committed local branch `rk3588-rewrite-mainline` at **`85ef988e6516`** ("iommu/rockchip: honor multi-page map counts"), committed in sibling worktree `/home/yi/Code/linux`; it carries the rewrite drivers plus the mainline-master DT/wiring work, the V4L2 RGA3 comparison tree discussed in §1, the same portable MPP KUnit allocation cleanup, the same aggregate/per-core debugfs hardware timing counters, the same KUnit timing-counter routing coverage, the same RGA KUnit stack cleanup, the same RGA2 packed-YUV420 fill support, the same GStreamer legacy RGA conversion profile and format-matrix coverage, the same RGA ABI direct-import clarification and direct-buffer fd/userptr classifier coverage, the same RKVDEC2 `rockchip,ccu-mode` SOFT-default/HARD-opt-in handling, the same MPP hardware-active reset/import cleanup coverage, and the Rockchip IOMMU `map_pages`/`unmap_pages` count fix shared with the 6.18 branch. |
| Validation | Focused compile gates pass at the committed tips (2026-07-03): both kernel trees passed `git diff --check`; [`tests/rewrite-build-gate.sh`](../tests/rewrite-build-gate.sh) reproduced the clean-source, fresh-output, KUnit-enabled rewrite object build for `../linux-6.18-rkvenc` at `710642eb58e3` and for `../linux` at `85ef988e6516`; it built `drivers/video/rockchip/mpp-rewrite/mpp_rewrite.o` and `drivers/video/rockchip/rga-rewrite/rga_rewrite.o` warning-free in both runs. The clean 6.18 gate also rebuilt `tools/bpf/resolve_btfids` with `CONFIG_DEBUG_INFO_BTF=y`, which is the host-tool coverage from the 6.18-only libbpf const backport. This slice also had focused object builds outside the archived rewrite gate: mainline `drivers/iommu/rockchip-iommu.o` built cleanly from a fresh `defconfig` output tree, and a clean temporary 6.18 source with the live forward-port diff built `drivers/iommu/rockchip-iommu.o`, `drivers/video/rockchip/mpp/mpp_service.o`, `mpp_common.o`, `mpp_iommu.o`, `mpp_rkvdec2.o`, `mpp_rkvdec2_link.o`, `mpp_rkvenc2.o`, `drivers/video/rockchip/rga3/rga2_reg_info.o`, and `rga3_reg_info.o` warning-free. Full dt-schema validation was not available in the local environment. A previous clean archive build of `rockchip/rk3588-rock-5b.dtb` passed at 6.18 tip `c24a9feebd7c`, and the same target passed on mainline tip `f05d39966e5d`; those DTB gates have not been rerun for the latest `710642eb58e3`/`85ef988e6516` tips. Both rewrite drivers carry optional KUnit coverage for pure ABI/helper logic, and the source-level ABI ledgers record the implemented/unsupported boundary per §2/§3; recent code slices add named and matrix GStreamer legacy `c_RkRgaBlit()` coverage for RGB-family/NV16/NV61-to-NV12 encoder preprocessing, NV12/NV21/NV16/NV61/compact-10-bit decoder output to RGB-family conversion, planar I420/YV12-style RGA2 fallback, direct-buffer fd/userptr classification for current `librga`/GStreamer legacy submissions, RKVDEC2 `rockchip,ccu-mode` normalization and SOFT-CCU register-programming coverage, hardware-active MPP reset/import cleanup coverage, and Rockchip IOMMU map/unmap count handling for large dma-buf mappings. This is still code/ABI-ledger progress rather than proof from a booted rewrite kernel. [`tests/abi-probe.sh`](../tests/abi-probe.sh) is the fast non-submit ABI log-diff gate for whichever implementation owns `/dev/mpp_service` and `/dev/rga`; [`tests/mpp-suite.sh`](../tests/mpp-suite.sh) is the official MPP test-suite wrapper for `mpp_info_test`, codec decode/encode, multi-thread, multi-instance, and rate-control cases; [`tests/mpp-suite-compare.sh`](../tests/mpp-suite-compare.sh) flags required MPP regressions against the forward-port summary; [`tests/librga-smoke.sh`](../tests/librga-smoke.sh) is the tiny direct librga/im2d smoke for virtual-address import, dma-heap dma-buf allocation plus `importbuffer_fd`, legacy `c_RkRgaBlit()` `BGRx` malloc to NV12 dma-buf, rotated NV12 dma-buf to BGRx dma-buf, and planar I420 dma-buf to NV12 dma-buf conversions, official Gaussian matrix `improcess(..., IM_SYNC | IM_GAUSS)`, sync copy/resize/fill, forced-core/priority copy, forced RGA2 pre-intr, and async fence chaining; [`tests/librga-suite.sh`](../tests/librga-suite.sh) is the official librga sample-suite wrapper with required Linux/RK3588 cases and diagnostic outside-slice cases; [`tests/librga-suite-compare.sh`](../tests/librga-suite-compare.sh) flags required sample regressions against the forward-port summary; [`tests/gstreamer-suite.sh`](../tests/gstreamer-suite.sh) is the JeffyCN GStreamer plugin wrapper for element inspection, asset-free encoder/RGA-conversion/restart-loop pipelines, generated-media decode/transcode, opt-in external-media decode/transcode/AFBC cases, the diagnostic GStreamer RGA format matrix, and opt-in Rockchip display/DMABuf sink cases; [`tests/gstreamer-suite-compare.sh`](../tests/gstreamer-suite-compare.sh) flags required GStreamer regressions against the forward-port summary; [`tests/rewrite-smoke.sh`](../tests/rewrite-smoke.sh) runs the ABI probe plus decode/encode/transcode consumer workloads and can include the GStreamer suite with `RUN_GSTREAMER=1`. The broader conformance plan is staged outside this repo at `../rockchip-conformance`: JeffyCN GStreamer Rockchip, official MPP tests, official librga samples, the Linux MPP/RGA/DRM demo, and RKMediaCodecDemo, with per-profile logs for rewrite vs forward-port runs. A 2026-07-03 source review of JeffyCN's GStreamer implementation identified the next userspace-visible priorities in §3; the support-runner gap is now closed, the kernel now has focused GStreamer RGA profile/format-matrix coverage and direct-buffer classification for the matching public legacy calls plus the official Gaussian matrix public API shape, but booted GStreamer state/allocator/RGA-conversion/display results are still missing before lower-priority diagnostic BSP profiles. **UNVERIFIED in this repo**: the workload gate and expanded conformance bundle have not yet passed on hardware through the rewrite; no validation record equivalent to status.md exists yet. |
