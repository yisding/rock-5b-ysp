...
| `MPP_CMD_POLL_HW_FINISH` / `MPP_CMD_POLL_HW_IRQ` | nonblocking `POLL_HW_FINISH` on a pending non-batch job returns `-EAGAIN` without consuming the session-visible active job; for `SET_SESSION_FD` batch polls the ioctl returns success and writes per-slot `mpp_bat_msg.ret` values (`EAGAIN` positive for pending, `0` for done, negative task errors such as `-EIO` for completed failures) so current libmpp's batch server keeps its poll timer alive and can classify each task; RKVENC2 slice result streaming advertises `POLL_BUTT`, detects split mode from the submitted register image (`slen_fifo` + slice split), stores IRQ slice-length words in a job-owned FIFO, copies up to userspace `count_max`, and falls through to normal completion/readback on the final slice; non-split IRQ polls use the full-frame finish path | |
...
- Optional **KUnit coverage** exists on both rewrite drivers: MPP parser helpers
  ...
  `SET_SESSION_FD`, batch poll status writeback, and public reset/file-close cleanup helpers via
