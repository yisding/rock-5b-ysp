...
| Board evidence | Published 6.18 kernel has broad conformance, bit-exact, sanitizer, root-gate, and production-performance evidence. | Boot `#29` (`g8042f13c5459`, 2026-08-02 14:46) completed the then-current exact 89/89 MPP plus 150/150 RGA KTAP with zero fatal signatures across the boot, live lockdep, kmemleak scanning, and every expected MPP/RGA core bound. It predates the August review fixes and current 92+152 manifest. Package `#30` is installed but unbooted and also predates the two current tip commits; no rewrite media or AV1 hardware result exists. |
...
| Current KUnit repair | Boot `#29` (`g8042f13c5459`, 2026-08-02) is the latest successful hardware run: exact 89/89 MPP plus 150/150 RGA, zero fatal signatures across the reference boot, live lockdep, kmemleak scanning, and all expected rewrite services registered. Package `#30` (`g2f05724a2003`) was installed but never booted and predates both the 2026-08-02 adversarial-review tail and the current tips. The source manifest is now **92 MPP + 152 RGA**. Its request-configuration and pattern-rotation oracle repair is build-verified only; all 244 cases still require a current-tip boot with a clean log interval, live lockdep, an aged clean kmemleak scan, restored services, and all expected cores before ABI or media qualification. |
| Current source tips | 6.18 `rk3588-rewrite-6.18@33c30ec6989e` and mainline `rk3588-rewrite-mainline@9e503f6b16df` are the maintained tips. They include the 2026-08-02 [adversarial-review repairs](rewrite-driver-adversarial-review-2026-08-02.md), the 2026-08-03 removal of an [unreachable mainline-only IOMMU IRQ-mask fallback](../../findings/2026-08-03-rewrite-rga-unreachable-iommu-irq-mask.md), and the 2026-08-04 KUnit contract repair: non-terminal request configuration no longer passes NULL to terminal cleanup, RGA3 pattern-blend rotation remains a deliberate rejection, and the positive rotation oracle uses the supported non-pattern path. The tracked MPP/RGA sources, Kconfig, ABI ledgers, and UAPI are byte-identical between the two maintained branches. VSI retains kernel-version-specific IOMMU attach APIs outside that identical set, with the same admission, retained-fault, callback-drain, and domain-retirement contract. |
...
| Validation | On 2026-08-04, the warning-fatal clean-archive `normal` profile passed for both maintained tips, compiling Rockchip and VSI IOMMU support, both KUnit-enabled rewrite objects, and the ROCK 5B DTB. The source audit reports **305 known signals, zero new, and zero absent** on both trees, and the identity gate passes for the tracked rewrite sources, Kconfig, ABI ledgers, and UAPI. The test-disabled, memory, and race profiles were not rerun at these tips and retain only their older evidence. Current-tip package identity, booted 92/152 KUnit, fatal-free interval, live lockdep, aged kmemleak, codec/RGA media, performance, and AV1 fault/AFBC proof remain open. |
