...
| Audit posture | 89 verified findings latent ([BSP audit](./bsp-audit.md)) | ownership-explicit and refcount-disciplined, with 85 MPP + 147 RGA KUnit cases on the shared rewrite and 89 MPP cases on the AV1 branch |
| Size snapshot | MPP ~15,822 lines; RGA3 19,173 code/build lines at the current forward oracle (`15,796` C + `3,305` headers + `72` Kconfig/Makefile) | shared tip: `mpp_rewrite.c` 14,061 C lines and `rga_rewrite.c` 23,865 C lines; dedicated AV1 tip: `mpp_rewrite.c` 15,445 lines |
...
| AV1 via RKMPP | implemented on the dedicated 6.18 AV1 branch; hardware-unvalidated | `rk3588-rewrite-av1-6.18@402fc9c0bd785` adds the separate AV1 hardware/IOMMU/backend plumbing without importing the BSP driver body. The forward-port bit-exact result remains the runtime oracle until the rewrite passes the generated AV1 diagnostics and recovery gates. |
...
| Item | State |
|------|--------------------|
| Code | Shared rewrite: `drivers/video/rockchip/mpp-rewrite/` (`mpp_rewrite.c` 14,061 lines; +`ABI.rst`, `Kconfig`, `Makefile`) + `drivers/video/rockchip/rga-rewrite/` (`rga_rewrite.c` 23,865 lines; +`ABI.rst`, `Kconfig`, `Makefile`), with 85 MPP and 147 RGA KUnit cases (232 total). Dedicated AV1 tip: `mpp_rewrite.c` 15,445 lines and 89 MPP cases (236 total with RGA). |
| AV1 6.18 state | **2026-07-26:** isolated worktree `/home/yi/Code/kernel/linux-6.18-rk-av1-rewrite`, branch `rk3588-rewrite-av1-6.18@402fc9c0bd785`, based on the complete shared rewrite tip `c5faabf9d00b0`. Commits `3327030e09029`, `a5624fa024766`, and `402fc9c0bd785` add the class-aware/dynamic shared architecture, RKMPP AV1 backend, VSI-IOMMU lifecycle, checked AFBC path, auxiliary IRQ descriptors, codec-neutral counters, KUnit coverage, and ABI documentation. Focused arm64 `W=1` builds and clean-archive `normal`, KASAN/fault-injection `memory`, and KCSAN/lockdep `race` profiles pass warning-free, including both IOMMU providers and the Rock 5B DTB. No booted KUnit or board evidence exists, and mainline has not been carried. |
