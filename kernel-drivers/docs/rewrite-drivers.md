...
| Size (observed 2026-07-03) | MPP ~15,822 lines + RGA3 ~19,171 lines (vendor-delta.md method) | MPP rewrite ~8,282 lines + RGA rewrite ~15,343 lines |
...
- **Version tuples are capability keys.** `librga` capability-probing expects
  ...
  (`rga_rewrite.c:14568-14582` — `{3, 2, 0x63318}` / `{3, 0, 0x76831}`,
...
- Optional **KUnit coverage** exists on both rewrite drivers: MPP parser helpers
  ...
  request staging, request reconfiguration resource replacement, mixed-task
