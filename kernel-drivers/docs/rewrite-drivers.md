...
path. The rewrite is the narrower, more explicit maintenance path whose
remaining uncertainty is concentrated at the hardware boundary.
The separate
[architecture comparison](./driver-architecture-comparison.md) expands this
summary into MPP/RGA ownership, scheduling, DMA/IOMMU, completion, recovery,
and teardown charts.

| Dimension | Forward-port (`mpp/`, `rga3/`) | Rewrite (`mpp-rewrite/`, `rga-rewrite/`) |
|-----------|--------------------------------|------------------------------------------|
| Primary objective | Preserve Rockchip behavior while moving the BSP stack to Linux 6.18. | Preserve the current ROCK 5B userspace contract while replacing BSP internals with public kernel APIs and explicit ownership. Current tips are 6.18 `7481df21ca2b1481a3c4b4d222e3ebed28692544` on `v6.18.42` and mainline `4a632e00c4cd729cb7aa473686bed3ccd2bb271c` on `v7.2-rc6`. |
| Code origin | Rockchip 6.1 BSP, ~87% unchanged (~90% Rockchip-authored), followed by the maintained forward-port/hardening series. | Independent implementation; current tips are 6.18 `7481df21ca2b1481a3c4b4d222e3ebed28692544` on `v6.18.42` and mainline `4a632e00c4cd729cb7aa473686bed3ccd2bb271c` on `v7.2-rc6`, including the AV1 backend, August adversarial repairs, librga DMA/fence/bounce compatibility, selected-core USERPTR mapping under power, per-session RKVDEC dispatch serialization, RGA command-buffer publication, Phase 1 ownership funnels, Phase 2 reset/cluster/recovery and IRQ-register leases, Phase 3A–G activation identity and retry closure, the Phase 3H ref-owning recovered-terminal claim and service quarantine tombstone, Phase 3I exact clean-terminal observations, Phase 3J typed external activation references and KUnit-only stack-shape correction, BSP-shaped MPP device discovery, explicit RGA3 shared-IRQ wiring, exact KUnit manifest, compile-time ABI layout contract, and reconciled ABI ledgers. |
...
| State and lifetime model | Modular BSP subsystem, but heavily global: service/request/memory managers route work into per-core scheduler state. The hardening tail repairs ownership races without replacing that model. | Session-owned IDs/imports, copied submitted jobs, retained hardware/import references, exact active-slot claims, generation-tagged recovery, and quarantine when ownership cannot return safely to the active slot. An IRQ-safe register lease binds accepted status to the live reset epoch and direct-core generation. Phase 3A–G establish activation identity, fresh retry storage, and retry-predecessor closure. Phase 3H makes the claim token own the active-slot job reference, retires recovered terminals only on typed core or group/core proof, and transfers exact restore refusal to a service tombstone that retains resources/dispatch and closes admission through reboot. Phase 3I retires exact clean claims only after immutable `NOT_PUBLISHED`, `IRQ_ACCEPTED`, or `CCU_DONE_ACCEPTED` observation; RKVDEC bus-idle status remains advisory and recovered proof remains separate. Phase 3J gives each activation a base bias and makes active, timeout, claim, retry, and quarantine identities typed `{activation, generation}` references paired with containing-job references. Dispatch/current/list remain borrowed and backend resources remain job-shaped. AV1 untrusted-stop failure retains `SLOTTED` active ownership for remove/shutdown retry. `RECLAIMABLE`, resource drain, final outcome arbitration, and RGA per-task execution remain planned rather than implemented. |
...
| Kernel-version coupling | Production work is pinned to 6.18 and carries `compat/` glue; resync cost is paid when BSP assumptions meet newer kernel APIs. | The tracked rewrite driver/Kconfig/ABI/uAPI files are byte-identical on 6.18 and the current mainline branch; surrounding kernel, DT, and IOMMU integration differs. Final-source Phase 3J KUnit-enabled and test-disabled 6.18 objects compile, and all eight warning-fatal profiles pass across both kernel lines; both still need independent boot proof. |
| Source organization | Many focused C/header files: easier subsystem-local navigation, but behavior crosses global managers, backend ops, and compatibility layers. | One large C file per driver plus an ABI ledger. Ownership flow is more local, but the current 22,176-line MPP and 26,466-line RGA translation units include large embedded KUnit regions and remain a review/merge burden. |
| In-tree testability | No comparable driver KUnit suite; confidence comes primarily from BSP history, repository reproducers, and extensive board runs. | 105 MPP + 152 RGA KUnit cases (257 total), fail-closed validators, debugfs counters, an exact 2,251/tree ownership audit, a 306/tree fixture-debt audit, and eight passing final-head warning-fatal build profiles. The tests have already exposed fixture/lifetime defects, so their presence is useful evidence rather than a correctness guarantee. |
| Board evidence | Published 6.18 kernel has broad conformance, bit-exact, sanitizer, root-gate, and production-performance evidence. | KASAN boot `#2` (`g19634f4eebba`, 2026-08-05) completed exact 92/92 MPP plus 152/152 RGA KTAP with a clean outer interval and live lockdep. Kernel #8 at `gf37186832202` later exposed residual same-session decode corruption, solo RGA3 vpp corruption, and an overlay-chain RGA3 IOMMU fault; those observations motivated the current source repairs. Phase 3J tips `7481df21ca2b1481a3c4b4d222e3ebed28692544` / `4a632e00c4cd729cb7aa473686bed3ccd2bb271c` have source and compile evidence but remain unbooted, and the RGA2 1 MiB SWIOTLB staging limitation remains open. No current-tip RGA media or AV1 hardware result exists. |
...
is **breadth and empirical maturity versus containment and maintenance
clarity**. The forward port wins today on supported hardware, compatibility,
measured correctness, and performance. The rewrite wins at the source level on
public-API portability, fail-closed scope, explicit ownership, executable
invariants, and auditable ABI boundaries.

Neither advantage erases the other track's risk. Forward-port bugs found during
KASAN and hostile close/reset testing clustered around global ownership and
unserialized retirement, exactly where the rewrite changes the model. But the
rewrite's recent fixture poisoning, recovery defects, DT-resource overlap, and
shared-IRQ miss show that a clearer design does not substitute for board
validation. Conversely, the forward port's lack of KUnit does not make it
unreliable: its much stronger runtime record is the evidence that matters for
shipping.

The two tracks have also converged semantically. The rewrite has ported or
dissolved forward-port fixes for 10-bit layout, import spans, fence/close
races, slice polling, and recovery, while retaining a different ownership
architecture. That makes the forward port a particularly strong differential
oracle rather than disposable scaffolding. Keep it as the default until the
rewrite passes the same media artifacts, hostile recovery gates, production
performance ceiling, and soak; then choose the rewrite for its lower long-term
kernel-integration cost, not because source cleanliness alone is presumed to
prove safety.
...
(`mpp-rewrite/Kconfig`, `rga-rewrite/Kconfig`). The rewrite binds the relevant
forward-port DT nodes: `rockchip,rkv-encoder-v2-core`,
`rockchip,rkv-decoder-v2`, both CCU compatibles, the separate
`rockchip,av1-decoder`, and the RGA2/RGA3 compatibles including mainline
`rockchip,rk3588-rga`.

Important codec boundary: RK3588 AV1 lives on a separate Verisilicon/VPU981
hardware block with its own VSI IOMMU and AFBC auxiliary registers/IRQ. Both
maintained rewrite branches now expose that block through the same
`/dev/mpp_service` ABI as the forward port. The backend began on the historical
`rk3588-rewrite-av1-6.18` spur and was merged into the maintained lines on
2026-07-29; it is source-inspected and compile-verified, but it has never
probed or decoded on a booted rewrite kernel. The forward port remains the only
path with bit-exact AV1 hardware evidence. See the
[AV1 rewrite backend design finding](../../findings/2026-07-29-av1-rewrite-backend-design-source-audit.md)
and its
[hardening-gap companion](../../findings/2026-07-29-av1-rewrite-branch-hardening-gap-and-backport.md).
The separate
...
original design, what landed, and the still-open hardware gates.
...
rewrite's hardware nodes. The current rewrite binds RKVDEC2/RKVENC2 plus their
CCUs and the separate AV1 block; it does not bind the legacy VDPU/VPU or JPEG
nodes that libmpp uses for older formats.
...
| AV1 via RKMPP | implemented in source, required for full current-tip qualification, hardware-unvalidated | The separate VPU981/VSI/AFBC backend is present on both rewrite branches. GStreamer/FFmpeg AV1 cases remain opt-in in the harness during bring-up, but a release-grade rewrite result must enable and require them. |
...
|---------|---------------------:|-----------------------:|
| Non-test C | 17,690 | 13,285 |
| Headers | 3,398 | 0 |
| Embedded KUnit | 0 | 10,710 |
| Kconfig + Makefile | 72 | 28 |
| **Driver code/build files** | **21,160** | **24,023** |
| ABI documentation | — | 665 |
| **Everything in the driver directory** | **21,160** | **24,688** |

Therefore the checked-in rewrite is 2,863 code/build lines (13.5%) larger when
KUnit is counted, or 3,528 lines larger when its `ABI.rst` is counted too. With
`ROCKCHIP_RGA_REWRITE_KUNIT_TEST` disabled, the rewrite has 13,313 code/build
lines — 7,847 lines (37.1%) fewer than the forward port. This is source
accounting, not an object-size or complexity measurement.

The main KUnit body occupies `rga_rewrite.c:8083-18780` (10,698 lines). Its
conditional include and isolated-service hooks bring the test-only source to
10,710 lines,
44.6% of the C file. At the current pin the file is physically laid out at a
high level as follows:

| `rga_rewrite.c` range | Lines | Main responsibility |
|-----------------------|------:|---------------------|
| 1-8082 | 8,082 | Registers, driver objects, sessions, DMA/userptr mapping and shadows, fences, recovery helpers, and first-stage validation (including KUnit include/service-isolation hooks) |
| 8083-18780 | 10,698 | KUnit implementation and 148 registered cases |
| 18781-23995 | 5,215 | RGA2/RGA3 profile validation and emission, scheduling/IRQ/recovery, ioctls, probe/remove, and session teardown |

MPP has the opposite raw-size result:

| Content | Forward-port `mpp/` | Rewrite `mpp-rewrite/` |
|---------|---------------------:|-----------------------:|
| Non-test C | 16,216 | 9,452 |
| Headers | 2,139 | 0 |
| Embedded KUnit | 0 | 4,666 |
| Kconfig + Makefile | 87 | 29 |
| **Driver code/build files** | **18,442** | **14,147** |
| ABI documentation | — | 673 |

The rewrite MPP directory is 4,295 code/build lines (23.3%) smaller even with
KUnit, and its non-KUnit code/build slice is 48.6% smaller. That broad
forward-port total also includes the 1,171-line AV1 backend and 1,742 lines of
material the rewrite intentionally omits: 1,445 lines of legacy-SoC hacks and
297 lines of compatibility headers. Against the
15,529-line forward-port RK3588 core with both categories excluded, the rewrite
is only 8.9% smaller with tests, or 39.0% smaller without them. The useful
conclusion is architectural, not a code-golf score: MPP becomes substantially
smaller when legacy hardware and vendor infrastructure are excluded, while
RGA's semantic format/operation matrix remains large regardless of ownership
model.
...
| `1fe46df86f1ca` (2026-07-23 recovery-hardening tip) | 23,479 | — | — | 2026-07-23 `harden rewrite driver recovery`: ~9k insert/~4.9k delete restructuring import/extent bookkeeping and recovery; RGA KUnit 122 → 147. (test/non-test split not recomputed at this tip) |
...
### Upstream-style V4L2 RGA3 in `../rock-5b/kernel/linux`

The separate upstream-oriented RGA support compared on 2026-07-02 was the sibling
`../rock-5b/kernel/linux` checkout on branch `rk3588-rewrite-mainline` at commit
...
| Register jobs | flat register-image materialization (`SET_REG_WRITE`), bounded readback retention (`SET_REG_READ`), and validated offset tuples (`SET_REG_ADDR_OFFSET`). Translated registers retain dma-buf provenance, so embedded and separate offsets are accumulated with checked arithmetic and must stay inside the allocation and 32-bit IOVA aperture, except for the end-exclusive limit pointer exactly one byte past the mapping that libmpp has programmed on the VEPU580 bitstream-top register since 2021 (rejecting it breaks encode on every release up to 1.0.11); literal non-fd registers keep BSP-style additive offsets but reject cumulative 32-bit wrap. A rejection names the offending register in the `translate-fail` event, a ratelimited `reject reg` log line, and the `rejected_job_count` counter. KUnit covers staging, duplicate-index additive apply, literal wrap rejection, cumulative dma-buf/32-bit bounds, malformed sizes, zero-size no-op, and cap handling. fd→IOVA translation always applies the built-in per-client tables (`rk_mpp_rkvdec_h264d_regs[]` etc., `:325-380`) plus any session entries, with translated dma-bufs mapped against the selected core's DMA device; VP9 RKVDEC coverage asserts the VP9 table translates only VP9-owned fd registers and rejects unknown RKVDEC formats. | |
...
| Fault/recovery | per-core timeout completion, public IOMMU fault callbacks, post-reset IOMMU refresh, and `-EIO` completion on matched active faults are implemented without private Rockchip IOMMU page-table walking. Provider callbacks are registered and cleared per physical IOMMU rather than per shared domain, and the reported controller/master must first match its exact physical core. HARD-CCU may execute a descriptor on a peer whose software `active_activation` slot is empty, so recovery reads that source core's BSP link `CFG_ADDR`, matches the descriptor IOVA to its software owner, and schedules the existing coordinator-wide fail-closed recovery there; software ownership is published and ordered before the `CFG_DONE` start doorbell so an immediate descriptor fault cannot miss it. An unreadable or unmatched descriptor falls back to any active job in the same HARD coordinator rather than silently scheduling the empty peer. After the coordinator is force-stopped, a peer whose run lock is contended gets an immediate deferred abort holding the exact activation's containing-job and hardware references. The target is pinned before the lock attempt, queued only while it still owns the slot, and rechecked after acquiring the lock; its timeout is canceled only after that exact claim, so containment neither waits for the ordinary 500 ms timeout, resets a replacement job, nor removes the replacement's watchdog. HARD-CCU resend requires successful coordinator and dependent-core resets; both the reset-preparation and restart passes pin each collected job's hardware under the session lock before touching IRQ/run state, so concurrent completion/removal cannot detach and free that core behind a retained job reference. Any reset failure aborts the affected queue, sets a visible `recovery_failed` quarantine, increments `recovery_failure_count`, and prevents reuse until reprobe. A coordinator failure applies that policy to every dependent core. A core with an attached IOMMU domain fails probe if the provider hook is unavailable instead of running without the intended recovery path; no-IOMMU operation remains allowed. Phase 3H recovered terminal paths carry the active slot's exact containing-job reference in the activation claim token and retire only with typed direct-core or hard-CCU group/core proof. If exact restore refuses, that reference and generation transfer to a service quarantine tombstone; diagnostic quarantine failure remains distinct from the retained core/group recovery statuses, resources and dispatch remain owned, selected-core/owned-CCU admission closes, remove cannot release provider/devres state, and shutdown retains the tombstone through reboot. Clean IRQ/`CCU_DONE`, pre-doorbell `START_FAILURE`, `RECLAIMABLE`, and final outcome arbitration remain outside this checkpoint. | |
...
- **Probe prerequisites**: every executable RGA2/RGA3 match requires an IRQ and
  ...
  `0x44` bytes for RGA3's command-state register. The BSP/forward-port
  `0x1000` RGA3 resource overlaps the separately described IOMMU's final
  `0x100` and works only because the vendor RGA maps without reserving; the
  resource-reserving rewrite requires the upstream/maxline `0x200` core
  window. The exact source comparison and reservation semantics are in the
  [device-tree guide](./device-tree.md#rga3-coreiommu-resource-ownership).
  Missing or truncated MMIO fails probe. With runtime PM and clocks enabled,
  probe also validates the actual version register before registering the
  IOMMU fault handler or public core; unsupported revisions fail with
...
- Optional **KUnit coverage** exists on both rewrite drivers: MPP parser helpers
  ...
  The first boot of that complete set established an additional fixture
  contract: a test that passes a fake hardware object into scheduling,
  abort/reset, workqueue, or refcount code must initialize the same topology
  and lifetime fields that probe initializes, or explicitly mark a reset-less
  fake terminally stopped. Stack work items use the `_ONSTACK` initializers and
  matching destroy helpers. Error-return helpers initialize observable output
  parameters before validation. The 2026-07-26
  [KUnit root-cause record](../../findings/2026-07-26-rewrite-kunit-failure-root-causes.md)
  separates those fixture rules from the driver fixes it exposed: fd 0 is a
  valid acquire-fence descriptor, acquire-fd collectors carry an explicit
  capacity, 4:2:2 semiplanar geometry does not inherit 4:2:0 vertical-evenness,
  special RGA2 features are not generic ROP, Y4/Y8 dither flags are legal in
  their documented subset, and rotated command dimensions do not replace the
  userspace destination-memory canvas during validation.
...
| Item | State |
|------|--------------------|
| Code | `drivers/video/rockchip/mpp-rewrite/` (`mpp_rewrite.c`; +`ABI.rst`, `Kconfig`, `Makefile`) + `drivers/video/rockchip/rga-rewrite/` (`rga_rewrite.c`; +`ABI.rst`, `Kconfig`, `Makefile`). The trees contain 105 MPP and 152 RGA KUnit cases (257 total). |
| Current KUnit repair | KASAN boot `Pc86b-Cad24` (`gdf22eeef8757`, 2026-08-05) passes the exact **92 MPP + 152 RGA** manifest: 244 results, zero failures/skips, clean outer-KTAP interval, and live lockdep. It also passes 12/12 official MPP and reaches 21/34 required librga cases. Source stamp `gf37186832202` later booted as kernel #8 and exposed the residual decoder and RGA3 failures documented in the live findings. The current 105/152 manifest and successor ownership changes remain unbooted; all 257 cases and the GStreamer/FFmpeg conformance workloads require exact-tip replay together with an aged clean kmemleak scan, restored services, and all expected cores. |
| Current source tips | 6.18 `rk3588-rewrite-6.18@7481df21ca2b1481a3c4b4d222e3ebed28692544` on `v6.18.42` and mainline `rk3588-rewrite-mainline@4a632e00c4cd729cb7aa473686bed3ccd2bb271c` on `v7.2-rc6` are the maintained tips. Phase 3A–G establish retained activation identity, exact slot/dispatch ownership, fresh hard-CCU retry successors, and typed retry-predecessor closure. Phase 3H makes the active claim token own the exact detached job reference, retires recovered terminals on typed direct-core or group/core proof, and transfers exact restore refusal into a ref-owning service quarantine tombstone. Phase 3I records immutable `NOT_PUBLISHED`, `IRQ_ACCEPTED`, or `CCU_DONE_ACCEPTED` observation/status before retiring each exact clean claim; an impossible finisher quarantines. Phase 3J adds a base reference bias and typed `{activation, generation}` external references paired with containing-job references for active, timeout, claim, retry, and quarantine ownership, with explicit clone/move/put semantics; the amended heads also carry the KUnit-only stack-shape correction. Dispatch/current/list remain borrowed and backend resources remain job-shaped. RKVDEC `BUS_IDLE` is advisory evidence, recovered proof remains separate, and AV1 untrusted-stop failure deliberately retains `SLOTTED` active ownership for remove/shutdown retry. `RECLAIMABLE`, activation-aware resource drain, CCU/link/DCHS and power migration, and final result arbitration remain open. The tracked MPP/RGA sources, Kconfig, ABI ledgers, and UAPI are byte-identical between the two maintained branches. VSI retains kernel-version-specific IOMMU attach APIs outside that identical set, with the same admission, retained-fault, callback-drain, and domain-retirement contract. |
...
| Validation | On 2026-08-09, Phase 3J pins byte-identical MPP SHA-256 `815ccaf6daa20a88592f1b9ba4860a29aeb35e2a26cae7554ade714420e29ee4` and strict 0/0/0 over 1,976 lines/tree. The final-source 6.18 KUnit-enabled object is 6,559,584 bytes at SHA-256 `8bd96a7ccb3fa60cfa23203055a0404b3f26d4dbd3e93a0b28224748cb7e337d`; the test-disabled object is 2,287,288 bytes at SHA-256 `4c7d90ba06743683c5b291e9b8d0dbfc910673cbf4c2a371f251de9c0ce98286`. Ownership passes at 2,251/tree, fixture debt at 306/tree, and the exact manifest is 105/152. All eight warning-fatal final-head profiles pass across both kernel lines with both IOMMU providers, both rewrite objects, and the Rock 5B DTB. The dedicated test-disabled policy/ABI gate passes on both heads: both KUnit options resolve disabled, the same providers/objects/DTB compile warning-fatally, and the deliberate MPP ABI mutation fails at compile time. The repository handoff gate passes; runtime execution remains pending. The predecessor 6.18 Phase 1 tip `ab69ece998642` was packaged as inspected P692f with stamp `(gab69ece99864)` but remains uninstalled and unbooted. No package or boot carries Phase 2 or Phase 3; exact-tip KUnit, corrected USERPTR GStreamer, MPP/FFmpeg, RGA3 vpp/overlay, aged kmemleak, final librga, performance, and AV1 fault/AFBC proof remain open. |
