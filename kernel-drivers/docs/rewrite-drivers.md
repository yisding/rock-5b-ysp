...
| State and lifetime model | Modular BSP subsystem, but heavily global: service/request/memory managers route work into per-core scheduler state. The hardening tail repairs ownership races without replacing that model. | Session-owned IDs/imports and copied jobs retain exact hardware/import references. One `rk_mpp_activation` owns each admitted attempt's generation/deadline, typed active/timeout/claim identities, closure/quarantine proof, CCU/link/DCHS/power/timing leases, terminal reason arbitration, exact drain, and reclaim boundary. Hard-CCU retry publishes a fresh successor and coherently transfers resources; one successful terminal owner releases resources, selected hardware, and dispatch before `DONE`. AV1 untrusted-stop and impossible recovery retain fail-closed quarantine ownership. RGA per-task execution remains planned rather than implemented. |
...
Phase 3 completion supersedes that last boundary. `rk_mpp_activation_resources`
now owns the CCU/link/DCHS/power/timing leases for one attempt; hard-CCU retry
moves the whole record under the coordinator/core locks. Terminal reasons and
results are accumulated and resolved with a stable priority, then one completion
tail drains resources, releases the exact selected core and session dispatch,
and alone publishes `DONE`. A retired predecessor with drained or handed-off
resources and no external owners becomes `RECLAIMABLE`; its list/base bias is
released and dynamically allocated storage is freed. Quarantine retains its
exact resources and continues to block unsafe teardown/reuse.
...
| Item | State |
|------|--------------------|
| Code | `drivers/video/rockchip/mpp-rewrite/` (`mpp_rewrite.c`; +`ABI.rst`, `Kconfig`, `Makefile`) + `drivers/video/rockchip/rga-rewrite/` (`rga_rewrite.c`; +`ABI.rst`, `Kconfig`, `Makefile`). The trees contain 108 MPP and 152 RGA KUnit cases (260 total). |
| Current KUnit repair | KASAN boot `Pc86b-Cad24` (`gdf22eeef8757`, 2026-08-05) passes the exact **92 MPP + 152 RGA** manifest: 244 results, zero failures/skips, clean outer-KTAP interval, and live lockdep. It also passes 12/12 official MPP and reaches 21/34 required librga cases. Source stamp `gf37186832202` later booted as kernel #8 and exposed the residual decoder and RGA3 failures documented in the live findings. The current 108/152 manifest and successor ownership changes remain unbooted; all 260 cases and the GStreamer/FFmpeg conformance workloads require exact-tip replay together with an aged clean kmemleak scan, restored services, and all expected cores. |
| Current source tips | 6.18 `rk3588-rewrite-6.18@77b60c9250ccccd8aa77c4b4426b7921e870a03d` on `v6.18.42` and mainline `rk3588-rewrite-mainline@0a645ea1df04225661e9611174abe3ca1451b07f` on `v7.2-rc6` are the maintained tips. Phase 3 gives each activation typed external ownership, immutable terminal evidence, a full attempt-bounded CCU/link/DCHS/power/timing resource record, coherent hard-CCU retry handoff, stable-priority reason arbitration, one drain-and-complete tail, exact selected-core/dispatch release, and `RECLAIMABLE` predecessor retirement. RKVDEC `BUS_IDLE` remains advisory evidence, recovered proof stays separate, and AV1 untrusted-stop or impossible restore retains fail-closed quarantine ownership. The tracked MPP/RGA sources, Kconfig, ABI ledgers, and UAPI are byte-identical between the two maintained branches. VSI retains kernel-version-specific IOMMU attach APIs outside that identical set, with the same admission, retained-fault, callback-drain, and domain-retirement contract. |
...
| Validation | On 2026-08-09, completed Phase 3 pins byte-identical MPP SHA-256 `33a0374b2009cb70a688cd02bfbc501b93fedb68a90d52db4ec654f82dc94f4e` and strict 0/0/0 over 3,253 lines/tree. The focused final-source 6.18 KUnit-enabled object is 6,886,200 bytes at SHA-256 `c88e58387dd0bd5eb109f441b31346f46cf32f206449639ba7dc6837aed70023`. Ownership passes at 2,314/tree, fixture debt at 306/tree, and the exact manifest is 108/152. All eight warning-fatal profiles and the dedicated test-disabled ABI-mutation gate pass on both kernel lines; runtime execution remains pending. The predecessor 6.18 Phase 1 tip `ab69ece998642` was packaged as inspected P692f with stamp `(gab69ece99864)` but remains uninstalled and unbooted. No package or boot carries Phase 2 or Phase 3; exact-tip KUnit, corrected USERPTR GStreamer, MPP/FFmpeg, RGA3 vpp/overlay, aged kmemleak, final librga, performance, and AV1 fault/AFBC proof remain open. |
