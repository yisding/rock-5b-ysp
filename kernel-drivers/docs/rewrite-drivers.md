...
| Size (observed 2026-07-04) | MPP ~15,822 lines + RGA3 ~19,171 lines (vendor-delta.md method) | MPP rewrite ~8,910 lines + RGA rewrite ~17,226 lines |
...
|------|--------------------|
| Code | `drivers/video/rockchip/mpp-rewrite/` (`mpp_rewrite.c` 8,910 lines; 9,245 total incl. `ABI.rst`, `Kconfig`, `Makefile`) + `drivers/video/rockchip/rga-rewrite/` (`rga_rewrite.c` 17,226 lines; 17,747 total incl. `ABI.rst`, `Kconfig`, `Makefile`) |
| 6.18 state | committed local branch `rk3588-rewrite-6.18` at **`7368fe6902f2`** ("media: rockchip: cover gstreamer 10-bit yuv scaling"), committed in dev worktree `/home/yi/Code/linux-6.18-rkvenc`. It includes the broad RGA/MPP ABI and performance-path work described in §2/§3, the Rock 5B DT self-containment commit for disabled decoder nodes/IOMMUs/SRAM pools, the BSP-derived forward-port recovery cleanup, Rockchip IOMMU `map_pages`/`unmap_pages` count handling for large dma-buf mappings, rewrite MPP/RGA fault-handler registration through the Rockchip provider-local public hook before generic fallback, explicit KUnit coverage for GStreamer decoder-side 8-bit RGBA/BGRA/RGBx/BGRx RGA output conversions, and compact NV12_10LE40/NV16_10LE40 decoder-output scaling to 8-bit NV12/NV16. |
| Mainline-master state | committed local branch `rk3588-rewrite-mainline` at **`9ad335a5daff`** ("media: rockchip: cover gstreamer 10-bit yuv scaling"), committed in sibling worktree `/home/yi/Code/linux`. It carries the same rewrite drivers and userspace-facing ABI coverage, the mainline DT/wiring work, the Rockchip IOMMU map-count fix, the minimal public `include/soc/rockchip/rockchip_iommu.h` provider fault hook used by the rewrites, and the same GStreamer decoder 8-bit RGB plus compact 10-bit YUV-output KUnit coverage. |
