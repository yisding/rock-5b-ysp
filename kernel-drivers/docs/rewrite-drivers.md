...
| Validation | On 2026-08-05, the warning-fatal clean-archive `normal` profile passed for both maintained tips, compiling Rockchip and VSI IOMMU support, both KUnit-enabled rewrite objects, and the ROCK 5B DTB. The source audit reports **305 known signals, zero new, and zero absent** on both trees, and the identity gate passes for the tracked rewrite sources, Kconfig, ABI ledgers, and UAPI. The exact-tip 6.18 KASAN image/DTB/headers/libc-dev packages also build as `P27bb-Cad24`; the image embeds `g37ae7459656b`, the packaged config SHA-256 is `8e8fa957ae7e4fa5776116ccf849c7c85486e7cc48ec1fc6ca8b7c8e49aa88b4`, and KASAN, lockdep, both rewrite drivers, and both KUnit suites are enabled. Predecessor 6.18 `19634f4eebba` has exact booted 92/152 KUnit, a clean interval with live lockdep, 12/12 official MPP, and partial librga evidence. The test-disabled, memory, and race profiles were not rerun at the fixed tips. Current-tip install/boot, booted KUnit, aged kmemleak, corrected full librga, performance, and AV1 fault/AFBC proof remain open. |
