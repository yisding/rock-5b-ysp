...
> copy/resize/fill, one legacy `c_RkRgaBlit()` malloc-backed BGRx to dma-buf
> NV12 conversion shaped like GStreamer encoder preprocessing, forced RGA3
> core-mask + priority submission, forced RGA2 `IM_PRE_INTR`, and an async
> acquire/release-fence chain, but these still need to be run on a booted
> rewrite kernel. The shipped,
...
- **Userspace-visible priorities after the GStreamer legacy-convert KUnit slice** are
  ...
  fallback; the missing evidence is still the booted plugin run.  The
  highest-value MPP
