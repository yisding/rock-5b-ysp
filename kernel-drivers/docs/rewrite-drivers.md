...
| Current KUnit repair | **2026-07-27:** lifecycle-fixed `P3138-Cad24` completed the exact 85/85 MPP plus 148/148 RGA plans and restored both services, proving the preceding boot-order repair. It still exposed warning-cleanliness and fixture-ownership defects despite green KTAP. The latest inspection found one more zeroed fixture lock: `rk_mpp_session_abort_hw_active_kunit` reached DCHS release without initializing `rkvenc_dchs_lock`, disabling lockdep. Current tips initialize that spinlock. The conformance parser now automatically rejects any fatal line or disabled lockdep before ABI/media work, while the MPP debug-state reader fails fast instead of waiting forever on an abandoned service mutex. The next boot must repeat all 233 cases with a fatal-signature-free interval, live lockdep, an aged clean kmemleak scan, restored services, and all expected cores before ABI or media qualification. |
| Current source tips | 6.18 `rk3588-rewrite-6.18@f6ebe28a3f66` and mainline `rk3588-rewrite-mainline@394d80552960` contain byte-identical rewrite driver and ABI files. They fix the abort fixture's uninitialized DCHS spinlock and make observational debug-state reads nonblocking over mutex/nested-allocation repairs `6b55e022ce49` / `9aa6ef7e97b2` and lifecycle repairs `db8251eec71a` / `fac707773158`. Earlier `835b19f81d2b` / `79a804a26e00` contain the heap-backed 2,928-byte MPP batch fixture and `3b41eca277c7` / `52d4dfa16825` fix the final capped Debug Objects stack fixture. The audited ledgers match the implemented Kconfig exclusions, RGA errno/validation boundary, 10-bit TILE stride, shared-IRQ and RGA3 capability policy, MPP terminal isolation, and VEPU580 slice-flush fixup. |
...
| Validation | On 2026-07-27, current 6.18 `f6ebe28a3f66` passed clean-archive normal, KASAN/fault-injection memory, and KCSAN/lockdep race profiles; the user stopped the current mainline matrix. Strict checkpatch and byte identity with mainline `394d80552960` also pass. Current-tip 6.18.40 KASAN/UBSAN/lockdep/kmemleak package `P91d6-Cad24` is payload-verified: exact patch 330 is applied, both 85/148-case suites and final repair symbols are present, intended instrumentation is enabled, vendor collisions are disabled, and the DTB retains ramoops plus disjoint RGA3 windows. Its 131.8 MiB image exceeds the stock load gap but fits the existing managed 188 MiB U-Boot map with about 56.2 MiB spare. The package remains uninstalled, so warning-clean KASAN/lockdep hardware proof and media conformance are still open. |
