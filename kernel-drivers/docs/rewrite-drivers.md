...
| Code origin | Rockchip 6.1 BSP, ~87% unchanged (~90% Rockchip-authored), followed by a 75-commit forward-port/hardening series through `12a7da02bea83`. | Independent implementation; the current 6.18 tip is `db8251eec71a`, including explicit RGA3 shared-IRQ wiring, heap-backed KUnit work and service fixtures, boot-lifecycle-safe runtime isolation during KUnit, and ABI ledgers reconciled with the stable implementation. |
...
| In-tree testability | No comparable driver KUnit suite; confidence comes primarily from BSP history, repository reproducers, and extensive board runs. | 85 MPP + 148 RGA KUnit cases (233 total), fail-closed validators, debugfs counters, and clean-source build profiles. The tests have already exposed fixture/lifetime defects, so their presence is useful evidence rather than a correctness guarantee. |
| Board evidence | Published 6.18 kernel has broad conformance, bit-exact, sanitizer, root-gate, and production-performance evidence; the latest `0074`/`0075` tail is also KASAN-verified. | Build `#6` passes the current 85+148 KTAP plan and binds RGA2 plus both RGA3 cores, but emits two fixture debug-object warnings that the earlier five-report cap hid. Package `P259b-Cad24` then wedged before userspace because its suites cleared already-probed service singletons. Lifecycle-repaired KASAN/lockdep package `P3138-Cad24` is package-verified but not yet boot- or media-validated. |
...
| Item | State |
|------|--------------------|
| Code | `drivers/video/rockchip/mpp-rewrite/` (`mpp_rewrite.c` 14,118 lines; +`ABI.rst`, `Kconfig`, `Makefile`) + `drivers/video/rockchip/rga-rewrite/` (`rga_rewrite.c` 23,995 lines; +`ABI.rst`, `Kconfig`, `Makefile`). The trees contain 85 MPP and 148 RGA KUnit cases (233 total). |
| Current KUnit repair | **2026-07-27:** build `#6` passes the complete 85/85 MPP plus 148/148 RGA KTAP plans and binds RGA2 plus both RGA3 cores, but exposes two Debug Objects reports in `rk_rga_timeout_target_replacement_kunit`. That fixture's stack `rk_rga_hw` embeds an ordinary delayed work, contributing work and timer objects. The previous five-report cap hid this pair, correcting the total to eight affected objects across four fixtures. The identical final fix moves the owner to KUnit-managed heap storage at 6.18 `3b41eca277c7` and mainline `52d4dfa16825`; later tips also move the large MPP batch service fixture off-stack. Package `P259b-Cad24` includes those fixture repairs but wedged because its invalid `late_initcall_sync()` assumption let both suites clear already-probed live singletons. Current tips explicitly unregister each runtime before the boot suite uses its singleton, restore it before initramfs, and reject post-boot reruns. The next boot must pass all 233 cases with no debug-object report and leave lockdep enabled before isolated ABI replay. |
| Current source tips | 6.18 `rk3588-rewrite-6.18@db8251eec71a` and mainline `rk3588-rewrite-mainline@fac707773158` contain byte-identical rewrite driver and ABI files. They repair the boot-order defect in `dbc36621b301` / `948db1b44c63`; their parents `835b19f81d2b` / `79a804a26e00` contain the heap-backed 2,928-byte MPP batch fixture. Earlier `3b41eca277c7` / `52d4dfa16825` fix the final capped Debug Objects stack fixture. The audited ledgers match the implemented Kconfig exclusions, RGA errno/validation boundary, 10-bit TILE stride, shared-IRQ and RGA3 capability policy, MPP terminal isolation, and VEPU580 slice-flush fixup. |
...
| Validation | On 2026-07-27, lifecycle-repaired tips 6.18 `db8251eec71a` and mainline `fac707773158` passed all six clean-archive builds: each tree's `normal`, KASAN/fault-injection `memory`, and KCSAN/lockdep `race` profiles built warning-free with the Rockchip IOMMU provider, both KUnit-enabled rewrite objects, and `rockchip/rk3588-rock-5b.dtb`. Focused 6.18 objects, strict checkpatch, initcall ordering, KUnit suite-exit verdict handling, and all rewrite asynchronous-work/blocking-wait sites were also checked. The 6.18 repair then built as package-verified KASAN/lockdep `P3138-Cad24`; its packaged `System.map` has both rewrite registrations at device-initcall level, its config contains the intended suites/instrumentation and no vendor collisions or DWC PCIe PMU, and its DTB preserves ramoops plus disjoint RGA3 windows. The preceding `P259b-Cad24` is no longer a qualification candidate. Note that an in-tree `mpp_rewrite.o` build proves nothing unless `ROCKCHIP_MPP_REWRITE_KUNIT_TEST=y`; the clean-archive and package gates explicitly require both suites. Device-free conformance validation passed earlier in baseline, rewrite-counter, and forced-RGA-userptr-IOMMU modes. The repaired package remains unbooted, so warning-clean KASAN/lockdep hardware proof and media conformance are still open. |
