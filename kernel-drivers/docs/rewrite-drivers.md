...
> record comparable to the forward-port's exists yet (§6). The in-repo direct
> `librga` smoke now covers virtual-address imports, dma-heap dma-buf
> allocation plus `importbuffer_fd`, sync copy/resize/fill, forced RGA3
> core-mask + priority submission, forced RGA2 `IM_PRE_INTR`, and an async
> acquire/release-fence chain, but it still needs to be run on a booted rewrite
> kernel. The shipped,
