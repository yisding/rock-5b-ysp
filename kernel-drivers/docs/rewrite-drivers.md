...
| Validation | On 2026-08-05, the warning-fatal clean-archive `normal` profile passed at 6.18 `571e261b26f79` and mainline `5db5ddf046825`, compiling Rockchip and VSI IOMMU support, both KUnit-enabled rewrite objects, and the ROCK 5B DTB. Strict checkpatch passes; the source audit reports **306 known signals, zero new, and zero absent** on both trees; the exact 92/152 manifest is unchanged; and the identity gate passes for the tracked rewrite sources, Kconfig, ABI ledgers, and UAPI. The earlier exact-tip 6.18 KASAN package `Pc86b-Cad24` embeds `gdf22eeef8757`, boots, passes exact KUnit with live lockdep, passes 12/12 MPP, and records 21/34 required librga cases. That run confirms the USERPTR SG-size cap and exposes the remaining RGA2 DMA-BUF/USERPTR bounce mechanisms fixed at the new tips. Current-tip `rewrite-debug` package `P49e6-Cad24` was then built and inspected: its image embeds `g571e261b26f7`, its config SHA-256 is `18b72117c018604844b7cb74adb3a24ea25e0134cefe6eb1adf82aea8fc8502d`, and it enables both rewrite/KUnit pairs under KASAN and lockdep while disabling all three vendor alternatives. The test-disabled, memory, and race profiles were not rerun. Current-tip install/boot, booted KUnit, aged kmemleak, final librga, performance, and AV1 fault/AFBC proof remain open. |
