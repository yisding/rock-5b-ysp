...
| Validation | On 2026-08-06, the warning-fatal clean-archive `normal` and KASAN/fault-injection `memory` profiles passed at 6.18 `67f323aebdf39` and mainline `7a6d4cb075a67`, compiling Rockchip and VSI IOMMU support, both KUnit-enabled rewrite objects, and the ROCK 5B DTB with zero warnings. The source audit reports **304 known signals, zero new, and zero absent** on both trees; the exact 92/152 manifest is unchanged; and the identity gate passes for the tracked rewrite sources, Kconfig, ABI ledgers, and UAPI. `rewrite-debug` package `P7215-Cad24` then built successfully on rolling Linux 6.18.43: its image embeds `g67f323aebdf3`, its config SHA-256 is `371568af516d28d604b397a2bd0d62a30eccbfbe5cd7630e93db653694509b8e`, and it enables both rewrite/KUnit pairs under KASAN and lockdep while disabling the vendor MPP, vendor multi-RGA, and upstream RGA alternatives. The packaged config is identical to `P49e6-Cad24`. The earlier booted package `Pc86b-Cad24` remains the latest runtime evidence; `P7215-Cad24` is not installed. The test-disabled and race profiles were not rerun. Exact-tip booted KUnit, the corrected USERPTR GStreamer replay, MPP proc/FFmpeg replay, aged kmemleak, final librga, performance, and AV1 fault/AFBC proof remain open. |
