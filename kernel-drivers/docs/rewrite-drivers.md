...
> rotate/translate/padding, packed-YUV422 fill, Y4/Y8 dither output, mixed
> RGA2/RGA3 task batches, async acquire/release fences, and per-core scheduler
> counters. The remaining gap is hardware validation: no conformance
...
| Size (observed 2026-07-03) | MPP ~15,822 lines + RGA3 ~19,171 lines (vendor-delta.md method) | MPP rewrite ~8,606 lines + RGA rewrite ~16,772 lines |
...
- Optional **KUnit coverage** exists on both rewrite drivers: MPP parser helpers
  ...
  packed-YUV422 fill, Y4/Y8 dither output, RFBC/AFBC/tile,
