...
| Size (observed 2026-07-03) | MPP ~15,822 lines + RGA3 ~19,171 lines (vendor-delta.md method) | MPP rewrite ~9,095 lines + RGA rewrite ~17,446 lines |
...
- **Userspace-visible priorities after the GStreamer legacy-convert KUnit slice** are
  ...
  adds named RGA KUnit coverage for RGB-family-to-NV12 encoder preprocessing,
  rotated NV12-to-RGB MPP-frame conversion, and planar I420/YV12-style RGA2
  fallback; the direct `librga` smoke now mirrors those JeffyCN legacy-convert
  shapes through public `c_RkRgaBlit()`.  The missing evidence is still the
...
- Optional **KUnit coverage** exists on both rewrite drivers: MPP parser helpers
  ...
  profile helpers, and ffmpeg-facing profile helpers via
