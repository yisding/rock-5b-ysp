...
| Validation | On 2026-07-28, current 6.18 `51ea9d1ca537` and mainline `03da898b03f1f` compile their KUnit-enabled rewrite objects warning-free; the clean-archive normal/test-disabled/memory/race matrix is the current handoff gate. The checked source audit scans every KUnit-symbol block, recognizes project allocation/FD wrappers, reports the same 325 known-debt signals and zero new signals on both trees, and the gate separately requires byte-identical sources, Kconfig, ABI ledgers, and UAPI. Parent-source 6.18.40 package `P91d6-Cad24` remains the latest boot: both then-current 85/148 suites completed, but the reset/import fixture disabled lockdep. The current 84/148 manifest, fatal-free interval, live lockdep, source/config/package identity, kmemleak, runtime, and media proof remain open on a successor boot. |
