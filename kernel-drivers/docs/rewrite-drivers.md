...
> rotate/translate/padding, packed-YUV422 fill, Y4/Y8 dither output, BPP
> palette sources, mixed RGA2/RGA3 task batches, async acquire/release fences,
> and per-core scheduler counters. The remaining gap is hardware validation: no conformance
...
| Size (observed 2026-07-03) | MPP ~15,822 lines + RGA3 ~19,171 lines (vendor-delta.md method) | MPP rewrite ~8,606 lines + RGA rewrite ~16,867 lines |
...
- **Per-core profile coverage** (what real librga/ffmpeg consumers need):
  ...
  - **RGA2**: solid fill, YUV fill including packed YUV422 destination
    ...
    Y4/Y8 dither output for current compact-CSC `librga` paths,
...
- **Remaining recognized RGA gaps** are now comparatively specific: physical
  ...
  Y4/Y8 full-CSC output; per-channel rotation; RGA3 RFBC/AFBC32x8; tile
...
- Optional **KUnit coverage** exists on both rewrite drivers: MPP parser helpers
  ...
  packed-YUV422 fill, Y4/Y8 dither output, BPP palette sources,
