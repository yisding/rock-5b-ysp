...
> rotate/translate/padding border copies, packed-YUV422 fill, Y4/Y8 compact/full-CSC dither
...
> and per-core scheduler counters. MPP applies DT
...
> copy/resize/fill, forced RGA3 core-mask + priority submission, forced RGA2
> `IM_PRE_INTR`, and an async acquire/release-fence chain, but these still need
> to be run on a booted rewrite kernel. The shipped,
...
| Size (observed 2026-07-03) | MPP ~15,822 lines + RGA3 ~19,171 lines (vendor-delta.md method) | MPP rewrite ~8,672 lines + RGA rewrite ~17,165 lines |
...
- Optional **KUnit coverage** exists on both rewrite drivers: MPP parser helpers
  ...
  packed-YUV422 fill, Y4/Y8 compact/full-CSC dither output, BPP palette sources,
...
|------|--------------------|
| Code | `drivers/video/rockchip/mpp-rewrite/` (`mpp_rewrite.c` 8,377 lines; 8,672 total incl. `ABI.rst`, `Kconfig`, `Makefile`) + `drivers/video/rockchip/rga-rewrite/` (`rga_rewrite.c` 16,662 lines; 17,165 total incl. `ABI.rst`, `Kconfig`, `Makefile`) |
| 6.18 state | public branch `linux-rock5b/rk3588-rewrite-6.18` at **`92c058e8ddbd`** ("media: rockchip: avoid kzalloc_obj in mpp rewrite tests"), matching clean dev worktree `/home/yi/Code/linux-6.18-rkvenc`. Since `611f5fe047fd`, the committed work added a large RGA feature-coverage push: userptr import coverage, acquire-fence ownership, per-task core masks, request lifetime fixes, RGA2/RGA3 mixed-task scheduling, RGA priority, RGA2 YUV fill/full-CSC/gray/Y400/RFBC/mosaic/ROP/gauss/quantize/alpha-bitmap/OSD/palette, RGA2 packed-YUV422 fill, Y4/Y8 compact/full-CSC dither-output coverage, and BPP palette source coverage, RGA3 resize/translate/rotate/flip/padding/color-key/tile8x8/AFBC/RFBC-facing/10-bit/P210/alpha-overlay/overlay-preprocess profiles, AFBC-to-AFBC ffmpeg filter-copy coverage, `immakeBorder()` reflect/wrap top/bottom and left/right side-edge coverage, RGA2 OSD external-flag/default-background/channel-invert/invert-calibration/external-color register coverage, RGA2 `IM_PRE_INTR` line-interrupt programming/line-only IRQ handling, plus MPP `POLL_HW_IRQ`, DCHS-id, hard-CCU resend/recovery, IOMMU-refresh, session-batch-split, `RELEASE_FD`, nonblocking poll coverage, public `RESET_SESSION`/file-close cleanup, CCU coordinator dependent queued/active abort coverage, and DT `rockchip,normal-rates` application through the public clock framework, plus RGA request-config staging and reconfiguration resource/acquire-fence/gauss replacement coverage, request-config ioctl staging/acquire-fd ownership/no-release-fence-export coverage, configured request cancel/file-close cleanup, async close cleanup for jobs pending on acquire fences and jobs already queued on hardware, last-hardware pending-acquire cleanup, RGA3 pattern-channel rotate rejection, the RK3588 `im2d_slt` RGB/RGBA-to-YUV three-channel alpha-blend profile, modern async request-submit acquire/release-fence coverage, legacy `RGA_BLIT_ASYNC` acquire/release-fence coverage for current `librga` single-task async calls, mixed-task RGA3-to-RGA2 core handoff/requeue coverage, 6.18 host `libbpf` const-correctness needed by clean BTF-enabled builds on current GCC, and MPP KUnit allocation portability. |
| Mainline-master state | public branch `linux-rock5b/rk3588-rewrite-mainline` at **`4b06912f6e42`** ("media: rockchip: avoid kzalloc_obj in mpp rewrite tests"), matching clean sibling worktree `/home/yi/Code/linux`; it carries the rewrite drivers plus the mainline-master DT/wiring work, the V4L2 RGA3 comparison tree discussed in §1, and the same portable MPP KUnit allocation cleanup. |
| Validation | Focused compile gates pass at the public tips (2026-07-03): 6.18 clean-source archive with fresh output dirs, `make O=/tmp/rkcompat-6.18-rga-gate.C0pdKp ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- -j8 drivers/video/rockchip/rga-rewrite/rga_rewrite.o` and `make O=/tmp/rkcompat-6.18-mpp-gate.xXXkia ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- -j8 drivers/video/rockchip/mpp-rewrite/mpp_rewrite.o`; mainline `make O=/tmp/rkcompat-mainline-rga-afbc-gcc ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- -j8 drivers/video/rockchip/rga-rewrite/rga_rewrite.o` and `make O=/tmp/rkcompat-mainline-mpp-gcc ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- -j8 drivers/video/rockchip/mpp-rewrite/mpp_rewrite.o`; `git diff --check` passed in both kernel trees. The clean 6.18 gate also rebuilt `tools/bpf/resolve_btfids` with `CONFIG_DEBUG_INFO_BTF=y`, which is the host-tool coverage from the latest 6.18-only libbpf const backport. Both rewrite drivers carry optional KUnit coverage for pure ABI/helper logic, and the source-level ABI ledgers record the implemented/unsupported boundary per §2/§3; the latest RGA slice accepts current `librga` `IM_PRE_INTR` for already-supported RGA2 profiles, programs BSP-compatible read/write line interrupt registers and read-hold mode, and clears line-only IRQs without completing jobs, after the prior RGA slice accepted current `librga` `IM_OSD_COLOR_EXTERNAL` for the existing RGA2 OSD profile and emitted BSP-compatible external normal/invert color registers, after the MPP slice applied DT `rockchip,normal-rates` through the public clock framework before enabling clocks, after the prior RGA KUnit slice compiled current `librga` `imosd`/`rga_alpha_osd_demo` external-flag, default-background, channel-invert, and invert-calibration register emission, after `immakeBorder()` reflect/wrap top/bottom and left/right side-edge command coverage, after AFBC-to-AFBC ffmpeg filter-copy command emission for current AFBC input plus AFBC output requests, including FBCD/FBCE mode bits, sparse writeback, source crop-top active offsets, header/payload strides, and header-size UV-base accounting, after the previous Y4/Y8 full-CSC dither-output acceptance for current RGB-to-YUV `librga` requests, BPP import stride sizing, BPP1/2/4/8 color-palette source mode selection, stride/offset emission, bad palette-mode rejection, update-palette mode validation, RGA Y4/Y8 compact-CSC dither-output coverage, packed-YUV422 fill coverage, MPP CCU coordinator-removal coverage, RGA pattern-channel rotate, last-hardware removal, file-close queued-job cleanup, pending-acquire close cleanup, MPP reset/release cleanup, RGA configured-request teardown, request-config ioctl staging/acquire-fd ownership, resource/acquire-fence/gauss reconfiguration, mixed-task handoff, and async acquire/release-fence paths. This is still code/ABI-ledger progress rather than proof from a booted rewrite kernel. [`tests/abi-probe.sh`](../tests/abi-probe.sh) is the fast non-submit ABI log-diff gate for whichever implementation owns `/dev/mpp_service` and `/dev/rga`; [`tests/mpp-suite.sh`](../tests/mpp-suite.sh) is the official MPP test-suite wrapper for `mpp_info_test`, codec decode/encode, multi-thread, multi-instance, and rate-control cases; [`tests/mpp-suite-compare.sh`](../tests/mpp-suite-compare.sh) flags required MPP regressions against the forward-port summary; [`tests/librga-smoke.sh`](../tests/librga-smoke.sh) is the tiny direct librga/im2d smoke for virtual-address import, dma-heap dma-buf allocation plus `importbuffer_fd`, sync copy/resize/fill, forced-core/priority copy, forced RGA2 pre-intr, and async fence chaining; [`tests/librga-suite.sh`](../tests/librga-suite.sh) is the official librga sample-suite wrapper with required Linux/RK3588 cases and diagnostic outside-slice cases; [`tests/librga-suite-compare.sh`](../tests/librga-suite-compare.sh) flags required sample regressions against the forward-port summary; [`tests/rewrite-smoke.sh`](../tests/rewrite-smoke.sh) runs the ABI probe plus decode/encode/transcode consumer workloads. The broader conformance plan is staged outside this repo at `../rockchip-conformance`: JeffyCN GStreamer Rockchip, official MPP tests, official librga samples, the Linux MPP/RGA/DRM demo, and RKMediaCodecDemo, with per-profile logs for rewrite vs forward-port runs. **UNVERIFIED in this repo**: the workload gate and expanded conformance bundle have not yet passed on hardware through the rewrite; no validation record equivalent to status.md exists yet. |
