...
| Current source tips | 6.18 `rk3588-rewrite-6.18@e41bdb50a9ab7` on `v6.18.42` and mainline `rk3588-rewrite-mainline@1c91ffc853f7a` on `v7.2-rc6` are the maintained tips. Their provisional Phase 1 funnels are followed by stable reset domains and an `rk_mpp_cluster` topology. The existing hard-CCU participant pulse now validates its pinned coordinator/cores through that cluster and one common reset domain, then records the physical assert/delay/deassert sequence as one non-interleavable epoch. Selection, descriptor admission, group power, IOMMU recovery, and quarantine remain unchanged. The tracked MPP/RGA sources, Kconfig, ABI ledgers, and UAPI are byte-identical between the two maintained branches. VSI retains kernel-version-specific IOMMU attach APIs outside that identical set, with the same admission, retained-fault, callback-drain, and domain-retirement contract. |
