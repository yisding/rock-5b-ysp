...
| Item | State |
|------|--------------------|
| Code | `drivers/video/rockchip/mpp-rewrite/` (`mpp_rewrite.c`; +`ABI.rst`, `Kconfig`, `Makefile`) + `drivers/video/rockchip/rga-rewrite/` (`rga_rewrite.c`; +`ABI.rst`, `Kconfig`, `Makefile`). The trees contain 90 MPP and 148 RGA KUnit cases (238 total). |
| Current KUnit repair | **2026-07-28:** lifecycle-fixed `P3138-Cad24` completed the then-current exact 85/85 MPP plus 148/148 RGA plans and restored both services, proving the preceding boot-order repair. It still exposed warning-cleanliness and fixture-ownership defects despite green KTAP. `P91d6-Cad24` then completed the same exact KTAP totals, but case 83 showed that `rk_mpp_reset_session_hw_active_import_kunit` repeated its sibling abort fixture's defect: its zeroed local service reached DCHS release without initializing `rkvenc_dchs_lock`, disabling lockdep before RGA. Current tips initialize both fixtures, make the lifecycle suites opt-in, remove the compile-time-owned ABI-layout runtime case, add AV1 AFBC status/admission coverage, and mechanically reject new lexical fixture-debt signals. The next boot must run the current 238 cases with a fatal-signature-free interval, live lockdep, an aged clean kmemleak scan, restored services, and all expected cores before ABI or media qualification. |
| Current source tips | 6.18 `rk3588-rewrite-6.18@9771d14cfa109` and mainline `rk3588-rewrite-mainline@5bfb81f90f05b` carry the 2026-07-30 [AV1/VSI fault, AFBC, PM, and teardown hardening](../../findings/2026-07-30-rewrite-av1-vsi-fault-afbc-lifecycle-races.md) plus a warning-fatal soft-CCU KUnit stack-frame repair. The MPP source and ABI ledger remain byte-identical; VSI has kernel-version-specific IOMMU attach APIs but the same admission, retained-fault, callback-drain, and domain-retirement contract. The preceding 2026-07-29 [review-round-2 fixes](../../findings/2026-07-29-rewrite-driver-review-round-2.md) remain in both tips: legacy librga pre-swapped 90/270-degree destination windows validate in canvas orientation, the hard-CCU chain has one writer, abort kicks the scheduler, shared paths derive the bound service, RGA fences use independent contexts and pin the module, shared-IRQ MMIO is power-gated, timed-out completed blits are salvaged, gauss defaults opaque, legacy FBC/tile imports use their real layout, and the session-abort KUnit double free is repaired. |
