...
| Size (observed 2026-07-03) | MPP ~15,822 lines + RGA3 ~19,171 lines (vendor-delta.md method) | MPP rewrite ~8,430 lines + RGA rewrite ~16,317 lines |
...
- **Version tuples are capability keys.** `librga` capability-probing expects
  ...
  (`rga_rewrite.c:15509-15523` — `{3, 2, 0x63318}` / `{3, 0, 0x76831}`,
  rendered `"%x.%01x.%05x"` at `:1277`, hence the hex-looking revision). Report
...
- **Legacy `RGA_CACHE_FLUSH`, `RGA_FLUSH`, `RGA_GET_RESULT`, and
  `RGA2_GET_RESULT` are safe as BSP-compatible no-ops** (`:15473-15478`) —
...
- **Buffer import**: `RGA_IOC_IMPORT_BUFFER`/`RELEASE_BUFFER` for dma-buf fds
  *and* user virtual addresses — VA imports pin user pages, build sg_tables via
  `sg_alloc_table_from_pages()` (`:14895`), map with the public DMA API, and
...
- **Acquire-fence ownership**: when a submitted task clears
  ...
  and jobs already queued on hardware.
...
- Optional **KUnit coverage** exists on both rewrite drivers: MPP parser helpers
  ...
  pending-acquire and hardware-queued jobs, legacy async blit and modern async
  request-submit acquire/release-fence ioctls,
