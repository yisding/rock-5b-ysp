...
| Size (observed 2026-07-04) | MPP ~15,822 lines + RGA3 ~19,171 lines (vendor-delta.md method) | MPP rewrite ~9,149 lines + RGA rewrite ~17,499 lines |
...
| `MPP_CMD_SET_SESSION_FD` | session switching, restricted to fds that are themselves `/dev/mpp_service` files; batch slots carry their userspace `mpp_bat_msg` status pointer so libmpp server polls get per-slot results, with bad-fd and `MPP_BAT_MSG_DONE` slots skipping the following task group instead of touching the previous session; the libmpp batch-server wait layout is recognized narrowly so repeated `SET_SESSION_FD` + `POLL_HW_FINISH|LAST_MSG` groups are scanned slot-by-slot instead of stopping after slot 0 | `:2371` |
...
| `MPP_CMD_POLL_HW_FINISH` / `MPP_CMD_POLL_HW_IRQ` | nonblocking `POLL_HW_FINISH` on a pending non-batch job returns `-EAGAIN` without consuming the session-visible active job; for `SET_SESSION_FD` batch polls the ioctl returns success and writes per-slot `mpp_bat_msg.ret` values (`EAGAIN` positive for pending, `0` for done, negative task errors such as `-EIO` for completed failures) so current libmpp's batch server keeps its poll timer alive and can classify each task; repeated wait groups continue through the inferred libmpp wait-array capacity, stop on fresh zeroed unused slots, and skip prior completed slots marked `MPP_BAT_MSG_DONE`; RKVENC2 slice result streaming advertises `POLL_BUTT`, detects split mode from the submitted register image (`slen_fifo` + slice split), stores IRQ slice-length words in a job-owned FIFO, copies up to userspace `count_max`, and falls through to normal completion/readback on the final slice; non-split IRQ polls use the full-frame finish path | |
...
- **Userspace-visible priorities after the GStreamer legacy sync-blit/no-op ioctl KUnit slices** are
  ...
  parallel decode, an explicit libmpp batch-server mixed decode case for
  multi-slot wait polling, and mixed H.264/H.265 decode->encode transcode
...
- Optional **KUnit coverage** exists on both rewrite drivers: MPP parser helpers
  ...
  `SET_SESSION_FD`, batch poll status/write-array continuation, and public reset/file-close cleanup helpers via
...
| Item | State (2026-07-04) |
|------|--------------------|
| Code | `drivers/video/rockchip/mpp-rewrite/` (`mpp_rewrite.c` 9,351 lines; 9,692 total incl. `ABI.rst`, `Kconfig`, `Makefile`) + `drivers/video/rockchip/rga-rewrite/` (`rga_rewrite.c` 17,499 lines; 18,020 total incl. `ABI.rst`, `Kconfig`, `Makefile`) |
| 6.18 state | committed local branch `rk3588-rewrite-6.18` at **`69b2e481b025`** ("media: rockchip: parse mpp batch wait slots"), committed in dev worktree `/home/yi/Code/linux-6.18-rkvenc`. It includes the broad RGA/MPP ABI and performance-path work described in §2/§3, the Rock 5B DT self-containment commit for disabled decoder nodes/IOMMUs/SRAM pools, the BSP-derived forward-port recovery cleanup, Rockchip IOMMU `map_pages`/`unmap_pages` count handling for large dma-buf mappings, rewrite MPP/RGA fault-handler registration through the Rockchip provider-local public hook before generic fallback, explicit KUnit coverage for GStreamer decoder-side 8-bit RGBA/BGRA/RGBx/BGRx RGA output conversions, compact NV12_10LE40/NV16_10LE40 decoder-output scaling to 8-bit NV12/NV16, the remaining 180/270-degree GStreamer public rotation values, VP9 RKVDEC fd-to-IOVA register translation/validation, direct legacy `RGA_BLIT_SYNC` wait/no-fence ioctl behaviour used by the default `c_RkRgaBlit()` path, legacy RGA flush/result no-op ioctl dispatch used by librga's post-blit compatibility path, `SET_ERR_REF_HACK` copy/discard control handling used by current libmpp probing, libmpp batch-server `mpp_bat_msg.ret` poll status writeback, and multi-slot libmpp batch-wait parsing through repeated per-task wait groups. |
| Mainline-master state | committed local branch `rk3588-rewrite-mainline` at **`b21247083a7f`** ("media: rockchip: parse mpp batch wait slots"), committed in sibling worktree `/home/yi/Code/linux`. It carries the same rewrite drivers and userspace-facing ABI coverage, the mainline DT/wiring work, the Rockchip IOMMU map-count fix, the minimal public `include/soc/rockchip/rockchip_iommu.h` provider fault hook used by the rewrites, and the same GStreamer decoder 8-bit RGB, compact 10-bit YUV-output, 180/270-degree rotation, VP9 RKVDEC translation, legacy sync-blit ioctl, legacy RGA no-op ioctl, MPP err-ref control, MPP batch-poll status, and MPP batch-wait layout KUnit coverage. |
| Validation | Focused compile gates pass at the committed tips available so far. On 2026-07-04, `ALLOW_DIRTY=1 kernel-drivers/tests/rewrite-build-gate.sh all` built from `git archive` copies and completed warning-free for `../linux-6.18-rkvenc` at `69b2e481b025` and `../linux` at `b21247083a7f`, producing the KUnit-enabled `drivers/video/rockchip/mpp-rewrite/mpp_rewrite.o` and `drivers/video/rockchip/rga-rewrite/rga_rewrite.o` targets for both kernels. `ALLOW_DIRTY=1` was used only because the 6.18 worktree has unrelated dirty forward-port files; the gate still built committed `HEAD` archives. `VALIDATE_ONLY=1 kernel-drivers/tests/rewrite-conformance-run.sh` also passed its device-free case-builder/comparator validation, including 131 GStreamer case builders, on 2026-07-04. This is still code/ABI-ledger progress rather than proof from a booted rewrite kernel. The broader conformance plan remains the staged `../rockchip-conformance` forward-port-vs-rewrite workflow for MPP, librga, JeffyCN GStreamer, and ffmpeg-rockchip; booted MPP official-test artifact/timing runs, GStreamer state/allocator/RGA-conversion/P010-P210/VP9/display/KMS-capture results, and expanded FFmpeg decoder/RGA-filter artifact/timing results are still missing before lower-priority diagnostic BSP profiles. **UNVERIFIED in this repo**: the workload gate and expanded conformance bundle have not yet passed on hardware through the rewrite; no validation record equivalent to status.md exists yet. |
