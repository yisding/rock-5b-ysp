...
> opt-in display-tail BGRA/XRGB rotation plus BGRA partial alpha blend,
...
| Size (observed 2026-07-06) | MPP ~15,822 lines + RGA3 ~19,171 lines (vendor-delta.md method) | MPP rewrite ~9,256 lines + RGA rewrite ~18,134 lines |
...
- **Userspace-visible priorities after the GStreamer legacy sync-blit/no-op ioctl KUnit slices** are
  ...
  opt-in display-tail BGRA/XRGB rotation plus BGRA partial alpha-blend smoke, and the current
...
- Optional **KUnit coverage** exists on both rewrite drivers: MPP parser helpers
  ...
  alpha-blend command emission, and ffmpeg-facing profile helpers via
