...
|-----------|--------------------------------|------------------------------------------|
| Primary objective | Preserve Rockchip behavior while moving the BSP stack to Linux 6.18. | Preserve the current ROCK 5B userspace contract while replacing BSP internals with public kernel APIs and explicit ownership. Current tips are 6.18 `74c1b98def888` on `v6.18.42` and mainline `dd3a88cd5629` on `v7.2-rc6`. |
| Code origin | Rockchip 6.1 BSP, ~87% unchanged (~90% Rockchip-authored), followed by the maintained forward-port/hardening series. | Independent implementation; current tips are 6.18 `74c1b98def888` on `v6.18.42` and mainline `dd3a88cd5629` on `v7.2-rc6`, including the AV1 backend, August adversarial repairs, librga DMA/fence/bounce compatibility, selected-core USERPTR mapping under power, per-session RKVDEC dispatch serialization, RGA command-buffer publication, Phase 1 ownership funnels, Phase 2 reset/cluster/recovery and IRQ-register leases, the Phase 3A embedded current-attempt generation/deadline record, the Phase 3B exact activation-dispatch owner, the Phase 3C activation-owned selected-core reference, the Phase 3D activation-typed active/timeout slots, the Phase 3E reasoned active-claim owner, the Phase 3F fresh retained hard-CCU retry storage, the Phase 3G typed retry-predecessor retirement proof, BSP-shaped MPP device discovery, explicit RGA3 shared-IRQ wiring, exact KUnit manifest, compile-time ABI layout contract, and reconciled ABI ledgers. |
...
| State and lifetime model | Modular BSP subsystem, but heavily global: service/request/memory managers route work into per-core scheduler state. The hardening tail repairs ownership races without replacing that model. | Session-owned IDs/imports, copied submitted jobs, retained hardware/import references, exact active-slot claims, generation-tagged recovery, and quarantine when reset cannot prove DMA stopped. An IRQ-safe register lease binds accepted status to the live reset epoch and direct-core generation. Phase 3A embeds the MPP current-attempt generation/deadline record; Phase 3B replaces paired dispatch booleans with one exact activation-owner pointer; Phase 3C moves the retained selected-core reference into that activation; Phase 3D stores its address in the active and timeout slots while retaining the same independent containing-job references; Phase 3E makes one reasoned helper the only active-slot detach and records provisional restorable claim state; Phase 3F allocates and retains a distinct hard-CCU retry successor; Phase 3G retains typed group/core closure proof before calling that predecessor retired. Ordinary terminal retirement/outcome arbitration and RGA per-task execution remain planned rather than implemented. |
...
| Validation | On 2026-08-09, both maintained Phase 3G tips pass the source-pinned 1792-signal production inventory, unchanged 306-signal KUnit-debt audit, exact 102/152 manifest, byte identity, strict checkpatch, and all eight warning-fatal clean-archive `normal`, `test-disabled`, `memory`, and `race` profiles. The predecessor 6.18 Phase 1 tip `ab69ece998642` was packaged as inspected P692f with stamp `(gab69ece99864)` but remains uninstalled and unbooted. No package or boot carries Phase 2 or Phase 3; exact-tip KUnit, corrected USERPTR GStreamer, MPP/FFmpeg, RGA3 vpp/overlay, aged kmemleak, final librga, performance, and AV1 fault/AFBC proof remain open. |
