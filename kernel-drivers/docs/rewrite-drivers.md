...
| Code origin | Rockchip 6.1 BSP, ~87% unchanged (~90% Rockchip-authored), followed by a 75-commit forward-port/hardening series through `12a7da02bea83`. | Independent implementation; the current 6.18 tip is `669697f23d3df`, including explicit RGA3 shared-IRQ wiring, heap-backed KUnit work and service fixtures, boot-lifecycle-safe runtime isolation during KUnit, initialized locked fixtures, assertion-safe nested-allocation cleanup, a nonblocking debug-state reader, opt-in lifecycle suites, a compile-time-owned ABI layout contract, and ABI ledgers reconciled with the stable implementation. |
...
| Current source tips | 6.18 `rk3588-rewrite-6.18@669697f23d3df` and mainline `rk3588-rewrite-mainline@a49eb7575f436` contain byte-identical rewrite driver and ABI files. They make both lifecycle KUnit suites explicitly opt-in and retire the MPP ABI-layout runtime case in favor of the existing, stronger `static_assert()` set. Their parents `0a2d7b9414f58` / `aa18488c8642b` contain the Kconfig-only change; earlier `9af4a8816f259` / `fb5040f08d833` initialize the reset/import fixture's DCHS spinlock over sibling abort-fixture/nonblocking-state repairs `f6ebe28a3f66` / `394d80552960`, mutex/nested-allocation repairs `6b55e022ce49` / `9aa6ef7e97b2`, and lifecycle repairs `db8251eec71a` / `fac707773158`. The audited ledgers match the implemented Kconfig exclusions, RGA errno/validation boundary, 10-bit TILE stride, shared-IRQ and RGA3 capability policy, MPP terminal isolation, and VEPU580 slice-flush fixup. |
...
| Validation | On 2026-07-28, current 6.18 `669697f23d3df` and mainline `a49eb7575f436` passed clean-archive normal and test-disabled provider/rewrite/DTB builds. The gate also proved ordinary `KUNIT_ALL_TESTS=y` leaves both lifecycle suites off, qualification config resolves both to `y`, and a deliberate ABI-size mutation fails the test-disabled MPP build through the existing static assertion. The checked source audit reports the same 248 known-debt signals and zero new signals on both trees. Preceding 6.18 `9af4a8816f259` passed KASAN/fault-injection memory and KCSAN/lockdep race profiles; its production path is unchanged by the current test-only reduction. Parent-source 6.18.40 package `P91d6-Cad24` remains the latest boot: both then-current 85/148 suites completed, but the reset/import fixture disabled lockdep. Warning-clean current-tip KASAN/lockdep hardware proof and media conformance remain open. |
