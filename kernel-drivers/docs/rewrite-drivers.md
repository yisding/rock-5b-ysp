...
> unsupported. A dedicated 6.18 AV1 branch also implements the RKMPP AV1DEC
> backend, but has source/build evidence only. RGA has grown from the initial
> blit/fill subset into a broad
...
| Audit posture | 89 verified findings latent ([BSP audit](./bsp-audit.md)) | ownership-explicit and refcount-disciplined, with 85 MPP + 147 RGA KUnit cases on the shared rewrite and 90 MPP cases on the AV1 branch |
| Size snapshot | MPP ~15,822 lines; RGA3 19,173 code/build lines at the current forward oracle (`15,796` C + `3,305` headers + `72` Kconfig/Makefile) | shared tip: `mpp_rewrite.c` 14,061 C lines and `rga_rewrite.c` 23,865 C lines; dedicated AV1 tip: `mpp_rewrite.c` 15,772 lines |
...
Important codec boundary: the shared MPP rewrite does not expose RK3588 AV1
through `/dev/mpp_service`, while the forward port does. The dedicated
`rk3588-rewrite-av1-6.18` branch now binds that block and exposes the RKMPP ABI,
but it has not been booted. AV1 lives on a separate
...
rewrite implementation is currently compile-verified only. The separate
...
| AV1 via RKMPP | implemented on the dedicated 6.18 AV1 branch; hardware-unvalidated | `rk3588-rewrite-av1-6.18@e58c57e50d0a0` adds the separate AV1 hardware/IOMMU/backend plumbing without importing the BSP driver body and closes the adversarial fault/AFBC/provenance findings. The forward-port bit-exact result remains the runtime oracle until the rewrite passes the generated AV1 diagnostics and recovery gates. |
...
| Item | State |
|------|--------------------|
| Code | Shared rewrite: `drivers/video/rockchip/mpp-rewrite/` (`mpp_rewrite.c` 14,061 lines; +`ABI.rst`, `Kconfig`, `Makefile`) + `drivers/video/rockchip/rga-rewrite/` (`rga_rewrite.c` 23,865 C lines; +`ABI.rst`, `Kconfig`, `Makefile`), with 85 MPP and 147 RGA KUnit cases (232 total). Dedicated AV1 tip: `mpp_rewrite.c` 15,772 lines and 90 MPP cases (237 total with RGA). |
| AV1 6.18 state | **2026-07-26:** isolated worktree `/home/yi/Code/kernel/linux-6.18-rk-av1-rewrite`, branch `rk3588-rewrite-av1-6.18@e58c57e50d0a0`, based on the complete shared rewrite tip `c5faabf9d00b0`. The original three AV1 commits add the class-aware/dynamic shared architecture, RKMPP AV1 backend, VSI-IOMMU lifecycle, checked AFBC path, auxiliary IRQ descriptors, codec-neutral counters, KUnit coverage, and ABI documentation. Adversarial commit `e58c57e50d0a0` closes five independently confirmed fault ownership/refresh, auxiliary-IRQ, DMA-provenance, and AFBC-capacity gaps and adds discriminating coverage. Focused GCC/Clang builds, strict patch check, and clean-archive `normal`, KASAN/fault-injection `memory`, and KCSAN/lockdep `race` profiles pass, including both IOMMU providers and the Rock 5B DTB. No booted KUnit or board evidence exists, and mainline has not been carried. |
