...
> flush-event, and state-loop pipelines so the
...
- **Userspace-visible priorities after the GStreamer legacy-convert KUnit slice** are
  ...
  post-flush output, and repeated state-loop reset; its
  diagnostic set also includes seek-event probes through the same
  `gstreamer-event-harness`, parallel H.264 encode, parallel H.264
  ...
  media-backed GStreamer expansion should add display, EOS loops, AFBC
