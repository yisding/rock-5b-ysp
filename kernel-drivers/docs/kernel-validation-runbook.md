...
its paired-evidence audit (`rewrite-evidence-audit.sh`) and 236 green booted
KUnit cases — see
...
| Extra required gates | GStreamer suite; RGA patch-gate probes (P010/NV15/legacy-blit/over-4G) | 236 booted KUnit on the AV1 branch; counter checks; paired evidence audit; KCSAN race kernel; fault-injection matrix; 72 h soak | recovery-first subsystem order; `public` before `wip`; blacklist handling for HDMI audio |
