...
its paired-evidence audit (`rewrite-evidence-audit.sh`) and 238 green booted
KUnit cases — see
...
| Extra required gates | GStreamer suite; RGA patch-gate probes (P010/NV15/legacy-blit/over-4G) | 238 booted KUnit; counter checks; paired evidence audit; KCSAN race kernel; fault-injection matrix; 72 h soak | recovery-first subsystem order; `public` before `wip`; blacklist handling for HDMI audio |
