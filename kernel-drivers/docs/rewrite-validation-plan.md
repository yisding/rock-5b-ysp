...
| Consumer conformance (MPP / librga / GStreamer / FFmpeg) | ✅ `*-suite.sh` + external [`kernel-drivers/tests/rewrite-conformance.md`](../tests/rewrite-conformance.md), including opt-in GStreamer display/KMS-capture and AV1 diagnostic cases | reuse; wire the pass/fail gate |
...
AV1 backend gap, then compares `artifacts.tsv` byte counts plus SHA-256s, with
