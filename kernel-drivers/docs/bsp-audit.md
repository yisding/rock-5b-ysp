...
git apply --reject $YSP/kernel-drivers/patches/cleanup-draft/mpp_rkvdec2.patch
#   …or git am to keep the commit message:
git am $YSP/kernel-drivers/patches/cleanup-draft/mpp_rkvdec2.patch
...
- [gotchas — Gotchas & workarounds](../../docs/gotchas.md): runtime traps in these **same files** (build/DT/driver/runtime). The audit is the static-analysis companion to gotchas.md's hands-on fixes.
- [kernel driver guide — How the drivers work](./how-the-drivers-work.md): the register-window / `mpp_check_req()` clamp it documents is exactly this audit's **`mpp_common.c:1943`** finding (the over-size request clamp computed the overflow amount instead of the remaining space).
