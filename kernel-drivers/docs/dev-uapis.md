...
- Buffers cross as **fds**: the kernel imports them (how-the-drivers-work.md §5), maps them in the
...
- The recipe lands in a fixed-size `task->reg[]`; the kernel **bounds-checks** each
  ...
  the `bsp-audit.md` audit found real OOB/clamp bugs exactly here, so treat any code
...
[BSP audit](./bsp-audit.md) scrutinised for memory-safety bugs.
...
> this repo only inside the forward-port patch** — [source-tree pins](../../docs/source-trees.md)
