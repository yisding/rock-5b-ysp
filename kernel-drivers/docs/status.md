...
| **VP9 decode** | The decoder driver builds VP9 support; H.264/H.265 are the only codecs hardware-validated so far. The GStreamer conformance wrapper now generates VP9 IVF input and requires `generated_dec_vp9_fakesink` plus `generated_dec_vp9_dmabuf` by default, with an optional direct `mpi_dec_test -t 10 …` (`MPP_VIDEO_CodingVP9 = 10`) recipe in [`tests/README.md`](../tests/README.md) § VP9 Decode. **TODO: run VP9 on forward-port and rewrite hardware and move this row to ✅ once logs match.** |
...
| **Expanded MPP/RGA/GStreamer conformance** | RGA is validated *through* ffmpeg's `scale_rkrga`, and the in-repo `librga-smoke.sh` covers direct im2d paths including virtual-address imports, dma-buf fd imports, GStreamer-style legacy `c_RkRgaBlit()` conversions, forced-core/pre-intr submission, and async fences. The support repo now has wrappers and comparators for the official MPP tests, official `airockchip/librga` sample suite, and JeffyCN GStreamer Rockchip plugin under `../rockchip-conformance`, including generated VP9 IVF decode and opt-in Rockchip display/DMABuf sink cases. MPP test binaries and the full librga sample build helper have been staged locally; the GStreamer plugin build wrapper is present but the current host still lacks the GStreamer development `.pc` packages. None of these expanded suites has paired forward-port/rewrite hardware logs yet. |
