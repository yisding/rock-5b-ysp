...
| **Full RGA standalone sample suite** | RGA is validated *through* ffmpeg's `scale_rkrga`, and the in-repo `librga-smoke.sh` covers a small direct im2d path including virtual-address imports, dma-buf fd imports, forced-core/pre-intr submission, and async fences. The broader official `airockchip/librga` sample suite is now staged in `../rockchip-conformance`, but it has not been run against either kernel profile yet. |
