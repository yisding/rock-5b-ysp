...
| **Full RGA standalone sample suite** | RGA is validated *through* ffmpeg's `scale_rkrga`, and the in-repo `librga-smoke.sh` covers a tiny direct im2d path. The broader official `airockchip/librga` sample suite is now staged in `../rockchip-conformance`, but it has not been run against either kernel profile yet. |
