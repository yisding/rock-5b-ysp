...
| Current state | The frozen two-patch non-AV1 base remains the July 4 hardware baseline and DKMS source. The maintained AV1/PPA series now runs through `0045`; KASAN verifies lifetime fixes `0042`/`0043`, corrected MPP/FFmpeg functional conformance passes, and the RGA ABI fixes pass booted KASAN ABI replay on debug build `Pb999-C4ad2`. Clean production build `Pf558-Cb831` predates `0044`/`0045`. Publication, exact-image board validation, RGA/GStreamer completion, and rollback remain open. The cleanup series is staged and currently fails its compile gate. See [`status.md`](../../status.md). |
...
| [`forward-port-rk3588-av1/`](forward-port-rk3588-av1) | Actively maintained 47-file RK3588 MPP/RGA/AV1 forward-port series (`0001`–`0048`, with `0012` omitted), including KASAN-derived lifetime fixes `0042`/`0043`, booted-replay-verified RGA ABI fixes `0044`/`0045`, and the new RGA legacy-virtual/memory-error/10-bit-stride fixes `0046`–`0048` (booted validation pending). This is the PPA/build source line; generated `.deb`s and worktrees stay outside git. | [`forward-port-rk3588-av1/README.md`](forward-port-rk3588-av1/README.md) |
