...
| Current state | There is one forward-port kernel line: the maintained series under [`forward-port-rk3588/`](forward-port-rk3588), which includes AV1 decode. The frozen two-patch pair below is the superseded July 4 import, kept as the DKMS source and as the provenance anchor for the older vendor-delta measurement; it is not a kernel to choose between. The maintained series now runs through `0069`: debug build `Pd222-C4ad2` hardware-validates through `0058`, while KASAN/lockdep package `Pabd5-C4ad2` carries the checkpatch- and compile-clean BSP-audit HIGH subset `0059`-`0069`. `Pabd5-C4ad2` was installed and booted 2026-07-22 (fingerprint-verified) with clean boot health, a bit-exact four-codec decode differential, and a clean KASAN MPP suite; the targeted `0059`-`0069` hostile-ioctl gates and the rest of the sweep on that boot remain open. The historical full cleanup series remains staged and retains its documented patch-0024 compile defect. See [`status.md`](../../status.md). |
...
| [`forward-port-rk3588/`](forward-port-rk3588) | **The** RK3588 MPP/RGA/AV1 forward-port series — 68 files, `0001`–`0069` with `0012` omitted. `Pd222-C4ad2` boot-validates through `0058`; `0059`–`0069` port every BSP-audit HIGH still present at that tip and are packaged in unbooted KASAN/lockdep build `Pabd5-C4ad2`. This is the PPA/build source line; generated `.deb`s and worktrees stay outside git. | [`forward-port-rk3588/README.md`](forward-port-rk3588/README.md), [per-patch provenance/backport catalog](../docs/patch-catalog.md) |
...
> **⚠️ Runtime gate PENDING for `0059`-`0069`** — compile status alone is not
> verification. Do not ship that HIGH subset without the targeted triggers and
> codec/RGA regression gate listed in `patches/cleanup-draft/verification.md`.
> The historical full cleanup series has likewise never completed its runtime
> gate.
