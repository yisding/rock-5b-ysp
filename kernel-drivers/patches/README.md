...
| [`forward-port-rk3588/`](forward-port-rk3588) | **The** RK3588 MPP/RGA/AV1 forward-port series — 74 files, contiguous `0001`–`0074`. `0058`–`0068` port every BSP-audit HIGH still present at the `0057` tip; the tail through `0071` is boot- and conformance-validated, and `0072`–`0074` are the compile-clean 10-bit RGA stride follow-ups awaiting their gate. This is the PPA/build source line; generated `.deb`s and worktrees stay outside git. | [`forward-port-rk3588/README.md`](forward-port-rk3588/README.md), [per-patch provenance/backport catalog](../docs/patch-catalog.md) |
...
> **⚠️ Runtime gate OWED for `0072`-`0074`** — compile status alone is not
