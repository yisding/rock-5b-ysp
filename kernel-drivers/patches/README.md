...
| Current state | There is one forward-port kernel line: the maintained series under [`forward-port-rk3588/`](forward-port-rk3588), which includes AV1 decode. The frozen two-patch pair below is the superseded July 4 import, kept as the DKMS source and as the provenance anchor for the older vendor-delta measurement; it is not a kernel to choose between. The maintained series is contiguous `0001`–`0075` (renumbered 2026-07-23; see the [series README](forward-port-rk3588/README.md) for the old→new map). The tail through `0071` — which includes the BSP-audit HIGH subset `0058`-`0068` — was built by Launchpad, installed from the PPA, booted, and passed the full conformance set plus root gates on 2026-07-24. `0072`–`0074` are the 10-bit RGA stride/UV-offset follow-ups and `0075` closes the RKVENC2 slice-FIFO terminal drop: all four are checkpatch- and compile-clean, and **their runtime gates are owed** — the `0072` gate was run on-board and failed, which is what `0074` fixes, and `0075` needs a `split_arg=4` run with the paired MPP userspace. The historical full cleanup series remains staged and retains its documented patch-0024 compile defect. See [`status.md`](../../status.md). |
...
| [`forward-port-rk3588/`](forward-port-rk3588) | **The** RK3588 MPP/RGA/AV1 forward-port series — 75 files, contiguous `0001`–`0075`. `0058`–`0068` port every BSP-audit HIGH still present at the `0057` tip; the tail through `0071` is boot- and conformance-validated, and `0072`–`0075` are the compile-clean 10-bit RGA stride and slice-FIFO follow-ups awaiting their gates. This is the PPA/build source line; generated `.deb`s and worktrees stay outside git. | [`forward-port-rk3588/README.md`](forward-port-rk3588/README.md), [per-patch provenance/backport catalog](../docs/patch-catalog.md) |
