...
| Current state | There is one forward-port kernel line: the contiguous `0001`–`0092` source series under [`forward-port-rk3588/`](forward-port-rk3588/README.md), with AV1 and IEP2 included. Source, export, and Published `20260804` source package align at `0092`; remote arm64 build `33467257` completed successfully and awaits binary ingestion, while the Published/booted binary remains at `0089`, so the three-patch safety tail has packaging but no runtime proof. The frozen two-patch pair below is the superseded July 4 import retained for DKMS and provenance, not a second kernel choice. The cleanup series remains historical audit work. See the validation boundary below and [`status.md`](../../status.md). |
