...
| Owns | The generated forward-port patches, current-mainline codec corrections, debug-only DT patch, `cleanup-split/`, and the historical `cleanup-draft/` verification record. |
...
| Current state | There is one forward-port kernel line: the contiguous `0001`–`0080` series under [`forward-port-rk3588/`](forward-port-rk3588/README.md), with AV1 included. The frozen two-patch pair below is the superseded July 4 import retained for DKMS and provenance, not a second kernel choice. The cleanup series remains historical audit work. See the validation boundary below and [`status.md`](../../status.md) for what is actually boot-proven. |
...
| [`forward-port-rk3588/`](forward-port-rk3588) | **The** 80-file RK3588 MPP/RGA/AV1 forward-port series and PPA/build source line. Its README is the mechanical index; the patch catalog owns provenance and BSP-backport classification. Generated `.deb`s and worktrees stay outside git. | [`forward-port-rk3588/README.md`](forward-port-rk3588/README.md), [per-patch provenance/backport catalog](../docs/patch-catalog.md) |
