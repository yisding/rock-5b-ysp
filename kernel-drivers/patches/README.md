...
| Current state | There is one forward-port kernel line: the contiguous `0001`–`0089` source series under [`forward-port-rk3588/`](forward-port-rk3588/README.md), with AV1 and the compile-only IEP2 tail included. The public/package line remains at `0087`; the frozen two-patch pair below is the superseded July 4 import retained for DKMS and provenance, not a second kernel choice. The cleanup series remains historical audit work. See the validation boundary below and [`status.md`](../../status.md) for what is actually boot-proven. |
...
| [`forward-port-rk3588/`](forward-port-rk3588) | **The** 89-file RK3588 MPP/RGA/AV1/IEP2 forward-port source series; its `0088`–`0089` IEP2 tail is newer than the current 87-file PPA line. Its README is the mechanical index; the patch catalog owns provenance and BSP-backport classification. Generated `.deb`s and worktrees stay outside git. | [`forward-port-rk3588/README.md`](forward-port-rk3588/README.md), [per-patch provenance/backport catalog](../docs/patch-catalog.md) |
