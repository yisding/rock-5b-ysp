...
| Current state | There is one forward-port kernel line: the contiguous `0001`–`0087` series under [`forward-port-rk3588/`](forward-port-rk3588/README.md), with AV1 included. The frozen two-patch pair below is the superseded July 4 import retained for DKMS and provenance, not a second kernel choice. The cleanup series remains historical audit work. See the validation boundary below and [`status.md`](../../status.md) for what is actually boot-proven. |
