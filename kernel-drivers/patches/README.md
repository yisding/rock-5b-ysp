...
| Owns | The generated forward-port patches, current-mainline codec corrections, the mainline `vsi-iommu` probe fixes, debug-only DT patch, `cleanup-split/`, and the historical `cleanup-draft/` verification record. |
...
| Current state | There is one forward-port kernel line: the contiguous `0001`–`0092` source series under [`forward-port-rk3588/`](forward-port-rk3588/README.md), with AV1 and IEP2 included. Source, export, Published binaries, and the installed image/DTB/headers align at `7d53bc7a3adc` / `0092` / `6.18.42+rk3588av1fwport20260804-0ubuntu1~rk1`. The exact artifact has broad green functional/recovery evidence and a flat two-hour encode soak; its two-hour decode workload and kernel scan pass while the strict fd-span oracle remains red. Exact-tail KASAN/lockdep and remaining targeted hostile paths are still open. The frozen two-patch pair below is the superseded July 4 import retained for DKMS and provenance, not a second kernel choice. The cleanup series remains historical audit work. See the validation boundary below and [`status.md`](../../status.md). |
...
| [`forward-port-rk3588/`](forward-port-rk3588) | **The** 92-file RK3588 MPP/RGA/AV1/IEP2 forward-port source series at `7d53bc7a3adc`; Published `6.18.42` source and binaries carry the complete `20260804` series, and the exact image/DTB/headers are installed, booted, and broadly production-validated. Its README is the mechanical index; the patch catalog owns provenance and BSP-backport classification. Generated `.deb`s and worktrees stay outside git. | [`forward-port-rk3588/README.md`](forward-port-rk3588/README.md), [per-patch provenance/backport catalog](../docs/patch-catalog.md) |
...
| [`iommu-vsi-probe-fixes/`](iommu-vsi-probe-fixes/README.md) | Three error-path fixes to **mainline's own** `drivers/iommu/vsi-iommu.c` — discarded ioremap error, clocks leaked on a `platform_get_irq()` failure, unchecked DMA-mask return. Prepared against `iommu/next`, checkpatch-clean, compile-clean, not hardware-tested. The only directory here that targets mainline rather than the forward port. | [`iommu-vsi-probe-fixes/README.md`](iommu-vsi-probe-fixes/README.md) |
...
> **Runtime status for `0072`–`0075`.** Compile status alone initially proved
> insufficient: the first `0072` board gate failed and led to the `0074`
> correction. The corrected 10-bit stride/UV and RKVENC2 slice paths later
> passed their dedicated `6.18.40` KASAN hardware gates, and the exact `0092`
> production package now has broad integrated RGA/codec runtime evidence. See
> the [6.18.40 KASAN finding](../../findings/2026-07-25-forward-port-6-18-40-kasan-full-validation.md)
> and the [current production finding](../../findings/2026-08-04-forward-port-6-18-42-0092-production-validation.md).
> The BSP-audit HIGH subset `0058`–`0068` likewise passed its targeted triggers
> and codec/RGA regression sweep on booted `Pabd5-C4ad2`; the historical full
> MEDIUM/LOW cleanup series has still never completed its runtime gate.
