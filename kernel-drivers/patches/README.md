...
| Current state | There is one forward-port kernel line: the contiguous `0001`–`0092` source series under [`forward-port-rk3588/`](forward-port-rk3588/README.md), with AV1 and IEP2 included. Source and export align at `0092`; the Published/booted `6.18.42` package remains at `0089`, so the three-patch safety tail is compile-verified only. The frozen two-patch pair below is the superseded July 4 import retained for DKMS and provenance, not a second kernel choice. The cleanup series remains historical audit work. See the validation boundary below and [`status.md`](../../status.md). |
