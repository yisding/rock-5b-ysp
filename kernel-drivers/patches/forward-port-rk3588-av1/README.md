...
- `0055` bounds-checks the MPP register-translation imports against the
  80-entry `struct mpp_task` storage, closing two unprivileged-reachable
  out-of-bounds writes (AV1-R1: `mpp_task_attach_fd()` off-by-one `>`→`>=`;
  AV1-R8: `mpp_extract_reg_offset_info()` floored count vs raw byte copy).
  Both corrupted adjacent task fields during task assembly; the fix is
  fail-closed and does not affect normal decode.
...
- `0059` handles `get_task_msgs()` allocation failure before
  `task_msgs_init()` dereferences the result, and releases the held session fd
  if allocation fails during `MPP_CMD_SET_SESSION_FD`.
- `0060` closes the `MPP_CMD_SET_SESSION_FD` type confusion: a session switch
  now requires `fd_file(f)->f_op == &rockchip_mpp_fops` before treating the
  foreign file's `private_data` as `struct mpp_session`. The opt-in
  `ABI_PROBE_ENABLE_MPP_FOREIGN_FD=1` gate exercises a valid `/dev/null` fd and
  requires `-EBADF`.
- `0061` bounds the userspace-supplied RKVDEC2 RCB register index against
  `RKVDEC_REG_NUM` and applies `array_index_nospec()` before writing the fixed
  task register array.
- `0062` tests each iterated decoder core's `disable` flag when building the
  CCU work-mode mask, instead of repeatedly testing the outer worker device.
- `0063` bounds RKVENC2 read/write request fan-out before indexing the fixed
  `w_reqs[]` and `r_reqs[]` arrays.
- `0064` balances the reference returned by `sync_file_get_fence()` on every
  RGA acquire-fence exit, including successful callback completion. The newer
  RGA reconciliation already handled the `-ENOENT` signal race, so the old
  cleanup series' companion rewrite was not needed.
- `0065` detaches queued RGA shutdown jobs under `irq_lock`, then performs the
  sleeping IOMMU/request cleanup after restoring interrupts.
- `0066` validates required multi-plane handle buffers before building the
  RGA2 MMU table, and skips optional zero-sized planes in the current transient
  bounce-mapping path.
- `0067` makes `rga_mm_get_buffer()` failure ownership symmetric: every error
  after `kref_get()` drops the reference and clears the caller's out-pointer.
- `0068` makes current per-channel cleanup idempotent (including RGA2 bounce
  mappings and DMA-mapped page tables) and unwinds buffers acquired before a
