...
DEBUG_CONFIG_NAMES=(
	"config-rock5b-debug-kernel.conf.sh"
	"config-rock5b-rewrite-debug-kernel.conf.sh"
)
...
FLAVOR_CONFIG_NAME=""     # Armbian named config (debug flavors); empty = stock path
...
# The *-debug flavors seed USER_KERNEL_CONFIG from /boot before enabling KASAN
# and lockdep. Armbian gives any userpatch config precedence over its
# production config, so leaving that seed behind can silently preserve a debug
# C#### hash even after the debug flavor is done. Remove only configs managed
# by this script; fail closed for an unrelated user-owned override.
reset_debug_kernel_config() {
	local name src dest fragment
	...
	for name in "${DEBUG_CONFIG_NAMES[@]}"; do
		src="$DEBUG_KERNEL_DIR/$name"
		dest="$USERPATCHES_DIR/$name"
		if [ -f "$dest" ] && [ -f "$src" ] && cmp -s "$src" "$dest"; then
			rm -v "$USER_KERNEL_CONFIG" "$dest" |
				sed 's/^/      removed stale debug config /'
			# Escalate the clean level only on a config-CLASS transition
			# (debug -> production, or one debug flavor -> another). A
			# same-flavor debug rebuild re-stages the identical config and
			# stays incremental.
			if [ "$name" != "config-${FLAVOR_CONFIG_NAME:-<none>}.conf.sh" ] &&
			   [ -z "$ARMBIAN_CLEAN_LEVEL" ]; then
				ARMBIAN_CLEAN_LEVEL="make-kernel"
				say "  forcing CLEAN_LEVEL=make-kernel to discard $name Kbuild metadata"
			fi
			return
		fi
	done
...
if [ -n "$FLAVOR_CONFIG_NAME" ]; then
	...
	say "  RECOVERY_READY=1 PHASH='$PH' bash $DEBUG_KERNEL_DIR/install-debug-kernel.sh"
else
	say "DONE. After install.md recovery prep, install with:"
