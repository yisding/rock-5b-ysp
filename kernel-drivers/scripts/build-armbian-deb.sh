...
#   ARMBIAN_KERNELBRANCH=branch:linux-6.18.y bash build-armbian-deb.sh # deliberate rebase only
#   ARMBIAN_USE_CCACHE=no bash build-armbian-deb.sh     # clean retry after config-class changes
#   ARMBIAN_CLEAN_LEVEL=make-kernel bash build-armbian-deb.sh # discard all Kbuild metadata
...
DEBUG_CONFIG_SOURCE="$HERE/debug-kernel/config-rock5b-debug-kernel.conf.sh"
DEBUG_CONFIG_DEST="$USERPATCHES_DIR/config-rock5b-debug-kernel.conf.sh"
USER_KERNEL_CONFIG="$USERPATCHES_DIR/linux-rockchip64-current.config"
...
# The heavy-debug wrapper seeds this file from /boot before enabling KASAN and
# lockdep.  Armbian gives any userpatch config precedence over its production
# config, so leaving that seed behind can silently preserve a debug C#### hash
# even after IOMMU_DEBUG is disabled.  Remove only the config managed by our
# tracked debug wrapper; fail closed for an unrelated user-owned override.
reset_debug_kernel_config() {
	if [ ! -f "$USER_KERNEL_CONFIG" ]; then
		return
	fi
	if [ -f "$DEBUG_CONFIG_DEST" ] &&
	   [ -f "$DEBUG_CONFIG_SOURCE" ] &&
	   cmp -s "$DEBUG_CONFIG_SOURCE" "$DEBUG_CONFIG_DEST"; then
		rm -v "$USER_KERNEL_CONFIG" "$DEBUG_CONFIG_DEST" |
			sed 's/^/      removed stale debug config /'
		if [ -z "$ARMBIAN_CLEAN_LEVEL" ]; then
			ARMBIAN_CLEAN_LEVEL="make-kernel"
			say "  forcing CLEAN_LEVEL=make-kernel to discard debug-build Kbuild metadata"
		fi
		return
	fi
	die "$USER_KERNEL_CONFIG would override the production kernel config; move it aside or set up an intentional build path"
}
...
say "STEP 3c: reject stale heavy-debug kernel config"
reset_debug_kernel_config
...
# =============================================================================
say "STEP 5: build (ccache=$ARMBIAN_USE_CCACHE; clean=${ARMBIAN_CLEAN_LEVEL:-incremental})"
