...
# IOMMU_DEBUG=yes stages an Armbian custom_kernel_config hook (userpatches/lib.config)
# that enables DMA_API_DEBUG / KALLSYMS_ALL / IOMMU_DEBUGFS etc. The stock config
# path applies no user .config, and userpatches are regenerated each build, so the
# fragment has to be (re)staged here rather than dropped in by hand.
IOMMU_DEBUG="${IOMMU_DEBUG:-no}"
USERPATCHES_DIR="$ARMBIAN_BUILD/userpatches"
LIBCONFIG="$USERPATCHES_DIR/lib.config"
DEBUG_HOOK_SRC="$HERE/../patches/iommu-debug/lib.config"
...
# Stage (or remove) the IOMMU/DMA debug custom_kernel_config hook. Idempotent and
# authoritative each run: IOMMU_DEBUG=yes installs it, anything else removes our
# copy so a debug build never silently leaks into the next production build. Guards
# against clobbering a hand-written lib.config that isn't ours.
stage_debug_config() {
	mkdir -p "$USERPATCHES_DIR"
	if [ -f "$LIBCONFIG" ] && ! grep -qF "$YSP_DEBUG_MARKER" "$LIBCONFIG"; then
		die "$LIBCONFIG exists and is not ours; merge the hook from $DEBUG_HOOK_SRC by hand"
	fi
	if [ "$IOMMU_DEBUG" = "yes" ]; then
		[ -f "$DEBUG_HOOK_SRC" ] || die "IOMMU_DEBUG=yes but hook not found: $DEBUG_HOOK_SRC"
		cp -v "$DEBUG_HOOK_SRC" "$LIBCONFIG" | sed 's/^/      /'
		say "  IOMMU_DEBUG=yes -> DMA_API_DEBUG/KALLSYMS_ALL/IOMMU_DEBUGFS will be enabled"
	elif [ -f "$LIBCONFIG" ]; then
		rm -v "$LIBCONFIG" | sed 's/^/      removed /'
	fi
}
