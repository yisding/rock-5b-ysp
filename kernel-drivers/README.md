...
| Rewrite drivers | Public-API-only, downstream-ABI-compatible reimplementations of `/dev/mpp_service` and `/dev/rga`. Current source includes VPU981 AV1, explicit exact-slot/generation recovery, stable reset domains, MPP cluster topology and hard-CCU reset ownership, plus refcounted member-core power leases; activation/task-execution and cluster admission/full-recovery ownership remain architecture work. | [`docs/rewrite-driver-architecture/`](docs/rewrite-driver-architecture/README.md), [`docs/driver-architecture-comparison.md`](docs/driver-architecture-comparison.md), [`docs/rewrite-ownership-refactor-plan.md`](docs/rewrite-ownership-refactor-plan.md), [`docs/rewrite-drivers.md`](docs/rewrite-drivers.md) |
