...
| Current state | The July 4 combined Armbian kernel remains the hardware-validated baseline. The maintained forward-port series includes KASAN-verified lifetime fixes `0042`/`0043` and RGA ABI fixes `0044`/`0045`, which pass booted KASAN ABI replay on rebuilt debug build `Pb999-C4ad2`; corrected MPP/FFmpeg conformance passes on KASAN, while RGA DMA ownership, GStreamer, publication, exact-image validation, and rollback remain. Clean exact-6.18.38 production build `Pf558-Cb831` predates the ABI fixes. DKMS compiles on 6.18 but its overlay is not boot-validated; audit-fix and rewrite tracks are not shippable replacements. See [`../status.md`](../status.md). |
