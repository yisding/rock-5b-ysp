...
| Rewrite drivers | Public-API-only, downstream-ABI-compatible reimplementations of `/dev/mpp_service` and `/dev/rga`. Current source includes VPU981 AV1, stable reset domains and MPP cluster ownership, one typed `rk_mpp_activation` per physical MPP attempt, one typed `rk_rga_task_exec` per RGA task/core attempt, sealed MPP register images, immutable RGA task plans, and fail-closed quarantine. Cluster admission/coordinator-power policy, RGA2 large-segment staging, source organization, and current-tip runtime qualification remain open. | [`docs/rewrite-driver-architecture/`](docs/rewrite-driver-architecture/README.md), [`docs/driver-architecture-comparison.md`](docs/driver-architecture-comparison.md), [`docs/rewrite-ownership-refactor-plan.md`](docs/rewrite-ownership-refactor-plan.md), [`docs/rewrite-drivers.md`](docs/rewrite-drivers.md) |
