...
A final `RgaApi.h` / `RkRgaBlit` / `overlay_rkrga` sweep found broader public
packaging and UI/desktop users: libretro/RetroArch's OGA KMS driver,
downstream Xorg modesetting EXA, Qt raster, pixman, SDL, and OpenHarmony/BSP
wrapper copies. These are useful evidence that display rotation, fill, copy,
simple source-over blending, and virtual-buffer support are public
expectations, but they still use the same `librga` API family rather than a
new kernel ioctl class.
The final symbol-level pass added BELABOX's `gstreamer-rockchip` fork,
BrightSign's NPU gaze extension, `drmclone`, and Rust/Zig wrapper examples to
the same pile. The concrete kernel-visible shape remains dma-buf or virtual
imports, legacy `c_RkRgaBlit()` conversion/rotation, IM2D resize/color-convert,
RGB565/BGRA/XRGB/NV12/NV21-family formats, and wrapper-level dynamic loading.
It does not add physical-address import, Android allocator/HWC semantics, old
DRM-RGA ioctls, or RGA2-Pro FBC source modes to the required profile.
...
| GStreamer MPP forks | `BELABOX/gstreamer-rockchip` `gst/rockchipmpp/gstmpp.c` at `6453290` | RGA is an MPP/GStreamer conversion helper: dma-buf input when present, virtual input fallback, dma-buf output, legacy `c_RkRgaBlit()`, env-gated disable, RGB/YUV format mapping, and 0/90/180/270 rotation | Same kernel surface as JeffyCN GStreamer plus the direct display-tail smoke: fd import, virtual import, legacy blit, RGB565/RGB-family/NV12-family formats, and rotation. Useful optional integration evidence, not a new ABI family. |
...
| UI/display/media-player stacks | SDL KMSDRM rotation patch in `knulli-linux` at `4cfc5d`; LVGL/RKADK sample at `6e1bb1`; `iambronze/mp4player` `media/rga_utils.cc` at `f7dfb8d` | GBM/dumb-buffer fds, virtual draw buffers, BGRA/XRGB/RGB565 `c_RkRgaBlit`, 90/180/270-degree rotation into scanout buffers, optional alpha/blend on virtual MPP frame copies | Not a media-server requirement, but common appliance-style usage. `LIBRGA_SMOKE_DISPLAY_TAIL=1` now covers fd-backed BGRA/XRGB/RGB565 rotation plus a deterministic BGRA partial-rectangle alpha blend artifact; it still is not a full GBM/DRM scanout test. |
| RetroArch / handheld display frontends | `libretro/RetroArch` OGA KMS driver at `6712969`; downstream Batocera/Knulli/MustardOS SDL or RetroArch patches | DRM dumb-buffer PRIME fds, RGB565/BGRA surfaces, `c_RkRgaColorFill()`, fd-backed `c_RkRgaBlit()`, 90/180/270-degree rotation, scaling, and BGRA source-over/message blending into scanout pages | Reinforces the optional display-tail smoke and legacy fill/blit/fd import coverage. It is not a reason to add Android HWC or old DRM-RGA ioctls to the `/dev/rga` rewrite. |
| OpenCV/camera/RTSP capture helpers | `nihui/opencv-mobile` RKAIQ V4L2 capture source at `3151145`; `pablocpas/rockchip-hdmi-capture-rtsp` RGA converter at `2e10fb8`; `brightsign-npu-gaze-extension` `src/inference.cpp` at `889ea95` | V4L2 exported dma-buf or MPP buffer fds, dma-heap destination buffers, dynamic `librga.so` loading, `importbuffer_fd`, `wrapbuffer_handle(_t)`, `wrapbuffer_virtualaddr`, `imresize`, `imcvtcolor(_t)`, YCrCb/NV12/YUYV/BGR/RGB conversion | Confirms camera/CV/streaming apps want the same fd-backed and virtual IM2D conversion path. The direct smoke covers the kernel-visible fd/virtual import and color-convert behavior; full camera/RTSP source lifecycle remains optional application coverage. |
...
- RetroArch OGA KMS/RGA driver: https://github.com/libretro/RetroArch/blob/6712969a7b7516cb348f6af70635b1b1c8822dee/gfx/drivers/oga_gfx.c
...
- Xorg modesetting EXA RGA path: https://github.com/khadas/external_xserver/blob/fad645a0ca0f438b62dc3a332b34388bf9fd9fd0/hw/xfree86/drivers/modesetting/exa.c
- Qt raster RGA patch: https://github.com/SwordofMorning/RK3588_Buildroot/blob/55c00ec37361dd1352957e2c32f138392cebf77c/package/qt5/qt5base/0018-HACK-qpaintengine_raster-Support-rga-in-fillRect-wit.patch
