...
| Current state | As of 2026-08-05, **`1.0.11+ysp13-0ubuntu1~rk1` is clean public release `main@70f26d9` and Published** as source `18657954`, successful arm64 build `33468630`, and live driver/config binaries. A fresh source-package extraction matches every tracked release file byte-for-byte. The same-version local build remains installed; its driver matches that deb payload at SHA-256 `ed578a241803f35f51ccc6afe38b1d132539ae48ca691b5a3180e37565fe56c0`. **H.264, VP9 Profile 0 and HEVC Main decode remain the default capability set**; HEVC Main10, VP9 Profile 2, and encode remain opt-in. Ysp13 preserves pre-decode NV12/P010 export identity and passes the exact 24-frame retained-export gate, focused lifecycle/sanitizers, all 17 conformance cases, and the complementary 1,440-frame zero-copy audit. Google Chrome 151 renders H.264 correctly instead of green through installed ysp13 and selects `VaapiVideoDecoder` for 640x480 VP9 Profile 0; its 384x240 `VpxVideoDecoder` choice matches Chromium's intentional below-360p software priority ([finding](../../findings/2026-08-04-google-chrome-rockchip-vaapi-green-stable-export.md)). The stock Google deb uses no sandbox-disabling flags, but `chrome://gpu` and a live `/proc` probe show an unsandboxed GPU process with `Seccomp: 0`. XtraDeb Chromium 151 still omits libva and exposes Hantro V4L2 VP8. Ysp12 retains the broader exact-kernel package campaign; the Published ysp13 binary has not replaced and replayed the same-version local build. Exact PPA-binary replay, sandbox-enabled Firefox, physical HDR, automated Google Chrome replay/GPU sandbox, HEVC-in-Chrome, XtraDeb Chromium packaging, clean-image install, and 512 MiB CMA remain. |
