...
| Current state | The `gl_FragCoord` u_blitter fix is upstream as the open 4-MR stack !42563 / !42679 / !42613 / !42614. The 2026-07-11 GitLab check found tips and pipelines unchanged and selected CI evidence still green; !42679 reported `need_rebase`. On 2026-07-22 a maintainer confirmed the varying failure is a hardware erratum and supplied a zero-valued depth-bias workaround, now verified for OpenGL and Vulkan raw varyings plus OpenGL ordinary TEX. On 2026-07-24 MR !43161's aspect-cutoff discussion was checked against the automatic triangle matrix reproducer; `1000` and `500` both miss measured failures, so the proposed Mesa predicate is path/hardware-scoped for the Panfrost internal fullscreen blitter on `arch >= 9 && arch < 11`, with a documented size-gated compromise only if maintainers reject the simpler predicate. The 2026-07-27 forced all-blit A/B made both affected R32UI geometries exact without corrupting four ordinary controls. On 2026-07-28 a balanced single-context fixed-clock benchmark resolved the affected `12288x1` cost: workaround-on adds `+0.50%` completion-side time (95% interval `+0.34%..+0.73%`) and `+0.62%` end-to-end wall time (`+0.44%..+1.01%`). Its off/off A/A interval includes zero; all 6,480 A/B samples were non-disjoint and all 242 GPU checks stayed at 500 MHz. A separate same-batch draw control measured `+0.451%` (`p10..p90 +0.081%..+0.666%`). These are workload-specific microbenchmark numbers, not a universal application slowdown. See the [erratum finding](../../findings/2026-07-22-mali-varying-depth-bias-erratum-workaround.md), the [triangle matrix finding](../../findings/2026-07-24-mali-oblong-triangle-matrix.md), the [benchmark result](../../findings/2026-07-27-mesa-all-blit-workaround-benchmark-results.md), the [fixed-clock benchmark finding](../../findings/2026-07-28-mesa-blit-benchmark-timing-boundary.md), and [`status.md`](../../status.md). |
...
| 2026-07-24 | **MR !43161 triangle matrix built and measured.** The new [`triangle_matrix_probe.c`](./reproducers/interp_probe/triangle_matrix_probe.c) sweeps exact vs oversized triangles, all corners, both windings, wide/tall axes, both coordinate directions, raw-varying and normalized ordinary-`texture()` sampling, and baseline vs zero polygon offset; `--all-sizes` now runs the known MR/local boundary suite automatically. [`exact_offset_scan2d.c`](./reproducers/interp_probe/exact_offset_scan2d.c) now has `--main-results`, a one-shot full-surface baseline-vs-zero-offset bit-equality reproducer: both dimensions power-of-two match bit-for-bit, while `3x4096`, `4096x4095`, `4095x4095`, and low-aspect non-power cases differ; `--full-grid-floor --max 500` fully tests every `1x1..500x500` size and finds 0 baseline integer-bin failures. On G610, `12288x1`, MR-reported `12848x14`, MR-reported `9350x11`, and `16307x15` fail integer-bin checks for every baseline option and pass for every offset option. `16307` also has an oversized-only transition band at sampled sizes including `16307x63` (`aspect=258.841`) and a full pass at `16307x64`; `16384x1` and `16384x16` pass. Expanded tests show `500` also misses failures (`16307x63`, `16383x96`), while some higher-aspect cases pass (`8191x16`, `10000x16`), and `exact_offset_scan2d --sample-major-pow2` shows larger-dimension power-of-two is not a bit-equality guarantee. Proposed Mesa predicate: install zero polygon offset for the Panfrost internal fullscreen blitter path on `arch >= 9 && arch < 11`; keep a size gate only as a maintainer-requested compromise, not as the exact hardware predicate. |
| 2026-07-28 | **MR !43161 fixed-clock A/B completed after repairing the per-blit timer boundary.** Panfrost writes elapsed-time markers into the currently bound FBO batch; the first harness ended its query before flushing the other pending destination batches, and counts above `--ring 4` reused destination FBOs. [`blit_workaround_bench.c`](./reproducers/blit_workaround_bench.c) now flushes the start marker, measured FBO batches, and end marker in order and rejects `max(count) > ring`. On the non-KASAN kernel with GPU fixed at `500 MHz` and CPU 6 fixed at `1.8 GHz`, all 12 affected `12288x1` A/B pairs passed the fit gate and workaround-on remained exact, but GPU elapsed was unresolved: central `-0.09%`, 95% interval `-1.90%..+2.11%`. A 30-block, 4,096-draw same-batch control reproduced the descriptor-path cost at `+0.451%` (`p10..p90 +0.081%..+0.666%`). Panfrost's 32 live-FBO-batch limit pipelines unique destinations, so increasing the fit to 1,024 FBOs did not tighten the total-throughput interval. |
| 2026-07-28 | **MR !43161 cost resolved with one-context, balanced-order A/B.** The two-context A/A exposed a `~0.30%` context-position bias, and the first one-context A/A exposed a false `+0.37%` count-order effect. The final `coalesced` schedule toggles the test-only driver decision in one EGL context, targets one FBO, and assigns each label one ascending and one descending fit per ABBA/BAAB block. Its off/off control is unresolved around zero (`tail -0.15%`, 95% `-0.27%..+0.15%`). The 30-block off/on run resolves workaround-on at `+0.50%` completion-side (`+0.34%..+0.73%`) and `+0.62%` end to end (`+0.44%..+1.01%`); off reproduced 11,744 mismatches, on had zero, all 6,480 samples were non-disjoint, and all 242 GPU checks read 500 MHz. |
