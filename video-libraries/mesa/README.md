...
| 2026-07-24 | **MR !43161 triangle matrix built and measured.** The new [`triangle_matrix_probe.c`](./reproducers/interp_probe/triangle_matrix_probe.c) sweeps exact vs oversized triangles, all corners, both windings, wide/tall axes, both coordinate directions, raw-varying and normalized ordinary-`texture()` sampling, and baseline vs zero polygon offset; `--all-sizes` now runs the known MR/local boundary suite automatically. On G610, `12288x1`, MR-reported `12848x14`, MR-reported `9350x11`, and `16307x15` fail for every baseline option and pass for every offset option. `16307` also has an oversized-only transition band at sampled sizes including `16307x63` (`aspect=258.841`) and a full pass at `16307x64`; `16384x1` and `16384x16` pass. Expanded tests show `500` also misses failures (`16307x63`, `16383x96`), while some higher-aspect cases pass (`8191x16`, `10000x16`), and `exact_offset_scan2d --sample-major-pow2` shows larger-dimension power-of-two is not an exactness guarantee. Proposed Mesa predicate: install zero polygon offset for the Panfrost internal fullscreen blitter path on `arch >= 9 && arch < 11`; keep a size gate only as a maintainer-requested compromise, not as the exact hardware predicate. |
