...
| 2026-07-28 | **MR !43161 per-blit benchmark boundary repaired.** Panfrost writes elapsed-time markers into the currently bound FBO batch; the first harness ended its query before flushing the other pending destination batches, so the reported `~1.5 us` GPU time excluded most measured work. Counts above `--ring 4` also reused destination FBOs, allowing fullscreen overwrites to share one tile batch and store. [`blit_workaround_bench.c`](./reproducers/blit_workaround_bench.c) now flushes the start marker, measured FBO batches, and end marker in order and rejects `max(count) > ring` for batched runs. Unlocked-clock `64x64` and affected `12288x1` smokes now scale with `N` at `R²=0.988` and `0.987`; these validate the boundary, not workaround cost. The runner now reports adjacent paired ratios, rejects low-`R²` fits, and produces a bootstrap interval/effect gate. The final percentage still requires the documented fixed-clock, non-KASAN run after an interactive root clock pin. |
