...
// This checks what actually happens for a wide array-layer readback with
// BLIT transfers enabled: exact (st avoided the sampled array blit), drift
// (documented known gap), or crash (bug).
...
// st_ReadPixels staging blit (RG32UI -> RGBA32UI expansion via u_blitter TXF).
// Any spatial shift shows up as output[i].r != i.
//
...
int main(int argc, char **argv) {
   ...
   unsigned *src = malloc((size_t)W * H * 2 * sizeof(unsigned));
   for (int i = 0; i < W * H; i++) { src[i*2+0] = (unsigned)i; src[i*2+1] = (unsigned)i; }

   GLuint tex;
   p_glGenTextures(1, &tex);
   p_glBindTexture(GL_TEXTURE_2D_ARRAY, tex);
   p_glTexParameteri(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
   p_glTexParameteri(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
   {
      void (*p_glTexImage3D)(GLenum,GLint,GLint,GLsizei,GLsizei,GLsizei,GLint,GLenum,GLenum,const void*) =
         (void*)eglGetProcAddress("glTexImage3D");
      unsigned *two = malloc((size_t)W * H * 2 * 2 * sizeof(unsigned));
      memcpy(two, src, (size_t)W * H * 2 * sizeof(unsigned));
      memcpy(two + (size_t)W * H * 2, src, (size_t)W * H * 2 * sizeof(unsigned));
      p_glTexImage3D(GL_TEXTURE_2D_ARRAY, 0, GL_RG32UI, W, H, 2, 0, GL_RG_INTEGER, GL_UNSIGNED_INT, two);
      free(two);
   }
   check_gl("texImage");
   ...
   {
      void (*p_glFramebufferTextureLayer)(GLenum,GLenum,GLuint,GLint,GLint) =
         (void*)eglGetProcAddress("glFramebufferTextureLayer");
      p_glFramebufferTextureLayer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, tex, 0, 1);
   }
   ...
   unsigned *dst = malloc((size_t)W * H * 4 * sizeof(unsigned));
   memset(dst, 0xCD, (size_t)W * H * 4 * sizeof(unsigned));
   p_glPixelStorei(GL_PACK_ALIGNMENT, 4);
   p_glReadPixels(0, 0, W, H, GL_RGBA_INTEGER, GL_UNSIGNED_INT, dst);
   check_gl("readpixels");
   ...
   for (int i = 0; i < W * H; i++) {
      unsigned r = dst[i*4+0];
      if (r != (unsigned)i) { mismatches++; if (first_mismatch < 0) first_mismatch = i; }
      if (r == 0 && i != 0) zeros++;
