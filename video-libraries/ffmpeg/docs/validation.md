...
| Local FFmpeg baseline package audit | Source-package build, focused FATE results, and package-policy checks | Adding the frei0r runtime plugin to test-time Build-Depends fixes both new FFmpeg 8.1 frei0r cases | [Baseline package validation](../../../packaging/ppa/ffmpeg-baseline/README.md#local-package-validation-boundary) |
| HEVC unused-following-reference repair | Source-inspected upstream fix, branch backports, package builds, and board decode | Unavailable `ST_FOLL`/`LT_FOLL` entries must be generated as reference-only placeholders rather than made fatal; the byte-identical NUT aliases produce 34 clean frames | MPP owns the paired [RADL parser repair](../../../vendor-libraries/mpp/docs/mpp-library-architecture.md#hevc-parser-state-must-follow-syntax-identity-not-poc-or-numeric-id-alone); broader HEVC and consumer matrices remain separate gates |
| Historical FFmpeg 8.1/Kodi compatibility package | Full local arch+indep build, FATE, Launchpad arm64 build, and source publication | Removing decoder `pix_fmts` metadata preserves the RKMPP `hw_config`; missing `RELEASE_NOTES` and arch-dependent `ffmpeg-doc` were packaging defects, not codec defects | [Frozen package lessons](../../../packaging/ppa/docs/building.md#ffmpeg81-package-lessons), [Kodi selection mechanism](../../../apps/kodi/docs/decoder-selection.md) |
